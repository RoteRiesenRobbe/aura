...
func TestWelcomeMarshalFlatbuf_RoundTrip(t *testing.T) {
	...
	sm := BerryhunterApi.GetRootAsServerMessage(b.FinishedBytes(), 0)
	assert.Equal(t, BerryhunterApi.ServerMessageBodyWelcome, sm.BodyType())

	var tbl flatbuffers.Table
	require := sm.Body(&tbl)
	assert.True(t, require)

	var welcome BerryhunterApi.Welcome
