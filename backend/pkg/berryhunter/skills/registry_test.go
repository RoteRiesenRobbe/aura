...
func TestRegistry_LoadsFromDisk(t *testing.T) {
	...
	r, err := RegistryFromFS(fsys)
	require.NoError(t, err)
	// 20 player skills (incl. SwiftPassive/ToughPassive, NovaBurst/Heal,
	...
	// ReaperAura vocabulary smoke, the Barrier shield smoke and the Recall
	// cast-time cooldown) + 9 mob skills (mobs/ subdirectory: 8 auras incl.
	// TotemAura + CompanionAura + HealerAura + CampfireAura + the
	// AngryMammothStomp cooldown)
	assert.Len(t, r.All(), 29)
