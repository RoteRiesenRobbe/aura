...
import (
	...
	"github.com/RoteRiesenRobbe/aura/pkg/aura/gen"
...
func (pr *PlaceableResource) PlayerTouches(player model.PlayerEntity, damage model.Damage) {
	pr.Placeable.PlayerTouches(player, damage)
	pr.EmbeddedResource.PlayerTouches(player, damage)
}
...
func determineResourceEntityType(stockItem items.Item) model.EntityType {
	...
	case "Berry":
		return model.EntityType(AuraApi.EntityTypeBerryBush)
	}
