...
// CONSTRAINT NAME on the same 23505 error, and all but one of those names are
// generated by Postgres rather than authored — so nothing but this test notices if a
...
func TestUniqueConstraintNames(t *testing.T) {
	...
	// The bloodline_unlocks primary key is the database half of P4 (no duplicate
	// picks), and AscendCharacter tells it from any other failure by this name.
	// Postgres generated it, so nothing but this notices a rename.
	insertUnlock := func() error {
		_, err := db.Pool.Exec(ctx,
			`INSERT INTO game.bloodline_unlocks (account_id, slot_index, unlock_key) VALUES ($1, 0, 'FrostShield')`,
			accountA)
		return err
	}
	require.NoError(t, insertUnlock())
	assert.Equal(t, "bloodline_unlocks_pkey", nameOf(t, insertUnlock()), "a duplicate bloodline unlock")

	// previous_character_id's inline UNIQUE — one sacrifice, at most one heir.
	// CreateCharacter maps it to the same retry as the slot conflict, because a
	// create losing this race is a create losing that one.
	_, err = db.Pool.Exec(ctx, `UPDATE game.characters SET sacrificed_at = now() WHERE name = 'Barney'`)
	require.NoError(t, err)
	var predecessorID int64
	require.NoError(t, db.Pool.QueryRow(ctx, `SELECT id FROM game.characters WHERE name = 'Barney'`).Scan(&predecessorID))
	claimHeir := func(name string) error {
		_, err := db.Pool.Exec(ctx,
			`INSERT INTO game.characters (account_id, slot_index, name, avatar, faction, previous_character_id)
			 VALUES ($1, 5, $2, 'default', 'aligned', $3)`, accountA, name, predecessorID)
		return err
	}
	require.NoError(t, claimHeir("Pebbles"))
	assert.Equal(t, "characters_previous_character_id_key", nameOf(t, claimHeir("BammBamm")),
		"a second heir claiming one predecessor")
}
