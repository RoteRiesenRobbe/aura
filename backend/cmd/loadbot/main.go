...
func spawnBot(id int) (*bot, error) {
	...
	// ⚑ MERGE, UNRESOLVED BY DESIGN (accounts-8a × main): the name is now the
	// server-issued character name from mintPlayTicket, so `-names`' nameOf(id)
	// no longer reaches the nameplate. `id` is kept so -orbit still rings.
	// See docs/backlog.md §49.
	b := &bot{id: id, name: name, conn: c, stop: make(chan struct{})}
