// automatically generated by the FlatBuffers compiler, do not modify

/* eslint-disable @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any, @typescript-eslint/no-non-null-assertion */

import * as flatbuffers from 'flatbuffers';

import { ScoreboardPlayer } from '../berryhunter-api/scoreboard-player.js';


export class Scoreboard {
  bb: flatbuffers.ByteBuffer|null = null;
  bb_pos = 0;
  __init(i:number, bb:flatbuffers.ByteBuffer):Scoreboard {
  this.bb_pos = i;
  this.bb = bb;
  return this;
}

static getRootAsScoreboard(bb:flatbuffers.ByteBuffer, obj?:Scoreboard):Scoreboard {
  return (obj || new Scoreboard()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

static getSizePrefixedRootAsScoreboard(bb:flatbuffers.ByteBuffer, obj?:Scoreboard):Scoreboard {
  bb.setPosition(bb.position() + flatbuffers.SIZE_PREFIX_LENGTH);
  return (obj || new Scoreboard()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

players(index: number, obj?:ScoreboardPlayer):ScoreboardPlayer|null {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? (obj || new ScoreboardPlayer()).__init(this.bb!.__indirect(this.bb!.__vector(this.bb_pos + offset) + index * 4), this.bb!) : null;
}

playersLength():number {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? this.bb!.__vector_len(this.bb_pos + offset) : 0;
}

static startScoreboard(builder:flatbuffers.Builder) {
  builder.startObject(1);
}

static addPlayers(builder:flatbuffers.Builder, playersOffset:flatbuffers.Offset) {
  builder.addFieldOffset(0, playersOffset, 0);
}

static createPlayersVector(builder:flatbuffers.Builder, data:flatbuffers.Offset[]):flatbuffers.Offset {
  builder.startVector(4, data.length, 4);
  for (let i = data.length - 1; i >= 0; i--) {
    builder.addOffset(data[i]!);
  }
  return builder.endVector();
}

static startPlayersVector(builder:flatbuffers.Builder, numElems:number) {
  builder.startVector(4, numElems, 4);
}

static endScoreboard(builder:flatbuffers.Builder):flatbuffers.Offset {
  const offset = builder.endObject();
  return offset;
}

static createScoreboard(builder:flatbuffers.Builder, playersOffset:flatbuffers.Offset):flatbuffers.Offset {
  Scoreboard.startScoreboard(builder);
  Scoreboard.addPlayers(builder, playersOffset);
  return Scoreboard.endScoreboard(builder);
}
}
