...
| Light | in-game | Creates light in dark areas; rendering-only (radius punches a hole in the client darkness overlay). Occupies the one active-aura slot → the light-vs-damage trade-off (since C2 Part 2 the dimmer **Torch passive** resolves it — entity light = max over active aura + passives; Light keeps the bigger group-support radius). | Early game, zone 1 → 2 tunnel tutorial (GDD §7). |
...
| Immolation | in-game | Leaves a fire dot on the nearest enemy. | Effect-foundations smoke content; now the fire line's mid-tier and a Wildfire ingredient. |
| Rejuvenation | in-game | Heal-over-time aura; the HoT lingers on allies after they leave range. | Skill-vocab smoke content; the boss-rare drop pattern's payload. |
