...
⚑ **Rider 2026-09-19, the skill inventory generator.** The PO asked for `docs/content-skill-inventory.md` brought up to date with a note that lagging is fine, new fields added and wrong process text REPLACED. Built by an Opus 5 agent: `tools/content-editor/skill-inventory.mjs` (`npm run inventory`) now owns the whole doc, rendered from the vocabulary fixture and `skill-presentation.mjs` (no per-type hand table), sources from `collectSkillReferences` extended with plain data fields (labels byte-identical, the rename guard and panel untouched), self-checks (every file once, counts sum, dangling names, idempotent render). The doc covers all 113 skills incl. the 38 mob-only ones with a Carried-by column, and its derived summary corrected the old snapshot: 3 milestones not 2, 18 teachings across 10 teachers not 20/12, 18 cheat-only (13 unplaced + 3 rigs + 2 prototypes) not 15, two ascension stones, Lantern also an ascension reward. The C4 checklist line, the editor README, the add-content skill step 5, the README index and the ability matrix say so now instead of "by hand, no generator". Verified: `node --test` 2/2 · smoke 0 findings / 113 · two runs byte-identical.
