...
| **D3** | ⚑ **REVERSED 2026-08-02 by feel-pass-2 chunk N4 (D4 there, PO 2026-08-01):** progress only counts **after acceptance, per stage entry**, and a talk-to target needs a **fresh talk** — the counters stay lifetime, but `Progress` now snapshots per-stage baselines (`KillBase`/`TalkBase`, `quests/ledger.go`) that the three read sites subtract, so a veteran no longer auto-completes at accept and *"kill 8 MORE"* is now the only expressible meaning. See `plan-feel-pass-2.md` §3.3/§4. *(Original ruling, for the reasoning:)* **Retroactive credit, via lifetime counters.** The ledger keeps per-species kill/harvest counters and a talked-to set for the character's whole life; an objective is satisfied when the counter meets the authored threshold. PO leaned retroactive pending cost; the cost analysis (§3) put the mechanical delta at ~zero, so it is locked. Known design consequence, accepted: a veteran auto-completes on accept ("you've already slain them? Take this." — the Gothic feel); *"kill 8 MORE"* is inexpressible until a per-quest opt-out flag, deliberately deferred. |
