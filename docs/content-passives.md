...
|---|---|---|---|
| SwiftPassive | in-game | +move speed (stat multiplier). | Pyromancer combo component ("Swift" in the recipe sketch). **Kill-drop @0.1 on every wolf (line-wide 2026-07-21; Wolf-only since C2).** |
...
| Antivenom | in-game *(C3)* | `resist_passive` ×0.7 poison, stronger per level — the first drop pair authored against its own source: spiders deal the zone's only poison, spiders drop the counter. | Spider kill-drop @0.1, venom spider @0.25 (nest = densest odds). `api/skills/antivenom.json`. |
