...
> here, but the numbers drift, so they are not repeated.
...
| Wolves | in-game *(C2)* | Animal / normal | Zone 1's first real combat mob; "pack" = clustered spawns (no pack mechanic). `wildlife_predator` — hunts boar/stag AND players (ambient predation). Speed 0.7 (PO rule: normal mobs clearly slower than the player). Drops Swift + KeenEye (both line-wide across the wolf line since 2026-07-21); the entry-rung wolf carries no signature drop of its own. `api/mobs/wolf.json`. |
| Elite wolf | in-game *(C2)* | Animal / elite | Dark-forest pack leader ("something big"); EliteWolfBite carries **execute + lifesteal** (feeds on wounded prey). Guards the Hermit approach. Drops Swift + KeenEye (line-wide) + Wild. `api/mobs/elite-wolf.json`. |
| Bear | in-game *(C2)* | Animal / normal | Forest tank; BearSwipe carries **berserker** (wounded animal rages). Drops ThickHide + Berserker. `api/mobs/bear.json`. |
...
| Kobold (melee) | in-game *(C3)* | Small Fantasy / normal | Hideout swarm fighter: KoboldStab small/fast (tick-contrast partner to the volley). First **flee-at-low-HP** combat mob (`fleeBelowHealthRatio 0.25`). `kobold` faction (hostile to players only). Drops **Light** — the zone's only Light source (§12 flag). `api/mobs/kobold.json`. |
| Kobold (ranged) | in-game *(C3)* | Small Fantasy / normal | The **uncapped-volley showcase**: KoboldVolley large radius, slow tick, `selector: "all"` — hits every valid target in range per volley. Stands behind the melee line. Also flees low HP; drops Light. `api/mobs/kobold-ranged.json`. |
...
| Soldier companion | in-game *(C6)* | Summon | The Call for Aid squad member: Companion body/behavior pattern (owned, caster-aligned, follows because Call for Aid authors `follows: true`), SoldierBlades kit reused — a soldier of the front, marked yours. Skill-spawned only. `api/mobs/soldier-companion.json`. |
...
| Dire wolf | in-game *(intermission ②)* | Animal / normal | Z2 wildlife step-up, apex of the Session-④ predator forest; WolfBite, curveLevel 6. Drops Swift + KeenEye (line-wide) + LongRangeStrike. **Own sprite since the unique-art pass (2026-07-21)** — was a Wolf reskin. `api/mobs/dire-wolf.json`. |
| Dire bear | in-game *(intermission ②)* | Animal / normal | Z2 tank half of the wildlife step-up; BearSwipe, curveLevel 7 — the Z2 band's hard normal. Drops Recover. **Own sprite since the unique-art pass (2026-07-21)** — was a Bear reskin. `api/mobs/dire-bear.json`. |
| Giant spider | in-game *(farm-band pre-chunk)* | Animal / normal | **cL8-17 farm band** (2026-07-21): the spider line grown huge — slow lurker, VenomSpit poison dot on a deep pool (the band's attrition check, guardrail-soft). XP 205 (kph-derived, facetank). Own sprite. No drops yet (PO call). Spawns: PO places in the editor. `api/mobs/giant-spider.json`. |
| Alpha wolf | in-game *(farm-band pre-chunk)* | Animal / normal | **cL8-17 farm band** (2026-07-21): apex of the wolf line (Wolf 2 → DireWolf 6 → Alpha 10); WolfBite, speed 0.74 under the PO normal ceiling (guardrail-soft). XP 275 (kph-derived, facetank). Own sprite. Drops Swift + KeenEye (line-wide) + Reaper — the line’s apex payload (2026-07-21). PO places spawns. `api/mobs/alpha-wolf.json`. |
| Marauder | in-game *(farm-band pre-chunk)* | Humanoid / normal | **cL8-17 farm band** (2026-07-21): hardened outlaws past the bandit camp — EliteBandit-derived NORMAL, BanditBlades **level 3** + deep pool = the band's HARD normal (kills the guardrail bot). XP 175 (kph-derived, kite ×0.5 — DireBear precedent). Own sprite. No drops yet. PO places spawns. `api/mobs/marauder.json`. |
| Fire elemental | in-game *(2026-07-21)* | Elemental / normal | **cL18, front-band** — a living flame on the `elemental` faction (hostile to players only; no quarrel with the orcs or the army). Carries FireElementalAura: a fire dot on **everyone** in range (`selector: "all"`), so it damages what stands near it rather than picking a target. Slow (0.55) — it advances, it doesn't chase. Drops **FireWard**, the drop pair authored against its own source (the mob that burns you drops the fire resist — Spider/Antivenom precedent); this is what finally gave FireWard a world source. Own sprite. PO places spawns. `api/mobs/fire-elemental.json`. |
| Greater fire elemental | in-game *(2026-07-21)* | Elemental / **elite** | **cL20 = Orc-equivalent**, the hardest non-boss content alongside the front. Carries the **same** FireElementalAura as its lesser sibling by PO ruling — the extra HP and damage come from the elite baseline (≈4× normal, Orc/EliteBandit precedent) plus f(cL20), not a separate skill. Slower still (0.45): a walking furnace, kiting is the counterplay. Drops the **FireTotem** cooldown. Own sprite, deliberately larger so the tier reads at a glance. `api/mobs/greater-fire-elemental.json`. |
| Fire totem | in-game *(2026-07-21)* | System / summon | The player-summoned totem behind the FireTotem cooldown — **never zone-spawned**. Totem precedent exactly (speed 0, layer 160 / mask 16, non-blocking); its aura burns **all** enemies in range, which is what separates it from the single-target plain Totem. `api/mobs/fire-totem.json`. |
