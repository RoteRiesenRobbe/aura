...
| Reinhard *(was `Farmer`)* | in-game *(C1)* | Zone 1, farm field south of the village | Teaches **Harvest**; offers and turns in `turnip-chore`, `boars-in-the-field` and `giant-rats-in-the-barn` (the first **three**-offer giver). | The peasant onboarding anchor (GDD §5 as amended in C1) + first harvest-mob loop (GDD §8). Harvest is *taught*, not granted — a fresh spellbook holds exactly the level-1 milestone (Damage, Q4). |
...
| Miller | authored, unplaced | Zone 1, the mill on the river | Teaches **nothing** (the TownCrier/Shepherd precedent); offers and turns in `the-sounder-at-the-mill`. | The Mill POI is a soft dead end (content-zone-design-guide.md §2.4), so he IS the reason to walk down there. Sprite `entityType: "Farmer"` — a deliberate reuse, as for the Shepherd, until the Zone 1 art pass. ⚑ **Unplaced:** the PO places the spawn in the editor. |
| Farmhand | authored *(placement rides the PO's zone pass)* | Zone 1, the farm | Teaches **nothing** and asks for **nothing** — one root node, no options. | ⭐ **The first NPC in the game that WALKS.** Every conversant before him authors `speed 0` under D4; D4's point was that an NPC is an ordinary actor, so content could later give one a walk without changing what it is — this is that content (speed 0.7 on a boustrophedon waypoint route, so he works the field in rows). ⚑ Hence `aggroRadius 3` rather than the standing 1.0: a man crossing a field should notice you before you are close enough to talk. ⭐ Also the only conversant with no options at all (the `baabara.json` terminal-node shape) — Reinhard on the same field already carries two quests and a teaching, so a second giver here would be noise. Sprite `entityType: "Farmer"`, the same deliberate reuse as the Shepherd and the Miller. |
| **Eliza** | authored *(placement rides the PO's zone pass)* | ⭐ Zone 1, the homestead — **the first NPC in the game** | Teaches **FirstAid** on the turn-in row; offers and turns in `dinner-for-the-family`. | ⭐ The opening arc (`content-zone-design-guide.md` §2.0). FirstAid is the dinner's **reward**, not a `Teach me something.` row — the-lost-lamp/Lantern shape, so the mother patches you up *after* your first hunt. Her `quest_at_stage … completed` greeting sits **above** root (L3) and is the handoff that points east to Reinhard. Sprite `entityType: "VillageHealer"`. ⭐ **Has a voice** (PO 2026-09-23, an exception to the plain-text pass): warm, and worried about you leaving home. ⚑ **Unplaced.** |
| **Hendrik** | authored *(placement rides the PO's zone pass)* | Zone 1, the homestead | Teaches **Wild** @L1 — the game's **first taught combat skill**. Carries **no quest rows at all**. | ⭐ The player's father. Wild had **no source anywhere in the game** before this, so the arc places a finished orphan. A fresh spellbook holds only the L1 `Damage` milestone, and Wild is a *wider ring bought with resource cost* — so the first lesson taught is a **trade**, not a bigger number. ⚑ No quest wiring needed: the `gather` stage carries a `talk_to` objective on him, satisfied by opening the panel. Sprite `entityType: "Wanderer"`. ⭐ **Has a voice** (PO 2026-09-23): stern, just and wise; an ungated `advice` row on root. ⚑ **Unplaced.** |
| **Benjamin** | authored *(placement rides the PO's zone pass)* | Zone 1, the homestead | Teaches **Harvest** @L1. Carries **no quest rows at all**. | ⭐ The player's uncle, and ⛔ **the first conversant the player cannot skip**: a Beet authors the Turnip's harvest lock, so the 6 beets are *unfinishable* until Harvest is learned. Reinhard down the road still teaches it too — kept on purpose so a player who somehow arrives at the farm first is not stuck. Sprite `entityType: "Farmer"`. ⭐ **Has a voice** (PO 2026-09-23): quirky, gives life and harvest advice, and ends every piece of it with *"Always remember this."* His `beets` node is where the arc explains switching the active aura to Harvest; an ungated `advice` row on root. ⚑ **Unplaced.** |
...
| Reinhard *(was `Farmer`)* | `village-welcome` | a talk_to target |
| Reinhard *(was `Farmer`)* | `turnip-chore` | offers and turns in; the **Harvest** teaching the objective needs sits behind his root `Teach me something.` row, and the quest brief points at it |
...
| Reinhard *(was `Farmer`)* | `boars-in-the-field` | offers and turns in (his **second** quest — the first two-offer giver); 6× Boar L2, 180 XP. Generic kill quest (plan-generic-kill-quests.md C1, 2026-08-07); rewards follow the L9 half-level rule |
...
| Hunter | `stags-for-the-table` | **turns it in**, and offers nothing; 3× Stag L14, 1600 XP (the L9 half-level rule at L14). ⚑ The inverse of every other wiring here: the askers and the payer are different people |
...
| Miller | `the-sounder-at-the-mill` | offers and turns in; **1× AlphaBoar L6**, 370 XP — ⚑ **the only kill quest in the game with a count of 1.** Every other one is a cull; this is a named fight, which is how the player learns an elite is a different kind of thing rather than more of the same. The elite premium is in the drop table (ThickHide + Hardy), not in the XP: the L9 half-level rule prices the TARGET'S LEVEL, so it pays exactly what the Lamplighter's three dire wolves pay |
