...
| `strike` `bite` | ONE jaw, drawn twice (the second mirrored), **hinged at the attacker** and closing over the victim | the **upper** jaw, **hinge at the left edge**, snout pointing right, teeth pointing down, **bite line on the bottom edge** | 128 × 48 |
...
of the canvas. The **hinge is the bottom-left corner**: that corner sits at the
attacker's mouth and never moves. The engine takes a second copy, mirrors it in
Y below the bite line, and rotates the pair about that corner from open to shut
over the layer's `ms`, reaching out over the victim. Nothing may hang below the
bite line, and nothing may sit left of the hinge.
...
**Code owns the motion.** `thrust`, `swing`, `overhead` and `bite` for a
...
| `spider-fang` | `strike` / `bite` | SpiderBite - Spider, 17 placements | 1 | P1 |
...
three under the same names**, and nothing else changes.
