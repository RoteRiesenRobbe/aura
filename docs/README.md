...
- **plan-xp-formula.md** — **designed 2026-08-05, not started**: kill XP stops being a flat authored number and becomes a computed, level-relative award — WoW-Classic-shaped, **anchored to the recipient's own level** (`killXPBase × killXPGrowth^(L−1)`, the exponential the sim's `XPModel` already carries), a bounded +5 %/level bonus for higher mobs, a linear taper to **exactly 0** for gray ones, per participant inside the existing reward fan-out. Closes both exploit ends: pull-through (a bystander at an endgame kill gets a bounded multiple of *their* at-level pay) and endless gray farming. 3 PO rulings, 5 landmines; `factors.experience` → relative `factors.xpFactor` (the Session-⑥ kite rule survives as `0.5`), quest `grant_xp` deliberately untouched. **No migration, no wire change.** ⚑ It is an **economy retune**, not just a patch — the authored kph-calibrated values stop being a balance input, and endgame pacing changes either way (§8.1 is the C2 PO pacing call)
- **plan-mob-levels.md** — **designed 2026-08-05, not started**: backlog §38 ratified — a spawn point may author an absolute `level`; the mob placed there stands at it, HP/damage/kill-XP all following automatically because all three derive live from `Mob.Level()` (entity-model 1b did the hard half). 4 PO rulings (absolute per-spawn `level` · XP priced by `plan-xp-formula.md`, **zero seam code — the two plans share no code and compose by construction** · CC axis flat for v1 · wolf family stays separate creatures, purely additive). ⚑ **DB none, FlatBuffers one appended field** — the nameplate/tint reads the static catalog today and must switch to the wire value (the real cost, C2). ⚑ Its §6 is the joint-fit analysis with the XP plan, incl. the one genuine shared seam (client gray-tint bands vs. the server's ZD gray threshold) and the one ordering rule (no overridden spawn in a live zone before the XP formula's C1). Unblocks `plan-test-world.md`'s 13–17 band hole as a side effect
