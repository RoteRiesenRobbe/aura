...
- **plan-entity-model.md** — **the Actor model** (backlog §31, designed 2026-07-26, **all chunks + post-review R1/R2/R5 shipped**; stays live for **R4**, now scoped by the PO to exactly two review-found badge fixes): converge the player/mob/NPC stat model onto one Actor + optional capabilities, with an authored `role` discriminator replacing today's inferred roles, and the NPC merge + interaction schema that unblocks WoW-Classic-style NPCs. 7 PO decisions; 5 chunks; **all of it runs before step 8**
