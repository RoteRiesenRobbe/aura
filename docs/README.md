...
- **plan-conversation-journal.md** — **the conversation & journal pass** (planned 2026-07-30 · **Q1 ✅ `d23670d7`, Q2–Q4 open**): the 13 items the PO's in-game walk of quest chunk C4 raised, verified against the running game and sorted into 4 chunks — **Q1 shipped**: the dialogue system (`blockedLine` deleted so a greyed row is simply inert, **a quest row shown iff its ledger op would succeed** via `Ledger.CanApply` — which is what makes an Accept row vanish while its sibling questions stay — talking in combat un-gated, a "Leave." row; ledger §6). Still open: server-composed objective tracking (`3/8 Wolves slain`, Q2), a two-pane journal (Q3), and the content/writing pass (Q4). Its §2 is the measurement record: what looks like a content bug and is not, and what looks systemic and is not
