...
- **plan-world-map.md** — **designed 2026-08-04, not started** (fast travel part 1, closes half of backlog §41): today's minimap becomes the **docked state of one map module**, with a full-screen state on a key + HUD button. Terrain and campfire positions need **no wire work** — the client already bundles every `api/zones/*.json` (`GroundTextureManager`). Contents are deliberately sparse (D6): **discovered campfires only** (undiscovered absent, not dimmed) + **other players** + self, no mobs, no quest markers. ⚑ The one server change is a **~1 Hz player roster**: other characters already reach the minimap, but only inside the 20 × 12 m AOI, so "show other players" is a streaming question, not a rendering one. 3 chunks, **no schema impact**
- **plan-flight-paths.md** — **designed 2026-08-04, not started** (fast travel part 2, closes backlog §41 with **option 2**, flight paths over campfire teleport): dwell at a fire → it becomes a flight node → fly fire-to-fire in a straight line at ~4× walk, zoomed out, aura off, abilities locked, **invisible to the ground but seeing everything below**. 8 PO rulings (complete graph · 2.5× viewport · dwell-discovered · time-only cost · no extra death brake · per-character · committed · disconnect resolves). Cheap for three reasons found in the survey: `Campfire.ID`'s comment **already anticipates** a waystone joining its namespace, `trackCampfireDwell` already fires exactly-at-threshold, and `playerSendState` is already per-viewer. ⚑ **Has a migration** (discovered set, per character) and gives `characters.home_campfire_id` its first writer. 5 chunks
