...
- **plan-pre-accounts-hygiene.md** — **config truth + wire vestiges** (planned 2026-07-28, not started): the last chunk before step 8 — the balancing harness disagrees with the live game about mob stop distance by 4×, the config a fresh deploy boots on is the most stale of five, the NPC-unattackability pin is a tautology, and two dead wire fields drag a whole harvest-era consumer chain. 4 PO decisions; 1 chunk; everything byte-identical except one recorded, signed-off re-baseline
- **plan-entity-model.md** — **the Actor model** (backlog §31, designed 2026-07-26, **all chunks + post-review R1/R2/R5 shipped**; §10 open questions and **R4 (visual polish) still open**): converge the player/mob/NPC stat model onto one Actor + optional capabilities, with an authored `role` discriminator replacing today's inferred roles, and the NPC merge + interaction schema that unblocks WoW-Classic-style NPCs. 7 PO decisions; 5 chunks; **all of it runs before step 8**
- **plan-faction-flips.md** — **runtime allegiance** (planned 2026-07-28, not started): fixes `plan-entity-model.md` L2 (`SetFaction` discards the authored aggro mask) by replacing one undefined setter with two defined verbs, and ships **charm** as its first consumer. 5 PO decisions; 2 chunks; the correctness chunk is behaviour-neutral
