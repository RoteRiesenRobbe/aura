...
- **plan-feel-pass-2.md** — **what came back from the R1–R3 checklist** (**PLANNED 2026-08-01, docs only**): the second PO feel pass on the priced catalog. The R-series landed on the surfaces a player reads (entry pricing generates tactics; one beat reads *"much better and more legible"*), so what came back is **presentation, one live bug and two design questions**. **5 PO decisions** (§4) and **5 chunks N1 → N5** (§5): the Focus bar tells the truth about shields · one cadence line + cost lines grouped by charge trigger · **dots leech** (no dot has *ever* leeched — R3 fixed the two direct damage payload sites and missed the third) · quest progress starts at acceptance (**F4**, reverses `archive/plan-quests.md` D3, and its baselines are persisted state so it wants to precede 8a) · ring pulse + HUD metronome. ⚑ Two items **parked with their measurement** in backlog §37 (§6)
