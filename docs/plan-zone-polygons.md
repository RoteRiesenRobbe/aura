...
- **P2 — the polygon primitive, draw only.** `world.Polygon` + validation ·
  ...
  already owns `scroll`.
- **P3 — filled blocking.** `PolygonColliders` beside `PathCorridors`: the inset
...
- **P4 — outlines.** `outlineProfile`/`outlineWidth` on both types · one shared
