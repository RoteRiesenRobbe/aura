...
function render(rows) {
    ...
    p('');
    p('> ⚑ **`current_file` and `format` were WRONG FROM BIRTH — not drifted.**');
    p('> Found 2026-09-24: **25 rows said `.svg` / `SVG`** — Tree, Boulder, Rock,');
    p('> 13 mobs, Campfire + Camp and 7 NPCs. The painted PNGs had landed on');
    p('> 2026-08-21 (`abe150ac`), a month *before* this tracker was created');
    p('> (`2d99ba95`, 2026-09-16), and both files already sat side by side in the');
    p('> tree at that commit. So the initial rows were sourced from something stale');
    p('> (the old README tables, most likely) rather than from the real load paths,');
    p('> and the error was copied forward. The 25 are corrected; **nothing else has');
    p('> been audited.**');
    p('>');
    p('> ⭐ **Next iteration: assume any hand-typed column may never have been');
    p('> right.** Both columns are still unchecked against disk. The audit is');
    p('> mechanical — parse `client-data/Graphics.ts` + `api/props/*.json`');
    p('> `sprite`, compare, fail `--check` on a mismatch — and it should sweep');
    p('> **every** row once, not just watch future art commits. ⛔ Judge a row');
    p('> against those load paths, never against "a .png exists next to it".');
    p('');
