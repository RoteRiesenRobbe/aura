...
- **Last completed:** Step-4 **chunk 3 (HoT payloads + revive) DONE + VERIFIED IN-GAME 2026-07-14, committed** → `docs/plan-skill-vocab.md` banner. HoT = 3 triggers via `hot_aura` (case-1 lingering, buff outlasts aura cadence) + `instant_hot` (self via `targetsSelf` / allies via `targetsAllies`, mirrors instant_shield); shared `hotPayload`; drain generalized `DueDotHits`→`DueBuffEvents`, `tickDots`→`tickBuffEvents`+`tickHotEvents`. Revive: Viewport-mask `nearestCorpseID` → precondition (reason 2 = no target, no cd) → `ConnState.ReviveAtCorpse(corpseID, healthFraction)` (rebuild at corpse, partial HP, race-safe); death overlay dismiss FREE (same Accept as respawn). Content Rejuvenation(29)/Recover(31)/Revive(32) cheat-granted; registry pin 32. **Prior:** chunk 4 (cast-time + Recall) VERIFIED IN-GAME + committed 4287977e. Key decisions (supersede §3.4/§3.5 where they differ): same-slot re-request ignored, any other deliberate input cancels (different cooldown still fires; aura switch; movement ≠ 0); **damage-interrupt is OPT-IN per skill** (`castInterruptedByDamage`, only Recall sets it — casts are combat vocabulary). Shipped: skill-def `castTicks`(+PerLevel)+flag; SkillComponent casting state + `CancelCastOnDamage`; `advanceCast` (consume at completion only, precondition re-checked there); activation-precondition + rejection-feedback primitive (`model.ActivationRejection`, player one-shots); hooks in `takeDamage` (dealt > 0, absorbed hits incl.)/input path/`SetSkillComponent`; `recall` effect type (17 total) + `ConnState.AnchorOf` seam + `jitterAround`; GameState appends cast_skill_id/ticks_left/ticks_total + activation_rejected_skill_id/reason; `#castBar`, rejection floating text, **teleport snap 600 px in _GameObject** (answers §3.8 for dash); Recall id 28, L2 milestone (12 entries + NEW pinned-table test), registry pin 29. Suite + build + tsc/webpack + boot smoke both content sources green.
- **Next:** Step-4 **chunk 5 = dash** (§3.8) — cooldown displace in the facing direction + stepped-probe collision clamp (no swept-circle in `phy`); a dash activation cancels a running cast for free (chunk-4 hook); teleport-smear already answered by Recall (600 px snap in _GameObject). NEW execution session. `docs/plan-skill-vocab.md` §3.8/§5
