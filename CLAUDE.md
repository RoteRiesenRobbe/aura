...
- **⭐ Last completed: SKILL VFX C2b, the other three kinds + the density slider** ✅ 2026-09-20 `[uncommitted]` (ledger: `docs/plan-skill-vfx.md` §13 C2b, spec §12d + look amendments §12d.7): ⭐ **`cast-pose`, `orbit` and `emitter` are real**, and ⭐ **an AMBIENT RECONCILER** (`SkillFx.setAmbient`, keyed by GameObject like the glows) finally dresses a RUNNING aura: fed from `Character` with `active_skill_id` alone (⚑ NOT interval-gated, or Lantern would never draw) and from `Mobs` with the catalog's new `auraSkillId` when `interval > 0`; ambient layers sit OUTSIDE the 96 budget, by design. ⭐ **Density Off / Low / Full**: `low` = counts ×0.4 + ambient emitters own-character only, `off` = no authored layer at all (glow + numbers stay), mobile default `low`. PO look, several rounds, each rebuilt: ⭐ **the bow moved to `hit`** (only when damage lands, aimed at the victim, ms 500; `cast-pose` is legal on `hit` now, the `HasFired` flips un-done) and ⭐ **a weapon is HELD** (hilt in the hand, length = the skill's reach, through closer mobs BY RULING). ⚑ A placeholder judged by COUNTERS can be invisible in PIXELS: the first emitters passed 13 harness legs as 4 px dots hidden behind the sprite. **Schema DB/wire/conf NONE; CATALOG one field, `auraSkillId` on `/mobs`; content: `visual` on 46 more files + 2 cheat-only skills (77, 78), pin 114 → 116, 129 layers.** Verified: 35 pkgs 0 failures · `-validate` 0 both ways · smoke 0/116 · editor 2/2 · frontend 812/44 · mutation ×5 · real boot `count=116` · `skill-fx.mjs` **13 legs PASS**. ⛔ NOT run: `hygiene-wire-prune`, two-window, loadbot, a phone. ⭐ **PO: "works, I think with this the chunk is done."**
...
- **Prior: SKILL VFX C2a, the engine + four kinds** ✅ 2026-09-19 `512d4afd` (ledger: `docs/plan-skill-vfx.md` §13 C2a, spec §12b + amendment §12c): ⭐ **a `SkillFx` manager on its own layer above `flyers`, below `darkness`**, fed once from `Backend.receiveSnapshot` off C1's `SkillEvent`s, budget 96 + pools, a deterministic math module, placeholder bodies (no atlas yet). Kinds `impact` · `projectile` · `beam` (+ `curve` and a VISUAL-ONLY `chain`), and ⭐ **`strike`, caster-anchored, which the FIRST PO look forced**: the victim-anchored wedge "doesn't work visually as an indicator", so `arc-swing` became `strike` and `impact` became a small opt-in burst (§12c, 28 files re-authored). ⭐ **The `hitStyle` lever is DELETED end to end** (Go + codec + wire + editor + client; `grep -i hitstyle` = 0) and `AuraTickIndicator.ts` is absorbed, so the wind-up glow now draws ABOVE entities. PO calls: author every damaging skill, no engine fallback · a cheat-only Lightning Strike (id 76), pin 113 → 114 · heal/shield/light wait for C2b. ⚑ `width` is PIXELS (the two halves disagreed and shipped a 0.35 px beam). **Schema DB/conf NONE; WIRE `aura_hit_style` deprecated, two slots; content: `visual` on 59 skills / 69 layers.** Verified: 35 pkgs 0 failures · `-validate` 0 both ways · smoke 0/114 · editor 2/2 · frontend 742/42 · mutation ×6 · real boot + join clean, `immune-feedback` PASS · ⭐ new harness `skill-fx.mjs` **7/7**. ⭐ **PO: "works for now as placeholders, ingame look passes."**

### Next

- **⭐ SKILL VFX owes only C3 (art) and C4 (world scale)** (`docs/plan-skill-vfx.md` §9; C0 + C1 + C2a 2026-09-19, C2b 2026-09-20, every one PO-passed): **C3** = the atlas contract doc, the first artist sheets in-repo, the body resolver's ERROR path armed in `-validate`, the editor's Visuals section; every body is a procedural placeholder until then. **C4** = density 10× with the slider at each level, frame time before/after, the cap tuned, mobile fill-rate on a real phone. ⚑ **The ambient layers are UNBUDGETED** (outside the 96 cap by design) and unmeasured: C4's first question. **Schema: NONE.**
