...
- **Next:** **Z2-hardening / cL8-17 farm-band PLAN chunk — new chat, plan-first** (PO 2026-07-20; C8 closed 2026-07-21): Z2 difficulty+density "up by a lot" + remaining high-tier farm content design; the farm-band pre-chunk (2026-07-21) already delivered the first three band normals + the guardrail band-check + the **gray-band decision (flat XP kept — decision CLOSED, camping accepted)**, so the plan chunk's remaining scope = Z2 difficulty/density design, more band content, elite/farm-area structure. **New mobs inherit the Session-⑥ XP rule** (derivation recipe re-validated in the pre-chunk: facetank kph while viable, else kite ×0.5). PO continues manual placements/corrections in the editor (campfires, density, road corridor, post-intro polish — crier/healer/farmer/field + farm-mob coords are first-pass). **Reconnect-token persistence** (reload loses character) = new small plan-first chunk when PO calls it. **Every C1+ mob MUST be authored tier + baseline** (raw `maxHealth` hard-fails; manual §1); combat mobs need NO harvest entries (gated damage tags — manual §1), but **gate obstacles must opt in**.
