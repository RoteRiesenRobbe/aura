...
- **Last completed:** **Step-7 B+C structural rename + branding — DONE + PO-verified in-game 2026-07-21, committed `aa509d95` (one atomic commit); full ledger: `plan-rebrand-cleanup.md` §5 banner. STEP 7 COMPLETE — the repo is now "Aura" end-to-end.** **Naming set (PO choice prompt at session start):** module path **`github.com/RoteRiesenRobbe/aura`**, package dir **`pkg/aura/`**, binary **`aurad`** (`cmd/aurad/`), FB namespace **`AuraApi`**, title **"Aura"**. Go module swept across all import sites + dirs git-mv'd; env var `BERRYHUNTERD_CONF`→`AURAD_CONF`; FB bindings regenerated (Go `pkg/api/AuraApi/`, TS `api/schema/js/aura-api/`, frontend barrel `AuraApi.ts` + 15 importers); Makefiles/goreleaser/CI/devops (`aurad.service`) renamed; dead root docker targets + 12 dead root scripts deleted; branding (startScreen `<h1>Aura</h1>`, webpack title/appName, `aura-frontend`, changelog header, credits, README rewrite); living docs + 4 project skills swept, "don't rename proactively" rule retired. **Kept by design:** berryhunter.io domain URLs (no replacement domain decided), Kringel social/rating links (A.2 ruling), historical docs/changelogs, mascot/splash/favicon art (**PO art call — drop-in paths: `frontend/src/features/user-interface/start-screen/assets/hunter.png` + `loadingScreen.jpg`, favicon source `frontend/src/features/user-interface/assets/logo.svg`**), wiki-generator (dormant — open PO keep-or-delete call). **Verified:** full suite `-count=1` green (29 pkgs), `make -C backend build` clean end-to-end, boot world + proving-grounds **0 warnings** counts unchanged (`78 skills/13 factions/47 mobs/10 recipes/856 props/349 spawns/5 campfires/14 npcs, 0 panics`), `tsc` + webpack prod clean, headless Playwright smoke, PO played in-game. **Watch:** webpack **config** changes (title/favicon) need a dev-server restart — HMR doesn't cover them.
