...
- **⭐ Last completed: conversation chunk Q4 — the content restructure ✅ DONE 2026-07-30 `3ccadb5e`, closing `plan-conversation-journal.md` (COMPLETE, ARCHIVED — ledgers `docs/archive/plan-conversation-journal.md` §6).** One PO ruling at chunk start: **the Miner is DROPPED** (§7 q3) — the lamp quest is the three-stage **"The Traveller's Lamp"** (retitled; he HAS a lamp and is done travelling, which is how the Lampless name survives), both kobold Lantern drops deleted, so **the turn-in row is Lantern's only source in the world** (L5b, pinned by the new `TestContent_LanternIsQuestOnlyAndHasASource`; the non-terminal-edge test left content with the Miner). **Every conversant is on R1's tree shape** — greeting at root, teachings behind named rows, each quest behind its own row with its brief as that node's text — with **zero `quest_at_stage` gates on quest rows** (the Q1 show-rule carries the whole lifecycle; the only conditional node left is the traveller's completed greeting, and the §15 greeting-hijack pattern is retired from content AND `manual-content-authoring.md` §6). Quest prose cut ≥50 % with the Q2 trackers authored (`{n}/{m}` plural overrides on the three count stages, "Return to …" on the four dialogue stages). **Damage is a level-1 milestone seeded at character creation** (`applyCreationMilestones()` in `player.New`; the crier teaches only Recall now) — ⚑ **seeded SILENTLY, a deliberate deviation from §4.6's parenthetical**: the client banners every Unlock message and all three respawn/reconnect paths overwrite the spellbook right after `New`, so a banner-sending call would flash "Level 1 reward" on every death; the silence is pinned by `TestNew_AppliesLevel1MilestoneAtCreation`. **Verified:** full Go suite + vet · guardrails + alloc `-count=2` · 84 vitest + typecheck + prod build · boot embedded AND `-content ../api` identical — 86 skills/15 factions/64 mobs/10 recipes/**2 milestone unlocks**/4 quests/777 props/485 spawns/5 campfires, 0 errors 0 warnings · harnesses each SOLO on a fresh server: **`chunkC4-quests` rewritten with the chunk (L2) 38/38 + 1 deliberate SKIP** (new legs: Damage-at-creation · Accept-vanishes/turn-in-appears · the four trackers · crier-teaches-no-Damage; ⚑ it was still reading the **pre-Q3 journal DOM** — Q3's gate never carried this script, the L2 lesson again) · `chunkC3-journal` **29/29** (half C now proves the `{n}/{m}` substitution live on "wolves slain") · conversation **31/31** · interact **14/14**. ⚑ One §29 ctx-loss run discarded per the standing rule; the fresh-server re-run was clean.
...
- **⭐ NEXT — step 8: accounts & persistence, opened as a DESIGN session, not a schema session.** The conversation pass — the PO's chosen precursor — is **closed**: Q1–Q4 all shipped 2026-07-30 and `plan-conversation-journal.md` is archived (see Last completed). Fold in **backlog §32** (does a charge survive death — the question most likely to want schema room), **backlog §36** (3 character slots / 3 bloodlines ⇒ the top-level persisted record is an **account**, not a character, from the first migration), and `plan-playtest-deploy.md` §Ops & security posture (persistence is the security tipping point). Ruling 4 (*only live players persist*) fixes the scope; the character record is written in **Actor vocabulary**. Detail in the step-8 bullet below. ⚑ The scoped-filler queue is EMPTY.
