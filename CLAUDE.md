...
- **Last completed: `plan-entity-model.md` Chunk 1b — dynamic levels + the summon-scaling collapse** (2026-07-26, backend + 6 content JSONs, 30 files + 1 new) ✅ `[uncommitted]` — **the chunk where numbers move**, and ⭐ **the numbers were priced BEFORE the first edit and PO-signed at design time** (2 choice prompts). Full ledger `docs/plan-entity-model.md` §11. **`MaxHealth()` is now fully derived** — `HP(baseMaxHealth × f(Level) × MaxHealthFactor())`, the same three factors the player's pool has — and **`Level()` returns the OWNER's current level when owned** (PO: track **live**, not snapshot at spawn). The registry stops pre-deriving: `Factors.MaxHealth` → **`Factors.BaseMaxHealth`** (authored baseline verbatim) and the frozen **`MobDefinition.PowerScale` is deleted** — `Curve` + `CurveLevel` already carry it. `RaiseMaxHealth` gone, `RestoreToFullHealth()` at the spawn site. **⭐ The finding that made the sign-off easy: summon OUTPUT does not move at ALL** — all 6 summon mobs are `curveLevel: 1`, so `f(1) × SummonPower × f(ownerLevel)` becomes `f(ownerLevel) × SummonPower` exactly (verified L1…L30: 1.0000 / 4.0210 / 65.5373 both sides); **L3's double-apply is avoided by REMOVING one factor from `casterPowerScale`, not by retuning**, and `SummonPower` survives as the linear per-skill knob. **The delta is HP, and it is large by design:** flat `+2/owner level` → the curve, so a Companion goes 118 → **1605** at L30 (a constant 60 % of a same-level player's pool at every level, vs ~4 % before). PO decision ①: **pure curve, authored bases unchanged**, and **`maxHealthPerOwnerLevel` RETIRED** from the schema + the 6 summon skills (rejected by the loader with a migration hint — the `factors.maxHealth` precedent; an ignored authored key is worse than no key). **4 decisions inside the chunk:** ① the **variance roll moved onto the BASE pool**, stored unrounded, so variance and curve compose in either order and the pool rounds exactly once; ② **`Level()` derives from the owner rather than a synced field** — a settable field would need a system pushing level-ups into every summon each tick and has no consumer today (YAGNI); ③ the **shrink clamp lives in `Update`** (growth deliberately NOT mirrored — health stays absolute and regenerates up, like the player's); ④ the spawn site refills **explicitly**, since `Heal` would pop a floating number on every summon. ⚑ **One trap only a diff caught:** `mobSpecOf` must now apply f(curveLevel) **and round it**, or presets model pools the server cannot spawn. Verified: TDD **red-first on all 4 behavioural pins**, each re-checked by reverting its mechanism; `go build`/`vet` clean; `go test` **exit 0, 27 pkgs**; simharness + **alloc** guardrails `-count=2`; boot `-content ../api` **0 errors 0 panics 0 warnings** (83 skills/14 factions/50 mobs/10 recipes/5 prop defs/1 milestone/777 props/471 spawns/5 campfires/14 npcs). **⭐ Sim battery, level curve, pack matrix AND the 50-mob preset roster BYTE-IDENTICAL to a HEAD build** (`git worktree`, not a remembered number) — expected, and it is evidence: **every number that moved is a summon number**. In-game smoke clean (0 console errors, 0 ctx losses, pack chases, bars render).
