...
- **⭐ Last completed: THE UNDERWORLD — U1→U4b, the first PLACED zone** ✅ 2026-09-08 (ledger: `docs/plan-underworld.md`; engine half committed `78a274bd`, U4b uncommitted): a zone file may now be authored around its own `{0,0}` and given a runtime **`origin`** that moves it far away in the SHARED coordinate space, and a **`CaveMouth`** interact row walks you between them behind a **directional curtain**. ⭐ **There is still ONE `phy.Space`, one process, no handoff** — zones are disjoint by **DISTANCE**, which the sparse hash-grid broadphase, the border walls, AOI streaming and every aura overlap already respect for free. This **amends `plan-release-map.md` D6 in its file-count clause only**; §8.3 (separate Spaces, handoffs, sharding) stays out of scope. ⭐ **The rejected alternative is recorded: a `worldLayer` tag with overlapping coordinates** — nicer to read, buys nothing the offset does not, and costs a new orthogonal filter axis every AOI query, aura overlap, targeting scan and broadphase insert must honour. ⭐ **U3b is the design change worth remembering: the destination moved from the mob DEFINITION to the PLACEMENT** (`spawns[].anchor`) — a def-level destination means one file per passage END, so two passages wanted four near-identical mob files; `plan-world-paths.md` D4 had already refused that exact shape. ⭐ **`Zone.Origin` is now an AUTHORING CONTRACT, not a free packing coordinate: +Y IS DEEPER** — U4b derives a row's direction byte by comparing the two zones' `OriginY`, so ⛔ **packing zones down Y for tidiness would make every crossing a descent**; a neighbour belongs in X. ⚑ **`ConversationOption.travel:ubyte` is DERIVED, never authored, and `anchor` mode ONLY (L15)** — `home_campfire`/`caster` resolve at step-through time by design, so a build-time direction can be stale; they report lateral and the client **upgrades on arrival**. ⚑ **Two boot-time landmines fixed that only exist once a second zone loads**: `startingSpawn` moved per-FILE → per-SET **and primary-zone-only** (else a fresh character spawns underground), and campfire ids are unique **set-wide** (`character_campfires.campfire_id` is bare TEXT — `plan-world-scale.md` §7, filed and unowned, now owned). ⚑ **A FOURTH zone-format writer exists and the completeness pin cannot see it**: `aura-world-format.js` copies map-level values onto Tiled's `TileMap` **by hand**, so a Tiled save silently dropped `origin` and the next boot refused; `verify.sh` now has a placed-zone leg (⚑ its fixture needs BOTH axes non-zero — `{0, 300}` passes its own mutation). **Schema: DB NONE · WIRE 2 appended (`Welcome.zone_names`, `ConversationOption.travel`) · CONF `game.zones` (singular kept as fallback) · CONTENT `Zone.Origin`, `Spawn.Anchor`, 2 mob defs.** Verified: build · vet · **`go test -count=1 ./...` EXIT 0** · tsc · **vitest 663/663** · prod build · `verify.sh` green · **mutation-verified ×4**. ⛔ **NOT in-game verified by me** — and three browser-only defects already came out of U2, which shipped with no in-game pass (a fog mask destroyed with its terrain layer, an unconditional `snapshot.player.position` deref, and a camera clamp that took a SIZE where it needed a RECTANGLE). ⚑ **OWED: U5 content** (the PO has begun — 12 spawns authored), **U0** (S1 lazy zone bundling), the **D6 amendment note** in `plan-release-map.md` §8, and **Q5**: the curtain now fires on the *shipped* portal pair and campfire recall, which were hard cuts.

- **Prior: WORLD PATHS C3 — animated water** ✅ 2026-09-07 (ledger: `docs/plan-world-paths.md` §12 C3): a profile can author **`scroll: {x, y}` in world units per second** and its TILE drifts — a `TilingSprite` over the mask footprint whose `tilePosition` is nudged per frame. ⭐ **The whole per-frame cost is two number writes per drifting surface**: nothing is rebuilt, the tile offset is a uniform. ⛔ **Never on the day/night filter machinery** (region-primitive L7). ⚑ **The sketch in §4.5 was wrong in one way and the fix is the lesson: "swap the blend path's node" would have made `scroll` a silent no-op on a `blend: 0` profile** — a drifting surface needs a mask to have a SHAPE, so an unfeathered one now gets a CHEAP mask (plain silhouette as stencil, no RenderTexture, no blur). All three cases (still-hard · still-feathered · drifting) collapsed into one `paintSurface`, which is what lets a **region** drift too — a lake is a `Water` region and animates free. ⚑ **`paintTerrainSurfaces` now returns `{masks, scrollers}`**; `MapTerrain` drops `.scrollers` DELIBERATELY (one still frame; a drifting river is a still river on the map). ⚑ Three traps pinned in comments: the `paused` guard first (else the river jumps by the whole pause), `regionScrollers` REPLACED not appended (the old sprites are destroyed), and `scroll` on a `texture: null` profile animating nothing on purpose (D14's fallback is a colour, and a colour has no phase). ⭐ **C3 was recorded as "blocked on art" and had silently unblocked**: `tools/make-water-tile.mjs` shipped the tile with the C1 Tiled fix. **Schema DB/wire/conf/content ALL NONE** — `profiles.json` is client-side by D12, so no `cp-defs`, no zone touched. Verified: tsc · **vitest 620/620** (+18) · prod webpack build · **mutation-verified ×2** (the authored-`{0,0}` keep, and the shared-default aliasing). ⛔ **Not in-game-verified by me**; the pixi half is untestable in vitest by the same split that leaves `buildBlendMask` untested.

- **Prior: WORLD PATHS C1 + C2 — the stroked polyline primitive** ✅ 2026-09-07 (ledger: `docs/plan-world-paths.md` §12): `zone.paths` — open POLYLINES naming the SAME profile a region does, stroked `width` units across as roads and rivers, and blocking movement when authored. ⭐ **The sibling of the region primitive, not a new system**: a region is a closed polygon FILLED, a path an open polyline STROKED, and Pixi 8's `StrokeStyle extends FillStyle` let `regionPaint()`'s `{texture, matrix}` feed `.stroke()` unchanged — profile table, D14 fallback, blend mask and map parity all reused, with ONE generalization (`buildBlendMask` takes a silhouette CALLBACK). ⭐ **Why it exists, measured: roads today are 372 hand-stamped `Sand` blobs, 69 % of world.json's 537-entry terrain array.** ⭐ **The design changed mid-plan and that is the lesson: bridges are PROPS, not paths** (PO) — which DELETED the first draft's last-wins clearing rule between paths, because props are a separate array so *any* prop whose DEFINITION authors `crossesPaths` clears wherever it sits. ⛔ The rejected alternative is recorded: authoring a GAP in the river fails structurally — the gap must exceed the river's width, and the deck the gap, so **a narrow bridge over a wide river becomes inexpressible**. ⚑ **Blocking is per-placement (`blocksMovement`, the word props already use), never per-profile** — the profile table is client-side by D12. ⚑ Corridors are `SolidRotatedAABB` chains on the border wall's own `AddStaticBody` seam, ⭐ **deliberately OFF `LayerViewportCollision`** so they never stream (zero wire cost); `maxCorridorSegment` caps them because the broadphase inserts by AABB. ⚑ **Sampling finds the bridge gap, it does NOT set body count** — an unbridged 40-unit segment emits ONE rect. ⭐ **The completeness pin worked as designed**: adding the field reddened all three legs before either writer was taught. **Schema DB/wire/conf NONE**; one zone array + one prop-def field, both absent-safe, so the feature is **inert at HEAD**. Verified: build · vet · **`go test -count=1 ./...` EXIT 0** · tsc · **vitest 597/597** · **`tools/tiled/verify.sh` all green** (byte-identical 263 581-byte round-trip) · bridge clearing **mutation-verified**. ⛔ **NOT verified in-game, deliberately: no zone authors a path yet.** ⚑ **OWED: C4** (re-author the sand roads — PO-ruled yes, a content judgement) and **C3** (animated water), C3 ⛔ **blocked on art** — `Water` authors `texture: null` and scrolling a flat colour is invisible.

### Next

- **⭐ THE UNDERWORLD owes U5 (content) + U0, and one PO look** (`docs/plan-underworld.md`): the engine is done end to end — U5 is **filling the room** rather than building it, and the PO has started (12 spawns authored in Tiled). ⭐ **The single biggest perf decision left in the feature is a CONTENT one: cave walls as blocking `paths`, not props** (§7.1 item 1). ⚑ **Q5 wants eyes before anything else**: the curtain is derived from the grant kind, so the **shipped** portal pair and campfire recall stop being hard cuts and get a lateral crossfade — free, probably an improvement, but a change to something already PO-verified. ⛔ **Nothing in U1→U4b has been walked in-game by me**, and U2 alone produced three browser-only defects. ⚑ **U6 = build-time zone placement is DESIGNED, NOT BUILT** (§7.2, PO-asked 2026-09-08): an authored `depth`, an auto-assigned origin, and a GENERATED placement file both sides read so the ⛔ **wire cost stays ZERO** — which is the constraint that picks the design, because a BOOT-time packer would force the origins onto the wire. ⚑ Deferred with two named triggers: a dungeon growing and forcing neighbours to be re-spaced, or the zone count passing ~10. ⭐ It REVERSES U4b's "no separate depth int" on purpose, and the reversal is sound: once the geometry is derived FROM the depth, the two cannot disagree. ⚑ Also owed: the **D6 amendment note** in `plan-release-map.md` §8 (✅ done 2026-09-08), and **U0** = `plan-world-scale.md` **S1** (lazy zone bundling — ⭐ its `ensureZoneLoaded` await seam is *exactly* what the curtain now hides, so the curtain is the load screen S1 always wanted).
...
- **⭐ SPELL BUILDER designed 2026-09-08, C0 next when the PO opens it** (`docs/plan-content-editor.md` **Part B**, PO-asked same day; ⚑ folded into the content editor's own plan, whose Part A had shipped 2026-08-27/28 without a ledger or an index line - fixed today; ⛔ **`plan-content-tooling.md` ARCHIVED as superseded (D10)**, the web editor is THE content tool, its survivors moved here): a **Skills tab in `tools/content-editor/`** over the loader's 34-type / 81-key vocabulary, writing `api/skills/<kebab>.json` directly. ⭐ The PO's example spell (fire vulnerability + fire damage, capped targets, authored cadence) is authorable TODAY as one file, and the per-type field table already exists in Go (`effectKeys`), so **C0 = `api/skill-vocabulary.json` pinned by a Go golden test** and the form renders from it (⛔ never a hand copy; per content kind, so a mob fixture can follow). ⚑ **Re-rules `plan-content-tooling.md` D7**: skills are human-authored in the tool. ⚑ **VFX = disabled "coming soon"** (no ruling exists); `hitStyle`, the parked `projectile` type and the dead `legacy` flag are out with it (§B4.8). Schema NONE. ⭐ **D9: a save is validated by the REAL Go loaders** (new `aurad -validate -content <dir>` mode, C2; no DB, all findings, the editor runs it on a temp copy) - no JS port of the ~70 skill rules. **C5 = the D5 registry lock** (tombstones, boot-enforced). 10 rulings in §B4.1, 8 unprompted proposals in §B9 (PO may veto). Six chunks C0-C5.
