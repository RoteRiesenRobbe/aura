...
- **⭐ Last completed: ASCENSION follow-up ① - THE ROW SAYS WHAT THE ABILITY DOES (`plan-ascension.md` §13.7 item 3 → ledger §13.9)** ✅ 2026-08-11, `[uncommitted]`: hover any conversation row that names an ability and read the spellbook's own tooltip, **on locked rows too**, which is the point, since a gate you cannot read the reward behind is indistinguishable from one that is merely hard. Ascension rewards first, then the `teach_skill` rows the same day on a PO ask, for **one field** and no second wire/codec/client change. No PO calls on the design (already taken). **Schema: DB NONE · FlatBuffers ONE APPENDED (`ConversationOption.skill_id:ushort`, field id 8, nothing renumbered) · conf NONE · content NONE.** ⚑ **THE NEXT DEPLOY IS A BOTH-SIDES ONE** (`devops/deploy.sh`, not `--content-only`) with a hard reload for connected players: a new client against an old server reads 0 everywhere. ⛑ **The teach half surfaced the C3 display-name defect in its SECOND home** (`presentOptions` derived from the raw name instead of `Display()`, dropping authored overrides; latent, since no NPC yet teaches one of the five skills that author one). ⚑ **A defect fixed at one call site is a defect CLASS until the others are swept**: the grep found four more, all correct. ⚑ Always the **level-1 preview**, never HUD's live `skillLevelOf`. ⚑ Locked rows carry `cursor: default` and no hover highlight but **not** `pointer-events: none`, one property from silently failing on exactly the rows the feature exists for. ⚑ **A quest row's reward stays unbuilt and is a PO call**: one row may pay several grants. Verified: TDD red-first on all three surfaces · **1 mutation, caught** · Go 0 FAIL (`-count=1`) bar the known `TestDwell` flake · vitest 264/264 · **`c2a` 27/27** (was 23) · **`chunk3b-ii-conversation` 28/34** (+3 over its 25/31 baseline, same six fails) · `c3-memorial-catalog` 14/14 · `chunkC4-quests` 37/37 · `hygiene-wire-prune` clean.
- **Prior: ASCENSION C3 — THE MEMORIAL AND THE CATALOG; `plan-ascension.md`'s LAST CHUNK (§13, seven steps)** ✅ 2026-08-11, `b995061b` — the loop is content-complete: `api/ascension/` holds an **eight-entry seed catalog** (five newly authored skills + a combination, ids 141-146, every one a verbatim re-type of a named parity source), the **memorial** stands beside the stone listing every character anyone has spent, and `kills_this_life` shipped with its evaluator on BOTH surfaces. **Schema: DB NONE · FlatBuffers NONE · conf NONE.** Four PO calls (D25 one global list with your own names marked · D26 the catalog's shape · D27 the hunt threshold · D28 reuse the locked row style), plus a same-day feedback pass (§13.7). ⛑ **THE CATALOG'S CONDITIONS WERE VALIDATED BY NOBODY**: `CrossValidate` walks mob nodes and stops, `ParseCondition` holds no registry, so a typo'd quest id or species parsed green and rendered **locked, unpickable and indistinguishable from a gate that is merely hard**. Now a constructor parameter: an unvalidated catalog does not compile. ⛑ **The row source re-implemented the server's OWN display-name rule** and dropped authored overrides (`Rime-Burst` → `Rime Burst`) — a C2a defect with no visible subject until C3 authored one. ⛑ **CONTENT MUTATIONS NEED `-count=1`, MEASURED** (five reported as surviving, all five caught once the cache was bypassed); the standing rule is in Gotchas below. ⚑ P10's "one hook, two consumers" held and cost nothing: the mux **is** a `RowSource`. Verified: ~90 pins, **36 mutations all caught** · Go 0 FAIL · `-race` clean · `db-test` green · boot 0 WARN/0 ERROR (95 skills / 67 mobs / 11 recipes / 8 rewards) · batteries byte-identical and parity **measured** · **`c3-memorial-catalog.mjs` 14/14** + `c2a` 23/23 + `c2b` 12/12. Ledger: `docs/plan-ascension.md` §13.8.
...
- **⭐ backlog §52 — LEAVING THE WORLD WITHOUT A RELOAD, now PO-asked (2026-08-11).** ⚑ Not ascension-specific: **every** exit is a page reload today, for one reason — *the client is built to boot once and has no teardown path*. Two shapes (`plan-ascension.md` §13.7): **(A)** a real teardown (dismantling the PixiJS scene and everything holding a reference into it — leaked state, lost GL contexts); **(B)** reset-not-teardown — keep the booted app and its GL context, make only the re-runnable parts re-runnable (fresh socket at Play, cleared `EntityManager`, the once-only `firstGameState` promise reset). B needs no server change and is plausibly one chunk. **backlog §48** ("there is one join path", `cd8fd4ba`) is blocked on this.
