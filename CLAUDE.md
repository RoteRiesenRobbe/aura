...
- **Last completed:** **Day/night cycle DEACTIVATED — the avatar-invisibility bug recurred. DONE 2026-07-22, `e648ab88`, PO-directed ad-hoc from live play; full ledger: `docs/plan-intermission-triage.md` §Day/night cycle DEACTIVATED.** PO hit it twice on the live server, 100% repro: **local avatar (and its aura ring) invisible with the aura ACTIVE, visible with it OFF — starting when night starts, ending when night ends, and triggering during the day→night TRANSITION**; name/level/HP bar keep rendering throughout (they sit on the night-exempt `namePlates` plate that `6afbee84` introduced). **This supersedes the `6afbee84` verdict** that the aura correlation was "a day/night confound" — it is real and PO-toggleable inside a single night; that fix solved a separate problem and did not close this. **Diagnosis (not proven):** the tint is applied **per layer** (`Game.ts` hands DayCycle ~25 containers, each its own `ColorMatrixFilter` pass); it is **not** a tint-to-black — the night matrix is a pure multiply of **×0.65/×0.71/×0.84**, recomputed by hand and confirmed live, so the avatar is not being darkened, it is **not being drawn**; the **aura is the switch** because `AuraRingStack` is the only thing that changes the `characters` layer's bounds (~40×40 px → 2× aura radius) and Pixi v8 sizes that layer's filter render-texture straight from them (`FilterSystem.push` → `getFastGlobalBounds` → `TexturePool.getOptimalTexture`); the **transition is the trigger** because a twilight fade changes `getNightFilterOpacity()` every tick, re-assigning `container.filters` across all ~25 layers at 30 Hz while that texture is being resized (steady day and steady night both skip that branch). **Change is a DEACTIVATION, not a fix — additive, 2 files, +36 lines, 0 deletions:** new `DAY_CYCLE_PRESENTATION_ENABLED = false` in `DayCycle.ts` (clock, `isDay`/`isNight`, dev-panel readout all still run; only the tint + dawn/dusk SFX are skipped) + a pointer comment in `Game.ts`. Untouched by design: `NightVisuals` values, `flood`/`lumaGreyscale`, the derived exempt set, `DayCycleJuice` + rooster/wolf assets, and the server-side `totalDayCycleSeconds`/`dayTimeSeconds`. **Re-enabling is one word → `true`; the first fix to try then is collapsing the ~25 per-layer passes into ONE filter on a single parent container** (also a real perf win). Verified: `tsc --noEmit` exit 0, `npm run build` clean, live probe day→dusk→night→dawn shows `charactersFilters: "undefined"` and avatar visible/alpha 1/bounds 39×39 in **every** phase, visual pass at `00:52 night` renders full daylight colour with the aura ring drawn. **Open:** root cause unproven — it **never reproduced headlessly** (software GL) across many driven cycles, so it looks GPU/driver dependent. Headless traps that cost real time: **`worldAlpha` is not a public Pixi v8 container property** (asserting on it fails in every phase and reads as a repro), and a **1 Hz full-scene `page.evaluate` walk starves the renderer** → solid-black screenshots at *day* as well as night (cross-check a day frame before believing a night one).
- **Prior:** **Load-test skill mode — capacity re-measured WITH combat. DONE 2026-07-22, `facadeee` (+ `1b0dbe73` embedded-content sync), PO-requested ad-hoc; full ledger: `devops/loadtest.md` §Results 2026-07-22 with combat.** **The published clustered ceiling (~80) was optimistic because the walking-bot ramp missed two structural costs**, both found by reading the sim before trusting the number: (a) **no mobs** — bots parked on the spawn campfire, an empty village, so MobSystem AI/aggro/pathing were absent entirely; (b) **aura sensor radius 0** — a fresh bot owns nothing (`player.go:742-751`) and its sensor is built at radius 0 (`player.go:77-82`), `SetRadius` being reached *only* while an aura is active (`skills.go:150-153`), so a walking bot contributes a zero-size AABB to a broadphase whose `bruteIntersectShapes` is O(n²) per cell (`phy/space.go:80-133`) — paid every tick by PhysicsSystem, **not** by the SkillSystem, and paid regardless of `tickInterval`. Measured live (clustered, empty server, `snap/s/bot` @100 bots): **28.7 walking on an empty spawn → 19.7 in a mob field → 17.1 with a Damage aura** ⇒ **mobs cost more than auras**, and the **real clustered ceiling is ~60–70, not ~80** (60 holds a full 30 Hz in all three configs; 80 is already off it once mobs are involved). The old "shave to ~50–70 real players" caveat was a *guess* — now measured, and about right. **The aura figure understates itself:** that run's mob census *fell* 20.7→11.1 per viewport (the auras were killing them) while the control's *climbed* 21.9→35.2 — slower while fighting fewer mobs. Run-to-run noise ~1.8 snap/s (two controls @100 gave 21.5/19.7), so the aura delta clears noise but not by much. **`loadbot` gains a skill mode** (`-token`/`-skills`/`-warp`/`-warpjitter`/`-god`): per bot `Cheat SKILL` → `Equip` → `Input.active_aura_slot`, spaced 120 ms because **every one-shot client channel is depth 2, drained once per tick**; skill ids resolve at runtime from `GET /skills`, never hardcoded. **Two traps that would have silently invalidated a run:** `SetActiveAura` resets `TickAccumulator` on *every* call, so re-sending the activation each tick would pin a `tickInterval 40` aura at 0 and it would never fire (sent exactly once); and gorilla forbids concurrent writers (setup + the 30 Hz input loop now share a mutex). **A rejected cheat token is silent on the wire** (`sys/cmd/cmd.go:296` logs server-side and drops; the client is never told) ⇒ the bot reads its loadout back from its own `GameState` and prints **`auras CONFIRMED LIVE n/n`** — if that reads 0/N the run measured nothing; mob census (aggro proven via `Mob.aura_radius`, `server.fbs:161`) prints for the control too, since an aura-vs-control comparison is only valid if both saw a similar mob population. Verified: `go build ./...` clean, `go test ./...` **exit 0**, boot `83 skills/14 factions/50 mobs/10 recipes/777 props/471 spawns/5 campfires (safeRadius 1.5)/14 npcs, 0 panics` — **counts pin refreshed 2026-07-22**, this run itself measured `82 skills/789 props/457 spawns` (props/spawns had moved 815/399 → 789/457 in the `24d40350` editor pass); since then **skills 82→83** (`78d1629f` gave GiantSpider its own bite) and **props/spawns 789/457 → 777/471** in the PO's new `api/zones/world.json` (terrain also 487→537; all four newly-used types already exist in `Graphics.ts` `groundTextureTypes`, and terrain is client-visual only so it reaches no server validator). **Open:** measured with **Damage L1 only** — one aura slot, no passives/cooldowns/skill levels; aura radius scales with skill level and a wider radius means more broadphase cells, so a maxed build should cost more. **PvP-off rests entirely on `targetsAllies: false`** (`skills.go:444-450`), so clustered players never damage each other — authoring `targetsAllies: true` on a `damage_aura` would silently enable PvP. **The live cheat token was pasted into a session transcript 2026-07-22** (value deliberately not recorded here) — rotate `/opt/aurad/tokens.list` at the next deploy if that matters (a standalone restart wipes characters).
...
- **Prior:** Playtest-1 feedback **Pass B (Communication & small fixes)** ✅ `75486ec9` 2026-07-22, not yet PO-verified — Light→**Lantern** rename, NPC sensor 1.5→1.0 (all 14), alert banner 22vh→**7vh** + `warning` kind (loadout-in-combat moves onto it), NPC bubbles **10 s**, active-voice gated tooltips, `XP 12/300` label, minimap compass, gold+ruled headers, Recover@Shaman + FirstAid@VillageHealer, Warlord banner hint; **items 1c+1d (unlock source attribution) SKIPPED — STILL OPEN**; full ledger: `docs/plan-playtest1-feedback.md` §Pass B ledger.
