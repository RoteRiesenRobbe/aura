...
- **⭐ Last completed: conversation chunk Q2 — server-composed objective tracking (`plan-conversation-journal.md` §6) ✅ DONE 2026-07-30 `1dfb57d8`.** R2 end to end: **`QuestProgress.objectives` appended LAST** (the table is positional — the schema comment now says why), composed in the ledger **event-driven and cached on `Progress`** (L4) — `Snapshot()` copies the slice header and never composes, pinned by `TestObjectiveLines_SnapshotDoesNotRecompose` asserting two snapshots share one backing array. Derived lines ride **load-resolved display names** (`Objective.TargetName` via `skills.DeriveDisplayName`, §35 C3's one path — composition never touches the mob registry): `kill`/`harvest` → `"3/8 Wolf slain"` with the count **capped at the threshold** (lifetime counters keep climbing while a sibling objective holds the stage, D3), `talk_to` ✓-marked once satisfied. **Three PO rulings:** ① **current stage only** (§7.1 closed — a completed quest carries no line, its diary is the record) · ② the authored per-stage **`tracker` override wins, with `{n}/{m}` substituted live** from the first countable objective (the loader rejects the placeholders on a stage with nothing to count) · ③ **mechanism only** — Q4 authors the trackers, so until then a quest resting on a dialogue stage shows no line. ⚑ The §4.4 hand-unmarshal warning held: BOTH client copies edited (`GameStateMessage.ts` + `JournalModel.ts`), render verbatim via `.journalObjective`. **Verified:** full suite + vet · guardrails/alloc `-count=2` · 79 vitest + typecheck + prod build · boot both ways identical (86 skills/15 factions/64 mobs/10 recipes/1 milestone/4 quests/777 props/485 spawns/5 campfires, 0 errors 0 warnings) · `chunkC3-journal` rewritten *with* the chunk (L2) **21/21 on a fresh server incl. the ⭐ real-kill leg (`0/8 Wolf slain` → `1/8`)** · `hygiene-wire-prune` clean after the field append. ⚑ One §29 ctx-loss run discarded per the standing rule; fresh-server re-runs clean ×3. **Knock-on for Q4:** the five non-terminal dialogue stages stay line-less until Q4 authors their trackers — pure content, the guard is live.
...
- **⭐ NEXT — `plan-conversation-journal.md` Q3, then Q4.** The PO's call between the conversation pass and step 8 fell to the conversation pass; **Q1 ✅ `d23670d7` and Q2 ✅ `1dfb57d8` both shipped 2026-07-30** (see Last completed). What remains: **Q3** — the two-pane journal (selection is client state over a per-tick re-send, exactly the `ConversationModel.update()` hazard; a defined fallback when the selected quest leaves the list; the sizing invariant must not mint a third hand-copy of the HUD strip's geometry — §4.5) · **Q4** — the content restructure to R1's shape (nine flat NPCs get real trees, prose ≥50 % shorter, R3's lamp rewrite **plus deleting the Lantern drop from both kobolds** — making the quest the aura's only source, L5b — Damage at level 1 via a creation-time milestone call, **and the five dialogue-stage `tracker` lines Q2's mechanism now waits on**). Q2's tracker vocabulary is live, so Q4 is fully unblocked; Q3 and Q4 are independent and can run in either order. **After the plan closes: step 8, accounts & persistence** (a DESIGN session, not a schema session) — detail in the bullet below. ⚑ The scoped-filler queue is EMPTY.
