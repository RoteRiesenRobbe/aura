...
- **Next:** **PO in-game pass over C4** (dev server left running; village → gates → camp → horde route), then **Step-6 chunk C5 (the front + Front-Aura) — execution session** per `plan-content-zones12.md` §13 C5: `human_army` + `orc` factions (army NOT hostile to `aligned`); **§9 lift 6** (friendly-to-players faction flag — the chunk's one code lift); army soldier XP 0 + Orc (very strong, XP very low); spike barricades; S blocked road + S exit past the orcs; Front NPC (level-gated teaching, TooLowLine, level anchor computed from the locked curve — closes that §11 item); **Front-Aura (§A)** multi-effect deliberately overstrong + sim presets + GDD §4/§5 amendments. Then C6–C8 in order, one chunk per session. **Every C1+ mob MUST be authored tier + baseline** (raw `maxHealth` hard-fails; manual §1); combat mobs need NO harvest entries (gated damage tags — manual §1), but **gate obstacles must opt in**. Standing locks: growth 1.12 × maxLevel 30 (≈27×, band ≈ +5, lower-first); regen slow ~1%/s, tier placeholders normal ≤ ~50% / elite ≤ ~25% / boss dies; downtime 15 s + chain 20; guardrail asserts land in C8 against real mobs. Dev cheats: GOD, WARP <x·120> <y·120>, SPEED [factor|off], XP, SKILL <name>. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; boot `-content ../api` for content iteration (schema/Go changes still need the rebuild).
