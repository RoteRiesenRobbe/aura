...
- **World map C1's tail is now the whole of part 1's remainder:** the mobile real-device pass (§7 requires it; headless perf transfers only as ratios) and the deferred `features/map/` rename. ⚑ C2 added phone surface (the 9 px docked marker) and C3 a second (the 7 px dot); the PO ruled **the desktop pass covers marker legibility** — no separate mobile item. ⚑ Also open from C3: the dot's colour + size are [PLACEHOLDER], and your own dot is invisible while you stand at your bound fire (the map's ruled draw order).
- **`plan-flight-paths.md` restarts at its C2** (the flight state machine): its C1 shipped inside world-map C2, so the next flight session is server-side movement, not persistence.
