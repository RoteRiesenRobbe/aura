...
- **⭐ SKILL VFX is the ranked next chunk** (`docs/plan-skill-vfx.md`, DESIGNED 2026-09-11, D1-D10 ruled, nothing built; the spell builder is complete and archived 2026-09-18): per-ability + per-mob VFX, a closed seven-kind `visual.layers[]` vocabulary, ⭐ honest attribution via a per-hit `SkillEvent` inside the four damage/heal funnels, own-caused numbers only. ⚑ Pulls the per-hit slice + the per-effect-art moratorium out of `plan-entity-presentation.md`. ⚑ C1 owes a loadbot measurement that may veto mob-vs-mob events.
