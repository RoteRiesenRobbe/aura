...
- **Last completed:** **Walking micro-resets → buffered render-delay interpolation (plan-render-jitter chunks 1+A+B). DONE 2026-07-24, committed + DEPLOYED LIVE, PO-validated LOCALLY ("feels good"), live re-feel/re-measure = acceptance.** Downstream follow-up to the input fix: with the input path clean (0 browser starvation) the PO still felt *slight* "stand-still-then-reset" while walking, and it **persisted solo** ⇒ client render side, not the input path. **Chunk 1 — instrument-first (`0e504c22`):** a dev-gated `[snapshot-arrival]` console line (GameState→GameState arrival intervals p50/p95/p99/max), deliberately snapshot-only — the existing `serverTickRate` dev line is time-since-ANY-message, corrupted by interleaved EntityMessage/Pong. Live-measured via `&develop`: **mean 33.3 ms = the true 30 Hz send rate** (the 30.3 seen on localhost was a loopback artifact — this RESOLVED the anomaly and *confirmed* Lever A's constant rather than overturning it), p99 ~40–43 ms, max ~72 ms. **Lever A — rate alignment (`8a29a75c`):** client `INPUT_TICKRATE` + `SERVER_TICKRATE` `33 → 1000/30` (33.333) — removes the per-tick 33-vs-33.3 micro-freeze AND the 10 % input-queue eviction; client-only (never move the server tick rate — it would shift every seconds→ticks conversion 1 %). **Lever B — buffered interpolation (`c5064732`):** replaced the reactive un-buffered chase in `_GameObject.ts:moveInterpolatedObjects` (lerp toward the single latest snapshot from its arrival moment) with the standard buffered scheme — each snapshot stored with client receive time, render `RENDER_DELAY_TICKS=2` (~66 ms, sized from the live p99/max) in the past, lerp between the two bracketing samples; underrun **freezes at newest, never extrapolates**; teleport (Recall/dash/respawn) still bypasses the buffer; `setPosition` seeds a synthetic left anchor on restart-from-idle so stop→start doesn't jump; uniform on players + mobs; fully contained to `_GameObject.ts`. **Tradeoff (documented):** +~66 ms render latency (no local prediction) — the correct, standard PvE choice and the base to extend from if prediction is ever added. **Verified:** `tsc --noEmit` + webpack prod exit 0; headless smokes (rAF shimmed onto setTimeout past the 6 fps hidden-page throttle) — movement tracks 414 px smoothly + buffer trims/settles to 0, teleport snaps 7460 px, 0 new console errors (the pre-existing null.split filtered); **deployed** `aura-game.duckdns.org` on an empty server (pid 54025, bundle `main.ec3610404a…`, boot 5 campfires / 14 npcs, 0 panics). Full ledger: `docs/plan-render-jitter.md`.
- **Prior:** Dropped-movement fix — held-state input model (plan-input-jitter chunks 1+A+B) ✅ `cb7f011f` 2026-07-24, PO-VALIDATED LIVE — instrument-first (per-player `inputstats` counters + starve-run histogram) overturned the TCP-HoL prime suspect ⇒ client-side Tock coalescing; server COASTS the last direction ≤`maxHoldTicks=15` starved ticks + client re-sends an explicit "stopped" input on release (`STOP_TAIL_TICKS=5`, no wire change); live re-measure 0 starved/coasted/stalled. Full ledger: `docs/plan-input-jitter.md`.
- **Prior:** Unlock source attribution (playtest-1 Pass-B 1c+1d) ✅ `2bfee286` 2026-07-24 — the **server** tags each unlock (`EntityMessage.kind=Unlock`, skill id + source label), the client composes `New <category>: <name>` + a source line from the 4 live grant sites (milestone / kill-drop / NPC-teach / recipe), plus an authored optional NPC `name` field. Full ledger: `docs/plan-unlock-attribution.md`.
