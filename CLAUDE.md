...
- **Last completed: §35 C2 — the unknown-key boot warning** (2026-07-29, backend only, `e7531444`) — a conf key that matches no `cfg.Config` field now WARNS at boot, one path-qualified line per key with the fix in the message (*"not a config key — delete it, or prefix it with _ to keep it as a comment"*); warn-not-fail per D2, `_`-prefix exempt per L2. New `cfg.UnknownKeys` (reflection walk of the raw map against the struct's json tags) + the `ReadConfig` wiring. ⚑ **One subtlety the plan didn't name:** `encoding/json` accepts case-insensitive field matches and APPLIES them, so the walker accepts them too — warning on `"Port"` would cry wolf on a key that works (pinned by subtest). Test-first off the exact pre-hygiene embedded conf (`c183ce12^`), which turned out to carry **8 dead keys, not the 7 the hygiene ledger counted**; and the boot-noise check is a **permanent test** (`TestTrackedConfs_HaveNoUnknownKeys` — all five tracked confs report zero unknown keys, so a retired field goes red the moment the files aren't pruned). **Verified:** build/vet clean, full suite green (27 pkgs); all five tracked confs + the gitignored local `conf.json` boot with **0 warnings**; a deliberate stale-key conf **booted to a running server and warned exactly twice**, path-qualified, `_` stash silent; battery default leg byte-identical vs the pre-C1 baseline (ReadConfig is outside the sim path — L9 — run anyway). Full ledger `docs/plan-conf-duplication.md` §7.
- **Prior: §35 C1 — total defaulting + shrink-to-deltas** ✅ `e7531444` 2026-07-29 — the defaulting layer made total (player pair 0.00033/0.05, test-first), env confs shrunk to genuine deltas, `conf_resolved_test.go` pins all five tracked confs + `{"game":{}}` to identical resolved tuning. Three findings beyond the survey: absent `server.port` bound a random port (now defaults 2000, plain-HTTP only) · `server.path` was dead config everywhere it appeared · `mob.healthGainTick: 0.0066667` was a rounded restatement that split the fleet at float32 — production already ran the true Go default; snapped to the exact decimal `0.006666667`, the ONE number that moved (3.3e-8/tick). Battery byte-identical, four variants booted clean. Full ledger `docs/plan-conf-duplication.md` §7.
