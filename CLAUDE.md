...
- **Last completed:** Phase 5 cleanup — all legacy aura code and wire artifacts removed. Gone: `applyDamageAura`/`applyHealAura`, `model.AuraType`, `ActiveAura`/`SetActiveAura`, `PlayerInput.Aura`, all `DamageAura*`/`HealAura*` config fields (structs + all `conf*.json`), the `aura`/`active_aura` wire fields, the `AuraType` enum, the `#auras` buttons + `AuraType`=slot hack + `InputMessage.aura`, the `[SkillSystem] tick` debug log, and dead `equip.go RemovePlayer`. Kept by decision: `aura_radius` on `Character` (now: effective radius of the active aura, 0 = none; fed by the re-sourced `player.AuraRadius()`), and the `-2` deactivate sentinel (**decided: stays** — collapsing onto `-1` is cosmetic-only at real input-path regression risk). ⚠️ The `Input` field-ID shift makes stale client bundles wire-incompatible — hard-reload clients after deploying.
- **The legacy-aura migration track is complete** (1a, 1b, collider re-sourcing, Phase 5). The skill system is the only aura path. **Next: Phase 6 (mob chapter)** — mob migration to SkillSystem, monster-kill unlocks, first JSON-only mob; natural fix window for the zombie-mob bug.
