...
- **⭐ DESIGNED 2026-08-23: the REGION PRIMITIVE** (`docs/plan-region-primitive.md` + `docs/plan-region-audio.md`; nothing built): `zone.regions` — polygons naming a **profile** (ground colour · footsteps · music · later atmosphere). ⭐ `tdd.md` §4.6's 2026-07-09 known-future, and **the same field `plan-release-map.md` §8/D6 specifies**, designed a day apart — D6 stays the ruling, the mechanism moves here (§2 reconciles; a square is a 4-point polygon). 5 PO rulings, incl. **one rule applied per property** (last containing region that declares it) and **`resolve()` is total** (any miss → the shipped default, never silence). ⚑ Schema NONE. ⚑ **THREE zone-format whitelists**, only Tiled's pinned — L1: an in-game save silently deletes an untaught field. ⚑ Widened 2026-08-24 (spike, deleted same day): §4.8 designs an optional ground-`texture` profile property (CC0 pack, polygon-clipped fills keep overdraw at ~1); adoption + blend width are PO calls at the C3 palette sitting.
