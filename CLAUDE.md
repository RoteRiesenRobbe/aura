...
- **Last completed:** **Step-6 C2 PART 1 (wildlife + factions + forest block-out) DONE + VERIFIED IN-GAME by PO (2026-07-17)** — per `plan-content-zones12.md` §13 C2, pre-marked split taken at the checkpoint. **Content:** `wildlife_predator`/`wildlife_prey` factions; Wolf/Bear/Boar/Stag/EliteWolf (tier+baseline; berserker on BearSwipe, execute+lifesteal on EliteWolfBite, first `bleed` tag on BoarGore; drops Swift / ThickHide+BerserkerAura / Hardy+Dash / none / LongRangeStrike; new player skills Hardy, ThickHide = first authored resist_passive, BerserkerAura, LongRangeStrike); NW dark-forest block-out in `world.json` via deterministic scratchpad generator (solid thicket walls + 3 entrances, carved corridors incl. the future bramble-shortcut, clearings, 9 darkAreas INSIDE the treeline, signpost NPC, path extension, open-country scatter — 196 props/48 spawns after the PO density pass ×2). **NPC-sprite lift (TDD):** optional zone-JSON npc `entityType` validated against the enum, `npc.SpriteFor`, editor round-trips it; 9 EntityTypes appended (Wolf…DogNpc, regen both sides). **PO-feedback fixes (same day, verified):** land rect fills exact bounds (beach ring moved OUTSIDE the wall — Game.ts waterMargin); wolf speed 0.7 / elite 0.75 (**PO rule: normal mobs clearly slower than the player; faster reserved for special must-kill mobs**); darkAreas overhang fixed (was reaching ~9u past the treeline → "invisible outside the forest" without light); **input-queue overflow now evicts oldest + merges one-shot commands** (aura switch/cooldowns) instead of blind-dropping newest (client.go pushInput, TDD — was the aura-selector stutter + "Input dropped" flood while moving) + 400ms HUD optimistic-highlight grace; aura tick-glow baseline alpha (no full blink-off); own-avatar darkness hole floored at tiny 40px (PO: darkness stays fully dark); Skills.ts gap fix (TurnipPull id 41 showed as "Skill #41"). Sim presets auto-derive from content (nothing to register). Verified: full suite + race green, sim pins byte-identical, boot 7 factions/196 props/48 spawns/2 npcs, PO in-game pass over all findings. **Prior: C1 DONE + VERIFIED (committed `a494bc26`; backlog §19 audio-memory record split out as `d1bce783`)** — Z1 farm start beat per `plan-content-zones12.md` §13 C1 + story §3 beats 1–4. **Rect-prop lift** (PO-approved add-on): `phy.SolidAABB` static solid rectangle (mirror of the border `InvAABB` — pushout to nearest face/corner, TDD incl. through-Space pins), prop JSON body = `radius` XOR `width`+`height` (hard-fails otherwise), `model/prop.NewRect` (BaseEntity stays circle-typed; Prop overrides body accessors, wire radius = max half-extent), mob steering `boxRepulsion` via exported `SolidAABB.ClosestPoint`, zone-editor rect markers + point-in-rect hit-test. **Content:** `api/zones/world.json` = THE playfield (240×120, W=Z1/E=Z2 design labels; conf default `game.zone: "world"`, proving-grounds stays `-zone` option; frontend auto-bundles) with farm block-out (2 House rect props — new `House`+`Turnip` EntityTypes appended + regen both sides, placeholder SVGs, House render class scales non-square from `api/props/house.json` aspect since SVG rasterization is square) + 16 Turnip spawns + Rübenfeld campfire (= fresh-spawn anchor, spawn-at-campfire default) + Sand path stubs N+E; Turnip mob (first authored `resistances` `{"*":0,"turnip":1}`, speed 0, NO skills — loader fine with empty loadout) + TurnipPull aura (`damageTags: ["turnip"]`, mirrors DamageAura numbers); **start-loadout flip** `initializePlayerSkills` → TurnipPull only (sim harness rides the name: `sim/world.go` soloRegistry now registers the synthetic aura as "TurnipPull" — pins stay byte-identical); Farmer NPC in-zone (tooLowLine = start line, ordered teachings DamageAura+Recall @L2, send-east idle line); **milestone first cut** L2 HealAura+Heal / L3 Recover / L4 Haste (removed: Recall/Swift/NovaBurst/Light/Immolation/Ignite/SummonTotem/SummonCompanion/Taunt/Fade — pinned test rewritten). **GDD §5 amended explicitly** (Damage L1-milestone → farmer-taught @L2, dev note flipped, draft table updated). **Gated damage tags (same-day follow-up, PO decision — replaces the interim per-mob rule):** `gatedDamageTags: true` on a damage payload = opt-in damage — only targets whose BASE resistances explicitly name a hit tag take damage (wildcard ≠ opt-in; `skills.GateOpensFor`, gate check in both takeDamage sites, flag rides `Damage.Gated`/`Factors.Gated` from both caster paths; gating the implicit [physical] default hard-fails at parse). TurnipPull carries it → combat mobs never author turnip entries; brambles/rockfall opt in like the turnip. **Join-panic fix (PO found on first join):** the player viewport is a dynamic `phy.Box` querier and blocking rect props carry the viewport layer — `SolidAABB.intersectWithBox` was a panic stub, now plain AABB overlap (boxes never resolve physically); pinned by viewport-streaming tests + verified with a real browser join (playwright smoke: join → in-world at farm, 0 panics, 0 client errors). Docs: manual §1b new-prop, content-auras/mobs/npcs/zone1/world updated. Verified: full suite green (27 pkgs), race green on touched pkgs, boot loads world (7 props/16 spawns/1 campfire/1 npc), zone generator = scratchpad script (committed output only). PO-verified checklist: fresh spawn Turnip-Pull-only → pull turnips → DamageAura does 0 to turnips → house collision solid all faces → Farmer too-low line at L1 → L2 milestones + teachings → Recall works → editor round-trips world.json.
...
- **Next:** **Step-6 chunk C2 PART 2 (forest interior) — execution session** per `plan-content-zones12.md` §13 C2 + the approved C2 plan: **§9 lift 2 (passive light — fold `LightRadius()` from passive slots in `model/player/player.go` + `model/mob/mob.go`, max not sum; loader already accepts light_aura on a passive; wire/frontend unchanged; TDD first)** + Torch skill (passive light_aura, id 46 next free) + Hermit NPC deep NW pocket (~(-110,-46), entityType Hermit, teaches Torch, tooLowLine); Dog NPC mid-forest clearing (~(-80,-30), entityType DogNpc, "Woof", teaches SummonCompanion); **bramble walls** (`api/mobs/bramble.json`: solid-mob pattern collisionLayer 99/mask 16, `{"*":0,"turnip":1}` opt-in, XP 0, speed 0, 3-5 spawns walling the shortcut-corridor mouth at ~(-112,-6..-12), long respawn) — EntityTypes Bramble/Hermit/DogNpc + SVGs + render classes already landed in Part 1; Torch needs its Skills.ts entry. Then C3–C8 in order. **Every C1+ mob MUST be authored tier + baseline** (raw `maxHealth` hard-fails; rule in `manual-content-authoring.md` §1); combat mobs need NO turnip entries (gated damage tags handle the chore gate — manual §1), but **brambles/rockfall must opt in** (`{"*": 0, "turnip": 1}` pattern). Then C3–C8 in order, one chunk per session. Standing locks: growth 1.12 × maxLevel 30 (≈27×, band ≈ +5, lower-first); regen slow ~1%/s, tier placeholders normal ≤ ~50% / elite ≤ ~25% / boss dies; downtime 15 s + chain 20; guardrail asserts land in C8 against real mobs. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; boot `-content ../api` for content iteration (schema/Go changes still need the rebuild).
