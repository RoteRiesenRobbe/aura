...
- **⭐ Last completed: PROP PLACEHOLDERS C1+C2** ✅ 2026-09-05 (ledger: `docs/plan-prop-placeholders.md` §10; spec + the three PO calls §4/§9, ⚑ read §4.2's **AMENDED** block first): a prop can now exist in-game **without art** — author `"entityType": "PropPlaceholder"` and it draws as its own authored footprint (circle or rect, at the authored aspect, honouring per-placement scale and rotation) with the prop NAME auto-fit inside it, the counterpart to `NpcPlaceholder`. ⭐ **The discriminator is the prop's NAME, and the plan's authored `id` was DROPPED mid-build on the PO's objection** — which was right: a prop is ALREADY keyed by name (every placement says `{"type": "House"}`, `GetByName` resolves it, duplicates already refused at boot), so an id would be a **second identity for the same thing**, hand-sequenced and one more field for every author and editor. `mob_id` earns its compactness by riding every mob every tick; this rides a handful of development-time stand-ins. **So `api/props/*.json` is UNTOUCHED and content impact is NONE** — `world/props.go`'s whole diff is one comment. ⚑ What a name cannot replace: **one reserved `PropPlaceholder = 77` enum value is still needed**, because the server hard-fails on an unknown `entityType` and a per-placeholder enum value would make every unarted prop a `.fbs` edit plus a regen of both binding sets. ⚑ **`Resource.prop_name:string` is appended last and written behind an entityType guard, and that guard IS the cost story**: `TestPropEntityFlatbufMarshal_RealPropCostsNothing` compares a real prop's bytes against a hand-built table with no `prop_name` slot at all — flipping the guard to always-true fails it (mutation-verified), while **every other test in the file stays green either way**. ⚑ A string must be created BEFORE the table starts (unlike the inline Vec2f/AABB structs), so the guard is one `isPlaceholder` decision spent on both sides of `ResourceStart`. ⭐ **The label fits the largest INSCRIBED RECTANGLE of the text's own aspect** (`2ra/√(a²+1)` by `2r/√(a²+1)`), not the bounding square — names are far wider than tall, so the square would throw away ~40 % of the usable width; containment is tested GEOMETRICALLY (the fitted corners stay inside the radius) so a wrong formula cannot pass by agreeing with itself. Below the legibility floor the label is DROPPED, never spilled. ⭐ **The name-on-the-wire also degrades better than an id would**: the SHAPE needs the compiled-in def but the LABEL does not, so a placeholder whose def this build has never seen still draws a **labelled** square. ⚑ **The drawing happens in the CONSTRUCTOR BODY, not `initShape`** — `initShape` runs inside `GameObject`'s `super()` chain, before any subclass field, so the 6th constructor argument is unreachable from `this` there, and `SimpleProp`'s `static bodyAspect` trick cannot work when ONE class serves EVERY definition. It returns an empty positioned+rotated `Container` instead, which also makes §4.3's "the label cannot leave the bounds" true by construction. ⚑ The pure geometry lives in its own `PropPlaceholderLayout.ts` because `Props.ts` reaches `require.context` at import time and is untestable under vitest. **Schema: DB NONE, content NONE**; FlatBuffers one appended enum value + one appended field, both binding sets regenerated. Verified: `go build` · `go vet` · full `go test -count=1` **fully green** · tsc · vitest **586/586** (+15) · prod build · palette regen · ⭐ headless in-game **16/16** over all five §8 cases (round · 2:0.6 rect · rotated · 2.5× scaled · a 26-char name hitting the min clamp), **re-run clean after the id → name rework**, against THROWAWAY content reverted afterwards — the shipped diff is the mechanism only. ⚑ **Two harness traps, now in the `verify` skill**: a stale `backend/aurad.exe` SHADOWS the freshly-built extensionless `aurad` (Windows PATHEXT — the server panicked on a field an hour old), and **`-content ../api` is SERVER-only** — `Props.ts` compiles prop defs in at webpack build time, so a prop added after the last `npm run build` renders as a shapeless square, which looks exactly like a broken wire field. ⭐ **Also this session, PO call: `api/mobs/martin.json` DELETED** — the collaborator's unplaced stub (TODO opening line, referenced by no zone or quest) had kept 3 `items/mobs` census tests red at HEAD for weeks; **the backend suite is fully green again**, and the real problem (censuses that hardcode a roster or a count, so any new content reddens them) is logged in `docs/feedback.md` 2026-09-05 needing a door. **OWED: the PO's in-game pass** (§8) — it settles §9's open question (the label currently rotates WITH the prop) and three [PLACEHOLDER] knobs; ⚑ **no placeholder prop ships as content**, so §10 ends with the five-line JSON that authors the first one.
- **Prior: WORLD SCALE S3 - MOB DORMANCY** ✅ 2026-08-29, `aac2852e` + `2364547e` (tuning), merged to main 2026-09-05 (ledger: `docs/plan-world-scale.md` §11 S3; ⚑ the plan doc itself was RECOVERED from a stash the same day — it had never been committed and its file was gone): a mob that nothing player-controlled is near stops thinking AND leaves the physics space, lifting both halves of F3. ⭐ **The cost changed SHAPE: area-linear → player-linear.** Measured over MobSystem.Update + Space.Update at 10 dispersed players: 500 mobs 0.97→0.61 ms · 5 000 mobs 14.38→2.12 ms · **14 640 mobs (the PO’s tiled 30× world, exactly) 61.5→5.7 ms, 10.8×** (5 repeats; ⚑ a single SHORT run reads ~32 ms for the OFF arm — repeat it) — i.e. from **186 % of the entire 33 ms tick budget spent on mobs alone, down to 17 %** (`mob_dormancy_bench_test.go`, kept). D3 pristine + D4 wake sources + D5 leave-the-space + D6 the wake volume DERIVED from the AOI box (wake 1.7× / sleep **1.9×** after the 2026-08-30 tuning pass, conf `game.mob.wakeMargin`/`sleepMargin`). ⚑ **D5 AS WRITTEN WOULD HAVE HANDED THE WIN BACK**: `RemoveShape` purges by walking EVERY shape in the space — ~2.6 ms per sleeping mob at 30×, and itself O(total mobs), so it would have re-introduced area-linearity on the transition path. New `phy.SleepShape` skips the purge, safe in three layers: tick ORDER (PhysicsSystem at priority 0 rebuilds every collision set later in the SAME tick that MobSystem at 20 sleeps a mob), READERS (nothing between priority 20 and 0 reads `Collisions()`), SEPARATION (≥22 u to any wake source). ⛔ A departure (death / disconnect / flight takeoff) must STILL use `RemoveShape`. ⭐ **Three findings the plan did not have**: **spectators are wake sources** (they stream viewports exactly like players — omitting them renders the pre-join start screen and every death overlay as an empty world) · **proximity alone cannot wake a mob** (one that slept on its spawn tick could be handed threat by an encounter script / the THREAT cheat and stay asleep with it forever — surfaced only as an order-dependent test failure, now pinned) · **D6's awake-count table was understated ~1.7×** — hysteresis means the steady state sits at the SLEEP box, so it measures ~53 mobs/player (47–57), not 32, which moves M0's "~52 dispersed players before break" to **~32** and made `sleepMargin` the knob to narrow first — **done 2026-08-30: 2.2 → 1.9, measured −26 % awake and −17 % tick consistently from 10 to 150 players, moving the mob subsystem ceiling ~75 → ~100 dispersed players**. ⭐ **L7 RULED: patrollers sleep** — freeze-and-resume is correct BY CONSTRUCTION (a dormant mob's Update never runs, so nothing writes its position, waypoint index or steering latch; it thaws mid-leg where it froze). Structures (campfires, braziers) excluded deliberately and narrowly. Free win: `onMobDeath`'s linear scan over every authored spawn became an O(1) index. ⚑ **`m.SetWakeSources(s)` in `core.NewGameWith` is THE ON-SWITCH** — a nil seam disables dormancy entirely, which is what keeps the sim harness and every pre-S3 test byte-identical (L6, satisfied structurally); **deleting that line silently restores the old cost and fails nothing.** ⭐ **L3 is CLOSED by D3, not standing** (the plan’s two clauses contradicted each other): a wounded mob is never ELIGIBLE to sleep, so it stays awake and regenerates — measured, a chipped mob is back to full at t=5.8 s and only sleeps at t=14.8 s, healed with ~9 s to spare. PO confirmed regen-before-dormancy is the wanted behaviour; relaxing D3’s full-health clause re-opens it for real. **Schema NONE.** Verified: `go build ./...` · `go vet ./...` · full `go test ./...` with **no new failures** — 5 pre-existing reds bisected against the committed world.json and confirmed unrelated (2 from the PO's uncommitted 30× `world.json`, which duplicates each unique ascension/memorial stone 30×; 3 from `martin.json` added in `6c2e6d5c` and never added to the content census lists). New legs: phy 3 · model/mob 12 · sys 16 · 1 simharness guardrail (L8's two invariants). ⭐ **TUNING PASS + L8 CORRECTION 2026-08-30**: `sleepMargin` 2.2 → 1.9 (−26 % awake, −17 % tick, ceiling ~75 → ~100 dispersed players), and ⚑ **the wake floor is `player.FlightViewportScale` (1.2), NOT 1** — L8 argued containment from `Zoom.ts`’s fixed GROUND field of view and missed that a FLYING player’s server-side AOI is itself scaled, which is the binding case (1.2 s of warning in flight vs 5.3 s on foot) and a [PLACEHOLDER] already retuned twice. Now clamped in `core/gameconf.go` and asserted in the guardrail; the const is exported for it. **OWED: the probe-ladder re-run on a real server, and §8’s in-game pass — walking in at EVERY zoom level, the totem-beside-a-sleeper case (L2), and now a FLIGHT leg over a dormant region.**
