...
- **⭐ Last completed: CODE HEALTH C1 - GATES + THE DEAD-CODE SWEEP** ✅ 2026-08-12, `[uncommitted]` (`docs/plan-code-health.md` §Chunk ledgers; C2-C7 open): **`npm test` now runs in CI** (its own workflow step, so the 287 vitest tests incl. every shared-constants pin's client half are finally defended), and the dead code nobody should ever restyle is gone: `features/rating/` whole, HUD.less's 324-line survival-era span (1845 → 1521 lines), `UtilsTest.ts`, the 0-byte `EMiniMapLayer.ts`, `BasicConfig.BACKEND`, and `Urls.ts`'s localhost→`local.berryhunter.io` rewrite + `developmentPort` (branch-scoped; the wsUrl-derived `catalogUrl`/`apiUrl` derivation stays). **Schema NONE; behaviour-neutral by criterion**, and the one behaviour change is a fix: a `-dev` boot on :2000 without `?wsUrl` now derives `ws://localhost:2000/game` instead of the broken `ws://local.berryhunter.io:2015/game`. Three grep-proven extensions in the ledger, the sharpest being `socialMedia.less`'s horizontal block, whose animation named a keyframe living in the never-imported `rating.less` - **a cross-file keyframe reference is a silent no-op the moment the defining file leaves the bundle**. `add-content` SKILL.md refreshed (triple map → catalog fetch; `gameObjectClasses` is an enum-keyed Record, so a missed entry is now a COMPILE error; pin refs :168/:168). Verified: vitest **287/287 before AND after** · typecheck · prod build · `go build` · boot 0 WARN/0 ERROR · `ctxloss-warning clean` PASS · zero-ref greps clean. ⚑ CI green needs the push - confirm on the first run after commit.
