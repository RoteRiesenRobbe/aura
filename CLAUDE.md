...
- **Last completed:** Step-4 **chunk 2 (shield layer) DONE 2026-07-13 + VERIFIED IN-GAME 2026-07-14, committed** → `docs/plan-skill-vocab.md` banner. Shipped: `shieldPayload` in `skills.Buffs` (top-up refresh, expiring-soonest `AbsorbShield`, `ShieldTotal`); absorb step in BOTH `takeDamage` sites after resist×DR — return widens to **dealt = absorbed + loss** (F6 §3.1/8-9 executable; lifesteal + mob threat count absorbs via the untouched chunk-1 callers, §4.2(a) pinned); damage/crit accumulators show real HP loss only; mob `tookDamage` + player combat stamp on dealt > 0; effect types `shield_aura` + `instant_shield` (16 total, key `shieldDurationTicks`, instant self-apply counts as a hit); `shield_hp:uint` wire Character+Mob (live value); HUD `.shieldIndicator` + overhead-bar segments in Character.ts AND Mobs.ts; Barrier id 27 smoke (cheat `SKILL Barrier`), count pin 28 skills. Suite + build + tsc/webpack + boot smoke both content sources green.
- **Next:** Step-4 chunk 4 = cast-time + interrupt primitive + **Recall** (runs before chunk 3 per §4.6; activation-precondition + rejection-feedback wire, cast bar, ConnectionStateSystem anchor seam) — execution session, NEW session — `docs/plan-skill-vocab.md` §5
