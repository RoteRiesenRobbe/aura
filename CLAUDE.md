...
- **⏸ Parked 2026-08-20 (PO choice, resume later): `plan-prototype-projectile.md`** - P1 shipped 2026-08-19 and its first PO session landed five fixes the same day; the SECOND in-game pass is OWED, not abandoned (§10 items 1, 2, 4, 5 + 13; setup `SKILL ThrowMine` / `SKILL ThrowBomb`, and ⚑ **GOD makes a cooldown free**, so the new cost is only feelable with god mode OFF). The verdict decides P2 (drift: frost ball + flying bolt, which now also owns the deferred travel-to-position ask) and P3 (mob thrower). Item 7 is ANSWERED: aim stays walk-direction, the cursor fallback is dropped. **In flight instead: `plan-npc-hails.md`** (planned 2026-08-17, conditional name-personalized walk-by greetings, one chunk, schema NONE) · `plan-play-bot.md` (C0-C3). ⚑ C4 leftover, unowned: `TICKING_TYPES` in `SkillTooltip.ts` is a hand-maintained per-type set with no completeness pin and a SILENT failure mode (a new ticking type just loses its cadence line). ⚑ Zone-editor C3 leftovers, unowned: dead Go legacy plumbing (Legacy/LegacyRefs + the unreachable loaders warning) · `wiki-generator/` broken at HEAD (needs the §28-deleted `client-data/Items`) · the reported stale `backend/pkg/api/mobs/` mirror (embedded boot hard-fail, found 2026-08-17) is still NOT reproducible here - two clean embedded boots since (census 101/59, then 102/60); re-verify on the finder's checkout before acting.

- **⭐ DIRECTION SET 2026-08-22: the next map we build is the FIRST RELEASE MAP, and CAMPS ARE CONTENT** (`docs/plan-release-map.md`, nothing built, the map itself not yet designed). Camp membership is a **completed quest**; exclusivity is a `quest_at_stage` gate on the nodes that offer, teach and turn in. The whole Gothic arc is authorable in shipped vocabulary: proving quests at every camp · a pledge that closes the rivals · a camp-only finale whose rival door renders *locked: complete "Pledge to the Ashfolk"* (`lockedWhenGated` + `describeConditions`) · camp-exclusive teaching (gate the teacher's node) · camp-gated ascension rewards (the catalog gate slot already takes `quest_at_stage`) · and a post-ascension reroll that reopens every camp for **free**, because the quest ledger is keyed `character_id`. **No new vocabulary, no Go, schema NONE.** ⛑ **Two engine rules constrain every camp file** (now in `manual-content-authoring.md` §6): a **terminal stage id never matches after completion** (`MatchesStage` needs `Running`), so *a durable choice must be its own quest id*, never a branch stage; and **`not_started` re-matches after an abandon**, so an exclusion seals at **completion**, not acceptance (author the pledge with no objective stage, accept + turn in in one conversation). ⛑ The permanence invariant is a **content pattern** (every pledge offer node carries every rival's `not_started`) guarded by a pin in `quests/content_test.go` - `applyGrant` re-checks node conditions per click, but nothing checks that you authored the full set. ⏸ **`plan-camps.md` is DEFERRED, not cancelled** (its C1 was always designed to land unused, P6): what it still buys is a *hostile* standing, losing standing without joining, and server-enforced permanence. ⛑ **Its standing wipe defers with it** - no standing state exists, so nothing to wipe and nothing to assert; the change to guard is now the OPPOSITE one, moving the **quest ledger** off `character_id` (per-account or per-slot, the way `bloodline_unlocks` is keyed) would silently kill the reroll. ⛔ **`plan-test-world.md` is DROPPED** and archived, never executed: its §1 transfers (F1-F4 map bounds + measured `world.json` densities 46.8 spawns / 74.9 props / 51.8 textures per 1000 sq units; ⛑ F2 has moved, a spawn may author its own `level`), its D1-D8 do not. Where the release map sits in the execution order, and whether it replaces `world.json`, are **open PO calls** (§7).
