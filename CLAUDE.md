...
- **Last completed:** **Step-7 A.6 bare-name skill renames — DONE + PO-verified in-game 2026-07-21, committed `24806352`; full ledger: `plan-rebrand-cleanup.md` §4 A.6 banner. Phase A complete.** **11 renames** of the JSON registry `name` keys: `Aura` suffix dropped (DamageAura→**Damage**, HealAura→**Heal**, WildAura→Wild, SlowAura→Slow, ImmolationAura→Immolation, ReaperAura→Reaper, PaladinAura→Paladin, BerserkerAura→Berserker), `Passive` suffix dropped (SwiftPassive→Swift, ToughPassive→Tough), and the Heal-vs-HealAura collision resolved cooldown-side: id 21 Heal→**FirstAid** (milestone L3 renamed with it). PO naming picks at execution: Damage + FirstAid. 12 JSON files git-mv'd to bare names (`damage.json`, `first-aid.json`, recipe `paladin.json`, …) — loaders + zone editor glob directories, nothing imports by filename. `Skills.ts` display names synced (incl. `Light Aura`→`Light`, `First Aid`); name-pinned Go tests, fixture keys, and stale filename comments updated. As audited: **no wire, no frontend id maps, no sim presets** (roster names derive from the registry). **Cheat/teaching names follow the registry** (e.g. `SKILL Damage`, `SKILL FirstAid`). **Verified:** suite `-count=1` green + `-race` on 6 touched package trees, `make -C backend build` (cp-defs), `tsc` + webpack prod build clean, boot world **0 warnings** counts unchanged (`78 skills/13 factions/47 mobs/10 recipes/856 props/349 spawns/5 campfires/14 npcs, 0 panics`), boot proving-grounds 0 warnings, FirstAid@L3/Haste@L7 in the milestone boot log, PO checked the spellbook names in-game.
