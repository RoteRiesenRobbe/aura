...
- **⭐ SKILL VFX C2b, the other three kinds + the slider, is the ranked next chunk** (`docs/plan-skill-vfx.md` §9; C0 + C1 + C2a done 2026-09-19, C2b-C4 unbuilt): `cast-pose`, `orbit` and `emitter` filled in (once-logging stubs today), the density setting (§7.3) with its mobile default, and ⭐ **the nine §4.3 PO examples authored as the acceptance set**. Heal / shield / light auras draw nothing until then, by C2a's PO call. ⚑ `arc-swing` is gone: it became C2a's caster-anchored `strike`. ⚑ Read the C2a ledger's implementation calls first. **Schema: NONE.**
