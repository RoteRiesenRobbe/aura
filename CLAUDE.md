...
- **⭐ SKILL VFX C1, the wire, is the ranked next chunk** (`docs/plan-skill-vfx.md`; C0 shipped 2026-09-19, C1-C4 unbuilt): the `SkillEvent` vector (FIRED + HIT, emitted INSIDE the four damage/heal funnels, per-viewer filter, both binding sets regenerated, floating numbers own-caused only, six per-tick fields deprecated). ⚑ C1 is not done without the loadbot leg (§5.3), which rules D10's mob-vs-mob fallback in or out. ⚑ Verify the DoT stream keeps its SKILL id (§3.3). C2a needs C0 + C1.
