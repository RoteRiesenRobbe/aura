...
- **Prior: PROP PLACEHOLDERS C1+C2** ✅ 2026-09-05 (ledger: `docs/plan-prop-placeholders.md` §10; ⚑ read §4.2's **AMENDED** block first): a prop can exist in-game **without art** — `"entityType": "PropPlaceholder"` draws its authored footprint with the prop NAME auto-fit inside; the counterpart to `NpcPlaceholder`. ⭐ **The discriminator is the prop's NAME; the plan's authored `id` was DROPPED mid-build on the PO's objection.** ⚑ One reserved `PropPlaceholder = 77` enum value is still needed (the server hard-fails on unknown `entityType`). ⚑ **`Resource.prop_name` is appended behind an entityType guard, and that guard IS the cost story** — `TestPropEntityFlatbufMarshal_RealPropCostsNothing` is mutation-verified. ⚑ Drawing happens in the **constructor body, not `initShape`** (which runs inside `super()`, before any subclass field). **DB/content NONE**; FlatBuffers one appended enum + one field. Verified: full `go test` · vitest 586/586 · in-game 16/16. ⚑ **Two harness traps, now in `verify`**: a stale `backend/aurad.exe` SHADOWS the extensionless `aurad`, and **`-content ../api` is SERVER-only**. ⭐ **`api/mobs/martin.json` DELETED** (PO call). **OWED: the PO's in-game pass** (§8).
