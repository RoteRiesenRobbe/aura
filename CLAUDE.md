...
- **⭐ Last completed: SPELL BUILDER C1: the Skills tab, READ-ONLY** ✅ 2026-09-11 `7b90fb5e` (ledger: `docs/plan-content-editor.md` §B12 C1): a **Skills** tab in `tools/content-editor/` renders all 72 player skills as the builder's form, every control disabled, no Save: identity · category block · one card per effect (shared/payload groups, a **per-level preview** `base + (level-1)·perLevel` with seconds beside every tick) · Visuals placeholder · **Obtained via** (five source kinds, jumps switch the sidebar tab). ⛔ L1 holds: a card renders `effectKeys[type] ∪ costKeys` from the served fixture; `skill-presentation.mjs` decides only how a key LOOKS, keyed by name, and `smoke.mjs` pins it against the fixture BOTH ways. ⚑ L2 was wrong (absent `tickInterval` = 1). ⚑ D6 missed the ascension catalog (now a source). **Two PO-look riders**: the stale `anchor` travel mode in `validate.mjs` fixed (TWO hand copies of the list, now one export); and ⭐ **the `_comment` ruling: an api/ comment is an AUTHORING NOTE, never a session ledger** ("deranged babbling"; rule in `manual-content-authoring.md`), all 102 skill comments rewritten in one line-2-only pass with a parse-equality proof (median 832 → 330 chars; other content kinds open in `feedback.md`). **Schema ALL NONE.** Verified: smoke 0/105 + mutation ×3 · browser harness `content-editor-skills-tab.mjs` 0 problems · `cp-defs` + `go test -count=1 ./...` EXIT 0, 35 pkgs. ✅ **PO form verdict PASSED 2026-09-11** ("works, that part is done"). **Next: C2** (`aurad -validate`).

- **Prior: SPELL BUILDER C0: the vocabulary fixture** ✅ 2026-09-10 `60fb44d2` (ledger: `docs/plan-content-editor.md` §B12 C0): **`api/skill-vocabulary.json` is a GENERATED file**, written only by `UPDATE_SKILL_VOCABULARY=1 go test -count=1 ./pkg/aura/skills/` and pinned by a golden test, so a new effect key, type or category reddens `go test` until regenerated and then reaches the editor with zero hand work (§B3). ⭐ It is a **COMPLEMENT of `api/shared-constants.json`** (which already carried `effectTypes`/`selectors`/`gateKeys`/`statNames`): a test forbids any list living in both files, `vocabulary.mjs` merges them. ⭐ `topLevelKeys` reflected from json tags lets `smoke.mjs` catch a typo'd top-level key for the first time (the loader has no `DisallowUnknownFields` there). Both are session judgements at the PO's delegation, reversal costs in the ledger. ⚑ L7 was wrong (no effect-level `_comment` exists). New: `pkg/aura/golden`, `tools/content-editor/{vocabulary,files,smoke}.mjs`, `npm run smoke`. **Schema ALL NONE.** Verified: red-first · `go test -count=1 ./...` EXIT 0, 35 pkgs · smoke 0/105 · mutation ×3. ⚑ Content pins read the EMBEDDED copy: `make -C backend cp-defs` before believing a red. C1 shipped 2026-09-11.
...
- **⭐ SKILL VFX: DESIGNED 2026-09-11; the spell builder's PO look passed the same day, so this and spell builder C2 are the two live candidates, order = PO call** (`docs/plan-skill-vfx.md`, D1-D10 PO-ruled, nothing built): per-ability + per-mob VFX for everyone (full by default, one density slider), a closed seven-kind vocabulary authored as `visual.layers[]` on the skill file (artist sprites, placeholder per kind), and ⭐ **honest attribution via a per-hit `SkillEvent` on the wire** (FIRED + HIT, emitted INSIDE the four damage/heal funnels so no future path can forget it, D9). Numbers become own-caused only, other players show none (D6). ⚑ **Pulls the per-hit slice and the per-effect-art moratorium OUT of `plan-entity-presentation.md` (§39)**; medallions stays parked (D8). Deletes the `hitStyle` lever end to end, absorbs `AuraTickIndicator`, deprecates six aggregate wire fields (all encode-only, sim byte-identical by construction). ⚑ C1 owes a loadbot measurement that may veto mob-vs-mob events (D10). Chunks C0 vocabulary · C1 wire · C2a/C2b engine · C3 art · C4 scale; C0 and C1 in either order. `prototype/skill-visuals` = quarry, delete after C2.

- **⭐ THE UNDERWORLD owes U5 (content) + U0, and one PO look** (`docs/plan-underworld.md`): the engine is done end to end; U5 is filling the room (12 spawns authored in Tiled so far). ⭐ The biggest perf decision left is CONTENT: cave walls as blocking `paths`, not props (§7.1 item 1). ⚑ **Q5 wants eyes first**: the curtain derives from the grant kind, so the SHIPPED portal pair and campfire recall change from hard cuts to a lateral crossfade, a change to something already PO-verified. ⛔ Nothing in U1→U4b has been walked in-game by me (U2 alone produced three browser-only defects). ⚑ **U6 = build-time zone placement is DESIGNED, NOT BUILT** (§7.2, PO-asked 2026-09-08): authored `depth`, auto-assigned origin, a GENERATED placement file both sides read, so the wire cost stays ZERO (a boot-time packer would force origins onto the wire); it deliberately reverses U4b's "no separate depth int", soundly, since geometry derived FROM depth cannot disagree with it. Triggers: a dungeon growing and forcing neighbours to re-space, or the zone count passing ~10. **U0** = `plan-world-scale.md` S1 (lazy zone bundling; its `ensureZoneLoaded` await seam is exactly what the curtain hides, so the curtain is S1's load screen).

- **⭐ WORLD PATHS owes C4 only** (`docs/plan-world-paths.md` §12): C4 = re-author the 372 `Sand` blobs as paths (PO-ruled 2026-09-07), a CONTENT judgement (where the roads run), not a transform of blob positions. It also arms the region primitive's OPEN look sitting (one region covers the whole shipped world, so no interior feathered edge exists to judge yet). ⚑ The path look sitting is live: water, road, forest and cliff paths are authored in `world.json` and the PO is judging widths, blends, textures and the `Water` drift ([PLACEHOLDER] `{0.4, 0.15}` u/s) in-game.

- **⭐ SPELL BUILDER: C1 shipped + PO-passed 2026-09-11, C2 next** (`docs/plan-content-editor.md` Part B, ledger §B12): the Skills tab in `tools/content-editor/` is live read-only and the form is accepted. C2 (`aurad -validate -content <dir>`, D9: all findings, no DB, the temp-copy save seam) · C3 (save, live fixture checks, type-change confirm, L2's absent-vs-0 tri-state, rename guard) · C4 (new-skill flow, icon + `spawnMob` pickers, post-save checklist, test-rig badge, docs) · C5 (registry lock). ⚑ Before C3: §B11 Q6, one reformat commit of `api/skills/`. ⚑ §B11 Q7 (Go validation for every tab) is still the structural answer to `validate.mjs`'s hand-ported rules (the `anchor` mode went stale once already).

- **⭐ SERVER PERF + WORLD SCALE, what's next** (`docs/plan-server-performance.md`, `docs/plan-world-scale.md`): perf chunks 0+3 shipped, 1/2/4/5 unstarted; scale S1 unstarted (S2 deferred into `plan-world-map.md`'s mobile pass). ⚑ **Re-read the perf ordering BEFORE chunk 1**: it sequences off encoding 57 % / physics 24 %, but M1-F3 measured PhysicsSystem at 74 % at density 10×, so chunk 4 may outrank chunk 1 for the clustered case (a design sitting). Cheap meanwhile: S1 (webpack `'lazy'` + one await seam; retires the L4 probe-zone footgun). ⛔ M1-F2's residues are recorded, NOT fixed, their own chunk: `SkillSystem` 6.6× / `StatusEffectsSystem` 11.7× walk dormant mobs, `Space.grid` never pruned; all need L6 byte-identical-sim care.
...
- **Feedback flow + the UI pass:** new feedback lands ONLY in `docs/feedback.md` (four exit doors; plan docs never double as intake). All UI work is in `docs/plan-ui-pass.md`: direction C "Inked Panel" RATIFIED (the §4 CORRECTION block IS the spec), §5 order C1-C11 RATIFIED, C1-C8 SHIPPED 2026-08-26 to 09-02 and PO-approved, **next: C9 (mobile)**. The two C8-play intake rows (tooltip → A · unlock VFX → B + louder entry-point trail) and the row-glow replay were ruled + fixed 2026-09-06 (`b8bd3d0b`, ledger §6 C8 rider); PO play owed. ⏸ `plan-onboarding-cleanup.md` DEFERRED, trigger: a human coworker about to join.
...
- **Known-inconclusive at HEAD**, all unowned: `chunk3-charm` 6–8/9 (D9 fragility) · `filler-batch` leg 1 (stale assert) · `chunk3b-ii-conversation` 28/34 (stale TownCrier assert + Leave-click race + Wanderer drift-pin, re-confirmed 2026-09-06) · `accounts.TestRepeatedFailuresAreThrottled` under `-race` only · `AuraTiledConvert.test.ts` byte-stability ×2 on a fresh checkout (they pin the PO's uncommitted world.json repair; the 30× world is GONE at HEAD, world is 263 KB / 144×72). ⚑ The 3 `items/mobs` census tests + the 3 `cmd/simharness` placement pins hardcode the roster, so ANY new mob/NPC reddens them (`docs/feedback.md` 2026-09-05, needs a door), and they read the EMBEDDED `backend/pkg/api/` copy: `make -C backend cp-defs` before believing a red. Backend suite fully green 2026-09-10 (35 pkgs); the frontend legs above not re-run. ⚑ Measure the rate before diagnosing a flake (a misread cost `chunk3b-interact` two chunks); ⚑ this host's wall clock is non-monotonic, suspect it before any elapsed-time red.
...
- **Gotchas & dev tools:** **A shape removed from `phy.Space` is purged from every other shape's collision set on the spot** (§54); never "optimize" that sweep away ([[project-ghost-references]]). ⚑ A content edit does NOT invalidate the Go test cache: after any `api/` change use `go test -count=1`. ⚑ **A ZONE edit is HALF-LIVE, the seam is BOOT TIME**: the client bundles `api/zones` through webpack (`GroundTextureManager.ts:77` `require.context`) and HMR re-reads every Tiled save, but `aurad` reads the zone ONCE, at boot, so a save RENDERS instantly while path corridors, prop colliders and spawns are still the booted zone (looks right, behaves wrong; cost a session 2026-09-07). **Restart the server after any Tiled save** (`./scripts/dev-restart-windows.sh server`). ⚑ Without `-content ../api`, `aurad` reads the EMBEDDED `backend/pkg/api/zones/`, refreshed only by `cp-defs` (`dev-restart-windows.sh` passes the flag). ⚑ The starting aura is pre-equipped but NOT active: the first press of `1` switches it on. **Cheats:** GOD, WARP `<x·120> <y·120>`, SPEED, XP, SKILL, ANNOUNCE, THREAT, QUEST (dump / `ACCEPT` / `ABANDON` / `ADVANCE`). `make -C backend build` runs `cp-defs`; boot `-content ../api` for content iteration.
