...
- **Last completed:** **Session ④ part 1 (C8 milestone settlement + world density pass) DONE 2026-07-20, PO-approved in-session (map artifact + choice prompts) + PO in-game density check — committed `03a377b1` (main, per PO directive); full ledger: `plan-content-zones12.md` §13 C8 Session-④ banner.** **(1) Milestone settlement (FINAL):** table = two rows — **Heal @L3, Haste @L7** (embedded `skills/milestone-unlocks.json` → needs rebuild, `-content` does NOT apply it; pin test re-pinned). World-placed instead: **HealAura → 2nd Hermit instance** at the turnip field, teach gate **@L2**; **Recover → DireBear drop 0.15** [placeholder → drop-rate pass] + made **self-only**. Lantern Post → **post-v1**; Stag → **dropless final**; Lacerate → **not adopted** (DamageBurst has physical+bleed since C4). Pins unchanged **77/44/10**. **(2) Density pass (2 rounds, PO-approved via before/after map artifact):** round 1 = 8 fill regions, **+185 props / +53 spawns** (incl. kobold outskirts north of the hideout) + **Z1 N–S road rerouted** to the main road (was dead-ending in the kobold camp); round 2 = PO standard *"a mob on screen at basically all times"* → screen = 17×9.5 u (`Zoom.ts` max zoom-out), sliding-window fill till no ⅔-screen window empty (middle woods = half-screen, wolf-heavy predator forest), **+95 themed spawns**. Zone: **805 props / 346 spawns / 13 npcs**; no-go respected (front, arena, village, tunnel, camps, clearings, seam); append-only verified vs HEAD. Map tooling → **backlog §22** (standalone browser map editor, post-C8). **Verified:** build clean, boot `77 skills/13 factions/44 mobs/10 recipes/805 props/346 spawns/2 campfires/13 npcs, 0 panics`, full suite green, PO in-game density check positive.
...
- **Next:** **Session ⑤ — C8 remainder (final feel items + PO walkthrough) — new chat** per `plan-content-zones12.md` §13 C8 banner: **drop-rate feel pass** now against final density (expected-farm-time table from kills/hour battery; PO leans RNG-as-default — "should be the default for most"; decides the §11 pity question; 16 drops: Session-② placeholders SlowAura .15 / Tough .25 / Nova .15 / Fade .1 / Reaper .5 + Session-④ Recover .15 + Light .08 the content-gating one); **campfire `healFractionOfMax` (0.12) + heal-cost curve (`10 −2/level`) final values**; **PO full-journey in-game walkthrough** — checklist: Step-0 Warbanner sequence (warlord journey pops trio NOT Warbanner; maxed Spearhead unlocks it), ceiling-kit heal/shield feel vs elites (sim under-reads the kit — damage axis only), teacher gates (Immolation 6 / Totem 5 / Revive 6 / **new HealAura Hermit 2**), Troll cL11 / Pyromancer cL6 / Dire cL6-7 tiers, new-density feel (mob-on-screen standard). Milestone-table final ✅ landed Session ④. C8 is the last §13 chunk. **Every C1+ mob MUST be authored tier + baseline** (raw `maxHealth` hard-fails; manual §1); combat mobs need NO harvest entries (gated damage tags — manual §1), but **gate obstacles must opt in**.
- **Deferred (per triage §Execution sequence):** **Post-C8 "combat readability"** — items **7** (category ring colors) + **15** (tier frame ring), two append-only Mob-table wire bytes. **Post-C8 tooling:** standalone browser map editor (**backlog §22**, seeded from the Session-④ density-pass renderer/generator in that session's scratchpad). **Step-7 rebrand** — items **22** (bare-name renames) + **12** (`legacy:true` tags). **Persistence step** — item **10** sacrifice loop (first consumer; GDD §11 amendment landed Session ①). **Anytime/annoying:** item **21** full `null.split` repro session. Placeholder values to settle in C8 remainder: heal cost `10 −2/level`, campfire `0.12`, drop chances (SlowAura .15 / Tough .25 / Nova .15 / Fade .1 / Reaper .5 / **Recover .15** / Light .08 + rest), Troll cL11 / Pyromancer cL6 / Dire cL6-7, teacher gates (Immolation 6 / Totem 5 / Revive 6 / HealAura Hermit 2). Standing locks: growth 1.12 × maxLevel 30 (≈27×, band ≈ +5, lower-first); **regen 0.00033 ≈ 1%/s FINAL** (C8 settlement 2026-07-19); **milestone table FINAL: Heal L3 / Haste L7** (C8 Session ④ 2026-07-20); **density standard: mob visible on every ⅔-screen (17×9.5 u) window** (C8 Session ④); tier frame: elite ≤ 25% facetank / boss kills the bot / normal = per-mob texture + Z1/Z2 band-check (front exempt); **downtime 10 s** + chain 20; guardrail asserts LANDED (`cmd/simharness/guardrail_test.go`, deterministic). Dev cheats: GOD, WARP <x·120> <y·120>, SPEED [factor|off], XP, SKILL <name>, ANNOUNCE <text>, THREAT [id]. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; boot `-content ../api` for content iteration (schema/Go changes still need the rebuild).
