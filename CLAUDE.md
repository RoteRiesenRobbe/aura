...
- **⭐ Chunk 3b-ii design session — the conversation panel planned in full (2026-07-27, DOCS ONLY, no code)** ⏳ `[uncommitted]` — `docs/plan-entity-model.md` §6b.9–6b.19 replaces the 20-line sketch with the full chunk plan (+512 lines): **9 PO decisions D15–D23 · 6 new landmines L21–L26 · a rewritten test-strategy row · §11 ledger stamped · 4 superseded decisions annotated in place.** **⭐ The reframe that did the work: the panel is a conversation TREE browser, not a teaching dialog** — and once that is the target, *everything is an option* collapses the feature onto one mechanism the schema already validates and nothing has ever read. The PO brief (three greeting choices · a hint branch that can later start quests · a pickable teaching list · a WoW-guard directions list · back navigation) needs **no new nesting**: a "list" is just a node whose options happen to be one-grant teachings, so a branch row and a grant row are the **same row**, and `next` — authored, load-validated since 3a, read by nothing — is the entire navigation mechanism. **⚑ The content audit that forced the fork: 12 of 14 conversants author exactly ONE option, with `text` empty and 1–3 grants stacked inside it; 2 author none.** So an options-only panel is literally one nameless button per NPC — which is why D15/D17 make grants and branches the same row and why the 11 un-migrated NPCs then need **zero content work** (`present()` expands a legacy multi-grant option into one row per grant). **3 rulings DELETE machinery:** ① **D17 retires the ordered grant walk** — a list is not a walk, so clicking *Ignite* teaches Ignite and nothing else; ⚑ that costs the **373-line ported evaluator suite**, the one thing 3b-i was careful not to touch, because it pins the walk. ② **D18 retires `trigger` entirely** — ⚑ the finding that forced it is that **`trigger` is a single value, so an NPC could never both call out as you pass AND open a tree on `E`**, which is exactly the NPC the brief describes; `interaction.ambient` becomes its own field and `approach` has no behaviour left to name (**L18 and L20's guard go with it**). PO asked whether `approach` could be reused for quests advancing on approach — ruled **no**: the reusable part is the rising-edge `seen` map + `speakToSensor`, which `ambient` keeps alive, while an approach-fired quest hook wants `onApproach: <nodeId>` plus one-shot-per-player-**forever** semantics a per-session map cannot give. ③ **D19 moves speech into the panel**, retiring D13's private `speak()` — the reply text already rides the streamed tree. **D16 is the architectural call:** while a conversation is open `GameState` carries the **whole personalised tree**; the client navigates instantly with a local back-stack and only *taking* an option goes to the server, so server state is **2 fields with no node bookkeeping** — every apply is validated on its own merits, never on the path taken to reach it, and `present()` serialises all nodes instead of walking them so authored cycles need no check. **`ServerMessageBody` stays untouched for the second chunk running** (L16); the cheaper catalog-served variant was killed by `catalog.go`'s own answer-key rule. **⚑ 3 landmines to read before the first edit:** **L21** — `present()` omits hidden rows, so a presented index is **not** the authored one; the wire must carry authored `option_index`/`grant_index` or a player learns the wrong skill, misfiring only *after* they have already learned something. **L22** — the mob loader is the one loader **without** `DisallowUnknownFields` (`definitions.go:284`), so deleting `trigger` from Go leaves **14 lying keys that boot green**; strip the content in the same step and pin it with a raw-JSON assertion. **L26** — no conversant authors `interaction.range`, so talk range is `aggroRadius: 1.0` on all 14; under D21 leaving that range now **tears a conversation down**, and D22 adds an actor that walks away on its own. **2 rules need content that does not exist:** D22 (an actor in conversation holds position, so a patrolling NPC can be stopped and talked to) — ⚑ **all 14 conversants author `speed: 0`, nothing patrols today**, so the chunk authors a moving `Wanderer` rather than shipping a hold nothing exercises; and D23's 2–3 representative trees (Emberkeeper = the teaching list with its 1/7/12 walls · TownCrier = the hint branch **plus** `ambient` · Wanderer = the patrol). Panel is **non-blocking, mouse-driven, bottom-centre above the action bars**, and **combat on either side ends the conversation** (D21) — which is what makes non-blocking safe. Ruled **one chunk, not a split**: every piece is unverifiable without the others.
