...
- **⭐ Last completed: PER-PLACEMENT PROP TRANSFORM — SCALE, THE WYSIWYG SPLIT, AND ROTATION** ✅ 2026-08-23, `8413714e` + `10adf1df` (ledger: `docs/plan-prop-scale.md` §10 — **the plan is COMPLETE**, all five chunks shipped: C1 · C1b · C1c · C1d · C2. ⚑ C2 was ⏸ deferred by D7 that morning — "do nothing and zero when it becomes a problem" — and the PO resumed it the same day; the defer paid, because the count to zero had already moved 797 → 798): resizing a prop in Tiled now changes its size in game — the thing the PO tried on 2026-08-22 and found did nothing. ⭐ **The whole design is one sentence: the box IS the scale.** One optional `scale` multiplier per placement, applied in ONE function (`Prop.VisualBody`), so it reaches both body forms and the collider behind them with **no per-type list anywhere** (D4) — a prop type added tomorrow inherits it untouched. ⚑ An untouched prop must derive EXACTLY 1 or all 807 placements grow a key and byte-stability dies; it does (`x/x` is exact in IEEE-754) and 1 normalises back to absent. ⭐ **L1 had a SECOND face the plan did not name, and it is the worse one**: naming `scale` in `getZoneAsJSON` is necessary and NOT sufficient — `readPropControls` rebuilds a prop from the in-game panel, which has no scale control, so nudging a Tiled-scaled prop would have reset it with the whitelist perfectly correct; **the loss would always have been somebody else's work**. ⭐ **C1b (D6) inverts what an authored body MEANS**: it is now the **VISUAL** footprint and collision is `body.collisionFactor` of it, so Tiled matches the game to the pixel. The client's hardcoded padding is deleted — and ⚑ the tree's flat `+ character.size` addend **did not scale**, which made per-placement scale non-linear on screen (2.00× the collider at 0.294, 1.27× at 2.045); retuning a tree's size is an `api/props/tree.json` edit now, pinned by a test that recomputes the OLD client formulas against the real content. ⭐ **C1c repairs two incidents C1b caused in CONTENT, not code** — both PO-reported from the live game: **742 props silently rescaled by one Ctrl+S** (574 trees at `0.714`, 168 minerals at `0.935` — the exact reciprocals of the migration factors) because a Tiled document open from before the palette regenerated still held the old boxes, and *the box is the scale*. ⚑ **Nothing can catch this**: the file is valid, the server accepts it, it round-trips byte-identically — it is only wrong against intent, so it is now a documented hazard (close the zone before regenerating). ⚑ Houses hid it for hours (body unchanged) and rocks were off by 6.5% (under the noticing threshold); only trees at 29% were visible, which reads as "trees are broken" and sent the first diagnosis after a **stale binary** — `backend/aurad` *was* three weeks old, but `dev-restart-windows.sh` builds and boots `aurad.exe`, so ⚑ **two binaries, one stale** is its own small trap. ⭐ **D8, from the same session: every polyline vertex is a waypoint, node 0 included.** It used to be dropped as "the origin", so N clicks gave N−1 waypoints and the node-0 handle was a silent no-op — cost the PO a Wanderer that ping-ponged between two points and ignored `loop`. ⚑ Neither reading was ever the ENGINE's (`patrol.go` treats the spawn purely as where the mob starts) and there was no settled convention to preserve: of the 7 routes in `world.json`, **2 include their spawn as waypoint 0 and 5 do not**. The risk was measured not argued — real Tiled does **not** normalise an off-origin first vertex. ⚑ Independent of the ruling: **with two waypoints `loop` and `pingpong` are the same walk**. **Schema: DB NONE · wire NONE** · content JSON one optional key per placement and one per prop type. ⭐ **Then the PO called the census pins what they are** ("it just hardcodes a certain number of objects on the map? that is a bad pattern") and all three went away: `ZoneModel.test.ts`'s `=== 17` talkers and simharness's `423`/`485` asserted **content nobody promised to freeze**, while the invariant each names in its own message — the serializer must not ADD a respawn; every combat spawn must reach a row — is about the **pipeline** and holds at any world size. Both now measure the input and reconcile the output against it (⚑ plus a floor, or a derived 0 would match a collapsed loader), and the ZoneModel one got **stronger**: it compares the SET, so swapping which spawns are respawn-free no longer passes. ⚑ The third `cmd/simharness` red was never a census at all — `os.Symlink` needs a privilege Windows withholds without Developer Mode, so it was red on ONE machine, which is worse than red everywhere because it teaches you to skim a failing package; now a plain recursive copy. ⭐ **C2 finally renders the field that had been authored-and-drawn-nowhere since the world-foundation chunk** — every editor has offered a rotation control for months and all of them were no-ops. ⭐ **The PO asked the cost before a line was written, and it is a flat +16 bytes per snapshot** — not per prop: at 1, 17, 60 or 200 props the delta is the same, because the float lands in `Resource`'s existing 8-byte alignment padding and the residual is one deduplicated vtable entry. ⚑ **An unrotated prop costs LITERALLY zero**: appended last + default 0 means the builder skips the value AND `WriteVtable` trims the trailing zero slot, so after the content pass the snapshot is byte-for-byte what it was — pinned by a test proven red at 56-vs-64 B, the only thing that would notice `rotation` being moved into the middle of the table. Server CPU/allocs unchanged (14 allocs either way). ⭐ **The client seam already existed and the classes were hardcoding `0` into it** (`GameObject`'s ctor rotation → `createInjectedSVG`), which is also the only cheap place: ⚑ the obvious `setRotation` path honours `LIMIT_TURN_RATE`, so every prop entering the viewport would join the per-frame `rotatingObjects` set and **spin into place** — a permanent working set of turning trees as you walk. Server side is one `Angle()` override, because `model.Entity` already had the accessor. ⭐ **C2b then DELETED D3's lie instead of living with it**, because the PO hit it in-game within the hour ("the colliders of rotated houses behave as if unrotated"). ⚑ **The reason it had been priced as out of scope was a WRONG CITATION**: both the plan and this entry sent it to backlog §34, which is *entity-vs-entity* collision — a **different problem, already decided against** in 2026-07-26. Nothing there covered a rotated static rect. The real price was one chunk, because three properties of THIS shape (props are static, so cos/sin resolve once at construction; only circles ever hit it, so there is no rect-vs-rect separating-axis test; and all the geometry funnels through `clamp()`) collapse general OBB collision to ~60 lines — and orienting `clamp` fixes intersection, push-out **and** mob steering at once, since `boxRepulsion` is written in terms of `ClosestPoint`. ⚑ Angle 0 is the EXACT identity (cos=1, sin=0, and the constructor short-circuits rather than trusting `cos32f(0)`), which is what made it safe under a world where every prop but three is unrotated — pinned by an `==` test, not a delta. ⚑ The broadphase bound had to grow to the enclosing AABB or the grid would never offer the pair to the narrowphase and the collider would vanish at exactly the corners the rotation swung out. ⭐ The old pin asserting the lie said in its own comment that failing meant somebody had made rect collision honest and it should be **replaced, not deleted** — which is exactly what happened. `-race` whole-repo: **ZERO data races** (the lone red is the known `-race`-only `accounts` timing test, which the run explains — 149 s under the detector against sub-second asserts). **798 of 810 rotations zeroed** (reading (a)); ⚑ intent could not have been lost and that was CHECKED — the 86 rotation lines that moved in the PO's pass are all Ctrl+D duplicates, and since nothing rendered them **no angle on disk had ever been seen**. ⭐ **The §4.1 both-bindings hazard has a mechanical check now**: the vendored `flatc_Windows_v24_3_25.exe` reproduces both checked-in binding sets byte-identically **offline**, so a half-applied regen is a one-command diff instead of a client reading `undefined` forever — and it sidesteps `flatcgen.go` re-downloading the compiler from GitHub every `go generate`. **Schema: DB NONE · wire YES (one appended `rotation:float`, both regens) · content JSON nothing new.** Verified: `go build` + `go vet` clean · `go test -count=1 ./...` **35 ok, ZERO red** (`cmd/simharness` had been red at HEAD before this work) · vitest **460/460** · `tsc --noEmit` clean · `verify.sh` **7/7** with `world.json` byte-identical in both newline conventions after the zeroing pass. ⚑ **`api/zones/world.json` stays UNCOMMITTED by PO choice** — the repair and the zeroing live in the working tree. PO in-game pass on C1/C1b/C1c **PASSED 2026-08-23**; the C2 one is owed.

- **Prior: TILED ZONE AUTHORING - ALL SEVEN CHUNKS** ✅ 2026-08-23, first commit `0b8ba13f` (ledger: `docs/archive/plan-tiled-authoring.md` §11 - the plan is COMPLETE + archived): `api/zones/world.json` now opens in **Tiled**, edits visually, and Ctrl+S writes it back **byte-identical**. ⭐ The feasibility answer that decided the tool: **none of Aura's world is a tilemap** - every array is freeform floats, no grid, no snapping - so only OBJECT layers matter, and there LDtk supports neither entity rotation (#841) nor flipping (#978), which would have left the 549-piece `terrain` array (backlog §58, the actual parked pain) uneditable. Tiled also takes `registerMapFormat` with both `read` and `write`, so there is **no intermediate .tmj and no converter step** - `world.json` stays the single source of truth and BOTH editors write it identically (D4: the in-game editor is not retired; it keeps the one thing Tiled cannot do, editing while standing in the live world). New vocab: **byte-stability as the acceptance criterion** (D6) · **the layer IS the meaning** (D5) · **inherit sentinels** (C6) · **the completeness pin** (C5). Chunks: C0 spike · C1 read+write all six arrays · C2 generated palette from the game's own assets · C4 save-time validation · C5 one-command content + the pin · C6 typed spawn form · C3 manual + verify leg + bookkeeping. ⭐ **C4 refuses what the server would reject, while you are still looking at the object**: a prop dropped on the `spawns` layer is named with its **Tiled object id** (Edit > Select Object by Id jumps to it) AND told which layer it belongs in; `terrain.type` is now validated **somewhere for the first time** (the server ignores it, the client dereferences `undefined` at render time). ⭐ **C6's sentinels are the whole trick**: Tiled cannot express "absent", so each tri-state knob borrows a value the loader already rejects - and ⚑ `wanderRadius` and `respawnTicks` **cannot use 0**, because 0 is a real authored value for both (forced-stationary, 19 spawns; respawn-next-tick, exactly the 17 NPCs). The mapping lives in ONE function (`readSpawn`), and because the class defaults ARE the sentinels the design is robust to whichever way Tiled treats default-valued properties - which is the half that cannot be tested headlessly. ⚑ **AuraProp deliberately gets NO members**: `blocksMovement` is a bool with no spare value, so a default would risk flipping all 777 props. ⭐ **C5's pin scrapes `zone.go`'s json tags** and fails red the moment the format grows a field the converter would silently drop - proven red on purpose before being trusted; its exception set holds exactly `legacy`. **Schema: NONE at every layer** (no game code, no wire, no DB) - dev tooling + static content only. Verified: vitest **439/439** (64 in the new `AuraTiledConvert.test.ts`) · `tsc --noEmit` clean · generator idempotent · real-Tiled round-trip byte-identical in both trailing-newline conventions, re-runnable as **`bash tools/tiled/verify.sh`**. ⭐ **The GUI pass happened same-day and found the one defect headless could not**: a plain-string property SHADOWS the class member that types it, so the mob dropdown only appeared after resetting the field. Fixed by setting enum properties as TYPED values (`tiled.propertyValue`, which THROWS on an unregistered type - hence a bare-string fallback for the no-project flow) and by overriding only what the file actually authored; ⚑ a typed enum reads back as an INDEX, never the string, so the generator now publishes `ENUM_VALUES`. **PO re-check PASSED 2026-08-23** ("dropdown works") - nothing outstanding on this plan. Also new: **`.gitattributes`** pinning `*.sh` and `api/zones/*.json` to LF (a CRLF checkout breaks install.sh on Linux and makes the byte-check report ~14600 false differences). (⚑ its closing "next" line named `plan-prop-scale.md` C2 as blocked on a rotation ruling; both shipped later the same day — see the entry above.)
