...
- **In flight: `plan-portal-spells.md`** - C1 Open Portal shipped 2026-08-18 (`[uncommitted]`, PO check pending); **C2 Pull Through is the next chunk** (`spawn_at_anchor` effect type + the `caster` destination mode + the D8 dwell-radius placement, §6 steps 5-8; re-verify its line refs first). Queued behind it: `plan-npc-hails.md` (planned 2026-08-17, conditional name-personalized walk-by greetings, one chunk, schema NONE) · `plan-play-bot.md` (C0-C3) · `plan-prototype-projectile.md` (designed 2026-08-15). ⚑ C4 leftover, unowned: `TICKING_TYPES` in `SkillTooltip.ts` is a hand-maintained per-type set with no completeness pin and a SILENT failure mode (a new ticking type just loses its cadence line). ⚑ Zone-editor C3 leftovers, unowned: dead Go legacy plumbing (Legacy/LegacyRefs + the unreachable loaders warning) · `wiki-generator/` broken at HEAD (needs the §28-deleted `client-data/Items`) · the reported stale `backend/pkg/api/mobs/` mirror (embedded boot hard-fail, found 2026-08-17) stays NOT reproducible here - 2026-08-18's embedded build+boot was clean; re-verify on the finder's checkout before acting.
