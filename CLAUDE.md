...
- **Last completed:** Dead resource + placeable + decay prune (backlog §26, plan-resource-decay-prune.md Chunk 1) ✅ `ee9d42e9` 2026-07-24. Removed the inherited Berryhunter cluster — `model/resource`, `model/placeable`, the `gen` package, `sys/decay.go`, `model/decay.go`, 3 codec marshal cases + 2 marshal funcs, `addPlaceableEntity` + `DecaySystem` (helpers 7→6, systems 16→15), 3 dead interfaces, and the 9 resource/placeable item JSONs. Campfires (mobs) + props (ride the `AnyEntityResource` wire path) untouched. **Deviation:** those JSONs were 100% of nested item content, so `items.go`'s embed narrowed to `*.json` — the item registry is now the `None` sentinel alone, making **backlog §28 (item-registry removal) trivial** (note left there). Verified `go build`/`vet`/`test` (29 pkgs), boot embedded + `-content ../api` both 0 panics / props:777 spawns:471 / 5 campfires / 1 item. **Next: Chunk 2 — frontend `Placeable.ts` decode-path sweep** (separate, keeps the schema; Tier 3 schema prune deferred into §28). Full ledger: `docs/plan-resource-decay-prune.md` §13.
- **Recent chunks (newest first; full ledgers in the plan docs):** render-jitter buffered-interp fix ✅ `0e504c22`/`8a29a75c`/`c5064732` (`plan-render-jitter.md`); input-jitter held-state fix ✅ `cb7f011f` (`plan-input-jitter.md`); unlock source attribution ✅ `2bfee286` (`plan-unlock-attribution.md`); idle-loop alloc fix ✅ `fe0044d0` + day/night DEACTIVATED ✅ `e648ab88` (`plan-intermission-triage.md`); playtest-1 Passes A/B/C ✅ — **plan fully executed** (`plan-playtest1-feedback.md`); F&F deploy LIVE ✅ `a7a2267d` → `https://aura-game.duckdns.org/` (`plan-playtest-deploy.md`); content pass C1–C8 + rebrand step 7 complete. Earlier chunks: roadmap.md "Execution order" + the plan-*.md §13 banners.
