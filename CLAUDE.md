...
- **⭐ Last completed: WORLD SCALE M1 — RE-MEASURE** ✅ 2026-09-06 (ledger: `docs/plan-world-scale.md` §11 M1): S3's owed headline number. ⭐ **The area ceiling went `~3.6×` → `~19×` (5.3×)** (break: p95 > 16.5 ms at 10 dispersed players), moving the release map's 50–60 % envelope to **~9.5–11.4× (~440×220 u)** — what `plan-release-map.md` §8.3 said it owed. ⭐ **The before-arm is the SAME BINARY with dormancy off via conf** (`wakeMargin`/`sleepMargin` 1000/1001), and it **reproduces M0.1 on all five of its rungs** — which is what licenses comparing the tables (a suspicion M0.1 was contaminated was raised, then **refuted**). ⛔ **The find is NOT world scale: a corpse panics `EntitiesMarshalFlatbuf`, so every player death aborts server ticks** — `CorpseEntity` (type 23) has no case in the Player/Mob/Prop switch (`codec/gamestate.go:551-576`) and corpses sit on `LayerViewportCollision`, the mask BOTH player and spectator viewports select. **1 024 panics / 1 523 ticks (67 %)** with deaths vs **0** with `-god`; `playerSendState` hits it too. Long-standing (`aa509d95`); `docs/feedback.md` 2026-09-06, **not fixed here**. ⚑ **It silently corrupts any capacity run without `-god`.** ⭐ Residues named: `SkillSystem` 6.6× / `StatusEffectsSystem` 11.7× still walk dormant mobs, `Space.grid` keys never pruned; **`NetSystem` flat 1.2×** — AOI+dormancy solved the networking axis. ⚑ **`density` mode: PhysicsSystem is 74 % of tick, not 24 %** — re-read `plan-server-performance.md`'s order before its chunk 1. **No production code**, **schema NONE**. Verified: build · vet · **`go test -count=1 ./...` EXIT 0, 35 pkgs, 0 failures**; L4 honoured. ⭐ **Leg C WALKED by the PO 2026-09-06** (zoom containment at EVERY level + the flight leg both PASS; totem/patroller inconclusive but fine; *"overall works well"*) — and it surfaced **M1-F5: two ways dormancy diverges from a credible world sim, both reachable today** — an unobserved mob-vs-mob fight never ends (mob-on-mob hostility is AUTHORED CONTENT), and a mob can freeze **mid-walk-home off its route**, because there is no return mode and the walk-home sits inside the IDLE path — exactly the case L7 did not cover. `docs/feedback.md` 2026-09-06.

- **Prior: PROP PLACEHOLDERS C1+C2** ✅ 2026-09-05 (ledger: `docs/plan-prop-placeholders.md` §10; ⚑ read §4.2's **AMENDED** block first): a prop can exist in-game **without art** — `"entityType": "PropPlaceholder"` draws its authored footprint (circle or rect, authored aspect, honouring per-placement scale + rotation) with the prop NAME auto-fit inside; the counterpart to `NpcPlaceholder`. ⭐ **The discriminator is the prop's NAME; the plan's authored `id` was DROPPED mid-build on the PO's objection** — a prop is already keyed by name, so an id would be a second identity for the same thing. **`api/props/*.json` UNTOUCHED, content impact NONE.** ⚑ One reserved `PropPlaceholder = 77` enum value is still needed (the server hard-fails on unknown `entityType`). ⚑ **`Resource.prop_name:string` is appended behind an entityType guard, and that guard IS the cost story** — `TestPropEntityFlatbufMarshal_RealPropCostsNothing` is mutation-verified. ⭐ The label fits the largest **inscribed rectangle of the text's own aspect**, tested GEOMETRICALLY; below the legibility floor it is DROPPED, never spilled. ⚑ Drawing happens in the **constructor body, not `initShape`** (which runs inside `super()`, before any subclass field). **DB NONE, content NONE**; FlatBuffers one appended enum + one appended field. Verified: full `go test` green · vitest **586/586** · ⭐ headless in-game **16/16**. ⚑ **Two harness traps, now in `verify`**: a stale `backend/aurad.exe` SHADOWS the extensionless `aurad`, and **`-content ../api` is SERVER-only**. ⭐ **Also: `api/mobs/martin.json` DELETED** (PO call) — the stub had kept 3 census tests red for weeks; the real defect is in `docs/feedback.md` 2026-09-05. **OWED: the PO's in-game pass** (§8).

- **Prior: WORLD SCALE S3 — MOB DORMANCY** ✅ 2026-08-29, `aac2852e` + `2364547e`, merged `1568c1f9` (ledger: `docs/plan-world-scale.md` §11 S3): a mob nothing player-controlled is near stops thinking AND leaves the physics space, lifting both halves of F3. ⭐ **The cost changed SHAPE: area-linear → player-linear** — at 14 640 mobs **61.5 → 5.7 ms, 10.8×**; ⭐ M1 above measured the whole-server consequence. Wake volume DERIVED from the AOI box (wake 1.7× / sleep 1.9×). ⚑ **`RemoveShape` must NEVER be used for the sleep transition** — it purges by walking EVERY shape (~2.6 ms per sleeping mob at 30×, itself O(total mobs), which would hand the win back); `phy.SleepShape` is the sanctioned pair, but ⛔ a real departure (death / disconnect / takeoff) STILL uses `RemoveShape`. ⚑ **`m.SetWakeSources(s)` in `core.NewGameWith` is THE ON-SWITCH** — a nil seam disables dormancy entirely (which is what keeps the sim byte-identical), and **deleting that line silently restores the old cost and fails nothing.** ⚑ **Spectators are wake sources** (omitting them empties the pre-join screen and every death overlay). ⚑ **The wake floor is `player.FlightViewportScale` (1.2), NOT 1** — a FLYING player's AOI is itself scaled, the binding case (1.2 s of warning vs 5.3 s on foot). **Schema NONE.**
...
- **Known-inconclusive at HEAD**, all unowned: ⭐ **the 3 `pkg/aura/items/mobs` census reds are CLOSED** (2026-09-05, PO call): `martin.json` deleted, backend suite fully green - but ⚑ **the tests are still the defect**, hardcoding a roster or a count so ANY new mob/NPC reddens them (`docs/feedback.md` 2026-09-05, needs a door) · `chunk3-charm` 6–8/9 (D9 fragility) · `filler-batch` leg 1 (stale assert) · `chunk3b-ii-conversation` 28/34 (stale TownCrier assert + Leave-click race + Wanderer drift-pin) · `accounts.TestRepeatedFailuresAreThrottled` under `-race` ONLY · **`AuraTiledConvert.test.ts` byte-stability ×2** on a fresh checkout - they pin the PO's UNCOMMITTED world.json repair (⚑ **that 30× `world.json` is GONE at HEAD** - tree clean, world is 263 KB / 144×72 - so S3's two `TestMemorial_*`/`TestAscensionSites_*` reds are closed and **M1 measured the whole backend suite EXIT 0, 35 pkgs, 0 failures on 2026-09-06**; the frontend legs above were NOT re-run). ⚑ **Measure the rate before diagnosing a flake** (a misread cost `chunk3b-interact` two chunks); ⚑ **this host's wall clock is non-monotonic** - suspect it before any elapsed-time red.
