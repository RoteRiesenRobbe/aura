...
- **⭐ DESIGNED 2026-08-23: the REGION PRIMITIVE** (`docs/plan-region-primitive.md` + `docs/plan-region-audio.md`, the authoritative records; ⚑ **C1 - per-region ground colour - BUILT 2026-08-25** in a parallel session, merged `c74292c3`; that session owns its wrap/ledger): `zone.regions` polygons naming a **profile** (ground colour · footsteps · music · later atmosphere; §4.8 adds an optional ground-`texture` property from the 2026-08-24 spike - adoption + blend width are PO calls at the C3 palette sitting). 5 PO rulings incl. **one rule applied per property** and **`resolve()` is total** (any miss → the shipped default, never silence). Schema NONE. ⚑ **THREE zone-format whitelists, only Tiled's pinned** - L1: an in-game save silently deletes an untaught field.
