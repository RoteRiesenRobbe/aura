...
- **World map C3 — the player roster** (`docs/plan-world-map.md` §6 + §4.3): a new ~1 Hz server message, one assembly point, dots on both states. ⚑ Its "the one chunk with a protocol change" billing is now **false** — C2 appended two `GameState` fields — but C3's is still the larger change (a new message). ⚑ Landmine 6 stands: a player inside the AOI arrives twice (30 Hz `GameState` *and* the 1 Hz roster), so pick one as authoritative or the dot stutters.
- **World map C1's tail:** the mobile real-device pass (§7 requires it; headless perf transfers only as ratios) and the deferred `features/map/` rename. ⚑ C2 added phone surface (the 9 px docked marker) and the PO ruled **the desktop pass covers it** — no separate mobile item.
