...
- **Last completed:** **Sim-harness Chunk 3 DONE — implemented + verified + PO-approved + committed 2026-07-16** per `docs/plan-sim-harness.md` §8: the 1-vs-N matrix. Sim core generalized to N mobs (`Scenario.PackSize`/`Pack()`, `World.Mobs` ring spawn, per-mob HP rolls in index order — N=1 byte-identical to chunks 1–2 pinned by regression test; `FightResult.Kills`); `sim/matrix.go` = `RunMatrix` (MaxTargets builds × pack 1..N, parallel cells): win rate + clear-time dist (wins) + kills-before-death dist (losses) per cell, per-build `OverwhelmPack` = first pack with win-rate <50% [PLACEHOLDER]. Surfaces: CLI `-matrix`, `POST /matrix`, web-explorer heatmap panel (validated sequential ramp); driver.mjs drives it. **Required real-system change `sys/targeting.go`: `selectTargets` sorts candidates by entity ID before capping — capped ties + per-target damage-roll order previously rode Go's randomized map iteration; in-game, equidistant ties now stick to the oldest entity (2 sys tests pin it) — flag to PO.** 10+2 new tests; suite + race + browser green. **Findings: at current baselines (TTD/TTK≈1.3) every build incl. uncapped is overwhelmed at pack 2 with 0 kills; at the target ratio (mob dmg 4) overwhelm moves with the cap (cap 1→pack 2, cap ≥2→pack 3) and multi-target caps clear covered packs in flat time; the overwhelm cliff is one pack size wide.** Chunk-2 recap (committed `eed434f5`): f(L) curve tool; same-tier flat (Philosophy A holds); ~4-level band at ratio ~1:2.5 → growth ≈1.15. Chunk-1 recap (`5faf7aa6`): 1v1 explorer, real ECS headless, `-serve` + presets.
...
- **Next:** **Chunk 4 of `docs/plan-sim-harness.md`** (chain runner) in a fresh session. **⚑ OPEN (PO): lock exact `growth` + max level from the chunk-2 tool output** (working read: ~4-level band at a ~1:2.5 TTK:TTD ratio → growth ≈1.15; the current-content ratio ≈1.3 compresses the band to +2 regardless of growth). After the harness: **Step 6 = the initial content pass (item 12)** — two large zones with intra-zone difficulty areas, full mob roster (hand-authored per-tier against the §5 curve), boss scripts, first real balance pass; live-game `f` multiplier lands at the top of step 6 (§5 Decision 5). `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; the committed Sage (proving-grounds (4,3), teaches `HealAura`@L1 + `Dash`@L5) is content — boot `-content ../api` for content iteration.
