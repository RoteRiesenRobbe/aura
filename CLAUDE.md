...
- **Last completed:** Step-5 (unlock-source systems, `docs/plan-npc-teaching.md`) **chunk 2 (entity + placement, backend) DONE + VERIFIED 2026-07-14, NOT committed.** `model/npc/npc.go`: `Npc` = embedded `BaseEntity` (static solid visual body, placeholder sprite `EntityTypeFlower` — a **Resource**-backed type, since a Mob sprite class expects health/aura wire fields the Resource payload lacks; placeholder visual radius 1.0) + separate `Sensor()` dynamic `phy.Circle` (radius = authored `Npc.Radius`, `Mask=LayerPlayerCollision`, `IsSensor`). `model.NpcEntity` interface (`Entity` + `Sensor() phy.DynamicCollider`); `game.AddEntity` switch case `model.NpcEntity` → `addNpcEntity` (before `model.Entity`): visual body → `AddStaticBody`, sensor → `p.Space().AddShape` **as a DYNAMIC shape**, + `NetSystem` + new `NpcSystem`. `sys/npc.go` `NpcSystem` (priority 20 like MobSystem, holds NPCs + **temporary** rising-edge log of players entering a sensor — replaced in chunk 3), registered in `NewGameWith`. Boot loop in `berryhunterd.go` places `zone.Npcs` after campfires. Rides the `Resource`/prop marshal path (`PropEntity` case), no `server.fbs` change. **CHECKPOINT RESOLVED:** a static shape's `Collisions()` is ALWAYS empty (`space.go:bruteIntersectShapes` only writes collisions onto dynamic shapes; static shapes aren't reset/resolved) — so the sensor MUST be dynamic (mob aggro works only because the whole mob is added dynamic) while the visual body stays static. Proven by 3 `model/npc/npc_test.go` tests (dynamic sensor reports overlapping player; ignores out-of-range; static sensor reports nothing — documents the trap). **Sanity:** `go build ./...` + full `go test ./...` (24 pkg ok, 0 fail) green; booted proving-grounds via `-content ../api`, no panic, `NpcSystem` in system list. **VERIFIED IN-GAME 2026-07-14:** the flower placeholder renders + is solid (collision), and the temp rising-edge sensor log fires on approach and RE-fires only on leave+re-enter (no per-tick spam) — the deferred chunk-2 items are now confirmed. **A temp Sage is staged for chunk 3** in SOURCE `api/zones/proving-grounds.json` at (4,3) (radius 3; teaches `HealAura`@L1 + `Dash`@L5 — the L5 gate exercises the too-low path; uncommitted, survives `make build` since it's the source; boot `-content ../api`). **Prior:** chunk 1 (schema + loader + TS mirror) DONE+VERIFIED 2026-07-14 (not committed) — `Teaching`/`Npc`/`Npcs` in `world/zone.go`, validate/resolve, skills registry threaded through `LoadZoneFS`, `ZoneModel.ts` mirror. Step 4 (skill-vocab fill) COMPLETE + committed (last 3e9ab8e4).
- **Next:** Step-5 **chunk 3 (NpcSystem grant + edge-trigger, backend) in a NEW session** → `docs/plan-npc-teaching.md`. Replace the temp `NpcSystem.Update` log with the real behavior: per-NPC `seen` rising-edge detection driving `onApproach(npc, p)` = ordered teachings, `HasDiscovered` skip, level-gate stop (`TooLowLine`), `Discover`+`ApplyRecipeCascade` grant-all-qualifying, lore fallback. Needs the teaching/lore payload threaded onto the NPC entity (deferred from chunk 2 — `Npc` currently carries only render+sensor). **Thread it via npc-local types built in the boot loop** (`skills.SkillDefinition`+level+line; `model/npc` already imports `skills` transitively via `model`), mirroring how `prop.New` takes decomposed params — do NOT import `world` into `model/npc`. Temp Sage staged at (4,3) teaches `HealAura`@L1 + `Dash`@L5. Whole core unit-tested with fake player/sensor (`sys/npc_test.go`, test cases in plan §Test strategy). Verify in-game: approach → skill in spellbook + unlock glow; no re-grant while standing. Chunks 4–6 follow (speech → frontend bubble → editor mode). **Note:** `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/` — the chunk-1 leftover temp NPCs in the embedded proving-grounds.json got cleaned this way (correct; that edit was meant to be reverted). Author temp content in SOURCE `api/zones/` + boot `-content ../api` to iterate without rebuild.
