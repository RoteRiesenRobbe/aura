...
- **Last completed:** Item 10 participation XP (v1-roadmap ✓). Mobs track damage contributors; on death **every participant gets the full XP** (no split). Healing counts: a successful heal registers the caster on the target (`NoteHealedBy`/`RecentHealers`, ~10s window [PLACEHOLDER], refreshed per heal, ticked in `player.Update`); recent healers of participants are rewarded too, deduplicated. Combat reset: full out-of-combat regen clears participants (no clock needed). Drops stay with the last toucher (item system dies with survival removal). Before that: Phase 6.1 mob migration (per-mob aura skill JSONs in `api/skills/mobs/`, `MobTouches(Factors)` double dispatch preserved, target flags enforced, sensor mask+radius per tick via `model.AuraMaskFor` — mob aura switching ready for boss scripts; load order items → skills → mobs) and the zombie-mob bug fix.
- **Remaining in Block 3:** 6.2 monster-kill unlocks (`unlocks` in mob JSON, per-participant rolls, glow via existing spellbook diff; needs one new player-facing aura skill as payload) → 6.3 AngryMammoth boss designation (JSON-only: biggest size+aura, guaranteed unlock source).
