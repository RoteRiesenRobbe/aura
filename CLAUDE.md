...
- **⭐ DIRECTION SET 2026-08-22: the next map is the FIRST RELEASE MAP, and CAMPS ARE CONTENT** (`docs/plan-release-map.md` - the authoritative record; nothing built, map not yet designed - it owes its own planning session, §7, incl. the execution-order slot + does-it-replace-world.json PO calls). Membership = a completed quest; exclusivity = `quest_at_stage` gates; the whole Gothic arc is authorable in shipped vocabulary, **no new vocab, no Go, schema NONE**. ⛑ The two engine rules constraining every camp file: `manual-content-authoring.md` §6; the permanence invariant is a content pattern pinned in `quests/content_test.go`. ⏸ `plan-camps.md` DEFERRED not cancelled (buys hostile standing + server-enforced permanence). ⛔ `plan-test-world.md` DROPPED, archived (its §1 F1-F4 bounds + measured densities survive).
