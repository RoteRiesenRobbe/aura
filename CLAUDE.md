...
- **Last completed:** **code-health triage session — 2 chunks DONE** (2026-07-24, ledgers in `docs/backlog.md` §30/§27.3.3 and §27.2.3/§25 B) ✅ `f095514a` + `2ec03ee7`, **headless-verified, NOT yet PO-verified in-game**. 19 files, **394 insertions / 96 deletions**. **Chunk A `f095514a` (vestige prune + guard):** deleted the whole `layers.placeables` container group (7 containers, both stage-assembly blocks, its no-op `nightExempt` entry, the `IGame` field — `cameraGroup` children **30 → 23**; nothing had rendered into any of them since §26/§28), the stranded `item: undefined` snapshot field, the orphaned `mineral-hit-sharp` alias + preload + mp3, and **8** dead minimap icons (**widened** from the 2 the backlog named — only `character`/`tree`/`stone` have readers); plus §27.3.3, an authored `tickInterval` ≤ 0 now **hard-fails at load** instead of being silently rewritten to 1 (all `api/skills/` prescanned: values are 1..120, nothing trips it). **Chunk B `2ec03ee7` (tuning visibility):** 3 hardcoded Go constants became conf knobs with **no behaviour change** — mob out-of-combat regen is now **`game.mob.healthGainTick`**, deliberately the *same name and unit* as `game.player.healthGainTick` so a later unification is a rename not a redesign, and `defaultCritFactor`/`healerThreatFactor` moved to a **new `game.combat` block** (not `game.player` — unlike the player-only `critChance` they apply to *every* acting entity). **PO call:** package-level setters at boot (`mob.SetHealthGainTick`, `sys.SetCombatFactors`, mirroring `mob.SeedProcess`) over signature churn — `NewMob` has ~100 call sites and the crit-factor readers are *free* functions with **58** test call sites. ⚠ **Authoring 0 restores the default, it does not disable** — healer threat can't be switched off by authoring 0 (**open PO question**). Also folded in §25 A#4 (a doc comment merged into its neighbour — same hunk, inseparable). **PO design question answered → new backlog §31** (player/mob/NPC stat convergence; see Code-health findings). Verified: `go build`/`vet` clean, `go test ./...` green (**27 pkgs** — prior banners' "29" was stale), guardrails replay identically `-count=2`, `tsc --noEmit` clean, webpack prod green, boot `-content ../api` **0 panics** (83 skills/14 factions/50 mobs/10 recipes/5 prop defs/777 props/471 spawns/5 campfires/14 npcs), **boot with both new conf blocks stripped resolves to the exact old constants** (the live server's upgrade path), join smoke both muxes 752/703 display objects + **0 console errors**. **⚠ Watch → §29 recurred once** (see Code-health findings).
...
- **Next: playtest round-3 chunk — healer combat state + role-as-loadout** (`plan-playtest-feedback.md` §Intake round 3, designed 2026-07-25, **not started**). Live bug: a lone healer regenerates through incoming damage and is effectively unkillable — "in combat" is derived from *having an aggro target*, not from *taking damage*, and the seek-healer path returns before the threat block entirely. Two parts: ① a damage-recency combat timer (ships standalone), ② **backlog §31 gap 5** — replace both `updateAggro` special-case early-returns (`seekHealer` + `isFollower`) with one loadout-driven mode selector keyed on aura *category*, so healer/hybrid/guardian/fighter are configurations, not types. **⚑ Landmine: mode thrash** — `SetActiveAura` zeroes the tick accumulator, so a mob flipping mode every tick silently deals and heals **zero**; needs hysteresis + a test pin. **⚑ Verify first:** `MedicCompanion` reads as already broken by the two early-returns colliding (code-only finding, unconfirmed in-game). PO decisions 2026-07-25: no universal auto-attack (parked in §31), support set = Heal+Shield, pacifist healers ignore their attacker, survivors-like fork **CLOSED — stays MMO-lite**.
