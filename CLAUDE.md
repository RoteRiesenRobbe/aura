...
- **Last completed:** Phase 6.2 monster-kill unlocks. `unlocks: [{skillName, chance}]` in the mob JSON (load-time resolution, absent chance = 1.0, invalid chance = startup failure); on death every rewarded participant (same set as item-10 XP: damagers + recent healers) rolls each unlock independently → `Discover()` → glow via the existing spellbook diff, no wire event. First content: `WildAura` (player skill ID 3, wide/low-dps damage variant, [PLACEHOLDER]) — guaranteed from AngryMammoth, 20% from SaberToothCat. Before that: Item 10 participation XP (full XP for all participants incl. ~10s-window healers, combat reset on full regen, drops stay with last toucher), Phase 6.1 mob migration, zombie-bug fix.
- **Remaining in Block 3:** 6.3 AngryMammoth boss designation (JSON-only: biggest size+aura; the guaranteed unlock is already in place) → then M2 (full block regression, two windows).
