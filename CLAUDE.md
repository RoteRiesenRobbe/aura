...
- **Open code-health findings** (documented 2026-07-24, none scheduled — `docs/backlog.md` §27 / `docs/research-code-quality.md` §10):
  - ① **Live bug, not fixed** — `MobSystem.Update` (`sys/mob.go:99`) removes dead mobs *inside* `for _, mob := range n.mobs` and `Remove` is synchronous (append-shifts the backing array), so each tick a mob dies one survivor is skipped + another double-updated (single-tick stutter, looks like netcode jitter). ~30 min test-first; nine sibling systems share the loop shape (§27.1).
