...
- **Last completed:** **Session ③ C8 part 1 (sim-side items) DONE 2026-07-19, PO-read in-session — commits `4e412ebf` `405b9e8c` `6a2f69a4` `ed9ffdff` `c55838e0` (main, per PO directive); full ledger: `plan-content-zones12.md` §13 C8 banner.** All sim/test/tooling — zero game-content change beyond Step 0's Warbanner recipe tier (Warbanner = Vanguard 5 + **Spearhead 5** + CallForAid 3; backlog §21). Items closed: **Step 0** recipe topology (option A); **full-roster presets** incl. new real-`dot_aura` sim support (AuraSpec `dotTicks`/`dotTickInterval`; pyro/VenomSpider/Totem/Brazier un-turreted, ImmolationAura+Wildfire join, roster 28); **kills/hour chain** over all 34 armed mobs at home brackets (near-binary: elites/bosses comply, normals 12 soft / 3 hard, kite always faster where feasible, flat cL1–23); **regen/downtime settlement** — regen 0.00033 ≈1%/s **FINAL** (self-heal L1 = +42% pace = the accelerator niche), downtime **10 s** (was 15); **guardrail asserts** (`cmd/simharness/guardrail_test.go`: elite ≤25%, boss dies, Z1/Z2 band-check, curated exemptions — deterministic 3.8 s, content passes); **§A ceiling calibration ACCEPTED** (Spearhead 2.28 ev/t = 3.4× best non-ceiling; 3-grunt packs in 1-grunt time; unique Orc-kite unlock; `TestGuardrails_CeilingOrdering` pins it). **PO rulings:** normal tier = per-mob texture + zone band-check (no per-mob ceiling); assert scope = real hostile roster only; front exempt (elite territory); ProvingBoss solo-kite OK. **Verified:** full suite green (29 pkgs) + `-race` (sim), Playwright explorer drive clean. Battery artifacts in session scratchpad (regenerate via run-simharness skill).
...
- **Next:** **Session ④ — C8 remainder (feel/decision items + PO walkthrough) — new chat** per `plan-content-zones12.md` §13 C8 banner: **drop-rate feel pass** (PO leans RNG-as-default — "should be the default for most"; decides the §11 pity question; Session-② placeholders SlowAura .15 / Tough .25 / Nova .15 / Fade .1 / Reaper .5); **campfire `healFractionOfMax` (0.12) + heal-cost curve (`10 −2/level`) final values**; **milestone-table final**; **PO full-journey in-game walkthrough** — checklist: Step-0 Warbanner sequence (warlord journey pops trio NOT Warbanner; maxed Spearhead unlocks it), ceiling-kit heal/shield feel vs elites (sim under-reads the kit — damage axis only), teacher gates (Immolation 6 / Totem 5 / Revive 6), Troll cL11 / Pyromancer cL6 / Dire cL6-7 tiers. C8 is the last §13 chunk. **Every C1+ mob MUST be authored tier + baseline** (raw `maxHealth` hard-fails; manual §1); combat mobs need NO harvest entries (gated damage tags — manual §1), but **gate obstacles must opt in**.
- **Deferred (per triage §Execution sequence):** **Post-C8 "combat readability"** — items **7** (category ring colors) + **15** (tier frame ring), two append-only Mob-table wire bytes. **Step-7 rebrand** — items **22** (bare-name renames) + **12** (`legacy:true` tags). **Persistence step** — item **10** sacrifice loop (first consumer; GDD §11 amendment landed Session ①). **Anytime/annoying:** item **21** full `null.split` repro session. Placeholder values from Session ② to settle in C8 remainder: heal cost `10 −2/level`, campfire `0.12`, drop chances (SlowAura .15 / Tough .25 / Nova .15 / Fade .1 / Reaper .5), Troll cL11 / Pyromancer cL6 / Dire cL6-7, teacher gates (Immolation 6 / Totem 5 / Revive 6). Standing locks: growth 1.12 × maxLevel 30 (≈27×, band ≈ +5, lower-first); **regen 0.00033 ≈ 1%/s FINAL** (C8 settlement 2026-07-19); tier frame: elite ≤ 25% facetank / boss kills the bot / normal = per-mob texture + Z1/Z2 band-check (front exempt); **downtime 10 s** + chain 20; guardrail asserts LANDED (`cmd/simharness/guardrail_test.go`, deterministic). Dev cheats: GOD, WARP <x·120> <y·120>, SPEED [factor|off], XP, SKILL <name>, ANNOUNCE <text>, THREAT [id]. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; boot `-content ../api` for content iteration (schema/Go changes still need the rebuild).
