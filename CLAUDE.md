...
- **Last completed:** **Idle-loop allocation fix — the EMPTY server was overloading. DONE 2026-07-22, `fe0044d0` (+ `ba266284` embedded-content sync), deployed live same day, PO-verified in-game ("movement feels the same"); full ledger: `docs/plan-intermission-triage.md` §Idle-loop allocation fix.** Found while deploying playtest-1 Pass A–C: `Overload! Systems at: N%` all over the live journal. **Most of it was self-inflicted** — 9 657 warnings/24 h collapse into two deliberate `loadbot` ramps (~9 550); real play made 8, at 103–121 %. **The real signal: it overloaded with ZERO players** (~30/h, up to 154 %, through the small hours). **Not the host** (`%steal` 0.00, CPU 83 % idle) — a *tail* problem: the live vCPU is **~3.4× slower** than the dev box on the same idle work (25.8 % vs 7.7 % of a core), which lands the tick tail right on the 33 ms line. **It never reproduced locally, even at `GOMAXPROCS=2`** — raw per-core speed, not core count. **Root cause: the idle loop allocated ~11 MB/s with nobody online** → a GC every ~350 ms whose tail clipped the budget. Four sites, all "rebuild the per-tick state from scratch": `Space.Update` re-`make`ing the whole broadphase grid (21.7 %), `resetCollisions` re-`make`ing a `ColliderSet` **per collider per tick** (14.5 %), `addCollision` into those fresh maps (24.2 %), and `Mob.blockerRepulsion` building a probe circle + `seen` map + hit slice **per mob per tick** (26.7 %) — the last one calling `QueryCircleStatics`, whose own doc says *"one-shot… create a circle, query, drop it"*. **Fix is pure allocation reuse, no behaviour change** (5 files + 3 new `*_alloc_test.go`, no wire, no content): grid + cell slices reused (tail `clear`ed so cells stop pinning removed shapes), new **`AppendCircleStatics(dst, c)`** for per-tick callers with `QueryCircleStatics` kept as the one-shot wrapper (its 8 other call sites untouched, cell-straddle de-dup now a linear scan), per-mob `steerProbe`/`steerHits` scratch, and `clear()` in `resetCollisions` + `StatusEffects.Clear` (safe — no reader retains those maps across a tick; `Effects()` copies). **TDD: all six pins failed first**, asserting `AllocsPerRun` is **zero** (not "cheap") with shapes moving between runs, plus an equivalence pin against the old query and an "actually clears" guard. Measured idle @`GOMAXPROCS=2`: allocation **11.2 → 0.16 MB/s (~70×)**, GC every ~350 ms → 4 total, `p95` **2713 → 1673 µs**, `p99` **3286 → 2024 µs**; the remainder is mob *respawn*, not per-tick churn. Verified: `go build ./...` exit 0, `go test ./...` **29 packages with tests pass / 0 failures** (21 have no test files — the commit message's "50 packages" counts all packages), boot `83 skills/14 factions/50 mobs/10 recipes/777 props/471 spawns/5 campfires (safeRadius 1.5)/14 npcs, 0 panics`. **Live confirmation from `/proc` over 20 s on the deployed binary: idle CPU 25.8 % → 17 % of a core (−34 %), RSS 25.6 → 18.1 MB.** **Open:** the overload verdict itself **needs a day of live logs** (0 since the deploy proves nothing — the old binary also had 26 clean idle minutes; rate to beat ~10–30/h idle, `journalctl -u aurad --since "-24h" | grep -c Overload`); **the load-test ceiling is unchanged structurally** — `bruteIntersectShapes` is still O(n²) per cell and ~39 % of loop CPU, so the published clustered ~60–70 was measured pre-fix and should shift up only from the GC savings; **live journal noise:** always `grep -v "TLS handshake"` (someone else's `api.cybex.net` DNS points at `159.69.148.73`).
