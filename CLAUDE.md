...
- **Last completed: `plan-entity-model.md` Chunk 3b-ii — the conversation panel** (2026-07-28, wire + backend + frontend + content, 45 files / 3 new source files) ✅ `ef4355f1` — **HARNESS-VERIFIED IN-GAME 28/28**, 1 deliberate SKIP, 0 console errors, 0 ctx losses (`.claude/skills/verify/chunk3b-ii-conversation.mjs`, kept); ⏳ PO sign-off rides the single deferred pass. **Talking is now a TREE you browse:** `E` opens the Emberkeeper's greeting with three branch rows, *Teach me* reaches the list with Torch available and Ignite/Immolate locked reading their authored walls, and clicking a row teaches **that row and nothing else** — D17's retirement of the ordered grant walk, visible from outside. `evaluate()` is GONE: `present()` (pure — it reads the spellbook and mutates nothing, which is the whole reason the split exists) + `applyGrant()` (one grant, validated on its own merits) replace it, and `next` is finally read after being authored and load-validated since 3a. **`ServerMessageBody` untouched for the SECOND chunk running** (L16 unexercised) — the whole personalised tree rides `GameState`, so server session state is 2 fields with no node bookkeeping. **⚑ The plan had a hole the pre-flight audit caught: the presented tree had nowhere legal to live.** §6b.12 put `present()` in `sys` and had `codec` marshal its result — but `sys` imports `codec`, so a `sys`-local tree type is unmarshalable; `model/conversation.go` holds the transport shapes. **2 rulings tightened inside the chunk:** L22 got a **boot-time tombstone** (`jsonInteraction` keeps `Trigger` solely to reject it) as well as the planned CI pin, because content is hand-edited against `-content ../api` by someone who does not run `go test`; and `requiredLevel: 1` **stopped being a gate** — players start at level 1, so 3a's rule demanded a refusal line no player can ever read. **⚑ 3 defects only the harness could find, all invisible to Go and vitest:** ① **the panel re-rendered ~30×/s** — the tree is re-sent every tick by design, so rows were destroyed and rebuilt continuously; `:hover` never survived and a click could land between an `<li>` being detached and its replacement appearing (it dropped clicks on ~half the runs, and would do the same to a player) — fixed with a view-signature check, pinned by 2 vitest cases; ② **`getInteractableEntityId()` read 0 whenever a panel was open**, self-inflicted by suppressing the badge *through* the tracked id — split into offered-vs-drawn; ③ **`wanderRadius` also RANDOMISES THE SPAWN POINT** (`spawnAt` offsets by `randomInDisc`), so the Wanderer is never where `world.json` says. **Content:** all 14 lose `trigger`, all gain `range: 2.0` [PLACEHOLDER] (L26 — talk range now TEARS DOWN a conversation); D23's three trees authored (Emberkeeper teaching list + hint leaf + directions · TownCrier hint branch **plus** the game's only `ambient` lines · Wanderer `speed 0.5 / wanderRadius 4`), and the other **11 needed zero content work** exactly as D17 predicted. **⚑ One acceptance item is a permanent SKIP by construction:** *being hit closes the panel* — no cheat can stamp player combat (`DAMAGE` bypasses `takeDamage`, `THREAT` is read-only), so the harness reports SKIP rather than a fake PASS; both directions are pinned in Go. **Verified:** `go build`/`vet`/`test` green, guardrails + alloc `-count=2`, frontend typecheck + **44 vitest** (up from 27) + prod build; boot **both ways** 0 errors 0 warnings 0 panics — 83 skills/15 factions/64 mobs/777 props/485 spawns/5 campfires. **⭐ Sim battery BYTE-IDENTICAL across all four runs** (TTK 6.67s / TTD 8.70s) — L14's predicted `Wanderer` roster row never materialised. In-game highlights: the Wanderer **holds position while talked to** (drift 1.29 → **0** → 1.21 units/4s) and the TownCrier calls out its ambient line **without** opening anything. Full ledger `docs/plan-entity-model.md` §11.
