...
- **Gotchas & dev tools:** **A shape removed from `phy.Space` is purged from every other shape's collision set on the spot** (§54) — never "optimize" that sweep away ([[project-ghost-references]]). ⚑ **A content edit does NOT invalidate the Go test cache** — after any `api/` change use `go test -count=1`. ⚑ **A ZONE edit is HALF-LIVE**: the client bundles `api/zones` straight through webpack (`GroundTextureManager.ts:77` `require.context`), but `aurad` reads the EMBEDDED copy at `backend/pkg/api/zones/`. So a Tiled save RENDERS instantly while every server-side consequence of it (path corridors, prop colliders, spawns) still runs the last `cp-defs` snapshot — geometry that looks right and behaves wrong. Cost a debugging session 2026-09-07 (water drew, did not block). Check `backend/pkg/api/zones/world.json` before suspecting the feature. ⚑ **The starting aura is pre-equipped but NOT active** — the first press of `1` switches it on. **Cheats:** GOD, WARP `<x·120> <y·120>`, SPEED, XP, SKILL, ANNOUNCE, THREAT, QUEST (dump / `ACCEPT` / `ABANDON` / `ADVANCE`). `make -C backend build` runs `cp-defs`; boot `-content ../api` for content iteration.
