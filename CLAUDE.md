...
- **Next:** Phase 9 — combinations *implementation* (design complete; steps listed in the doc's Phase 9 entry: recipe registry in `api/recipes/`, trigger hooks on discovery+level-increase, cascade, hard-fail validation, 1–2 secret starter recipes). Alternative: Block 2 (survival removal + resource unification, roadmap items 1+2) or item 11 (aura targeting). Roadmap: docs/v1-roadmap.md.
