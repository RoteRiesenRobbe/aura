...
- **Prior: WORLD SCALE M1 — RE-MEASURE + the corpse fix it found** ✅ 2026-09-06 (ledger: `docs/plan-world-scale.md` §11 M1): S3's owed headline number. ⭐ **The area ceiling went `~3.6×` → `~19×` (5.3×)** (break: p95 > 16.5 ms at 10 dispersed players), putting the release map's 50–60 % envelope at **~9.5–11.4× (~440×220 u)**. ⚑ **The before-arm is the SAME BINARY with dormancy off via conf** (`wakeMargin`/`sleepMargin` 1000/1001) — that is what licenses comparing the tables. ⛔ **M1's find was NOT world scale, and is ✅ FIXED: a corpse panicked `EntitiesMarshalFlatbuf`, so every player death aborted server ticks** — `CorpseEntity` (type 23) had no case in the switch and corpses sit on `LayerViewportCollision`, the mask BOTH viewports select. **1 024/1 523 ticks (67 %) → 0/2 100.** ⭐ **It had never worked on any build** — corpses now render for the first time. ⚑ **`default:` there still `panic()`s — see Open PO calls.** ⚑ **`density` mode: PhysicsSystem is 74 % of tick, not 24 %.** **Schema/DB/content NONE.** Verified: build · vet · **`go test -count=1 ./...` EXIT 0, 35 pkgs**. ⭐ **Leg C WALKED by the PO**, surfacing **M1-F5** (below).

### Next

- **⭐ WORLD PATHS owes C4 and C3** (`docs/plan-world-paths.md` §12): **C4 = re-author the 372 `Sand` blobs as paths** (PO-ruled 2026-09-07). ⚑ It is a CONTENT judgement — where the roads actually run — not a transform derivable from scattered blob positions, and it is also what first puts a path in the world, so **the in-game leg belongs to it**. **C3 (animated water) is ⛔ blocked on art**: `Water` authors `texture: null` because `features/regions/assets/ground` holds no water tile, and scrolling a flat colour is invisible. ⭐ C4 is also what finally arms the region primitive's OPEN look sitting — the shipped world has exactly one region covering everything, so no interior feathered edge exists to judge.
...
- **Gotchas & dev tools:** **A shape removed from `phy.Space` is purged from every other shape's collision set on the spot** (§54) — never "optimize" that sweep away ([[project-ghost-references]]). ⚑ **A content edit does NOT invalidate the Go test cache** — after any `api/` change use `go test -count=1`. ⚑ **The starting aura is pre-equipped but NOT active** — the first press of `1` switches it on. **Cheats:** GOD, WARP `<x·120> <y·120>`, SPEED, XP, SKILL, ANNOUNCE, THREAT, QUEST (dump / `ACCEPT` / `ABANDON` / `ADVANCE`). `make -C backend build` runs `cp-defs`; boot `-content ../api` for content iteration.
