...
- **⭐ Last completed: UI PASS C4b - THE UNLOCK BREADCRUMB TRAIL** ✅ 2026-08-29, `c22eadc0` (ledger: `docs/plan-ui-pass.md` §6 C4b; spec + rulings §5 C4b, detailed + built in back-to-back sessions): a light pulse leads to every spell discovered THIS SESSION until it is seen - open buttons + mobile ☰ while the book is shut → category tab → directional pager step → the row itself; seen = on-page **0.5 s dwell** (built at 1 s, halved at the PO look; PLACEHOLDER). ⭐ **D1 SESSION-ONLY, fully frontend** (reload clears; the declined server option stays priced in §5 - `character_flags` is insert-not-migration) · **D4 the baseline never pulses** - ⚑ the trail rides the SAME diff as the `.unlocked` stamp in `updateSpellbook`; a second diff computed anywhere else silently breaks the all-seen baseline. ⚑ The dwell is WALL-CLOCK `setTimeout`, never rAF (headless pages throttle rAF to ~6 fps). ⚑ `.breadcrumb` animates box-shadow ONLY so it composes with `.hasPoints`; known limit, flagged at the look with no change asked: on a ROW the id-scoped `.unlocked` glow wins the `animation` property (CSS animations do not merge) - C6's call if the pulse should win. **Schema NONE.** Verified first-hand: vitest 554/554 (+11 red-first) · tsc · prod build · new `c4b-breadcrumb` 17/17 at both dwell values · `c3-spellbook` 26/26 held · spot-checks green · `chunk2-follower` leg 6 INCONCLUSIVE settled IDENTICAL at HEAD (pre-existing D9 fragility) · mobile probe. Opus-built, reviewed line-by-line; ⭐ PO played 2026-08-29, the one change (dwell halved) applied + re-verified same session.
- **Prior: UI PASS C4 - SKILL ICONS** ✅ 2026-08-28, `1400e4fc` (ledger: `docs/plan-ui-pass.md` §6 C4; spec + rulings §5 C4, PO choice prompts same day): every one of the 72 `api/skills` definitions authors `icon: "author/name"` (game-icons.net), served through the `/skills` catalog via a one-field Go rider (the `displayName` precedent; mob-embedded skills serve `""`) - ⭐ **D1 all 72** incl. cheat rigs · **D2 spellbook rows** are the visible surface (slots/bar wait for C5) · **D3 functional placeholder VOCABULARY** (shared geometric glyphs by damage type/role, all to be replaced; landed at 33 glyphs vs the ruled ~15-25, deviation flagged + PO-approved). New **`.ink-token`** (2.2rem, zero extra row height, mobile needed NO reset) is the ONE direction-C glyph treatment - C5 re-sizes it, never restates it; `IconToken.ts` falls back to an initial letter. Pipeline: `scripts/fetch-skill-icons.mjs` vendors stripped SVGs (`currentColor` tint, no runtime HTTP, CC BY NOTICE); completeness pinned twice (Go content test + `SkillIcons.test.ts`). Recall/Camp ride `UTILITY_ICONS` (exact-set pin, Ascend absent by design). ⚑ SURFACED pre-existing: 3 census tests in `items/mobs` red at HEAD on the collaborator's Martin NPC (the stale gitignored embedded copy hid it until cp-defs ran) - unowned, see Open items. **Schema NONE** (⚑ first non-pure-client UI chunk: content + Go + client). Verified: vitest 543/543 · tsc · prod build · `go test -count=1` (skills, cmd/aurad) · `c3-spellbook` 26/26 held · new `c4-skill-icons` 6/6 · spot-checks green · mobile probe. Built by an Opus 5 agent, reviewed line-by-line; ⭐ PO approved 2026-08-28, same look slotted C4b (breadcrumb trail) before C5.
...
- **⭐ REGION PRIMITIVE: C1-C5 ALL SHIPPED** (C5 `4937a977`, ledger §12; audio consumers unscheduled, `plan-region-audio.md`). ⏸ **`docs/plan-region-primitive.md` stays live for the OPEN look sitting** (texture picks, the 0.35 scale, the seam, blend width, mask density). ⚑ The standing C4/C5 landmines and the no-second-region seam caveat (`c4-region-texture` leg 5 inconclusive by construction) live in the plan doc + [[project-region-primitive]]. Schema NONE throughout.
...
- **Feedback flow + the UI pass:** new feedback lands ONLY in **`docs/feedback.md`** (four exit doors; plan docs never double as intake). **All UI work is consolidated in `docs/plan-ui-pass.md`**: direction C "Inked Panel" RATIFIED (⚑ the §4 CORRECTION block - the RENDERED mockup CSS, no wiggly forms - is the spec for C6+); §5 order C1-C11 RATIFIED; **C1-C4b SHIPPED** (C1+C2 2026-08-26, C3+C4 2026-08-28, C4b breadcrumb trail 2026-08-29), all PO-approved in-game; **next: C5 (the ONE ability bar; its icon prerequisite is met)**. ⏸ `plan-onboarding-cleanup.md` DEFERRED, trigger: a human coworker is actually about to join.
...
- **Known-inconclusive at HEAD**, all unowned: ⭐ **3 census tests in `pkg/aura/items/mobs` RED at HEAD** - the collaborator's Martin NPC (`6c2e6d5c`) never bumped the hardcoded counts; the stale gitignored `backend/pkg/api/mobs/` copy kept the suite green by accident until UI-pass C4's cp-defs sync (remove/restore-proven). Fix = bump 3 counts or hand to the collaborator · `chunk3-charm.mjs` 6–8/9 (accepted D9 fragility) · `filler-batch.mjs` leg 1 (stale assertion) · `chunk3b-ii-conversation.mjs` 28/34 (leg 67 stale TownCrier assert, fixable; rest = Leave-click race + Wanderer drift-pin) · `accounts.TestRepeatedFailuresAreThrottled` red under `-race` ONLY · **`AuraTiledConvert.test.ts` byte-stability ×2 red on a fresh checkout** - they pin the PO's world.json repair, UNCOMMITTED by PO choice; green on the checkout holding that tree. ⚑ **Measure the rate before diagnosing a flake** - a misread cost `chunk3b-interact` two chunks.
