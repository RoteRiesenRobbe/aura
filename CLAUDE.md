...
- **Last completed: chunk A — a lost WebGL context now says so** (2026-07-26, frontend only, 1 file + 2 new + 1 new harness) ✅ `6c8bde2e` — **⏳ PO in-game check pending.** backlog §29 option A; full ledger `docs/backlog.md` §29.2.2. One `webglcontextlost` listener on the **world** canvas → a labelled `console.error('[webgl] world context lost …')` + the existing red `warning` banner. Executed to plan, **no deviations**: no `preventDefault`, no `webglcontextrestored` listener (that would half-implement option B), minimap canvas deliberately untouched. **⚑ The plan's riskiest assumption held** — a clean boot measures **5 GL contexts created, 2 deliberately lost** by pixi's capability probes and **0 warnings** (3 consecutive runs), so scoping to `application.canvas` really does see zero false positives; a banner on every clean boot would have been worse than no banner. Forced mid-boot loss: exactly 1 warning + banner, and the screenshot is the point — blank world with spellbook/slots/HP/XP intact, **FPS 60/60/60**, `Websocket PLAYING`, tick advancing. **This labels the failure, it does not fix it**: `ctxloss-repro.mjs` still yields the three `null.split` TypeErrors unchanged, the trigger is still unknown, and rendering is still dead ⇒ **§29 stays open** (option C is the only lever at the cause; a *player*-facing sighting promotes it). **2 known limitations, both by plan:** a loss *during* `init()` is unlabelled (`application.canvas` does not exist yet), and the banner is best-effort while the log is load-bearing (`AlertBanner.show()` no-ops before HUD setup — and mid-boot is the reproduced case). TDD: 6 vitest tests, red first on 2 behavioural assertions; **3 are negative pins** that make the plan's decisions falsifiable. New `.claude/skills/verify/ctxloss-warning.mjs` (`clean`/`forced`, non-zero exit on mismatch) — **re-run its `clean` mode after any boot-path change**, it is the cry-wolf check. Verified: `npm test` 21/21, typecheck + prod build clean, `go build ./...` clean.
