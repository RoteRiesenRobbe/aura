...
- **Last completed: Swift becomes a movement cooldown** (2026-07-29, backend + frontend + content + docs, 27 files) ✅ `a29fe986` — **the first item picked off the open-questions sweep**, and not a numbers item: the movement slot was *empty*. Dash is a blink and **Haste is aura cadence despite its name**, so nothing in the game gave sustained speed and a flat `stat_multiplier` passive was the weakest possible occupant. ⭐ **The shape: `speed_burst` is the movement twin of `tick_rate`** — both are self-targeted, cooldown-fired buffs of a factor plus a duration, so the 25th effect type is an existing one mirrored rather than a new pattern (payload in `skills.Buffs`, same stream rules — per skill the strongest wins, across skills they multiply — same `applier` seam, so **mob content can carry a sprint too**). ⭐ **`Buffs.MovementFactor()` is the ONE place the movement axis composes**: speed × (1 − strongest slow), floored at 0, read by *both* movement sites, because two independent readers would eventually disagree about which wins. **⚑ It closed a latent asymmetry nobody had noticed — `SlowFraction()` had exactly ONE reader** (`mob.stepLength`), so a slow applied to a **player** sat in the buff store unread. **Players are now slowable**; nothing applies one today (no mob authors a `slow_aura`), so no behaviour moved, but the *content surface* did — a mob slow aura is now a content decision instead of silently inert. ⚑ Enemy-targeted slows still cannot reach another player (targeting is faction-relative; two players share `FactionAligned`), but an **ally**-targeted one now would: a working griefing lever against GDD §9 where before it was inert — a review rule for future authoring, not a defect today. **⚑ The pip took bit 7 — the LAST bit of the `applied_effects` ubyte**; the next payload wanting a pip must widen the wire first (`backlog.md` §39 does that anyway). **Content:** `swift.json` → `category: cooldown`, 1.5× (+0.1/level) for 150 t (+30/level), CD 600 (−60/level), all [PLACEHOLDER] — Pass 1b re-touches them; keeps id 10 and its four wolf drops, so **Wolf's first-drop moment is unchanged and only what it drops changed**. Also **retired `TestContent_TeachingOrderMatchesPreMigrationZone`** (PO) — it proved the 3a/3b-ii moves were payload-preserving and content has since deliberately moved past it (`3b1b3ef6`); the half that was never about the migration survives as `TestContent_EveryGrantIsAResolvedTeach`. **Verified:** `go build`/`vet`/`test ./...` clean (**24 new Go tests**), frontend typecheck + **57 vitest** + prod build; boot `-content ../api` **0 errors 0 warnings 0 panics — 15 factions/86 skills/64 mobs/10 recipes/1 milestone/777 props/485 spawns**; **sim battery BYTE-IDENTICAL against HEAD on all four legs** (diffed against a HEAD worktree build; TTK 6.67 s / TTD 8.70 s stand). **In-game harness `swift-cooldown.mjs` 6/6** (0 console errors, 0 WebGL losses): unbuffed **1.32 u/s** → sprinting **2.01 u/s** = **1.52×** against the authored 1.5, clean separation across all 8 legs. **⚑⚑ The harness lesson — it cost four runs and faked two different product failures:** the first warp target sat in a **~2-unit pocket between blocking props**, so every walk measured the pocket, not the pace — identical 2.04 u legs whether sprinting or not (a flat 1.00× *"the buff does nothing"*), plus 0.00 u legs whenever two consecutive walks pushed the same way, which correlated so perfectly with key reuse that it sent the script chasing an input-handling bug that did not exist. On open ground the player walks a clean, time-proportional **1.5 u/s** — throttled headless rAF does **not** slow it, because the server coasts on held movement for up to `maxHoldTicks` (15). The script now warps to the most open whole-unit tile in the zone (computed from `world.json`) and reports **INCONCLUSIVE** if the unbuffed baseline is slow, rather than a ratio that can only flatter or fake the result. Pinned in the `verify` skill. Full ledger `docs/plan-playtest-feedback.md` §Ledgers.
