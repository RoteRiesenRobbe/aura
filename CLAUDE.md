...
- **Last completed:** **Phase 7 complete** — 7.1–7.3 (skill leveling + skill points) implemented and verified in-game (commit 442d2b50): per-skill levels in the spellbook (`map[SkillID]int`), derived point economy (flat 1 point/level, level 1 free on discovery, free respec, `skillPointsPerLevel` in conf.json), `SpendSkillPoint` wire message + `spellbook_levels`/`skill_points` on GameState, EquipSystem equips at stored level (gap closed), spend UI (`− 2/5 +` per entry, gold points badge) in the spellbook panel. 7.4: **full combinations design written** (skill-system-design.md → Combination System; all 16 catalog questions decided 2026-07-04). Before that: Block 3 (Phase 6 mob chapter, participation XP); players and mobs run on one SkillSystem.
- **Next:** Phase 8 — passives (8.1: DerivedStats, passive slots, spellbook category sections) then cooldowns (8.2: `cooldown_activations` input, ability bar, mob cooldown AI); first passive + first cooldown unlock via milestones. Alternative next block: survival removal + resource unification (roadmap items 1+2). Roadmap: docs/v1-roadmap.md.
...
- **Deferred tech debt / known bugs:**
  ...
  - Frontend `Skills.ts` hardcodes skill ID → name *and* maxLevel (for the "2/5" badge), duplicating the backend registry — sync manually when skills change; revisit (wire or generated file) when the skill list grows.
