...
- **Last completed:** **Dropped-movement fix — held-state input model (plan-input-jitter chunks 1+A+B). DONE 2026-07-24, [uncommitted], PO-VALIDATED LIVE.** Live report: *"I kept stopping while walking — the keys just didn't register."* **Chunk 1 (instrumentation, shipped + measured):** per-player input-transport counters (`starved`/`coasted`/`stalled`/`evicted` + a compile-locked 7-bucket starve-run histogram) on a rolling quiet-by-default `slog` `inputstats` line; `model.InputTransportStats` read via a narrow type-assert (NOT on the `Client` interface — the sim `nopClient` + 6 test fakes stay untouched), atomics on the client struct, tick-side plain fields, `AllocsPerRun==0` pin held. The live 14-min run **overturned the TCP-HoL prime suspect** — no eviction bursts, `q_mean` 0.7 ⇒ **client-side**: the browser Tock timer (`tocktimer/tock.js:53`) coalesces missed input ticks under render jank, so the queue starved ~20 % of ticks and `pickInput` *deleted* the movement. **Chunks A+B (held-state fix):** A — `pickInput` COASTS the last direction up to `maxHoldTicks=15` starved ticks (was a 1-tick bridge), cleared on death so a coast can't cross a respawn; B — the client re-sends an explicit zero-movement "stopped" input on key release (`STOP_TAIL_TICKS=5`, **no wire change**) so the coast can't ghost-walk. **Verified:** `go build`+`go test ./...` exit 0, alloc pins green, boot `-content ../api` 0 panics, headless smoke moved 365 u then **0.0000 residual drift**; **live re-measure (20 control bots + PO): browser input queue 0 starved / 0 coasted / 0 stalled across all 7 windows — the bug is GONE**; bots confirmed the coast covers network gaps (bot 1706 coasted 113/188 starved). Full ledger: `docs/plan-input-jitter.md`. **FOLLOW-UP (separate issue, PLANNED):** the re-measure exposed 10 % eviction / `q_mean` 1.09 (client-fast 30.303 vs 30.0 Hz) and the PO still feels *slight* "micro-resets" while walking (persists **solo**) — that is **downstream** snapshot-render jitter, NOT the input path (0 starvation proves it); own plan `docs/plan-render-jitter.md`.
