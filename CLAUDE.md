...
- **Last completed:** **Phase 8 complete (all three skill categories live), verified in-game** — 8.2 cooldowns: `cooldown_activations` on Input (queued at input time, fired same tick), `instant_damage` via new `phy.Space.QueryCircle` (one-shot query against last broadphase, no sensor lifecycle), whiff rule (player activation always consumes; mob AI fires-when-ready-and-hits-something, else holds), cooldown move semantics (one slot per skill, like passives), `cooldown_slots`+`cooldown_remaining_ticks` on GameState. UI: bottom-center action bars (aura slots as 2×2 grid: col1=slots 1+2, col2=slots 3+4; cooldowns right), spellbook+passives top-left; hotkeys **1–4 aura toggle, Q/E cooldowns** (Q removed from legacy alt-action, SHIFT stays); unlock glow = 5s double pulse full gold. Burst VFX: `BurstFired` status effect + `burst_radius` (px) on Character/Mob → exact-size gold fade ring for players AND mobs. New effect types: `slow_aura` (transient `Mob.ApplySlow`, re-applied per tick, strongest wins), `self_heal` (caster only, mobs can't), stat `damageReduction` (in takeDamage). Content: NovaBurst (milestone L4), AngryMammothStomp (boss), SlowAura (Mammoth 20%), Heal (milestone L2), ToughPassive (Dodo 5%) — 13 skills, 4 milestones. Before that: 8.1 passives, Phase 7 (leveling+points; combinations design written → skill-system-design.md → Combination System).
- **Next:** **Block 2 in progress** (survival removal + resource unification, roadmap items 1+2) — full staged plan + status in **docs/block2-resource-and-survival-removal.md**. Stages 1, 2, 3a, 3b **done** (single resource; DayCycle/heater removed; wire `satiety`/`body_temperature`→`level_progress`/`level`; item actions + harvest + inventory loot/crafting + food/tool defs removed; resources decorative; campfires inert stubs). **3c next** = frontend item-system teardown, then **3d** = wire cleanup (remove dormant `inventory`/`equipment`/`Action`/`OngoingAction` wire + Go types). After Block 2: Phase 9 (combinations impl; steps in skill-system-design.md Phase 9) or item 11 (aura targeting).
- **Current state:** new players start with DamageAura in slot 0 on spawn, server-authoritative from spawn. Unlock sources live: milestones (HealAura+Heal L2, SwiftPassive L3, NovaBurst L4), kill drops (WildAura: SaberToothCat 20%/boss 100%; SlowAura: Mammoth 20%; ToughPassive: Dodo 5%). Skill points: earned per player level, spent/refunded freely in the spellbook panel, equipped skills scale live. All combat participants (incl. healers, ~10s window) get full XP on mob death. `XP <amount>` cheat for manual leveling.
- **Deferred tech debt / known bugs:**
  ...
  - **BUG — `KILL` cheat no longer kills (found 2026-07-04, Block 2 testing):** one-shot zeroing of `Health` is reverted before death is detected. `KILL` sets `Health = 0` in `CommandSystem` (prio −50); `UpdateSystem` (also −50, runs after) regenerates any `Health != Max` via `updateVitalSigns`, bumping it to a tiny positive value the same tick, before `ConnectionStateSystem` (prio 10, `sys/state.go`) checks `Health == 0` next tick → player survives at ~minimum health. Continuous-damage deaths still work (`SkillSystem` −65 re-pins to 0 each tick after regen). Likely pre-existing (regen predates Block 2). **Fix candidate:** don't regen at `Health == 0` (0 = dead) — one-line guard in `updateVitalSigns` + a `KILL`→dead test. See block2 doc.
