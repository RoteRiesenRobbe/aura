...
- **Last completed:** **Session ⑤ (C8 drop-rate pass FINAL + NPC pass + walkthrough part 1) DONE 2026-07-20, PO-driven walkthrough in-session — committed `1ef67776` + `ac44bae5` + `d5263355` (main, per PO directive); full ledger: `plan-content-zones12.md` §13 C8 Session-⑤ banner. Parallel session landed `e2643cdb` (mob pathfinding detour-commit + camp watchdog) — no overlap.** **(1) Drop table FINAL** (kph-derived event pacing Z1 ≈10 min / Z2 ≈20 min expected; **no pity — pure per-kill RNG FINAL**, closes GDD §11 TBD): Light .05×2 / Hardy .03 / Dash .025 / ThickHide .04 / Berserker .03 / Swift .10 / Fade .015 / SlowAura .03 / NovaBurst .05 / Recover .02; Antivenom + elites kept; **WildAura → DireWolf .06** (live home; legacy carriers only in proving-grounds → Step-7 cleanup); **OrcWarlord + Rejuvenation .10** = first boss rare re-farm drop (pattern: guaranteed identity + low-chance hunt slot; FireWard = last orphan, reserved for fire content). **(2) NPC pass:** body 1.0→**0.35**, sensor 3→**1.5** (all 13), **Farmer = EntityType 62** via full 5-file wire path. **(3) Walkthrough fixes:** aura-swap-into-active-slot left no aura active (invisible under darkness) — equip re-activates the slot, test-pinned; **Vanguard shield 13→~2.7 HP/s** (damage checked vs §A league = in-line, untouched); **regen taper 1.0@L1→0.4@max** (amends the 1%/s lock top-end only, pinned). **Settled FINAL:** campfire `0.12`, heal cost `10 −2/level`, drop table, §11 no-pity. Teachers read OK; tier readability vague-but-consistent = acceptable (real fix = deferred items 7+15). **Verified:** build clean, suite green, boot `77 skills/13 factions/44 mobs/10 recipes/805 props/346 spawns/2 campfires/13 npcs, 0 panics`, headless client smoke, PO in-game re-checks.
