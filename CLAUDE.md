...
- **Last completed:** **Applied-effects pips: buff/debuff visibility on avatars — DONE + PO-verified in-game 2026-07-21 ("works"), committed `1358b9bc`; full ledger: `plan-intermission-triage.md` §Applied-effects pips.** PO-picked ad hoc (ahead of queue item ②): a dot/buff/debuff was invisible until its first damage tick — now colour pips appear under the overhead HP bar the moment it lands, players and mobs alike. **PO picks (choice prompts):** colour pips at the nameplate/overhead bar (not tint/ring/icons — no icon assets exist, pips reuse the item-7 colour language); **all kinds except shield** (shield_hp + absorb segment already show it); **presence only** (no duration on the wire). **Two directions, two fields:** `aura_category` = what an entity *projects*; new `applied_effects` = what is applied *to* it (`skills.Buffs` — previously nothing of that store hit the wire except derived `shield_hp`). **Backend:** `skills.AppliedEffect` bitmask (dot/slow/hot/resist/tickrate) via `Buffs.AppliedEffects()`; exhaustiveness **compile-enforced** (stronger than the aura_category table): `appliedBit()` sits on the `buffPayload` interface, so a new payload kind fails to *build* without a pip decision. TDD `TestAppliedEffects_*`. **Wire (append-only, no ID shift):** `Mob.applied_effects` + `Character.applied_effects`, `ubyte`, both bindings regenerated. **Frontend:** one shared `EffectPips` renderer for characters+mobs; dot/slow/hot reuse the ring colours (now exported as `AURA_CATEGORY_COLORS`, single source); resist teal `0x5fbfb0` + tickrate orange `0xe0812e` are first-ever colours [PLACEHOLDER]; player pips on the unfiltered nameplate plate (night-readable), mob pips with the on-body bar. **Gotcha (smoke-caught):** the own player bypasses `EntityManager` (`Player.updateFromBackend`) and needed its own `setAppliedEffects` call. **Watch item (cost a PO round-trip):** regenerating the FB **TS bindings needs a webpack dev-server restart** — `api/schema/js/` is outside `frontend/src`, HMR doesn't watch it (`entity.appliedEffects is not a function` while the prod build was fine); sibling of the config-change-restart item. Also: `pkill -f` self-kill applies to any pattern in the invoking command (`pkill -f "npm run start"` exit 144) — kill by PID. **Verified:** `go build` clean, full suite green, `tsc` clean, prod build clean, boot `-content ../api` `81 skills/14 factions/50 mobs/10 recipes/847 props/383 spawns/5 campfires/14 npcs, 0 panics`; headless smoke with real before/after — no pip at spawn, purple dot pip under the player's bar inside the FireElemental burn aura (screenshot-confirmed on :2000 prod AND :2001 dev server post-restart), no runtime errors.
