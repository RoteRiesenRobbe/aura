...
- **Last completed:** **Intermission fixes mini-chunk (Session ① of `plan-intermission-triage.md`) DONE (2026-07-19), one session — resumed after an interrupted worker session; AUTOMATED in-game verify PASS, NOT yet PO-verified, NOT committed** — per `plan-intermission-triage.md` §Execution sequence Session ①​ (that doc is the authoritative DECIDED ledger). **Items:** **19** bandit radii (healer 2.5→1.0 / volley 5.0→3.5, pure JSON); **1** heal self-kill clamp (TDD — scaled self-cost pre-computed and clamped to leave the caster ≥1 HP; a caster already at 1 HP skips the whole effect, no heal/no cost; zero-cost components Vanguard/Warbanner unaffected); **14** respawn full-health re-stamp `Health = MaxHealth()` AFTER skills restore (Hardy-carrier test — was respawning at the base pool); **16-code** portrait rotation freeze — `Mob.setRotation` ignores the wire heading + `Character.setRotation` pins the LOCAL avatar too (PO ruling) + `angryMammoth→demon` placeholder retired; **5** data-driven `startingSpawn` campfire flag — schema→`CampfireAnchor`→boot loop, `defaultSpawnPosition` picks only flagged fires (defensive fallback for bare test zones), `z.validate()` hard-fails campfires-but-none-flagged; west village fire flagged, proving-grounds `(0,0)` flagged; **11** Harvest via Farmer — `initializePlayerSkills` now returns an EMPTY component (spawn truly empty), Harvest added as the Farmer's first ungated teaching, **`sim/world.go` now equips its own synthetic aura** (the sim relied on the old freebie → fixed), 4 player tests rewritten; **21-partial** `Items.ts` `== null` guard; **3/9** manual drift DRIFT-A/B/C + portrait-style checklist + NPC end-to-end walkthrough (zone-editor §5c); **GDD §11 + roadmap** — character sacrifice pulled into v1 scope (item 10). **Verified:** `go build` + full suite + `-race` (sys/world/player/sim/simharness) green; boot clean — 75 skills/12 factions/40 mobs/10 recipes/620 props/185 spawns/**2 campfires/10 npcs**, startingSpawn validation passes, 0 panics; **headless browser** — fresh spawn @(-58.4, 23.4) = west fire (item 5), spellbook empty at spawn (item 11), Farmer teaches Harvest on approach (item 11), local avatar rotation pinned to π/2 before+after WARP (item 16), 0 client errors on warm re-run. **Watch item recurred:** 1st-run triple `null.split` pageerror (C4 item), same first-run-after-fresh-build signature, gone on the re-run — still unlocated (= deferred item-21 full repro). **Item-3/9 code niceties also DONE this session (PO said "do them now"):** DRIFT-D 8-mode comment fixed in `_ZoneEditorPanel.ts`, vestigial `'zone'` editor default → `'world'` in `ZoneEditor.ts`, and a **Resource-backed NPC `entityType` validator** in `world/zone.go` `resolve()` — rejects an NPC entityType that names a MOB wire sprite (blocklist, so `Signpost`-style resource sprites stay valid; +2 tests). Both zones boot clean under it.
- **Prior:** **Step-6 C7 (Recipe net) DONE (2026-07-18), full chunk in one session — PO-VERIFIED IN-GAME 2026-07-18, committed `53868697` ("all in all, it works"; two findings handled same session: Vanguard/Warbanner overshields cut to ~⅓ (5+2 / 6+2.5), and the Wildfire/Suppression/Barrier ingredient placement gap recorded — Ignite/Immolation sourceless, SlowAura/ToughPassive only on never-spawned legacy mobs → C8 places them, §11 + content-recipes.md audit)** — per `plan-content-zones12.md` §13 C7 (banner there records the full ledger). **PO rulings:** capstone = YES → **Warbanner** aura (Vanguard 5 + CallForAid 3, base ingredients only); Vanguard trio = **specialist** results; thresholds = all ingredients maxed net-wide; CallForAid partners = Taunt + HealAura (PO pick over the Heal cooldown); gap fills = fire + kiter; extra PO recipe Hardy 3 + ToughPassive 3 → **Barrier** (the existing cheat-only instant-shield becomes the result — cooldown because that's the rarer category after the net (20 auras/18 cooldowns), closes the §11 "Barrier home" item and avoided the instant_resist lift → **chunk stayed zero-lift**). **Content:** 10 recipes (`api/recipes/`, ids 1–10 incl. Paladin); 8 result skills 52–59 — Spearhead (~115% damage, 3 targets) / Lifewarden (2-target no-cost heal) / Shockwave (~2× DamageBurst) / Warbanner (all Vanguard components ~110% + slow — best generalist, each component below its specialist) / HoldTheLine (detaunt + 3 ShieldbearerCompanions) / FieldMedics (2 soldiers + MedicCompanion) / Wildfire (2-target fire dot @~70%) / Suppression (LRS-range damage+slow @~70%); summons reuse RallyDrum/HealerAura mob skills (zero new mob skills); EntityTypes 58–59 + SVGs; client ring styles (Lifewarden support, Warbanner dual); registry pin 67→**75**, recipe pin **10** + `TestRecipes_C7Net` cascade test; sim player-aura presets auto-derive Spearhead/Warbanner/Suppression (§A "never a surprise"); coverage audit in `content-recipes.md` (gaps = deliberate post-v1). **Verified:** suite + race green; boot 75 skills/12 factions/40 mobs/10 recipes/620 props/185 spawns/10 npcs + encounter registered; browser smoke 2 chars × real spellbook spends — **all 9 results unlock in-game** with gold banners (48-point net vs 29-point char = deliberate build split), Warbanner dual ring active, FieldMedics + HoldTheLine squads spawn (~80 s cooldowns), 0 client errors runs 2–3. **Watch item recurred:** 1st-run triple `null.split` pageerror (C4 item), same first-run-after-fresh-build signature, gone on re-runs; still unlocated.
- **Prior:** **Step-6 C6 (Ork World Boss, §B) DONE (2026-07-18), full chunk in one session — PO-VERIFIED IN-GAME 2026-07-18 ("works as intended"), committed `5961b29a`** — per `plan-content-zones12.md` §13 C6 (banner there records the full ledger). **PO rulings:** fight = "Warbanner gate + waves" sketch (invuln while either of 2 totems stands; OrcGrunt waves at 66/33%; one-shot banner re-gate at 33%); enrage = **rotating Frenzy** (mob-fired tick_rate cooldown — NO §9 lift 5); Boss-Aura = **Call for Aid** id 51 (cooldown, THREE spawn effects → 3 SoldierCompanions in one cast — fireCooldown loops effects, zero code); boss cL23 boss tier (above the L20 band); totems = gate + shield_aura allies-only; wipe = base leash+walk-home+regen observed by the script; respawn **~5 min** [PLACEHOLDER] + return broadcast; kill broadcast **with credit names**; arena = light dressing. **Two lifts (TDD):** (1) **alert/broadcast** — `chat.Broadcast` (EntityMessage id 0 → every player) + `ANNOUNCE <text>` cheat + client **AlertBanner** (upper third, queue, fade; unlock banners ride the existing spellbook diff, new discoveries only); (2) **zone anchors** — `anchors` schema section (unique names, in-bounds) + `Zone.AnchorPos` + full editor tool (place/rename/delete, crosshair markers; manual §5b) — the encounter resolves warlord-home/warbanner-1/-2/wave-mouth at registration, boot **hard-fails on a missing anchor**. **Script:** `encounter/warlord.go` (first designed encounter; gate re-derived AFTER replants — no 1-tick vuln window; wipe re-arm keyed on `engaged` so pre-pull banner kills stay down; `System.Despawn` + `Mob.KillCreditNames` seams). Content: mobs 35–38 (OrcWarlord XP 600 / WarbannerTotem killable XP 0 / OrcGrunt XP 5 / SoldierCompanion), mob skills 128–131, pin 62→67, EntityTypes 54–57 + SVGs, arena boulders + 4 anchors in world.json (append-only, flood-fill green: 4 anchors/185 spawns/10 npcs). Roadmap 9f REMOVED from v1 scope. **Verified:** suite + race green; boot 67 skills/12 factions/38 mobs/620 props/185/10 + encounter registered; browser smoke 0 client errors (ANNOUNCE banner, gold unlock banner, warlord `invulnerable=true` with both banners in THREAT dump, Call for Aid → spellbook → slot Q → 3 companions server-side; arena-side companions die to the at-level war at owner L1 — correct). Wave/re-gate/wipe/kill beats unit-pinned. PO in-game pass 2026-07-18: works as intended.
- **Prior:** **Step-6 C5 (the front + Front-Aura) DONE (2026-07-18), full chunk in one session — PO-VERIFIED IN-GAME 2026-07-18, committed `96cea32f`** — per `plan-content-zones12.md` §13 C5 (banner there records the full ledger). **PO rulings:** Front-Aura = **Vanguard** (Damage+Heal+Shield: full DamageAura ×2 targets + free HealAura + RallyDrum-class shield allies+self — the §A sanctioned power outlier, GDD §4/§5 amended); **anchor L20** ("journey's final step in v1" — re-banded the front: soldier cL18 / orc cL20, closes the §11 anchor item); sim presets = derived player damage-aura presets. **§9 lift 6 landed (TDD):** faction `friendlyToPlayers` → `model.PlayerFriendly` → `mayHarm` skips friendly targets for aligned casters (players + owned summons). Content: `human_army`+`orc` factions; mobs 32–34 (ArmySoldier XP 0 / Orc elite XP 15 / SpikeBarricade brazier, players-only by faction choice); skills 125–127 + Vanguard 50; pin 58→62; EntityTypes +4. Zone: checkpoint opened (middle C4 cap GateWall removed — the one non-append change), soldier line vs orc line (unattended war), 9 barricades + funnel, **west arena x 23–33 kept empty = C6 boss canvas**, S-exit teaser, FrontCaptain @20 real TooLowLine → 615 props / 185 spawns / 10 npcs (flood-fill verified). **Verified:** suite+race green; boot 62/12/34/615/185/10; browser smoke — gate holds at L1 / teaches at cap, Vanguard equips+activates (self-shield visible), war fights unattended, 0 client errors runs 2–3. **Watch item recurred:** the 1st-run triple `null.split` pageerror (C4 item) — once on first run after fresh build, unreproducible with stacks armed; still unlocated. PO in-game pass 2026-07-18: passed (front feel, checkpoint + S exit, soldier immunity, Vanguard).
...
- **Next:** **Session ② — pre-C8 lifts + content (PO directive: run with ultracode) — execution session (new chat)** per `plan-intermission-triage.md` §Execution sequence: item-**2** heal-cost linear scaling lift (schema+loader+apply+UI number — PO ruling (a), heal>cost from L1) + item-**13** campfire `healFractionOfMax` lift (value stays C8) + **6** companion follow-target jitter (PO ruling (c)) + **18** Z2 wildlife variant defs + **20** placements (fire-teacher NPC for Ignite/Immolation, SlowAura/ToughPassive/Fade/NovaBurst/SummonTotem sources, Revive/Reaper world ports) — **un-breaks Wildfire/Suppression/Barrier before C8 balances them.** (Session ① is done pending the PO's own in-game pass + commit.)
