...
- **Last completed:** **Playtest-1 feedback triage (planning session) — DONE 2026-07-22, committed `ff1d5465`; full plan: `docs/plan-playtest1-feedback.md`.** First *external* playtest (tester with little game/MMO experience). Headline: **"Die Mechanic an sich ist fun"** — failures cluster in missing onboarding, NPC/unlock communication, and too-easy/uncontrolled balance. **PO decisions (choice prompts, 2026-07-22):** density standard **KEPT** (difficulty via mob HP cL4–5+ and Z2 damage up, not fewer spawns); **gray-aggro removed** (mobs `cL ≤ player cL − 5` [PLACEHOLDER] no longer aggro; flat gray XP stays DECIDED); **drop philosophy hybrid** (most ability drops → elite/boss, a few hand-picked low-chance commons); **campfire hard safe-zone** (mobs never enter the radius, chase breaks); cLv difficulty indicator = **WoW-style nameplate colors**; **equip flow click-to-bind** (spellbook click → slot click/key). **Execution order: Pass A (Balance & AI) → Pass B (Communication & small fixes) → Pass C (UI features), slotting in FRONT of step-8 persistence planning.** Own planning rounds later: tutorial/onboarding strip (extend the village-arrival intro, not overlays), quest/hint-journal question, aura differentiation (matches the "Wild trap pick" note). No code this session.
