...
- **Last completed: R4 — the last two interact-badge defects** (2026-07-29, frontend only, 3 files + 1 new harness) ✅ `a4225e8b` — **the chunk that CLOSES `plan-entity-model.md`; the doc is now archived** (`docs/archive/plan-entity-model.md`). Both fixes are **latent-by-content**, which is the whole character of the chunk: neither is reachable with anything authored today, and both arm the moment content moves one step. ⭐ **The anchor:** `InteractBadge` now takes the container it is drawn INTO and the container it MEASURES as two separate arguments — attached to `Mob.shape` (so the cap still fades and darkens with the mob), anchored off `actualShape` (the art alone), because the group also carries the aura ring stack, the dwell ring, the tick indicator and the health bar. **Measured, not argued:** with an aura on the badged actor the group's top goes −42 → **−211.5 px**, so pre-R4 would have parked the key cap at −232.5 instead of −62.5 — **170 px above the NPC's head, permanently**, since `build()` runs once. ⭐ **The corpse:** `fadeOutAndHide` now calls `setInteractable(false)` beside the existing `setAuraRadius(0)` — same idiom, same reason. The ordering the review claimed is **confirmed**: `Backend.receiveSnapshot` runs `newSnapshot` at :294 and `updateInteractBadge` at :373, so on the removal tick the object is already out of `objects` and the client-side retarget can never land — ⚑ which is why it must be fixed there and not in `Backend`, which has no handle on an object it cannot resolve. **⚑ The health bar was never part of it** — it hangs BELOW the mob (`bar.y = max(30, size*0.9)`), so it only inflated the group's *bottom*; only the ring reaches above the art. **Verified:** new `r4-badge.mjs` run **twice against the same build** with a throwaway `role: structure` + `TotemAura` on the Hermit (reverted) — **7/7 both legs**, 0 console errors, 0 WebGL losses: *aura* group top −211.5 → badge y **−62.5** (delta 170 px vs pre-R4); *vanilla* group top −42 → badge y **−62.5** (delta **0.5 px**, i.e. a no-op on shipped content — exactly why it was latent); corpse leg 0 badges across ~12 samples with 10–20 fading corpses on screen. Plus `go build`/`vet`/`test ./...` clean (no Go changed), typecheck + **57 vitest** + prod build, boot `-content ../api` **0 errors 0 warnings — 15 factions/86 skills/64 mobs/10 recipes/1 milestone/5 props/777 props/485 spawns/5 campfires**, `npc-portraits.mjs` clean. **⚑ FINDING — a full sweep of all 15 browser harnesses (2026-07-29, the first ever run in one go) found FOUR rotten and one dead, none of them caused by recent code.** `chunk3a-npc-merge.mjs` was **DELETED** (0/6 — every check asserted that *approaching* an NPC teaches you, the behaviour 3b-i deliberately reversed; not repairable, since its premise *was* the behaviour). `chunk3b-interact.mjs` (6/15) was **narrowed to the verb** — who is offered, what the key does, the badge lifecycle — because 3b-ii had moved teaching behind a panel row click; everything *inside* the panel now belongs solely to `chunk3b-ii-conversation.mjs` (29/29 + 1 SKIP), so a content edit breaks one harness instead of two. That script's own rot was an exact teaching-row **count** that `3b1b3ef6` grew; it now asserts the rows by name and covers the fourth. And `chunk2-calm` / `chunk2-follower` / `chunk3-charm` each had one assertion that reported a working feature as broken — calm's `last >= first` failed by **0.11 units** on a settling mob, and the other two required a pet to be **alive and visible** during the fight when **D9 says it gets focused and dies in ~8 s**; both now assert on **XP rising with every aura slot empty**, the attributable evidence (the player deals no damage, so a rising XP bar can only have come through the pet). All now green: 7/7 · 6/6 · 9/9 · 14/14 · 29/29. **⚑⚑ Two causes were environmental, not code, and both cost runs before they were understood:** ① **a long-lived server invalidates every venue** — mobs wander far from their authored spawns, so a spot picked by reading `world.json` stops describing the world; a restart alone restored three of charm's four checks with no code change; ② **conversants stand in CLUSTERS and the server offers the NEAREST** — Farmer, Hermit and TownCrier sit within ~3 units, so a script warping "to the Farmer" gets the Hermit and *walking away until the badge goes out* just walks into the next one's range. Badge-lifecycle work now runs on the **isolated** Emberkeeper (30.5 units from any other conversant). **⚑⚑ And the new harness faked the product bug THREE times** (all pinned in its header): it warped to the Farmer and the server offered the **Hermit** — 0.6 units vs 2.6, the only one inside the 2.0 talk range — so the first clean-looking 7/7 **measured an actor with no aura and proved nothing**; the removal latch then fired twice on the live, legitimately-badged NPC, once because *"some corpse is fading somewhere"* is not the same event as *"my actor was removed"*, and once because reading badge/corpses/position as **three round trips** let the warp land between them. The probe is now a **single atomic `evaluate`**, and the script asserts its own precondition rather than assuming the intended actor was reached. Full ledger `docs/archive/plan-entity-model.md` §11.
