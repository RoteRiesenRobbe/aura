...
- **Last completed: `plan-entity-model.md` Chunk 2 — the authored `role` discriminator** (2026-07-27, backend + 14 content JSONs, 42 files + 5 new) ✅ `0be771bd` — **planned and executed in one session** (§5 is the plan, §11 the ledger). ⏳ PO in-game check pending on the **follower half only** (see below). **A mob now says what it IS instead of implying it with a stat value:** `creature` (default) / `structure` / `follower` on the definition, validated against **one source table** via `mobs.ParseRole` — the `tierRanks` precedent, and the only list: the JSON loader, `sim.MobSpec` and the `-mob-role` flag all resolve through it. **Three reads retired:** `auraAlwaysOn := Factors.Speed <= 0` (the stored bool is deleted), `isFollower() = owner != nil && velocity > 0` → `role == follower && owner != nil` (D4 — role is the authored *intent*, ownership the runtime *precondition*, so an editor-placed companion degrades to creature behaviour rather than standing inert), and the **10 dummy `aggroRadius: 0.1`** values (optional for structures now). ⚑ **Four `speed <= 0` reads deliberately SURVIVE** — `moveTowardsScaled`, `moveAwayFrom` and the two wander/waypoint validators are statements about *movement*: *"speed 0 means it cannot walk"* stays true, *"speed 0 means it is a turret"* is what went. `roleSlots` → **`loadoutSlots`** (the combat role is a different axis and the word collision was live in one file). **⭐ The finding that resized the chunk: `speed: 0` is a MECHANISM inside the SIM, not just a value** (new landmine **L9**) — `chain.go` pins the kite-stance mob at 0 precisely so *"its aura stays always on and it still fights back"*, and the kite stance is half the **chain battery, which is where the level curve comes from**. PO ruling: **explicit role in the sim, no shorthand** (*"aren't we moving away from speed defining what something is?"*) — an inference in the balancing harness is worse than one in the model layer, not better. The explorer needed a real string-knob branch: its `KNOBS` table is uniformly numeric and `buildRequest()` runs every value through `parseFloat`, so a string would have `NaN`-ed the run silently. **3 PO rulings this session:** ① `aggroRadius` optional for **structures only** — the 2 follower dummies become an authored **3.5 [PLACEHOLDER]** (inert today: a non-support follower's sensor is read by nothing); ② the sim authors its role explicitly; ③ **no loader guard** on `speed: 0` without `role: structure` — *"a turret is something I would want to build"*, so the **50-def content census** is the safety net instead. **Content:** 36 creature / 10 structure / 4 follower; all 14 `_comment` blocks rewritten (they were *teaching* the retired inference); `FireTotem`/`Totem` are now live proof that role and ownership are **orthogonal** — structures **with** an owner. Verified: TDD **red-first, with each pin authoring a role that CONTRADICTS the old inference** (a **speed-0.7 structure** keeps its aura on, a **speed-0 creature** gates it, an **owned structure** does not follow) so none can pass against a speed read; `go build`/`vet` clean; `go test` **exit 0, 27 pkgs**; simharness + **alloc** guardrails `-count=2`; boot `-content ../api` **0 errors 0 panics 0 warnings** (83 skills/14 factions/50 mobs/10 recipes/5 prop defs/1 milestone/777 props/471 spawns/5 campfires/14 npcs) — **the real gate here**, since the sim never loads authored content (L4). **⭐ Sim battery, level curve, pack matrix AND the chain battery BYTE-IDENTICAL to a HEAD build** (`git worktree`) — **the kite row included, which IS L9 holding**; the 50-mob preset roster moves in **exactly 12 cells, all `aggroRadius`** (10 structures → 0, 2 follower dummies → 3.5), the predicted **L10** delta and nothing else. **In-game smoke 3/3** (`.claude/skills/verify/chunk2-roles.mjs`, kept): campfire **+46 HP in 8 s** where regen alone gives ~8 (the *rate* is the evidence), poison pool kills an idle player unprovoked, bramble stops a 3.2-unit walk at the wall face — **0 console errors, 0 ctx losses**. ⚑ **Two harness traps, now comments in the script:** the dev console input `stopPropagation()`s keydown, so **WASD is swallowed while it holds focus** (the first bramble run "passed" while the player never moved), and **screen-up is DECREASING world y**. **Not covered in-game: the follower half** (summoning needs a cooldown equipped through the aura panel) — pinned by the Go suite instead; **PO: summon a companion, confirm it trails you and fights what you fight.**
