...
- **Last completed:** **Session ⑦ — C8 walkthrough close-out + village-arrival intro rework — DONE + PO-verified in-game 2026-07-21 ("the intro feels much better now"), committed `f9345739`; full ledger: `plan-content-zones12.md` §13 C8 Session-⑦ banner. C8 is CLOSED.** The Session-⑥-deferred low-band cheese landed live (group in the turnip field power-levels everyone) → PO ruling: **turnip XP 30→0** (pure Harvest demo target; 16 old-field spawns → 5 at a new farm field) and the start reframed as **adventurers arriving in a wolf-beset village** — **TownCrier NPC (new EntityType 65**, full 5-file wire path) at village center teaches DamageAura @L1 on arrival + Recall @L3 with the go-east/militia hook; Farmer moved south (−57/28.6) beside a new Sand-patch field, teaches only Harvest (ungated); healer hermit pulled into the village (−54.9/25.6, Hermit type + lore breadcrumb kept, HealAura @L2 unchanged); "adventurer" rename dialogue-only. PO editor pass folded in: wolves 86→97 (+1 DireWolf), RoadWarner → south-road Wanderer_1 + 3rd campfire, KoboldSign/TunnelSign deleted on purpose. Stale sim pin fixed (`TestLoadPlayerAuraPresets_DotSkillsDerive` expected Immolation 10 vs 10.5 dot comp — suite was red at HEAD since `635a44e3`). Walkthrough remainder blanket-verified in-game, no new triage. **Verified:** suite + `-race` green, boot `78 skills/13 factions/44 mobs/10 recipes/798 props/347 spawns/3 campfires/14 npcs, 0 panics`, headless smoke (TownCrier renders own sprite + teaches @L1, Harvest unlock banner pops). **Watch:** the zone editor does NOT round-trip NPC `entityType` (hand-restore on every editor-added NPC) and its Download lands outside the repo (`world .json`).
...
- **Next:** **Z2-hardening / cL8-17 farm-band PLAN chunk — new chat, plan-first** (PO 2026-07-20; C8 closed 2026-07-21): Z2 difficulty+density "up by a lot" + dedicated high-tier farm content; design target = the guardrail-flagged cL8-17 normals gap; execution in a later session. **Owns the gray-band XP rule decision** (turnip cheese resolved Session ⑦ via XP 0 — remaining low-band cheese is live-mob camping, e.g. wolves/Healer/Mammoth) **+ new mobs inherit the Session-⑥ XP rule**. PO continues manual placements/corrections in the editor (campfires, density, road corridor, post-intro polish — crier/healer/farmer/field coords are first-pass). **Reconnect-token persistence** (reload loses character) = new small plan-first chunk when PO calls it. **Every C1+ mob MUST be authored tier + baseline** (raw `maxHealth` hard-fails; manual §1); combat mobs need NO harvest entries (gated damage tags — manual §1), but **gate obstacles must opt in**.
- **Deferred (per triage §Execution sequence):** **Post-C8 "combat readability"** — items **7** (category ring colors) + **15** (tier frame ring), two append-only Mob-table wire bytes. **Post-C8 tooling:** standalone browser map editor (**backlog §22**, seeded from the Session-④ density-pass renderer/generator in that session's scratchpad). **Step-7 rebrand** — items **22** (bare-name renames) + **12** (`legacy:true` tags). **Persistence step** — item **10** sacrifice loop (first consumer; GDD §11 amendment landed Session ①). **Anytime/annoying:** item **21** full `null.split` repro session. Placeholder values still open (all re-felt OK in-game Session ⑦ 2026-07-21, none declared FINAL): Troll cL11 / Pyromancer cL6 / Dire cL6-7 tiers, teacher gates Immolation 6 / Totem 5 / Revive 6, regen-taper floor 0.4, Vanguard shield 4+1/90t, WolfBite radius 1.0, **XP table v1** (rule `3600 × 1.15^(cL−1)` ÷ kph, Session ⑥; turnip → **0 XP** Session ⑦, L1→2 on wolves), **crit rework v2** (base `0.05` conf / default critFactor ×2 / KeenEye 2%/level ×5 / DireWolf drop 0.06 / dot +5% comp — backlog §23). Standing locks: growth 1.12 × maxLevel 30 (≈27×, band ≈ +5, lower-first); **regen 0.00033 ≈ 1%/s FINAL × level taper 1.0→0.4** (C8 settlements 2026-07-19 + 2026-07-20); **drop table FINAL + §11 no-pity FINAL + campfire `0.12` FINAL + heal cost `10 −2/level` FINAL** (C8 Session ⑤ 2026-07-20; KeenEye→DireWolf 0.06 added post-FINAL by PO choice 2026-07-20); **milestone table FINAL: Heal L3 / Haste L7** (C8 Session ④ 2026-07-20); **density standard: mob visible on every ⅔-screen (17×9.5 u) window** (C8 Session ④); tier frame: elite ≤ 25% facetank / boss kills the bot / normal = per-mob texture + Z1/Z2 band-check (front exempt); **downtime 10 s** + chain 20; guardrail asserts LANDED (`cmd/simharness/guardrail_test.go`, deterministic). Dev cheats: GOD, WARP <x·120> <y·120>, SPEED [factor|off], XP, SKILL <name>, ANNOUNCE <text>, THREAT [id]. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; boot `-content ../api` for content iteration (schema/Go changes still need the rebuild).
