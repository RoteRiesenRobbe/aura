...
- **Last completed:** **Wolf-line drop reshuffle + milestone trim (ad-hoc tuning session) — DONE 2026-07-21, PO-driven in-session, committed `0ef4b738`; full ledger: `plan-intermission-triage.md` §Wolf-line drop reshuffle.** PO was playtesting, found the four-wolf drop set confusing, retuned it live. **Framing ruling (PO 2026-07-21): the C8 "drop table FINAL" and "milestone table FINAL" locks are first-pass settlements, NOT frozen values** — as in any MMO, drop sources/rates and unlock placement tune continuously on feedback; FINAL here means "decided well enough to build on", not "never revisit". Treat both tables (and essentially every [PLACEHOLDER]) as tuning-open. **Changes:** FirstAid leaves the milestone table (the village Hermit teaches it @L2, *earlier* than the L3 milestone granted it — dead weight) ⇒ **Haste @L7 is the only milestone unlock left**; wolf line becomes Swift .1 + KeenEye .06 **line-wide** plus one signature aura each — Wolf (cL2) nothing more, **EliteWolf (cL5) Wild .5**, **DireWolf (cL6) LongRangeStrike .2**, **AlphaWolf (cL10) Reaper .2** (it was the only dropless wolf despite being the apex). "Easiest drop = least interesting skill" **confirmed intended** (generous variety, not power). **Reachability flag raised + resolved in-session:** the literal spec would have made **Swift world-unreachable** (Wolf was its only non-legacy source — would have regressed the step-7 A.5 guarantee); PO resolved by going line-wide. Post-change sweep over mobs+teachings+recipes+milestones: nothing orphaned (`FireWard` remains a *pre-existing* known gap → item 12). **Watch:** the milestone table is `//go:embed`-ed under `backend/pkg/aura/skills/`, **not** `api/` — `-content ../api` does not cover it, `make -C backend build` required. **Verified:** `go build` clean, **full suite green** (exit 0) incl. the rewritten milestone pin test, boot clean with **1 milestone unlock** (was 2) — `78 skills/13 factions/47 mobs/10 recipes/850 props/380 spawns/5 campfires/14 npcs, 0 panics`. **Not yet PO-verified in-game** (server left running). PO's own zone-editor pass rode along: props 856→850, spawns 349→**380** (DireWolf +8, EliteWolf +3, Boar +17, Kobold +4, …). **Next tuning notes (recorded, not acted on):** Wild ~72% of baseline DPS at r1.4 is close to a **trap pick** (barely outranges DamageAura's 1.0, so the DPS loss buys no safety) vs LongRangeStrike ~64% at r2.6–3.0 which outranges every wolf bite = a real playstyle; **Reaper caps at maxLevel 3** so its 13.5 max DPS ≈ LRS's 12.8 despite four more curve levels (raise to 5 if the apex drop should feel apex).
