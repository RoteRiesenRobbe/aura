...
- **⭐ REGION PRIMITIVE: C1-C4 ALL SHIPPED (`4fd694fa`); C5 DESIGNED, UNSCHEDULED** (`docs/plan-region-primitive.md` §4.9 is a COMPLETE spec written against the shipped code; audio consumers unscheduled, `plan-region-audio.md`). ⭐ **C5 = soft borders, the blend band** (`3990151c`, spec §4.10): a per-region alpha ramp at the polygon edge, per-profile `blend` width, hard edges until the seam reads badly in game (D15). Three PO calls open, the sharpest being whether COLOUR regions get bands too — free in code, a reversal of D5 in design. ⚑ Shipped in C4 instead: `texture`+`scale` in the profile table, `regionPaint` replacing `regionColor` and owning D14's fallback **within one profile** (never a chain of `resolve()` calls - that would take the texture from an outer region and the colour from an inner one), the same three lines at BOTH draw sites (`Game.ts` + the `MapTerrain` bake - §4.7 is not optional), per-zone `Assets.load` at `startRendering`. ⛔ never SVG (webpack inlines it INTO the bundle) · ⛔ never `GroundTextureTypes` (preloads at import, blocks boot) · ⛔ never `registerGameObjectSVG` (crops rasters). ⚑ **D18's fork was SETTLED at C4**: 16 independent textures, over a new `tint` key - which stays available and is the thing that would separate a base profile from its *darker variation* without a second file (D14's objection was ONE key with two meanings, never tinting as such; the look pass hit exactly this, pointing Ashen Fields and Volcano at the same tile). ⏸ The look sitting is still OPEN: the picks, the 0.35 scale and the seam all want a PO pass in front of the game, and `Forest`/`Ice` currently author no tile. Schema NONE at both chunks; neither changes a zone-file field.
