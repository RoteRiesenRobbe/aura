...
- **Last completed:** **Playtest-1 feedback Pass A (Balance & AI) — DONE 2026-07-22, `b0171ffb`, NOT yet PO-verified in-game; full ledger: `docs/plan-playtest1-feedback.md` §Pass A ledger.** Both halves in one session. **A1 (Go/AI, TDD):** gray-aggro gate (new optional `model.Leveled`, player-only so mob-vs-mob is untouched; gates `findAggroTarget` ONLY — threat retention untouched, so gray mobs still retaliate; band **−5** [PLACEHOLDER]); campfire **hard safe-zone** (`model/mob/safezone.go`, radius = the visible heal ring **1.5**; acquisition skips in-zone targets, an aggro target reaching the fire breaks the chase outright, movement clamped in the single choke point `Mob.moveTo()`; aligned + `friendlyToPlayers` exempt; boot-time world data from `zone.campfires`, nil in sim/tests); **aggro radius ×0.6** on 27 non-legacy mobs (RallyDrummer 6→**4.5** not 3.6 — its r4.0 ally shield must not out-reach its own sensor). **A2 (content tuning):** respawn **×2** (399 spawns; trash 900→1800 = 60 s; the 5-min tier restored per PO — and for the record the **boss was never in scope**, `OrcWarlord` is encounter-controlled at `encounter/warlord.go:47`, not a zone spawn); **mob HP graduated** ×1.2 cL4–6 / ×1.35 cL7–11 / ×1.5 cL12+ **with XP scaled by the same factor** (kph-derived XP/h per band holds, so leveling does not slow); **Z2 damage ×1.25**; AlphaWolf **0.95** + Marauder **0.92** now outrun the player (parity = 0.909), DireWolf 0.88 just under; **Vanguard gate 20→15**; Heal radius 1.0→**1.5** (+0.1/lvl) + tick 120→**80**; **drop reshuffle to rare carriers** (30→29 entries, **zero orphans** — every strip is a move; trash stripped, Wolf keeps only Swift **.04** as the first-drop moment, Orc gains **Tough .2**). Verified: `go test ./...` **exit 0 / 29 pkgs**, roster **band-check holds** (every zone keeps both a soft and a hard normal), boot `82 skills/14 factions/50 mobs/10 recipes/815 props/399 spawns/5 campfires (safeRadius 1.5)/14 npcs, 0 panics`, browser smoke 6/6, 9 new Go tests. **Open at the feel pass:** Heal self-cost/second rose 1.5× with its throughput; KeenEye came off Wolf (deviates from the 2026-07-21 line-wide ruling); campfire splash gap + edge-plinking are inherent to the 1.0 radius factor.
