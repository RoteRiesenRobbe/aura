...
- **⭐ Last completed: SPELL BUILDER C4: the new-skill flow and the bookkeeping** ✅ 2026-09-12 `71183777` (ledger: `docs/plan-content-editor.md` §B12 C4): **"+ New" writes `api/skills/<kebab>.json`** (auto-id = max over BOTH folders + 1; category UNSET on open, PO call), an **icon picker** over the 33 vendored glyphs (the current value always offered), a **`spawnMob` picker over ALL mobs grouped by role** + an "edit in Mobs" jump (⚑ §B4.6's follower/structure filter was measured WRONG: the portal mobs are creatures, Go has no role rule; PO-ruled), the **test-rig badge**, and an always-present **After saving** section: the `start-cmds=GOD,SKILL <name>` link + Copy, and after a save the **checklist from the RESPONSE** (restart · registry pin, count computed from disk · inventory row BY HAND, no generator exists, PO-ruled · placement). `save-skill.mjs` gained `isNew`; no JS duplicate guard (`registry.go` refuses through the seam, proven). ⚑ Go has NO `icon` rule: smoke leg (i) twins `SkillIcons.test.ts`, a named D9 exception. ⚑ A tab-authored NEW file's key order differs from hand files (L14). **Schema ALL NONE; content: the PO's tab-authored `summonspider.json` ships (id 152, cheat-only) + the registry pin 105 → 106.** Verified: unit 0 · smoke 0/106 + 33 glyphs · harness 0 problems incl. the new-skill leg with mandatory cleanup · the PO's example aura POSTed as a NEW skill, then a REAL 15 s boot `count=106` · `go test -count=1 ./...` EXIT 0, 35 pkgs · `-validate` 0 findings at 106 files · mutation ×5. ⭐ **PO look, the PO's words: "works and behaves as I guess would be technically expected"**: the PO authored AND cast `SummonSpider` (a `spawn` of the wild `Spider`), the first tab-created skill walked in game - it fought for its caster but STOOD STILL (follow permission lives on the MOB's `role`), and two UI fixes shipped from the look (the icon picker's highlight now follows the click; the checklist renders under the header in an "After this save" box, not at the form's bottom). **Same day, from the PO's first tab-authored spell: `docs/plan-summon-follows.md` DESIGNED, the next chunk; C5 after it.**

- **Prior: SPELL BUILDER C3: editing and saving** ✅ 2026-09-12 `dd05b9f4` (ledger: `docs/plan-content-editor.md` §B12 C3): the Skills tab is WRITABLE. Every control edits the raw object in place (unrendered keys survive; a blank number / unchecked bool / empty pick DELETES the key, the L2 tri-state), effect cards add · delete · reorder · retype (a confirm names every dropped key), and `POST /api/save/skill` runs `save-skill.mjs`: path guard → id guard → ⭐ **rename guard** (ONE `skill-references.mjs` scan feeds both it and the "Obtained via" panel) → the C2 seam → write. ⚑ Refusals are 200 + a `stage`, a thrown seam 500 (L12). PO ruled: **§B11 Q6 CLOSED, no reformat** (the style lands per file on first save) · `_comment` editable. ⚑ L14: `delete`+reassign moves a key to the END of the file. **Schema ALL NONE, content one comment-only tab save; Go only in the rider.** Verified: smoke 0/105 legs (a)-(h) · the harness REWRITTEN for the editable tab, 0 problems · a real 14 s boot on a tab-saved two-effect skill · mutation ×6. ⭐ **Same-day PO-look rider: the loader now refuses an effect type on a category that never dispatches it** (`effectCategories` aura 9 · cooldown 21 · passive 5; a `stat_multiplier` on an aura loaded clean and did NOTHING, the rule lived only in three runtime switches), fixture-carried and picker-filtered. ⭐ **PO verdict: PASSED** ("Seems to work"); the PO's own tab save of `damage.json` ships in the commit, the tab's first real content edit; two presentation notes stay open.
...
- **⭐ SKILL VFX: DESIGNED 2026-09-11, nothing built; it and spell builder C5 are the two live candidates, order = PO call** (`docs/plan-skill-vfx.md`, D1-D10 PO-ruled): per-ability + per-mob VFX, a closed seven-kind `visual.layers[]` vocabulary, ⭐ **honest attribution via a per-hit `SkillEvent` emitted inside the four damage/heal funnels** (D9), own-caused numbers only (D6). ⚑ Pulls the per-hit slice and the per-effect-art moratorium OUT of `plan-entity-presentation.md` (§39); medallions parked (D8). ⚑ C1 owes a loadbot measurement that may veto mob-vs-mob events (D10). ⚑ `prototype/skill-visuals` = quarry, delete after C2.
...
- **⭐ NEXT CHUNK: `follows`, the summon that is a pet** (`docs/plan-summon-follows.md`, DESIGNED 2026-09-12, D1 PO-ruled UNIFORM, nothing built): a `follows: true` key on the `spawn` effect sets a runtime flag the follow check reads beside charm's link; the role-based branch is retired, the four companion skills author the key, `role: follower` becomes a classification (Q1: retire it later). ⚑ Guard = a LOAD-TIME refusal of a follower-role `spawnMob` without `follows` (L3). ⚑ Never carry it on `charmer` (`EndCharm` reverts faction on any departure, L1). Exit test = the PO's untracked `summonspider.json` (L7) following, plus `CallForAid` still heeling.

- **⭐ SPELL BUILDER: C4 shipped 2026-09-12 `71183777`, PO-looked, then C5** (`docs/plan-content-editor.md` Part B, ledgers §B12): the Skills tab creates, edits and saves through the Go seam. C5 = the registry lock (`api/registry-lock.json`, tombstones, boot + `-validate` enforcement; closes L3/L4). ⚑ §B11 Q7 (Go validation for every tab, retiring `validate.mjs`'s port) is one line per tab away, still a separate call. Open after the C3/C4 looks, presentational: the red hint box, L14 key order, the stacked ICON hints.
