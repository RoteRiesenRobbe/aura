...
- **Last completed:** **Step-6 chunk C0 DONE + VERIFIED IN-GAME (2026-07-16)** — scaling & authoring foundation per `plan-content-zones12.md` §13 (execution record in the C0 section). Live `f(L) = growth^(L−1)`: one shared formula `pkg/berryhunter/curve` (sim harness aliases it — structurally drift-free), conf `game.player.levelGrowth`/`maxLevel` (working lock 1.12 × 30; `curve.Default()` + `cfg.ReadConfig` = the single default point so player + mob derivation never diverge). Player: `MaxHealth = base × f(L) × (1+passive)` (multiplicative), level cap 30, `PowerScale()`; SkillSystem: single seam `casterPowerScale` multiplies HP-side output only (damage/heal/dot/hot/shield/flat self-heal/selfDamageHP; fraction-of-max self-heal rides max HP, not f twice; owned summons = `SummonPower × f(ownerLevel)`). Mobs: **tier+baseline authoring** — JSON `tier` (pure label normal/elite/boss) + `curveLevel` + `factors.baseMaxHealth`, loader derives `maxHealth` + def `PowerScale` (applied to mob skill HP at cast time), **raw `factors.maxHealth` hard-fails at load**; all 13 mob JSONs migrated at curveLevel 1 (byte-identical). PO decisions: per-mob baseline shape, migrate-all-now, multiplicative HP passive, summons ride f. Verified: suite + race green, sim chunk-1–4 pins byte-identical, harness spot-run matches (eff ≈0.22 flat), in-game L1 100/100 → L20 861/861 (=100×1.12¹⁹) → cap L30 2675/2675. Deferred to C8: summon max-HP scaling (linear `maxHealthPerOwnerLevel` vs exponential f). Docs: tdd §4.1 flipped live, `manual-content-authoring.md` §1 tier+baseline rule. Prior: sim-harness plan COMPLETE (all 4 chunks, `plan-sim-harness.md`); step-6 plan + chunking C0–C8 (`plan-content-zones12.md` §13; lift 1 CUT → Rally+Flee backlog §18, drummer = shield_aura, recipe net before balance).
...
- **Next:** **Step-6 chunk C1 (Z1 farm start beat) — execution session** per `plan-content-zones12.md` §13: new playfield zone JSON (full Z1+Z2 bounds, farm block-out), Turnip mob (first authored `resistances` + turnip wildcard) + Turnip-Pull aura, **start-loadout flip** to Turnip-Pull only, Farmer NPC (start line + ordered teachings Damage + Recall @L2 + `TooLowLine`, lands the GDD §5 amendment), milestone-table first cut. **Every C1+ mob MUST be authored tier + baseline** (`tier`+`curveLevel`+`baseMaxHealth`, skills at baseline values — raw `maxHealth` hard-fails since C0; rule in `manual-content-authoring.md` §1). Then C2–C8 in order, one chunk per session. Standing locks: growth 1.12 × maxLevel 30 (≈27×, band ≈ +5, lower-first); regen slow ~1%/s, tier placeholders normal ≤ ~50% / elite ≤ ~25% / boss dies; downtime 15 s + chain 20; guardrail asserts land in C8 against real mobs. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; boot `-content ../api` for content iteration.
