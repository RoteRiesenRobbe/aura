...
- **⭐ Last completed: SPELL BUILDER C0: the vocabulary fixture** ✅ 2026-09-10 `60fb44d2` (ledger: `docs/plan-content-editor.md` §B12 C0; built by an Opus subagent): **`api/skill-vocabulary.json` is a GENERATED file**, written only by `UPDATE_SKILL_VOCABULARY=1 go test -count=1 ./pkg/aura/skills/` and pinned by a golden test, so a new effect key, type, category or damage type reddens `go test` until regenerated and then reaches the editor with zero hand work (§B3). ⭐ **The design changed on measurement: `api/shared-constants.json` ALREADY carried `effectTypes`/`selectors`/`gateKeys`/`statNames`, Go- and client-pinned**, so the fixture is a COMPLEMENT (effectKeys · topLevelKeys · categories · costKeys · renamedKeys · factionScoped · damageTypes · resistWildcard), a test forbids any list living in both files, and `vocabulary.mjs` merges the two at read time; supersedes the §B4.2 sketch, closes §B11 Q5. ⭐ **`topLevelKeys` is reflected from the struct's json tags** (both decisions are session judgements at the PO's delegation, vetoable; reversal costs in the ledger), which lets `smoke.mjs` check every shipped file's top-level keys for the FIRST time (the loader parses skill JSON without `DisallowUnknownFields`; 0 strays / 105 files). ⚑ **L7 was wrong**: no effect carries `_comment` and the loader would refuse one. New: `pkg/aura/golden` (compare-or-rewrite helper, skills its first caller), `tools/content-editor/{vocabulary,files,smoke}.mjs`, `skillVocabulary` on `/api/data`, `npm run smoke`. **Schema ALL NONE** (not embedded; `cp-defs` copies directories only). Verified: red-first · build · vet · **`go test -count=1 ./...` EXIT 0, 35 pkgs** · smoke 0 findings/105 files · **mutation ×3**. ⚑ **`go test ./...` was RED on arrival for a STALE reason**: `backend/pkg/api/` lacked U4b's two cave mobs, so six census/placement pins failed with `unknown mob "CaveMouth"`; `make -C backend cp-defs` → green. **Next: C1**, the read-only tab.
...
- **Prior: WORLD PATHS C3: animated water** ✅ 2026-09-07 (ledger: `docs/plan-world-paths.md` §12 C3; full banner in `docs/archive/status-history.md`): a profile authors **`scroll: {x, y}`** in world units/s and its TILE drifts, a `TilingSprite` whose `tilePosition` is nudged per frame: two number writes per surface, nothing rebuilt. ⛔ Never on the day/night filter machinery. ⚑ The §4.5 sketch would have made `scroll` a silent no-op on a `blend: 0` profile: a drifting surface needs a mask for its SHAPE, so an unfeathered one gets a CHEAP stencil mask, and all three cases collapsed into one `paintSurface`, which lets a `Water` REGION drift free. ⚑ `paintTerrainSurfaces` returns `{masks, scrollers}`; `MapTerrain` drops `.scrollers` deliberately. Three traps pinned in comments (`paused` guard first · `regionScrollers` REPLACED · `scroll` on `texture: null` animates nothing). ⭐ C3 had silently unblocked: `tools/make-water-tile.mjs` shipped the tile with C1. **Schema ALL NONE.** Verified: tsc · vitest 620/620 · prod build · mutation ×2. ⛔ Not in-game-verified by me.
...
- **⭐ SPELL BUILDER: C0 shipped 2026-09-10, C1 next** (`docs/plan-content-editor.md` **Part B**, ledger §B12): a **Skills tab in `tools/content-editor/`** over the loader's 34-type / 81-key vocabulary, writing `api/skills/<kebab>.json` directly. **C1 = the tab READ-ONLY**, rendered from `skillVocabulary` on `/api/data` (the generated fixture merged with shared-constants; ⛔ never a hand-typed field list, L1): sidebar groups, the presentation table with its completeness assert, the per-level preview, seconds beside ticks, the Visuals placeholder, the sources panel, the §B4.8 exclusions. ⭐ Read-only first so the PO judges the form against all 72 real skills before a write path exists. Then C2 (`aurad -validate`, D9) · C3 (save) · C4 (new-skill flow + docs) · C5 (registry lock). ⚑ Open before C3: §B11 Q6, one reformat commit of `api/skills/` (85 of 105 files reformat under `prettyJson`). ⚑ C1 should know `readSkillVocabulary` throws inside `/api/data`.
...
- **Known-inconclusive at HEAD**, all unowned: `chunk3-charm` 6–8/9 (D9 fragility) · `filler-batch` leg 1 (stale assert) · `chunk3b-ii-conversation` **28/34** (stale TownCrier assert + Leave-click race + Wanderer drift-pin — re-confirmed 2026-09-06) · `accounts.TestRepeatedFailuresAreThrottled` under `-race` ONLY · **`AuraTiledConvert.test.ts` byte-stability ×2** on a fresh checkout (they pin the PO's uncommitted world.json repair; the 30× `world.json` is GONE at HEAD — world is 263 KB / 144×72). ⚑ **The 3 `items/mobs` census tests + the 3 `cmd/simharness` placement pins hardcode the roster**, so ANY new mob or NPC reddens them (`docs/feedback.md` 2026-09-05, needs a door); ⚑ **they read the EMBEDDED `backend/pkg/api/` copy**, so after pulling content run `make -C backend cp-defs` before believing a red (a stale embed read as "red at HEAD" 2026-09-10). Backend suite **fully green 2026-09-10, 35 pkgs**; the frontend legs above were not re-run. ⚑ **Measure the rate before diagnosing a flake** (a misread cost `chunk3b-interact` two chunks); ⚑ **this host's wall clock is non-monotonic** — suspect it before any elapsed-time red.
