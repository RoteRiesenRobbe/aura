...
- **Last completed:** EntityType name-fallback validation **§27.2.1 DONE** (test-first, `plan-entitytype-validation.md`) ✅ `c3938be7` 2026-07-24. Turned the last live-server crash risk in the §27 sweep into a boot error: a mob whose wire EntityType falls back to the def `name` (no `entityType` override) was validated **nowhere** — a bad name loaded clean and killed the whole process via `log.Fatalf` at first spawn (spawn effect / encounter spawns build mobs at cast time). Three coupled edits: ① new shared resolver `mobs.ResolveEntityType(override, name)` — single source of truth for the name/override → wire-type mapping (collapses the `NewMob` `types` map vs loader `EnumValuesEntityType` DRY smell); ② loader validates the **effective** lookup with distinct per-knob messages; ③ `NewMob` uses the resolver + **panics** instead of `log.Fatalf` (last `log.Fatalf` in `model/`). **Guard kept, not deleted** — unreachable for loaded content but live for direct construction (sim/tests), where it catches the `EntityType(0)=DebugCircle` zero-value trap. Pins: `TestResolveEntityType`, `TestMapMobDefinition_UnknownNameFallbackFails` (regression — fails on pre-fix loader), `TestNewMob_PanicsOnUnresolvedEntityType`. Verified `go build`/`vet` clean + `go test ./...` green (29 pkgs); boot `-content ../api` = 50 mobs 0 panics; negative boot check fails at load not first spawn. No wire/schema/JSON change. Full ledger: `docs/plan-entitytype-validation.md`.
