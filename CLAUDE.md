...
- **Last completed:** **Phase 8.1 (passives) implemented and verified in-game** — `DerivedStats` on SkillComponent (stat_multiplier: movementSpeed in core/input.go, maxHealth in maxHealthFactor; recomputed on equip/unequip/level change), category-aware EquipSystem (+ mob loadout routing), **a passive occupies only one slot** (re-equip moves it; aura duplicates stay allowed — nothing stacks there), `passive_slots` on GameState, spellbook category sections (empty = invisible), passives panel, `SwiftPassive` (id 10) via level-3 milestone. Before that: Phase 7 complete (leveling+points, commit 442d2b50; combinations design written — skill-system-design.md → Combination System, all 16 catalog questions decided).
- **Next:** Phase 8.2 — cooldowns: `cooldown_activations: [ubyte]` on Input, `instant_damage` via temporary sensor, `CdTicks` bookkeeping, cooldown slots + remaining ticks on the wire, ability bar (hotkeys + click), mob fire-when-ready AI, `NovaBurst` via milestone. Then Phase 9 (combinations) or Block 2 (survival removal + resource unification). Roadmap: docs/v1-roadmap.md.
- **Current state:** new players start with DamageAura in slot 0 on spawn, panel highlight + ring correct from spawn, server-authoritative. HealAura unlocks at level 2 (gold glow), `WildAura` drops from SaberToothCat (20%) and guaranteed from the boss; equip/activate via Aura Slots panel. Skill points: earned per player level, spent/refunded freely in the spellbook panel, equipped skills scale live (levels sync into all slots each spend). All combat participants (incl. healers, ~10s window) get full XP on mob death. `XP <amount>` cheat for manual leveling.
- **Deferred tech debt / known bugs:**
  ...
  - ⚠️ Testing gotcha: `go:embed` patterns don't include subdirectories (`*.json **/*.json`!), and disk-based registry tests can't catch embed gaps — pinned by `pkg/api/skills/skills_test.go`. Before manual tests: `pkill berryhunterd`, rebuild, and check the boot log (`Loaded skill definitions count=8` as of Phase 8.1) — a stale server process silently masks new behavior.
