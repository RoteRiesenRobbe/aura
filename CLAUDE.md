...
- **Known-inconclusive:** `chunk3-charm.mjs` red 6–8/9 across four runs (the accepted D9 fragility), HEAD baseline hung — not cleared. · **`sys.TestDwell_TakeoffDropsAnInProgressCount` now fails DETERMINISTICALLY at HEAD** (2026-08-07, "fixture: one tick short of the threshold"; stash-proven to predate the steering fix) — it was the documented sometimes-flake, and a test that failed sometimes and now fails always has *changed state*; needs its own look.
