...
- **⭐ ZONE POLYGONS: P1 shipped, P2-P4 next in one pass** (`docs/plan-zone-polygons.md`): **P2** the polygon primitive draw-only (its own array + `AuraPolygon` on the SHARED `paths` layer, D5) · **P3** filled blocking — the inset **rotated boundary stroke** over a **coarse interior fill**, auto-coarsening at the cap (D6) · **P4** outlines on both types. ⚑ **Three [PLACEHOLDER] numbers are still open** (§9): interior cell size (proposal **2 u**), boundary thickness **`T`** (proposal **1 u**, and the one that is safety-critical — L11, pick it against the FASTEST mover, not the player) and the per-polygon body cap (proposal **256**, interior fill only). ⭐ **P1 proved the shape-is-the-flag posture works end to end**, which is what P2's class discriminator on a shared layer rests on.
