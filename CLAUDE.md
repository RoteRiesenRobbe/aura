...
- **Last completed:** Step-5 (unlock-source systems, `docs/plan-npc-teaching.md`) **chunk 5 (frontend bubble hoist + latest-wins + `Chat.showMessage` guard) DONE + VERIFIED IN-GAME by PO 2026-07-15, committed.** The speech-bubble machinery (`messages[]`/`messagesGroup`/`say`/expiry + the follow-group position-mirror) was **hoisted from `Character` to the shared base `GameObject`** (`_GameObject.ts`), made **lazy** (built on first `say()`; a silent object pays nothing) and self-cleaning (released in `hide()`, which now also backs a new `GameObject.remove()`), so the NPC's `Resource`-mapped render object (Flower) can speak. `say(message, latestWins=false)` gained a **single-slot latest-wins branch** (`removeChildren()` before pushing the new `Text`) for NPC lines; player chat keeps stacking. `Chat.showMessage` (`chat/logic/Chat.ts`) hardened with an `isDefined` guard (fixes the latent throw when the speaker isn't client-tracked) and picks `latestWins = !(gameObject instanceof Character)` so NPCs are single-slot, players stack. **Also:** the Sage gained an idle-lore fallback line (`"lines":["Nice to see you again, traveller."]` in `api/zones/proving-grounds.json`) — **zero code change**, the chunk-3 `onApproach` already speaks `Lines` when neither a grant nor the too-low gate applies (`sys/npc.go:139`); embedded copy synced. **Sanity:** `tsc --noEmit` clean; `npm run build` (webpack prod) 0 errors; backend boots `-content ../api` → `placed npcs count=1`, no panic. **VERIFIED IN-GAME by PO:** bubble renders over the Sage's head; too-low line; lore/greeting fallback; latest-wins. **Prior:** chunk 4 (speech origination, backend) DONE+committed (`6322218c`); chunk 3 (grant + edge-trigger) `35f3b151`; chunks 1–2 `f4ebe912`, `9d71f9ad`. Step 4 (skill-vocab fill) COMPLETE + committed.
- **Next:** Step-5 **chunk 6 (zone-editor `npc` placement mode) in a NEW session** → `docs/plan-npc-teaching.md`. New `EditorMode` `'npc'`, marker draw + hit-test + place/select, a nested teachings/lore config panel, export via `getZoneAsJSON` (the TS `ZoneNpc`/`ZoneTeaching` mirror already exists from chunk 1). Precedent: the campfire/dark modes in `zone-editor/logic/_ZoneEditorPanel.ts` + `ZoneEditor.ts`. Verify: place an NPC in-game, export, reload, it teaches. This is the final chunk of Step 5; after it, Step 6 per `docs/roadmap.md`. **Note:** `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; the Sage (proving-grounds (4,3), teaches `HealAura`@L1 + `Dash`@L5, idle line) is committed content — boot `-content ../api` for content iteration.
