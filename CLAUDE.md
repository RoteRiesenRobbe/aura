...
- **Last completed:** **UI-polish chunk 1 — skill-catalog endpoint + hover tooltips (step 8, plan-ui-polish.md) — DONE 2026-07-21, PO-VERIFIED in-game 2026-07-21 ×2 ("it works and is correct" + text-size/color follow-up "it's great"), `ae51d8b5`; full ledger: `docs/plan-ui-polish.md` §Chunk 1 ledger.** `GET /skills` serves the PARSED registry (both muxes, CORS `*`, marshaled once at boot; JSON tags on the REAL `SkillDefinition`/`EffectDef`/payload structs, enum wire strings via `MarshalJSON` derived from the parse maps; `displayName` authored-override-else-derived CamelCase→spaces — exactly 4 overrides authored: Call for Aid, Damage-Burst, Long-Range Strike, Hold the Line). Frontend: `Skills.ts` rewritten as the catalog module — **the three hand-sync maps are deleted** (item-8 debt closed), same-signature accessors degrade to `Skill #<id>`/`'aura'` on fetch failure; new `SkillTooltip.ts` = pure formatter (all ~20 effect types, current→next values, unknown-type console-warn tripwire) + shared anchored tooltip on delegated hover over spellbook + all three loadout lists. **PO rulings:** full-detail stat lines; element-anchored; follow-up: shared-line dedupe (identical radius/targets print once) + font bump 1.5rem [PLACEHOLDER] + **colored label only** (effect labels tint in `AURA_CATEGORY_COLORS`); accepted judgment call: cadence folds into the main line ("every 1.32s" / ", refreshed every 0.99s"), no standalone "Ticks every" line. **Verified:** `go build` + full suite green (+4 catalog tests), `tsc` + prod build clean, boot `-content ../api` `81 skills/14 factions/50 mobs/10 recipes/828 props/373 spawns/5 campfires/14 npcs, 0 panics`, Playwright smoke **10/10 PASS** (incl. Warbanner dedupe + 4 label colors + blocked-fetch fallback), PO in-game ×2. Watch: `#gameUI` is a zero-size shell — Playwright must wait on `classList`, not visibility; `AURA_CATEGORY_COLORS` (AuraRings.ts) is now also a HUD dependency. Commit includes the PO's parallel zone-editor `world.json` pass (embedded copy sync; source committed parallel as `ac8eb9b5`).
- **Prior:** **② Reconnect-token persistence ✅ `a8e82851`** 2026-07-21 PO-verified ("tested and works") — reload restores the character: `Join`/`Accept` `reconnect_token` (append-only), ConnState `stashByToken` (disconnect stashes, name stays reserved, TTL ~10 min [PLACEHOLDER]), sessionStorage + auto-rejoin, `Game.removePlayer()` null-guard drive-by; Playwright smoke 9/9; watch: stash-reserved names block new joiners for the TTL window; full ledger: `docs/plan-reconnect-token.md`.
