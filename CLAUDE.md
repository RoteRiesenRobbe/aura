...
- **Last completed:** **Sim-harness Chunk 1 IMPLEMENTED (2026-07-15, pending PO review + commit)** per `docs/plan-sim-harness.md`: new `backend/pkg/berryhunter/sim/` (`world.go` = `simGame` model.Game stub + minimal real system set StatusEffects→Mob→Physics→Update→Skill, real `player.New`/`mob.NewMob` from synthetic specs; `scenario.go` = AuraSpec/PlayerSpec/MobSpec + TTK/TTD constructors; `runner.go` = fight-to-death + N-seeded-run p10/p50/p90 distributions; `report.go` = table + JSON artifact) + thin `backend/cmd/simharness/` (flags for all combatant numbers, defaults mirror current content [PLACEHOLDER]; `make -C backend simharness.build` works via the `%.build` pattern; **plus PO-requested `-serve localhost:8081` local web explorer** — embedded single-file HTML, vanilla JS, `POST /run` reuses the sim spec structs, live knobs → histograms/percentiles + artifact download; `Distribution.Values` carries the sorted raw seconds; **mob-preset dropdown** = `GET /mobs` maps the real authored mob registry onto MobSpecs, embedded `pkg/api` default / `-content ../api` live — first §4 real-content bridge; no-damage-aura mobs map to harmless turrets). One seam added: `sys.SkillSystem.SeedRNG(seed)` (variance/crit rolls reproducible; live game stays time-seeded). Key determinism decision: the mob's spawn-HP variance is rolled by the RUN's seeded rng (not the mob's entity-ID rng) → same (scenario, baseSeed, n) reproduces exactly. 7 sanity tests green (exact TTK tick-12 / TTD tick-20 cadence pins, chase terminates, timeout, fixed-seed reproducibility, variance spread); `go build ./...` + full `go test ./...` green; CLI smoke-run OK. **First finding: current content numbers → TTD ≈ 8.7 s vs the ~20–25 s working target.** Design/`f` context: see plan §5 (Philosophy A, steep ~12%/level [PLACEHOLDER], max level ~25–35).
...
- **Next:** **PO review of Chunk 1** (in-game surface = none; review = CLI run `go run ./cmd/simharness` from `backend/` + code), then commit, then **execute Chunk 2 of `docs/plan-sim-harness.md`** (implement + validate the decided `f` curve: fixture generator `f(L)=growth^(L-1)` HP-values-only, level-typical player + same-tier mob per level, TTK/TTD/kills-per-level across the span, cross-tier level-gap band, linked-triple table — PO locks exact `growth`/max-level from the data). Then chunks 3→4, pausing for PO review between. After the harness: **Step 6 = the initial content pass (item 12)** — two large zones with intra-zone difficulty areas, full mob roster (hand-authored per-tier against the §5 curve), boss scripts, first real balance pass. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; the committed Sage (proving-grounds (4,3), teaches `HealAura`@L1 + `Dash`@L5) is content — boot `-content ../api` for content iteration.
