...
- **Last completed:** Step-5 (unlock-source systems, `docs/plan-npc-teaching.md`) **chunk 3 (`NpcSystem` grant + edge-trigger, backend) DONE + VERIFIED IN-GAME 2026-07-14, committed.** The chunk-2 temp `NpcSystem.Update` log is replaced by real behavior: on the rising edge (chunk-2 `seen` map, unchanged) it runs `onApproach(n, p)` (free func in `sys/npc.go`) = ordered `n.Teachings()`, `HasDiscovered` skip, `Level >= RequiredLevel` grant-all-qualifying (`sc.Discover(id)` + `p.ApplyRecipeCascade()`, once per grant), else emit `TooLowLine` and **stop**, else lore `Lines` fallback when nothing is taught. Returns `[]string` lines but does **not** speak yet — a temp chunk-3 `slog "npc onApproach"` records them; **chunk 4 replaces that log with the `EntityMessage` fan-out.** Grants are instant; client renders the unlock glow from the spellbook diff (no wire event). **Payload now on the entity:** new `model.Teaching{Def *skills.SkillDefinition; RequiredLevel uint32; Line string}` + `model.NpcEntity` gains `Teachings()/TooLowLine()/Lines()`; `npc.New(pos, radius, teachings []model.Teaching, tooLowLine, lines)` stores + exposes them; boot loop maps `zone.Npc.Teachings`→`[]model.Teaching`. **DESIGN DECISION (plan deviation):** `Teaching` lives in `model` (NOT `model/npc`) — the plan floated an "npc-local" type, but `model.NpcEntity` referencing it would cycle, and the plan also wants grant logic + tests in `NpcSystem`/`sys/npc_test.go`. Resolution honors the real constraint (`model/npc` does NOT import `world`; `Teaching` carries the *resolved* `*skills.SkillDefinition`, not a name). `onApproach` reads through narrow local `teacher`/`learner` interfaces (the `skillEntity` precedent) so the core is fake-testable. **Sanity:** `go build ./...` clean; full `go test ./...` 0 fail; **8 new `sys/npc_test.go` tests** green (multi-teaching ordered grant, level-gate stop, first-too-low, back-to-back multi-unlock, idempotent re-approach, lore fallback ×2 [pure-lore + all-learned sage], and a **rising-edge `Update` test against a real `phy.Space`**: enter→grant+one `onApproach`; stand→no re-fire; leave→nothing; re-enter→re-trigger). Booted `-content ../api`: no panic, `20 *sys.NpcSystem` registered, `placed npcs count=1` (the Sage, committed in HEAD `d250f7de`). **VERIFIED IN-GAME 2026-07-14 by PO.** **Prior:** chunks 1 (schema+loader+TS mirror) + 2 (entity+placement) DONE+VERIFIED, committed (`f4ebe912`, `9d71f9ad`). Step 4 (skill-vocab fill) COMPLETE + committed.
- **Next:** Step-5 **chunk 4 (speech origination, backend) in a NEW session** → `docs/plan-npc-teaching.md`. Reuses the existing `EntityMessage` wire (no `server.fbs` change). Replace the temp chunk-3 `slog "npc onApproach"` in `NpcSystem.Update` with: build one `codec.EntityMessageFlatbufMarshal(builder, npc.ID, strings.Join(lines,"\n"))` and fan it out (`SendMessage`) to the players in `n.Sensor().Collisions()` — the sensor ⊆ their viewport, so the client already tracks the NPC (sidesteps the `Chat.showMessage` throw-on-untracked bug). Latest-wins bubble is automatic (one shared `entity_id`, newest `say`). Pattern: `sys/chat/system.go:38` per-viewport fan-out; `codec/chat.go:8`. `onApproach` already returns the combined `[]string` — chunk 4 only wires the speak. Verify in-game: two nearby players both see the bubble; low-level player triggers the too-low line. Chunks 5 (frontend bubble hoist + latest-wins + `Chat.showMessage` guard) → 6 (zone-editor `npc` mode) follow. **Note:** `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; author temp content in SOURCE `api/zones/` + boot `-content ../api` to iterate without rebuild. The Sage NPC (proving-grounds (4,3), teaches `HealAura`@L1 + `Dash`@L5) is committed content, usable directly for chunk-4 verification.
