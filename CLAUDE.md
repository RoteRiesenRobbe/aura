...
- **Two plans in flight** (`plan-code-health.md` completed whole + archived 2026-08-14): `plan-zone-editor-structure.md` **C1 ✅ 2026-08-15 - next is C2** (controls + the respawn tri-state + the talker round-trip that fixes the uneditable ascension stones; then C3 retires the legacy roster, the one wire-touching chunk) · `plan-play-bot.md` untouched (C0-C3: extract transport → the bot that fights → quests → the soak finding).
