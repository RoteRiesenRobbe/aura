...
- **Last completed:** **Playtest-1 feedback Pass C items 1+2 (UI features) — DONE 2026-07-22, `22028dc4`, NOT yet PO-verified in-game; full ledger: `docs/plan-playtest1-feedback.md` §Pass C ledger.** **Item 3 (darkness multiply blend) SPLIT OUT to its own session** — rendering-tech chunk with its own diagnosis phase, no shared code. **PO rulings:** nameplate = **name + level** ("Alpha Wolf 10"); bands = **WoW-classic** (`≥+5` red / `+3..+4` orange / `−2..+2` yellow / `−5..−3` green / `≤−6` gray — matches the ≈+5 curve band); **gray-aggro NOT re-landed** (decision 2 stays reverted — ship colours, play, then decide); **no equip-flow hint text** (teaching the flow belongs to the tutorial round, so discoverability is unchanged by this pass). **C1 click-to-bind:** the click half already existed since the panel was built — only the *"or press slot key"* half was missing. New **`tryEquipPending(category, slot)`** is the single implementation shared by the three slot-click handlers AND both hotkey entry points (keyboard/mouse can't drift on the combat lock or category rule; three inlined copies collapse into one); a **category mismatch consumes the key** (else a pending passive + `1` would silently fire your aura); **Escape cancels** (a forgotten pending selection used to swallow every slot keypress); passives stay click-only (no key assigned, none invented). **C2 nameplates — NO wire/schema change:** mobs had **no name text at all**, so this *adds* the plate. `MobDefinition` already carries Name+CurveLevel+Tier and the snapshot has always sent `mob_id` (never read client-side) ⇒ new **`GET /mobs`** (`mobs.CatalogHandler`, mirroring `skills.CatalogHandler`, marshaled once at boot — per-species data must not ride a 30 Hz snapshot). **Minimal projection pinned by test** (`{id,name,displayName,curveLevel,tier,combatTarget}`): the full definition would leak drops/HP/resistances = an out-of-game answer key vs the zero-hint policy. **`combatTarget` = grants XP && not friendlyToPlayers** — derived, not authored; campfires/braziers/companions/totems/brambles/rockfalls/pools/barricades are all MobDefinitions, and the zero-XP set turned out to be *exactly* those. Plate lives on the **unfiltered `namePlates` overlay** (Character.plate pattern) — the night filter would recolour a tint rendered in `shape`, and the tint IS the feature; plate mirrors `shape.alpha` so it fades with the corpse fade. Tint **recomputed per frame, not pushed on level-up** (it depends on the *player's* level; a push needs every mob subscribed — 2 int ops, PixiJS touched only on band change) ⇒ levelling up recolours the world. Drive-by DRY: `deriveDisplayName` → exported **`skills.DeriveDisplayName`** used by both catalogs, ws→http derivation → shared **`catalogUrl()`** in `Urls.ts`. Verified: `go build` clean, `go test ./...` **exit 0** (incl. 6 new mob-catalog tests), `tsc --noEmit` clean + prod build green, boot `82 skills/14 factions/50 mobs/10 recipes/815 props/399 spawns/5 campfires (safeRadius 1.5)/14 npcs, 0 panics` (unchanged — no content added), **C1 Playwright 8/8 + C2 5/5, no JS errors**, visual: same wolf **yellow @L1 → green @L7 → gray @L21**, campfire + NPCs plateless. **Open at the feel pass:** `combatTarget` has two visible consequences to judge — **Turnip gets a plate** ("Turnip 1", grants 1 XP) and **Fire Totem / Poison Pool get none** (0 XP despite being hostile), both one-line flips; nameplate **font 16 + gap 16px** [PLACEHOLDER] read small at default zoom; **gray-aggro can now be re-evaluated** (its missing level signal is on screen); **slot hotkeys look broken under Playwright unless held ~1.3 s** — edge-triggered off the rAF-driven `Controls.update`, which a headless page throttles far below the 33 ms `INPUT_TICKRATE`, so a 200 ms tap falls between samples while raw `window` listeners (Escape/chat/console) still work; harness artifact only, recorded in `.claude/skills/verify/SKILL.md`. All new numbers [PLACEHOLDER].
