...
- **Prior: backlog §46 — ONE AUTHENTICATOR** ✅ `2be3a3dd` 2026-08-03 — the anonymous secret no longer rides every request; it is spent **once** at `POST /api/session/anonymous`, so `resolveCaller` is a single cookie branch and the "JWT wins" arbitration (home of the two-tab logout bug) is gone. The secret stays a non-expiring **recovery** credential; the boot path exchanges first and forgets only on explicit `session_expired`. Verified: Go 33/33 vs real Postgres · 4 mutations · vitest 163/163 · `chunk2-accounts` 24/24 · `chunk4-persistence` 16/16. Full ledger: `docs/plan-accounts-frontend.md` §10a.

### Next

- **World map C2 — campfire markers** (`docs/plan-world-map.md` §6), then C3 (the player roster, the one wire change). ⚑ **Do backlog §53 first**: static minimap icons sit at player-relative coordinates, so C2's markers — which come from bundled *absolute* data — would land correctly beside trees that do not, reading as C2 being broken.
