...
- **Next:** **Step-6 chunk C2 (Z1 wildlife + dark forest) — execution session** per `plan-content-zones12.md` §13: `wildlife_predator`/`wildlife_prey` factions + matrix, Wolf/Bear/Boar/Stag/Elite Wolf, dark forest NW (thicket + darkAreas), §9 lift 2 (passive light) + Hermit teaches Torch, Dog NPC, bramble walls (Turnip-Pull-gated solid mobs), forest clue signpost; pre-marked split: wildlife+factions / forest interior. **Every C1+ mob MUST be authored tier + baseline** (raw `maxHealth` hard-fails; rule in `manual-content-authoring.md` §1); combat mobs need NO turnip entries (gated damage tags handle the chore gate — manual §1), but **brambles/rockfall must opt in** (`{"*": 0, "turnip": 1}` pattern). Then C3–C8 in order, one chunk per session. Standing locks: growth 1.12 × maxLevel 30 (≈27×, band ≈ +5, lower-first); regen slow ~1%/s, tier placeholders normal ≤ ~50% / elite ≤ ~25% / boss dies; downtime 15 s + chain 20; guardrail asserts land in C8 against real mobs. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; boot `-content ../api` for content iteration (schema/Go changes still need the rebuild).
