...
- **Last completed:** **Crit rework v2 mini-chunk (backlog §23) DONE 2026-07-20, PO-driven in-session (choice prompts + same-day v2 rulings; in-game re-feel pending Session ⑦) — committed `635a44e3`; full ledgers: backlog §23 DONE banner + `plan-skill-vocab.md` §4.3 v2 amendment.** Crit is now a **character-driven stackable stat**: total per-hit chance = **character base 5%** (conf `game.player.critChance`, all conf JSONs + ReadConfig default) + `critChance` passive stat + skill-authored level-scaled chance (new `critChancePerLevel` key; chance-only valid at global default factor ×2 [PLACEHOLDER], factor-only invalid). **KeenEye passive id 60** (2%/level ×5, **DireWolf rare drop 0.06** — post-FINAL drop-table amendment, PO choice) → registry pin **78** + Skills.ts synced. **ReaperAura authored 25%/×2 pair REMOVED** (~−12% sustained EV, PO accepted); DamageAura's briefly-added 1%/level authored crit removed same day (made the starter aura top non-ceiling); **dot compensation +~5% damageHP** (Immolation 10.5/2.1, Wildfire 7.4/1.5, Ignite 6.3/1.6 — dots never crit). Acting-entity doctrine: mobs/summons get no base (EliteBanditSlash pair untouched); dot-freeze path untouched; RNG-draw discipline kept (crit-free casters on pair-less effects draw nothing). Sim wired (`PlayerSpec.critChance` → PlayerConfig, level-scaled preset crit): §A ceiling ordering + tier guardrails green. **Verified:** suite + `-race` green at HEAD, boot `78 skills/13 factions/44 mobs/10 recipes/805 props/346 spawns/2 campfires/15 npcs, 0 panics`, in-game smoke (stat-driven crit renders the standard big-orange pop — pixel-detected).
