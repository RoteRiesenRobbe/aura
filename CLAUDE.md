...
- **⭐ Last completed: ASCENSION SITES C1 - A SITE OWNS ITS PRICE** ✅ 2026-08-11, `[uncommitted]`: the level cap is no longer the entry rule for ascending - **a life costs whatever the stone you are standing at authored** - and a second stone at the front proves it by asking for level 25 + a completed *Thin the Orc Line* where the village stone asks for 30. **Schema: DB NONE · FlatBuffers NONE · conf NONE**; Go + one content file, so an ordinary both-parts deploy with **no** hard reload. Shipped as **C1a** (behaviour-neutral: `RowSource` takes the NODE, so a provider can tell two stones apart) then **C1b** (the price). Three PO calls (the stone's price · its place · P2 pulled forward from C3). ⛑ **THE GATE SNAPSHOT CANNOT BE A CLOSURE**: `reattach` installs the STASHED SkillComponent, cast state included, into a **freshly built player entity**, so a mid-channel reconnect would re-judge against a detached object - the pick carries DATA (`AscensionPick{Key, Gate any}`) and `sys` supplies the live player. ⛑ **A NEW MOB BREAKS THREE CONTENT CENSUSES** in `items/mobs`, and they read `api/` from disk, so `-count=1` or a stale green hides it (it did, in this session). Verified: TDD red-first, **6 mutations caught** · Go 0 FAIL (`-count=1`, 34 pkg) · `-race` clean · boot 68 mobs / 488 spawns, 0 WARN/0 ERROR · **`c2a` 29/29 · `c3-memorial-catalog` 14/14** (exact baselines) · new `c1-front-stone` **9/12 + 3 inconclusive**, the three being a price no harness can pay (an elite Orc is 16× a dire wolf's HP; **PO: leave it, record the gap**). Ledger: `docs/plan-ascension-sites.md` §9.
...
- **⭐ ASCENSION SITES: C1 shipped, C2 and C3 open** (`docs/plan-ascension-sites.md` §9). Next is **C2 - the gate reads**: a navigation row whose destination is gated renders LOCKED with the gate named (`describeConditions` already covers the vocabulary) instead of vanishing, opt-in per row so no quest tree leaks hidden nodes. Small, and it is what makes the front stone's price legible - today it is only as readable as the fallback line authored by hand. Then **C3 - the site owns its rewards** (node `Rewards`, boot cross-validation, and the index-space hazard: present and apply must derive the row order from one authored list). P2 is done.
