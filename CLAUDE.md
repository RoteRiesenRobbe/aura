...
- **Last completed:** Step-4 **chunk 1 (damage-vocabulary batch) DONE + VERIFIED IN-GAME 2026-07-13, committed** → `docs/plan-skill-vocab.md` banner. §4.1 confirmed w/ **immunity-strip rider** (temporary immunity removal via cooldowns/auras/boss scripts = required future seam, demand-driven, recorded in skills/resist.go); §4.2 (a)+(b) confirmed; berserker reads the ACTING entity's HP (summon rages on its own wounds). Shipped: `"*"` wildcard resist, execute/berserker/crit/lifesteal on damage_aura+instant_damage (F6 §3.1 composition = shipped decision record w/ executable pin), `player.takeDamage` returns dealt, `model.ApplyLifesteal` in all four *Touches, `crit_taken` wire Character+Mob + 1.8× crit numbers, ReaperAura id 7 smoke (cheat `SKILL ReaperAura`), count pin 27 skills. Suite + build + tsc/webpack + boot smoke both content sources green.
- **Next:** Step-4 chunk 2 = shield layer (shieldPayload + absorb step in both takeDamage sites + `shield_hp` wire + HUD segment; §3.2 semantics) — execution session, NEW session — `docs/plan-skill-vocab.md` §5
