...
- **Last completed:** **Milestone table moves into `api/` + restart robustness (ad-hoc session) — DONE 2026-07-21, PO-driven in-session, committed `[uncommitted]`; full ledger: `plan-intermission-triage.md` §Milestone table moves into api/.** Closes the `-content` coverage gap the previous session logged as a recurring Watch item. **Finding: the split was historical, not principled** — the table dates to `4afc7fb4` (Phase 3.2) when it was a small Go-side table; `-content`/`contentSources` only landed yesterday in `aa509d95` and never revisited it, just wrote a comment rationalising it ("code-adjacent config"). It is `[{level, skillName}]` resolved against the skill registry — structurally the same as `api/recipes/`, and tuned on PO feedback like any content. **Change (backend-only, no wire, no content values):** `api/milestones/milestone-unlocks.json` git-mv'd out of `backend/pkg/aura/skills/`; new `backend/pkg/api/milestones/` embed pkg; `../api/milestones` added to the Makefile `cp-defs` list (fixed list, not a glob); `DefaultMilestoneUnlocks(r)` → `MilestoneUnlocksFromFS(fsys, r)`; `milestones fs.FS` on `contentSources`, hard-failing `sub("milestones")` in `diskContent()`. ⇒ **`-content ../api` now covers 100% of tunable content — the "milestone edits need `make -C backend build`" caveat is retired.** **Restart robustness:** `pkill -f aurad` (the form the `playtest` skill taught) **kills its own shell** — `-f` matches the full command line and the invoking shell contains `aurad`; burned three attempts (exit 144), leaving the stale server alive each time; `'[a]urad'` does not help either. Skill doc fixed to **`pkill -x aurad`** (name-exact; a shell is named `bash`, so self-match is impossible). **Verified:** `go build` clean, **full suite green** (exit 0); booted **both** ways (`source=embedded` + `source=../api`), each `Loaded milestone unlocks count=1` — `81 skills/14 factions/50 mobs/10 recipes/847 props/383 spawns/5 campfires/14 npcs, 0 panics`; **end-to-end proof:** a second milestone entry went live on a plain restart with **no rebuild** (`count=2`), then reverted. New `TestEmbeddedMilestones_MatchSource` guards a forgotten `cp-defs` (embedded stale while `-content` correct) and was **proved non-vacuous** by deliberately drifting the copy. Not PO-verified in-game (no runtime surface changed).
- **Prior (ad-hoc, full ledger `plan-intermission-triage.md` §Wolf-line drop reshuffle):** **Wolf-line drop reshuffle + milestone trim ✅ `f9a64db5`** 2026-07-21 — PO found the four-wolf drop set confusing while playtesting and retuned it live. **Framing ruling (PO 2026-07-21): the C8 "drop table FINAL" and "milestone table FINAL" locks are first-pass settlements, NOT frozen values** — FINAL means "decided well enough to build on", not "never revisit"; treat both tables (and essentially every [PLACEHOLDER]) as tuning-open. FirstAid left the milestone table ⇒ **Haste @L7 is the only milestone unlock left**; wolf line = Swift .1 + KeenEye .06 line-wide + EliteWolf Wild .5 / DireWolf LongRangeStrike .2 / AlphaWolf Reaper .2. Swift-unreachability flag raised + resolved in-session (line-wide). Two open tuning notes (Wild reads as a trap pick, Reaper maxLevel 3) carried in **Deferred** below.
