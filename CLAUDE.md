...
- **Last completed: round-5 batch — both verification-session items DONE** (2026-07-26, backend only, 4 files, **⏳ PO TEST PENDING**, 6-point acceptance checklist + full ledger at `docs/plan-playtest-feedback.md` §Round-5 chunk ledger). ① **Shield auras now draw a tick indicator** — one line, `EffectTypeShieldAura` joins the `HasVisibleTickCadence` whitelist; everything downstream was already live and category-agnostic, so **no frontend change**. The predicate is shared by `player.go:644` and `mob.go:475` ⇒ player and mob indicators moved together. Hits `RallyDrum` + `WarbannerShield` only; `Vanguard` already indicated at its damage cadence (`Effects[0]`) and is untouched. ② **Pacifists flee when attacked with nothing to support** — exactly the 4 designed pieces, no additions: `modeFlee`, one `selectMode` case ranked **below support** (reaching it means there is nobody to heal; above engage for readability only, since a pacifist has no combat slot), one `Update` movement case on the existing `moveAwayFrom()`, direction from `highestThreatTarget()`. `applyMode` untouched — `modeFlee` falls through to slot −1, already what an unemployed pacifist does. **1 implementation deviation:** `highestThreatTarget()` **prunes dead rows as it reads**, so it is resolved once into a local instead of being called in both the `case` and the body. **⚑ The edge worth knowing:** campfires/totems/braziers are pacifists too and now reach `modeFlee` when damaged — **inert both ways** (they are `auraAlwaysOn` ⇒ early return before any aura gating, and `moveAwayFrom` refuses a zero-velocity mob), pinned by its own test rather than reasoned about. TDD: 5 new mob tests + 2 skills tests, all verified red first. Verified: `go build`/`vet` clean, `go test ./...` **exit 0**, guardrails `-count=2` clean, boot clean with unchanged counts. **Not done:** no in-game click-through — checklist items 1/5/6 are feel judgements, and decision 4 deliberately deferred the drummer's flee speed to the PO's eye.
- **Prior: ⭐ ALL THREE PENDING CHUNKS PO-VERIFIED IN-GAME** (2026-07-26, one session): **round 3** `03b152f4`, **round 4** `eaae2e69`, **rolling-filler batch** `dab4dae0` — every checklist item on all three passed, and both ⚠ judgement calls resolved as shipped (the `RallyDrummer` reading as a pacifist is **accepted**; the whole-point tooltip HP rounding is **signed off**). **2 new items raised in that session, neither a defect in any of the three** (`docs/plan-playtest-feedback.md` §Intake round 5): ① **a shield aura draws no tick indicator** — `shield_aura` is simply missing from the `HasVisibleTickCadence` whitelist (`skills/definition.go:66`) so the wire interval is 0; everything below it already works, so it is a one-line fix affecting `RallyDrum`/`WarbannerShield` only (`Vanguard` reads `Effects[0]` = its damage cadence either way), and the same predicate drives the player indicator. ② **pacifist mobs should flee when attacked with nothing to support** — **a logic change, NOT config**: `factors.fleeBelowHealthRatio` exists but the flee branch (`mob.go:687`) is nested under `case m.aggroTarget != nil` and a pacifist never acquires one (round-3 ruling, `mob.go:904`), so authoring the ratio today does nothing; and the ask fires at full health anyway, so it is a **mode**, not a threshold. Lands cleanly in the round-3 architecture with no new plumbing: `modeFlee` + one `selectMode` case ranked below support (`isPacifist() && InCombat() && supportTarget == nil` — `InCombat()` already means "hit within ~3.3 s", which also ends the flee for free), the existing `moveAwayFrom()`, and `highestThreatTarget()` for the direction since the pacifist threat table is already populated (`noteThreat` never gates on pacifism). `applyMode` needs no change — `modeFlee` falls through to slot −1. **Does not undo round 3:** mob step is `0.055 × speed` vs the player's `0.05`, so `BanditHealer` runs at **66 %** and `RallyDrummer` at **50 %** of player speed — still catchable. **✅ ALL 4 BLOCKING DECISIONS TAKEN 2026-07-26, every one the minimal option ⇒ the chunk is designed and ready to execute:** ① scope = **pacifists only** (the ~4 support-without-combat mobs; everything else byte-identical) · ② **universal within that scope, no authoring flag** (YAGNI — add `fleeWhenHelpless` when content wants a stoic healer) · ③ **flee away from the top-threat attacker**, not toward allies (the "runs to fetch the guards" read is better story but needs an ally search outside the sensor and can walk the healer *into* the player) · ④ **authored speed, no flee multiplier** — the drummer staying catchable at 50 % is exactly what protects round 3's fix; judge the waddle in-game. No content edits, no conf knobs, no wire change.
...
- **Next: PO test of the round-5 batch, then step 8 (see below).** The round-5 batch is the only thing pending — 6-point checklist at the top of §Round-5 chunk ledger. In short: the `RallyDrummer` (27.3, 19.7) must now show a **tick indicator** on its shield ring; a lone `BanditHealer` (50, −21.5) must **run from you** yet still die when you keep up; next to a wounded bandit it must **keep healing instead of fleeing**; the flee must stop ~3.3 s after the last hit; **judge whether the drummer's 50 %-speed retreat reads as fleeing or as comedy** (decision 4 shipped it unmodified on purpose); and ordinary damage mobs must be completely unchanged. After that nothing blocks step 8. **Round 6 queued three pickups, none blocking:** the **orc-grunt fix** (config-only if you take the radius or the anchor — `-content ../api` + restart, no rebuild), **mob soft separation** (decided, `steering.go`), and **target stickiness** (`targeting.go`, unscheduled). PO decisions on record: **2026-07-26 round 6 — the resource's action-economy/survivability duality is INTENTIONAL with a permanently free base damage aura; collision = SOFT SEPARATION, hard collision and player↔player both rejected; 3 grunt options sanctioned**; **2026-07-26 — `RallyDrummer` pacifism ACCEPTED, tooltip whole-point HP rounding SIGNED OFF**; round-3 2026-07-25 — no universal auto-attack (parked in §31), support set = Heal+Shield, pacifist healers ignore their attacker, survivors-like fork **CLOSED — stays MMO-lite**; round-4 2026-07-25 — tooltips show **absolute numbers**, and 2026-07-26 — add a real JS test runner rather than skip the frontend guard.
