...
- **⭐ SKILL VFX C2a, the engine + three kinds, is the ranked next chunk** (`docs/plan-skill-vfx.md`; C0 + C1 done 2026-09-19, C2a-C4 unbuilt): the `SkillFx` manager on its own layer below darkness, budget + pools, the math module, body resolver with placeholders, `impact` / `projectile` / `beam`, the `hitStyle` lever DELETED and `aura_hit_style` deprecated with it, `AuraTickIndicator` absorbed. ⚑ Owes a PO look with screenshots. ⚑ Read the C1 ledger's "Landmines for C2a" first.
