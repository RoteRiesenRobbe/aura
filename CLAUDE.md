...
- **⭐ Prior design session: the QUEST SYSTEM (backlog §42) — DESIGNED IN FULL** ✅ `8d6440f6` 2026-07-29 (docs only) — new **`docs/plan-quests.md`** (D1–D13 PO-ruled · 10 landmines · chunks C0–C4, none started), indexed in `docs/README.md`. **⭐ CODE-REVIEWED 2026-07-30** ✅ `e9676836` (docs only; 3 parallel line-level sweeps — interaction container · XP/credit path · wire + content loading) — corrections folded into the plan tagged *(code review)*, **4 new PO rulings D14–D17** (minimal-projection `/quests` catalog, titles + diary prose only · **Pass 3 item 1 — the ruled-but-unbuilt presence-counts attribution rework — ships BEFORE C1 as prerequisite chunk P** (D15) · journal = `J` + HUD button · stage-entry alert banner) **+ 5 new landmines L11–L15**. The review's biggest catch: **the quest ledger must join the death/reconnect stash** — the player struct is rebuilt on death/reconnect and only `progression`+`SkillComponent` survive (5 sites, `sys/state.go`), or every death silently wipes quest progress (L11). Also material: ledger state rides **GameState every tick** like the spellbook — the plan's original "EntityMessage + full-state-at-join" model had no precedent (`Welcome` is a shared buffer; EntityMessage is lossy, garnish-only — L8) · species/conversant keys = **`MobID`**, never EntityType (9 sprite overrides) or entity id (resets per respawn) — plus a dup-id boot guard and a legacy-species lint (L12) · counters fire on **participation, not XP amount** (28/64 defs author `experience: 0` — L13) · the band lock is an **offline authoring budget with zero runtime clamp**, so quest XP is sized by hand in C4 (L9) · a new grant kind is ~5 files, the loader resolves `skill` unconditionally today (§5). **D4 restated:** quest credit = XP credit **at the same event, whatever the attribution rule is** — the hook (`rewardPlayer`) sits downstream of the participant set, so quests inherit the D15 rework and charm/summon `CreditTo()` routing for free; §42 stays the tension/pillar record, the pillar itself was ruled earlier the same day (GDD §8 → "Quests & the Journal": journal-carried, **no markers ever**). ⭐ **The load-bearing shape: a quest is a stage graph and BRANCHING IS DIALOGUE-SHAPED** — objective stages (kill-N / harvest-N / talk-to) auto-advance off events against **lifetime counters**; dialogue stages advance only via authored rows, and several rows on several NPCs can move the same stage to *different* next stages with *different* rewards, which is how "two NPCs complete the quest" and the later **camps** concept are content, not features (D9). The quest file owns stages + diary prose; the world's conversants own who offers/advances it (D11 — conversant-only starts, signs count for free). **The rulings in brief:** verbs kill-N/talk-to/harvest-N, discover-location deferred (couples §41) · **retroactive credit** via lifetime per-species counters + talked-to set (mechanical delta vs after-accept ≈ zero; accepted cost: a veteran auto-completes on accept, "kill 8 MORE" waits for a deferred flag) · **presence counts** — the counter increments where XP does, so quests and XP never disagree · per-character state, one-shot + `repeatable` schema room · journal = **Gothic 1's shape** (grouped per quest, running/done) · skill-trade turn-in (D8) and consequences/quest-turns-hostile (D10) are **schema room only** — un-learning is THE open question blocking skill-trade authoring · **abandon yes, failure no** (D13). ⚑ **D12 reorders the roadmap: the first pass builds BEFORE step 8**, session-scoped like the spellbook, so persistence receives a live ledger (roadmap "Execution order" has the pre-step-8 insert). ⚑ **C0 is a real prerequisite, not hygiene:** §8b's N1 trap (a row that both grants and navigates is broken at BOTH ends) is exactly the shape of every turn-in row; C0 fixes both ends + the 255-index guard. ⚑ New landmine found while ruling D13: **abandon + mid-quest `grant_xp` is a loopable XP faucet** (counters survive abandon ⇒ objective stages re-complete instantly) — L10's loader lint pins `grant_xp` to terminal edges.
...
- **Last completed: §35 C3 + C4 — the pins and the mirrors, closing the conf-duplication plan** (2026-07-30, tests + wire + frontend, `[uncommitted]`) — **`plan-conf-duplication.md` is COMPLETE and ARCHIVED** (`docs/archive/`). **C3 (tests only, zero production code):** the tier-3 Go literals pinned against `conf.default.json` — sim's `DefaultRegenTick`/XP 300·1.2/margin asserted on **the config a built sim world actually runs** (`NewWorld(...).game.config`, not restated literals), `model/mob`'s `defaultChaseIntoAuraMargin`, and both `combatRegenGraceTicks = 100` pinned in-package naming their twin (L11 — mirrors stay deliberate, drift now goes red; red direction proven by perturbing the JSON). **C4a:** `spacedName()` **deleted** — the summon tooltip resolves the served `/mobs` `displayName` via `mobDisplayName()` (`client-data/Mobs.ts`), raw-name degrade; the old rule copy was genuinely red in the new test (it split names even with no catalog). **C4b:** `ActivationRejection` is a **wire enum in `server.fbs`, values pinned 0/1/2** (L8); both binding sets regenerated as a **zero diff** apart from the new enum, the Go model constants derive from the generated enum (the `status_effects.go` pattern), the client map keys by the generated names + `Skills.test.ts` asserts every member has a message — the renumber-shows-wrong-message defect class is dead. ⚑ The client imports the enum FILE, not the `AuraApi` barrel (the barrel drags `flatbuffers` into client-data and breaks vitest resolution). **C4c:** new **`api/shared-constants.json`** (pip bits · ring category bits · tier ranks · viewport 20×12 · tickrate 30) asserted by `cmd/aurad/shared_constants_test.go` AND `SharedConstants.test.ts` (TS side exhaustive both directions; one perturbed bit failed both languages); the client tier-frame table now keys off a named `TierRank` enum, and the three client bit enums went `const enum` → `enum` (member enumeration is a compile error on const enums; behavior-identical). Fixture is deliberately outside `cp-defs`/loaders. **Verified:** build/vet/full Go suite clean · 66/66 vitest · typecheck · prod build · boot `-content ../api` 0 errors 0 warnings, counts pinned (15 factions/86 skills/64 mobs/10 recipes/1 milestone/5 prop defs/777 props/485 spawns/5 campfires) · harness gate: `round4-tooltip.mjs` all checks passed (first run was a §29 context loss, re-run clean), `hygiene-wire-prune.mjs` 647 sprites/0 errors/0 ctx losses. Wire bytes identical ⇒ no battery. Manual in-game rejection-message spot check handed to the PO per the per-bug model. Full ledgers `docs/archive/plan-conf-duplication.md` §7.
...
- **⭐ NEXT — quest chunks P + C0–C4 (`docs/plan-quests.md`), THEN step 8, accounts & persistence (a DESIGN session, not a schema session).** Quest ruling **D12** put the quest first pass in front of step 8 (see the ⭐ quest banner at the top; roadmap "Execution order" carries the pre-step-8 insert); the 2026-07-30 code review added **prerequisite chunk P = Pass 3 item 1 of `plan-playtest-feedback.md`** (presence-counts attribution, D15) so the sequence is **P → C1→C2→C3→C4** — **C0 (interaction hardening: the §8b N1 both-ends fix + the ≤254 index guard + the two stale-comment fixes) is independent of everything and is standalone filler any time**. `plan-faction-flips.md` is **COMPLETE and ARCHIVED** (chunks 1 ✅ · 2 ✅ · 3 ✅ · in-world teachers ✅ `3b1b3ef6` · all §8 questions closed), pre-accounts hygiene is complete, and the open-questions sweep above cleared the standing PO queue. The step-8 detail — ruling 4, backlog §32, the ops/security fold-in, **now also the live quest ledger from D12** (`quests`/`killCounts`/`talkedTo`, plan §10) — is in the bullet below. ⚑ **The scoped-filler queue is EMPTY:** §35 C3–C4 shipped 2026-07-30 (see Last completed — that completed and archived `plan-conf-duplication.md`), after **Swift → cooldown** `a29fe986` and **R4** `a4225e8b` had already shipped; quest chunk C0 is the natural next filler-sized piece.
