...
- **Last completed:** Step-5 (unlock-source systems, `docs/plan-npc-teaching.md`) **chunk 6 (zone-editor `npc` placement mode) DONE + VERIFIED IN-GAME by PO 2026-07-15 → Step 5 COMPLETE.** The editor NPC mode is committed in **`00574d4c` ("Add NPCs to Editor")**: `ZoneModel.ts` (`ZoneNpc`/`ZoneTeaching` interfaces + `npcs[]` + `add/update/removeNpc` + `getZoneAsJSON` omit-when-empty, teachings only when non-empty → round-trip diff-clean); `ZoneEditor.ts` (`'npc'` selection kind, `npcMarkers`, `drawNpcMarker` = sensor-radius disc + centre dot + `Type (n)` label, `hitTestNpc`, `place/update/removeNpc`, rebuild/redraw wiring); `_ZoneEditorPanel.ts` (`'npc'` `EditorMode`, mode radio, `npcControls`, an ordered teaching sub-list [skill dropdown from the registry, add/remove], update/delete/deselect, count label, client-side mirrors of the backend loader's hard-fails); `groundTexturePanel.html` (all `zoneEditor_npc*` DOM + the `npc` mode radio). **Follow-up bug fix (uncommitted, in working tree):** the "Lore lines" `<textarea>` swallowed all keystrokes — **two coupled defects**: (a) the `preventShortcutPropagation` guard's `querySelectorAll('input, button, a, select')` omitted `textarea` (`_GroundTexturesPanel.ts` ×2, `_ZoneEditorPanel.ts` ×1), so the field was never registered; (b) `preventShortcutPropagation`'s type-switch (`common/logic/Utils.ts`) had **no `HTMLTextAreaElement` branch**, so even once selected it fell through to the `console.warn('Unsupported Element')` no-op. Fixed both: added `textarea` to the 3 selectors + a `HTMLTextAreaElement` case (full free-text capture like the `'text'` input case; stopping propagation also keeps Enter inserting a newline instead of firing a game shortcut). **Sanity:** `tsc --noEmit` clean. **VERIFIED IN-GAME by PO 2026-07-15:** place→export→reload→teaches (the chunk-6 gate); and after the fix, typing + newlines into Lore lines work. **Prior:** chunk 5 (frontend bubble hoist) `2984b447`; chunk 4 (speech origination) `6322218c`; chunk 3 `35f3b151`; chunks 1–2 `f4ebe912`, `9d71f9ad`. Step 4 (skill-vocab fill) COMPLETE + committed.
- **Next:** **Step 5 is COMPLETE.** Per `docs/roadmap.md`, the next work item is the **pre-step-6 gate: the simulation harness** (GDD §5 metrics — TTK / survival / kills-per-level + the 1-vs-N matrix + the stand-still-bot *sustainable kills/hour over a chain incl. modeled regen + downtime, per level bracket*), then **Step 6 = the initial content pass (item 12)** — real zones, full mob roster, boss scripts, first real balance pass. Both are new, **plan-first** sessions (write a `docs/plan-*.md` before code). **Note:** the textarea keyboard-capture fix above is **uncommitted** — commit it (the `00574d4c` editor work already landed) before starting new work. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; the committed Sage (proving-grounds (4,3), teaches `HealAura`@L1 + `Dash`@L5 + idle lore line) is content — boot `-content ../api` for content iteration.
