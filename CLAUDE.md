...
- **Last completed: pre-accounts hygiene, SESSION 2 — H1a, the chase margin** (2026-07-28, backend, 3 files, +38/−5) ✅ `[uncommitted]` — the part priced as *"the only one that moves a number"*, split off precisely so a moved number could not ride through a review of four diffs that changed nothing. ⭐ **It moved nothing, and that is the finding — TTK 6.67 s / TTD 8.70 s STAND**, are not superseded, and stay the baseline every later chunk measures against. D1's *"accepted cost: a one-time re-baseline"* was a price that never came due. **What was fixed:** two Go defaults held one value and disagreed 4× — `gameconf.go:48` normalizes to **0.2** (the live path; all five non-test `NewMob` callers pass the normalized value) while `mob.go`'s "0 means unset" fallback said **0.05**, and `sim/world.go:75` had copied that test-only number under a comment claiming it mirrored conf. Both now 0.2, run as two separately-verified steps (L-H1). ⚑ **Chased down rather than accepted as a lucky green: no battery scenario ever makes a mob approach**, so `chaseIntoAuraMargin` is not exercised by the instrument at all — facetank spawns the mob **on** the player (`distance = 0`), kite pins it at the ring centre with `Speed = 0` + `Role = structure` (`chain.go:150-160`), the 1v1 and level sweeps start at `-distance 0.5` while `stopDistance` is 1.20/1.05, and the guardrail roster leg is `RunChain` facetank. **The drift was LATENT, not active** — which does not make the fix pointless, it makes it cheap: the harness was one `-distance 3` away from silently modelling a game that stops 0.15 units short of the real one. ⚑ **The one red test is the measurement, reported not re-fitted.** Exactly one of 155 test `NewMob` sites leaned on the fallback — `TestMob_StopsChasingInsideAuraStopDistance` — and *invisibly*: `stopDistance` went 0.7 → 0.55, flipping its "hold position" probe at 0.6, while its own comment did the arithmetic with 0.05. The probe was **not** moved to fit; the test now **authors** `margin = 0.2`, so the geometry it pins is the geometry the game has, plus a new pin on the fallback value itself. ⚑ **Deliberately NOT unified:** `gameconf.go:48` and the new `defaultChaseIntoAuraMargin` are still two independent 0.2s naming each other in comments — collapsing them is backlog **§35 tier 3**, still open, and the DRY fix would drag config normalization into a model package. **Verified:** `go build`/`vet`/`test -timeout 180s ./...` green, guardrails + alloc `-count=2` green, boot `-content ../api` **0 errors 0 warnings 0 panics — 83 skills/15 factions/64 mobs/10 recipes/1 milestone/777 props/485 spawns/5 campfires** with the live config reporting `MobChaseIntoAuraMargin: 0.2`. No frontend surface. ⚑ **Harness note: three guardrail log lines reorder run to run** — the tier-band membership lists (`guardrail_test.go:256`) and the `strongest non-ceiling … 0.670 ev/tick` tie (`:179`) come out of Go map iteration (three runs named three different skills for the same value). Set-equality is identical and every survival-rate line is byte-identical; **diff that output raw and it looks like a regression.** The archived §11 now also pins the four battery commands, which session 1 ran but never wrote down — and records that the roster leg is a `go test -run Guardrails`, not a CLI run (`-content` is a serve/preset flag). Full ledger `docs/archive/plan-pre-accounts-hygiene.md` §11.
...
- **⭐ NEXT: step 8 — accounts & persistence (a DESIGN session, not a schema session).** The pre-accounts hygiene chunk is **COMPLETE** (both sessions ✅, doc archived), so the last thing standing before step 8 is gone. ⚑ **`docs/plan-faction-flips.md` chunk 1 is behaviour-neutral, small, and can run before, after or alongside that design session** — only its chunk 2 (charm) carries a real verification tail. See the ⭐ banner at the top for its reframe and the two landmines (L-E faction is global; L-C charm is invisible to the client). The step-8 detail — ruling 4, backlog §32, the ops/security fold-in — is in the bullet below.
