...
- **One plan in flight** (`plan-zone-editor-structure.md` completed whole + archived 2026-08-16; `plan-code-health.md` likewise 2026-08-14): `plan-play-bot.md` untouched (C0-C3: extract transport → the bot that fights → quests → the soak finding). **Two more designed 2026-08-15, nothing built:** `plan-effect-types.md` (round 1: vulnerability, retaliate damage, invulnerability, ally speed - four chunks, schema NONE) · `plan-prototype-projectile.md`. ⚑ Zone-editor C3 leftovers, owned by nobody: the Go legacy plumbing is now dead code (Legacy/LegacyRefs fields + the unreachable loaders warning) · `wiki-generator/` is broken at HEAD independently of C3 (requires the §28-deleted `client-data/Items`).
