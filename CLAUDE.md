...
- **Last completed:** **Combat readability: category aura rings + mob tier frames (triage items 7 + 15) — DONE + PO-verified in-game 2026-07-21 ("looks better"), committed `e8b67289`; full ledger: `plan-intermission-triage.md` §Combat readability.** First chunk of the post-C8 PO priority queue; both items shipped in one pass since they share the same append-only Mob-table extension. **PO picks:** item 7 → category colour (identical for players and mobs); item 15 → frame ring (normal unmarked / elite silver / boss gold); multi-effect auras → **bitmask, dual rings kept**. **Mid-chunk PO correction:** the first cut drew one ring per category at *decreasing radii*, which reads as different areas of effect — false, since every effect shares one aura radius. Bands now stack **inward from the aura edge** (a multi-category aura = a *thicker border*), outermost edge = the true radius. **Wire (append-only, no ID shift):** `Mob.aura_category` 21 + `Mob.tier` 22, `Character.aura_category` 29, all `ubyte`. **Backend:** `skills.AuraCategory` derived from the existing `EffectDef.Type` (no new authored field) as an **exhaustive table, not a switch with a default** — a new `EffectType` fails a test instead of silently rendering ringless; `mobs.TierRank`, with `tierRanks` now the **single source for both** loader validation and wire encoding; `SkillComponent.AuraCategories()` delegated by player+mob (the `LightRadius` pattern). **Frontend = net debt removal:** one shared `AuraRingStack` replaces **three** implementations, deletes `damageAura.svg`+`healAura.svg` and (after the geometry fix) the SVG path entirely — `assets/effects/` is gone; the hardcoded skill-ID ring switch and **all 8 `*_SKILL_ID` constants** deleted from `Skills.ts`. **Finding: the old dual ring was already wrong** — Warbanner carries **four** categories, Vanguard three, but the client only had two sprites. **Watch item (cost a PO round-trip):** `_GameObject`'s ctor calls `initShape()` **before subclass field initialisers**, so fields assigned there must be declared **without** an initialiser (as `actualShape` already was) — `= null` silently overwrote it; compounded by `isDefined` being `!== undefined` (**true for null**). **Verified:** `go build` clean, **full suite green** (exit 0), `tsc` clean, prod build clean, boot `-content ../api` `81 skills/14 factions/50 mobs/10 recipes/847 props/383 spawns/5 campfires/14 npcs, 0 panics`; headless client smoke **no runtime errors**, **proved non-vacuous** by reintroducing the defect and catching the PO's exact error 8×.
