...
- **⭐ SKILL VFX owes the PO look at C3a-ii, then C3b, then the phone check** (`docs/plan-skill-vfx.md` §13 C3a-ii): the look = the rim bite, the white fangs (two 42 px fangs may want their own length rule), the spit drawn on application and every refresh, the four cooldown waves. **C3b** = the editor's Visuals section (§12f.5; `applied` is a fourth column). ⚑ **The phone check carries THREE decisions**: cap 96 vs 192, fill rate, the packer trigger (§12f.2). ⚑ The pyromancer's new `damage_aura` roughly doubled its output, unmeasured. ⚑ The harness census misses snapshots headless (ledger Findings).
