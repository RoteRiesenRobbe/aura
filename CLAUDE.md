...
- **Last completed:** **Sim-harness PLANNING session (2026-07-15) — `docs/plan-sim-harness.md` written + APPROVED; the `f(character level)` design was settled with the PO (recorded in GDD §5 + tdd §4.1).** The harness is a **balancing / what-if explorer** (numbers → outcomes), not primarily a gate; it **drives the real ECS headlessly** (no re-modeled math), reports every metric as a **distribution** (variance + crit kept in-model), and ships a **standalone cmd + saved artifact**. **`f` decision (Philosophy A):** same-tier fights are **scale-invariant** (TTK/TTD constant at every level); `f` is *not* a same-tier balance knob but does progression-feel + **baseline relevance** (a found aura = `base(1)×f(level)`, instantly usable, even early auras found late) + uniform outleveling; directed growth = skill points + slots/unlocks + zone progression. **Curve is STEEP** (WoW-Classic-punishing): `f`'s rate is cross-tier-defining → target **~4-level doable band ≈ ~12%/level** [PLACEHOLDER, supersedes the old 6.9%/50×-over-60], **max level ~25–35** (linked triple: band ↔ max level ↔ total inflation, ~15–30×). Live-game `f` wiring stays a step-6 task. 4 chunks: 1) 1v1 TTK/TTD explorer → 2) implement+validate the curve → 3) 1-vs-N matrix → 4) sustainable-kills/hr chain + facetank-vs-kite. **Next session = execute Chunk 1.** No production code was written this session.
...
- **Next:** **Execute Chunk 1 of `docs/plan-sim-harness.md`** (plan APPROVED, design settled — see "Last completed"): a new `backend/pkg/berryhunter/sim/` package that builds a minimal deterministic world, drives the real ECS systems headlessly, and reports 1v1 **TTK / TTD as distributions** (N seeded runs, variance + crit on) from explicit combatant numbers, plus a thin `cmd/simharness/` printing a table + writing a JSON artifact. Sanity gates per CLAUDE.md (`go build ./...`, `go test`). Then chunks 2→3→4, pausing for PO review between. After the harness: **Step 6 = the initial content pass (item 12)** — two large zones with intra-zone difficulty areas, full mob roster (hand-authored per-tier against the §5 curve), boss scripts, first real balance pass. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; the committed Sage (proving-grounds (4,3), teaches `HealAura`@L1 + `Dash`@L5) is content — boot `-content ../api` for content iteration.
