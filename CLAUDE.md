...
- **Last completed: `plan-entity-model.md` Chunk 1a — one derived-stat formula** (2026-07-26, backend only, 10 files + 1 new) ✅ `cf9a10c7` — **⭐ acceptance criterion met: sim battery + level curve + pack matrix BYTE-IDENTICAL** to the pre-edit baseline (TTK 6.67s / TTD 8.70s), which is the whole reason 1a opened the plan: 1b's real balance deltas now measure against a clean baseline. Full ledger `docs/plan-entity-model.md` §11. Three factor methods on `DerivedStats` (`MaxHealthFactor` / `DamageReductionFactor` / `MovementSpeedFactor`), player migrated onto them unchanged, **`*Mob` now reads all three** — the three latent player-only stats are closed. Plus the two riders: **`game.mob.walkingSpeedPerTick`** (⚑ L1 held — **0.055 preserved**, the player's 0.05 was never adopted; the standing constructor TODO is gone) and **`Level()` on `*Mob`** with `PowerScale()` derived from it. `Derived.Resistances` deliberately untouched (audit deferral). **3 decisions inside the chunk:** ① **every pool cap goes through `MaxHealth()`** — regen target, `Heal`'s `AddCapped`, `HealthRatio`, the full-health participant reset (`==` → `>=`); the getter alone would widen the wire number while the mob still could not heal past its base pool, and `m.maxHealth` is now explicitly the **base** pool, the shape 1b generalizes; ② the **DR clamp moved inside the shared factor**, so no call site re-checks it; ③ the speed factor rides **`stepLength()`, the consumption site** — folding it into the stored `velocity` (set once at construction) would freeze the spawn loadout. ⚑ **The flagged plumbing was real:** live `PowerScale()` needed `curve.Curve` threaded into `MobDefinition` (only the float was retained); the zero-value curve is neutral, so sim/test defs read 1 exactly as the old `PowerScale <= 0` guard did — but a mapper that forgets the field would silently flatten every mob to 1, so the registry test pins it. Verified: TDD **red-first on all 3 behavioural pins** (asserting HP pool / damage taken / distance moved, never `Derived` — L6), `go build`/`vet` clean, `go test ./...` **exit 0, 27 pkgs**, simharness + **alloc** guardrails `-count=2` (the new multiply sits on the per-tick `steer` path), boot `-content ../api` **0 errors 0 panics 0 warnings** (83 skills/14 factions/50 mobs/10 recipes/5 prop defs/1 milestone/777 props/471 spawns/5 campfires/14 npcs) with `mob.walkingSpeedPerTick: 0.055` in the tuning-knob line. **No in-game check needed** — nothing is player-visible, because no mob authors a `stat_multiplier` passive, which is the same property that makes byte-identity provable.
