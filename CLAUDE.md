...
- **⭐ Last completed: ZONE AUTO-DISCOVERY — the directory IS the zone list** ✅ 2026-09-10 (no plan doc; THIS ENTRY IS THE RECORD): every `.json` in `api/zones/` loads on every boot, so adding a zone is dropping a file in. ⭐ **`game.zone` + `game.zones` GONE, replaced by `game.startZone`** (Go default `"world"`), which names ONLY the primary zone — fresh-character spawn, the bounds on the wire, the `Welcome` name — and no longer selects what loads, because the filesystem already says that. ⛔ **CLI `-zone`/`-zones` GONE too**; `-start-zone` replaces them and an old invocation dies with `flag provided but not defined` (both `dev-restart*.sh` pass only `-dev -content ../api`). ⚑ **The deliberate trade: a half-authored zone in `api/zones/` now BREAKS THE BOOT** where it was silently ignored — which IS the fix, since a present-but-unloaded zone looks installed and is not (`tunnel.json` sat there a session doing nothing). Park WIP outside the directory. ⭐ **The durable finding is about TESTS, from PO feedback** (*"tests that break when i edit the map are kind of dumb"*): the passage round-trip pin hardcoded anchor NAMES + hand-picked distances, and the D8 clearance pin asserted a **made-up `8.0` against a real bound of `2.75`** (dwell `0.75` = aura `1.5` × `CampfireDwellRadiusFactor`, + the door’s `interaction.range` `2.0`) — false positives on every map edit, proving nothing when green. Both now DERIVE: round-trip pins **nearest-door ORDERING** (no names, no thresholds — it survives moving a passage anywhere); clearance reads the registry. **Schema: DB/WIRE/CONTENT NONE · CONF BREAKING** (an old key now warns as unknown and is ignored, not fatal; `devops/conf.json` updated, `deploy.sh` copies it verbatim). Verified: build · vet · **`go test -count=1 ./...` EXIT 0** · **mutation-verified ×2**. ⛔ **NOT booted by me — no Docker/`.env.local` on this host**, so `main()`’s resolution and a 3-zone boot log are unproven.

- **Prior: THE UNDERWORLD — U1→U4b, the first PLACED zone** ✅ 2026-09-08 (ledger: `docs/plan-underworld.md`; engine half committed `78a274bd`, U4b uncommitted): a zone file may now be authored around its own `{0,0}` and given a runtime **`origin`** that moves it far away in the SHARED coordinate space, and a **`CaveMouth`** interact row walks you between them behind a **directional curtain**. ⭐ **There is still ONE `phy.Space`, one process, no handoff** — zones are disjoint by **DISTANCE**, which the sparse hash-grid broadphase, the border walls, AOI streaming and every aura overlap already respect for free. This **amends `plan-release-map.md` D6 in its file-count clause only**; §8.3 (separate Spaces, handoffs, sharding) stays out of scope. ⭐ **The rejected alternative is recorded: a `worldLayer` tag with overlapping coordinates** — nicer to read, buys nothing the offset does not, and costs a new orthogonal filter axis every AOI query, aura overlap, targeting scan and broadphase insert must honour. ⭐ **U3b is the design change worth remembering: the destination moved from the mob DEFINITION to the PLACEMENT** (`spawns[].anchor`) — a def-level destination means one file per passage END, so two passages wanted four near-identical mob files; `plan-world-paths.md` D4 had already refused that exact shape. ⭐ **`Zone.Origin` is now an AUTHORING CONTRACT, not a free packing coordinate: +Y IS DEEPER** — U4b derives a row's direction byte by comparing the two zones' `OriginY`, so ⛔ **packing zones down Y for tidiness would make every crossing a descent**; a neighbour belongs in X. ⚑ **`ConversationOption.travel:ubyte` is DERIVED, never authored, and `anchor` mode ONLY (L15)** — `home_campfire`/`caster` resolve at step-through time by design, so a build-time direction can be stale; they report lateral and the client **upgrades on arrival**. ⚑ **Two boot-time landmines fixed that only exist once a second zone loads**: `startingSpawn` moved per-FILE → per-SET **and primary-zone-only** (else a fresh character spawns underground), and campfire ids are unique **set-wide** (`character_campfires.campfire_id` is bare TEXT — `plan-world-scale.md` §7, filed and unowned, now owned). ⚑ **A FOURTH zone-format writer exists and the completeness pin cannot see it**: `aura-world-format.js` copies map-level values onto Tiled's `TileMap` **by hand**, so a Tiled save silently dropped `origin` and the next boot refused; `verify.sh` now has a placed-zone leg (⚑ its fixture needs BOTH axes non-zero — `{0, 300}` passes its own mutation). **Schema: DB NONE · WIRE 2 appended (`Welcome.zone_names`, `ConversationOption.travel`) · CONF `game.zones` (⚑ SUPERSEDED 2026-09-10 — both zone keys replaced by `game.startZone`, see above) · CONTENT `Zone.Origin`, `Spawn.Anchor`, 2 mob defs.** Verified: build · vet · **`go test -count=1 ./...` EXIT 0** · tsc · **vitest 663/663** · prod build · `verify.sh` green · **mutation-verified ×4**. ⛔ **NOT in-game verified by me** — and three browser-only defects already came out of U2, which shipped with no in-game pass (a fog mask destroyed with its terrain layer, an unconditional `snapshot.player.position` deref, and a camera clamp that took a SIZE where it needed a RECTANGLE). ⚑ **OWED: U0** (S1 lazy zone bundling) and **Q5** (the curtain now fires on the *shipped* portal pair and campfire recall, which were hard cuts); the **D6 amendment note** ✅ landed `5994897c`. ⛔ **U5 (content) DROPPED 2026-09-10 (PO): level authoring is manual** — but §7.3 re-homes what it was carrying, and the **in-game pass is NOT content and does not drop with it**.
