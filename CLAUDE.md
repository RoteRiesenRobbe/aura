...
- **Last completed:** **Playtest-1 feedback Pass C item 3 (darkness) — DONE 2026-07-22, `[uncommitted]`, PO-approved in-session ("all good now"), in-game feel pass outstanding; full ledger: `docs/plan-playtest1-feedback.md` §Pass C item 3 ledger. PASS C COMPLETE ⇒ the whole playtest-1 feedback plan is executed.** **Both halves of the plan's premise were wrong, found by measuring first** (per the handover note): (a) `FLOOD_OPACITY` is the **day/night cycle**, a *different system* from authored dark areas — night retains **70%** of daylight (genuinely "merely grayer"), which made it the obvious suspect and the wrong one; **PO ruling: day/night untouched** ("just a small visual"), the tester's report was the tunnel and the PO witnessed it ("props in the dark still visible enough to easily navigate"); (b) **`flood()` already emits a pure multiply** (`diag(k_r,k_g,k_b)`, zero offset column ⇒ ×0.65/×0.71/×0.84) — a "multiply blend instead of the alpha flood" would have changed *nothing*; night fails to darken only because the flood **colour** is bright (even opacity 1.0 reaches just 51%). Measured, identical crop: day-open **67.7** / night-open **47.2 (70%)** / dark-area **6.1 (9%)**; model validated vs measurement (predicted `(33.2,52.5,45.4)` vs measured `(31.0,52.3,44.6)`) ⇒ **future darkness constants can be priced analytically, no game run needed**. **Why 9% still read as navigable:** an alpha-black overlay **preserves relative contrast** — props stayed 6% brighter than the ground, so every silhouette survived intact (dim but fully readable shape info) ⇒ "grayer", not "darker", and nudging the constant would not have helped. **Fix = `MAX_ALPHA` 0.94 → 1** (dark areas now measure exactly `(0,0,0)`) — **not a new design call**: `gdd.md` §6.5 "field of view heavily restricted" + a **PO ruling from 2026-07-17 written into `Player.ts:99`** ("darkness stays fully dark — the hole may cover the avatar itself and nothing more") with `MIN_SELF_LIGHT_PX 40` guaranteeing you see your own avatar; the residual 6% was the placeholder, never the design. **Plus `DarknessOverlay.isHidden(x,y)` + 1 line in `Mobs.updatePlate()`**: mob plates hide in unlit darkness — they ride the night-exempt `namePlates` overlay *above* the darkness layer, so at full black "Venom Spider 4" in orange over an invisible spider was the only legible thing on screen, cannibalising the light role whose stated value is "spotting targets" (`gdd.md:482`). Hit-test geometry is stored **alongside** the sprites, not derived from them (sprite radii include the fade; the fade must NOT count as hidden); dynamic lights count at **full** radius so a plate returns exactly when the mob starts to emerge. **The one real risk was PixiJS short-circuiting the `AlphaFilter` at alpha 1** (would let `erase` punch through to the page background instead of the world) — disproved by sampling green ground pixels at the self-glow rim. Verified: **52 mob plates × 5 positions, 0 mismatches**, checked by walking the live scene graph (`window.game.character.plate.parent` IS the namePlates layer) against `world.json` circles — not by eyeballing; discriminating case = campfire pocket, same dark circle: `Dire Wolf litByFire=true visible=true` vs `Bear litByFire=false visible=false`; `tsc --noEmit` clean + prod build green, `go build`/`go test ./...` **exit 0**, boot unchanged `82 skills/14 factions/50 mobs/10 recipes/815 props/399 spawns/5 campfires (safeRadius 1.5)/14 npcs, 0 panics`. **Open:** the tunnel is now a **total blackout** (whole viewport `(0,0,0)`; corridor wider than the screen ⇒ nothing lit to steer by) and **no campfire exists in the corridor** (nearest `(-21.26,-23.51)`, ~9 units off the west end) — content-side lever if it plays too harshly; **still leaking through darkness**: other players' plates (arguably correct — light-support fantasy), **floating damage numbers** (most likely next complaint — numbers painted over an invisible mob) and NPC bubbles; **light does nothing at night** (holes only exist inside authored circles ⇒ Lantern/campfire are no-ops after dark); `MAX_ALPHA` is **no longer [PLACEHOLDER]**, `EDGE_FADE 2`/`LIGHT_CORE_FRACTION 0.55`/`MIN_SELF_LIGHT_PX 40` still are (the self-glow is now load-bearing). **Harness gotcha (recorded in `verify` skill): after `WARP`, wait ~20 s — the camera interpolation crawl (backlog §20) makes an early screenshot render the PREVIOUS position, silently; the first measurement run was contaminated end-to-end and produced exactly inverted results.**
