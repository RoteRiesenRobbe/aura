...
- **Last completed:** Render-jitter fix — buffered render-delay interpolation (plan-render-jitter chunks 1+A+B) ✅ `0e504c22`/`8a29a75c`/`c5064732` 2026-07-24, deployed live, PO-validated locally. Lever A aligned the client tickrates (`INPUT_TICKRATE`/`SERVER_TICKRATE` `33 → 1000/30`); Lever B replaced the reactive chase in `_GameObject.ts` with buffered interpolation (`RENDER_DELAY_TICKS=2` ≈66 ms, freeze-not-extrapolate, teleport bypass), trading +~66 ms render latency for smoothness (the standard PvE choice). **Acceptance = a live re-feel + fresh `[snapshot-arrival]` re-measure** — drop `RENDER_DELAY_TICKS` to 1 if the latency reads laggy. Full ledger: `docs/plan-render-jitter.md`.
- **Recent chunks (newest first; full ledgers in the plan docs):** input-jitter held-state fix ✅ `cb7f011f` (`plan-input-jitter.md`); unlock source attribution ✅ `2bfee286` (`plan-unlock-attribution.md`); idle-loop alloc fix ✅ `fe0044d0` + day/night DEACTIVATED ✅ `e648ab88` (`plan-intermission-triage.md`); playtest-1 Passes A/B/C ✅ — **plan fully executed** (`plan-playtest1-feedback.md`); F&F deploy LIVE ✅ `a7a2267d` → `https://aura-game.duckdns.org/` (`plan-playtest-deploy.md`); content pass C1–C8 + rebrand step 7 complete. Earlier chunks: roadmap.md "Execution order" + the plan-*.md §13 banners.
...
- **Open code-health findings** (documented 2026-07-24, none scheduled — `docs/backlog.md` §27 / `docs/research-code-quality.md` §10):
  - ① **✅ FIXED 2026-07-24** — `MobSystem.Update` mutation-during-iteration (dead mobs removed *inside* `for _, mob := range n.mobs` with synchronous append-shift `Remove` → one survivor skipped + another double-updated per dead mob/tick, single-tick stutter that looked like netcode jitter). Fixed test-first: collect dead in the loop, remove after; pinned by `TestMobSystem_RemovingDeadMobDoesNotSkipOrDoubleUpdateSurvivors`. Blast-radius clear (`state.go` snapshots; `decay.go` twin is dead code). §27.1.
  - ② **Drop-RNG determinism — PO ruled a bug** — `NewMob` seeds per-mob RNG from the entity ID alone (`mob.go:150`) + `ecs.NewBasic()` counts from 1 each process ⇒ a fresh server rolls the same HP variance + first drop for the Nth spawn every restart (visible live). Fix = mix a per-process seed (keep streams independent; sim harness/guardrails need their own explicit seed) — §27.2.2.
  - ③ **✅ FIXED 2026-07-24** (`39152958`, §27.4 order items 2+4, test-first) — `skills/definition.go` guard coverage evened out: `mapToEffectDef` now has a `default:` that hard-fails any effect type forgotten from the payload switch (with explicit no-op cases for the two payload-less types `light_aura`/`recall`), plus inert-config guards for 0-damage (siege auras with only `structureDamageFraction` stay valid), 0-heal (`heal_aura` + `self_heal`), and `radius ≤ 0` on any geometry type (data-driven off `effectKeys`, no hardcoded type list). 6 new tests + 9 fixtures completed; suite green, boot-safe (83 skills, 0 panics). §27.3.
