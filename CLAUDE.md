...
- **✅ GENERIC KILL QUESTS ARE DONE AND ARCHIVED** (`docs/archive/plan-generic-kill-quests.md`, C1+C2 both 2026-08-07 `a66e6be5`) — nine quests, 10 of 14 placed conversants offer one. ⚑ One watch item rides forward: **the Emberkeeper's `bandits-at-the-shrine` is the first human-target kill quest and the PO has not seen it in play** — if it reads badly, GiantSpider L14 ×5 is the recorded fallback (archived plan §8). Reward numbers follow the L9 half-level rule and are [PLACEHOLDER] like everything else. **Ascension C1 remains the visibly-next roadmap item.**
