...
- **⭐ Runtime allegiance — CHUNK 1 SHIPPED 2026-07-28 (see Last completed); the plan below is the design session that produced it** ✅ `074d1a53` — new **`docs/plan-faction-flips.md`**, indexed in `docs/README.md`. **5 PO decisions D1–D5 · 2 chunks · 10 landmines L-A–L-J · 7 open questions.** ⭐ **The reframe:** `SetFaction`'s `^f.Bit()` is not a wrong *value*, it is an **undefined destination** — correct for `aligned`, undefined for everything else. So the fix is not "look up the right mask", it is to replace one general-looking setter with two **defined verbs** (`Align()` / `RevertFaction()`), ~20 lines, **both existing callers bit-for-bit unchanged**. **3 findings that sized it:** ① **the restore direction needs no registry** — `MobDefinition.AggroMask` **is** the faction's authored mask (`definitions.go:409`), so reverting reads `m.definition`; ② **there is no runtime faction registry to look up anyway** — `factions.Registry` is boot-only (`loadFactions` → `loadMobs` → discarded), so flips to a *third* faction are new plumbing nobody wants (YAGNI); ③ ⚑ **the overwrite is LOAD-BEARING today** — `mayHarm` (`skills.go:403`) routes every harmful effect through `MayHarm`, and the built-in `aligned` faction is declared `AggroMask: 0` (`factions.go:109`), so the "obvious" registry-derived fix would **silently strip harm rights from every companion, totem and campfire**. Also corrected: L2 has **three** mask readers, not one (acquisition `mob.go:1228` · **damage eligibility** `mob.go:1396` · sensor mask `mob.go:681`). **The 5 rulings:** ① scope = correctness **+ charm as the first consumer** (proven by use, not by test only) · ② a charmed mob is **credited to its charmer but keeps its own level** · ③ charm is a **cooldown hitting the nearest enemy in radius** (GDD §9 forbids target-clicking) · ④ **charmability is an authored per-mob flag** — tier-gating rejected as exactly the inferred-from-a-number pattern chunk 2 retired · ⑤ plan now, execute next session. ⭐ **D2 came out cheaper than priced: it does NOT re-open chunk 1b.** `owner` answers two questions at once — *whose level am I?* (`Level():765`, one site) and *who gets my kill credit?* (three `model.Owned` sites); a summon answers the same player to both, a charmed mob answers **nobody** then **the charmer**. **Split the question, not the field** — `Level()`/`MaxHealth()`/`PowerScale()` keep reading `owner` (1b untouched, no number moves), attribution reads a new `CreditTo()` = charmer ?? owner. And `applyPlayerDamageAura:503` **already** separates `caster` (attribution) from `acting` (stats), so the player's crit and damage factors correctly do **not** leak into a charmed mob's hits. **⚑ 2 landmines worth knowing before chunk 2:** **L-E — faction is GLOBAL, so charm makes a mob immune to *every* player's auras**, not just the charmer's, against GDD §9 "no griefing by design" (D4's flag contains the boss case; open question 4 is the one to settle first); and **L-C — charm is invisible to the client** — `combatTarget` derives from the *definition* at catalog load (`catalog.go:63`) and faction is not on the wire at all, so a charmed wolf keeps its nameplate and bar and looks unchanged. **L-B kept as a pin:** since 1b, `SetOwner` **re-levels** — reaching for the obvious field would shrink a charmed elite to the charmer's level. All numbers **[PLACEHOLDER]**. Full ledger `docs/plan-faction-flips.md`.
...
- **Last completed: `plan-faction-flips.md` chunk 2 — calm, the disengage cooldown** (2026-07-29, backend + frontend + content, 30 files) ✅ `[uncommitted]` — **a wolf can now be told to stop.** Calm ships as ruled: it never touches faction, it **drops the live aggro link**, it takes **everything** in the radius (a pack aggros as a pack), and **any** damage ends it — the calmer's own aura included. ⭐ **Two plan corrections, both found by auditing §5 against HEAD before the first edit** (the habit that caught 3b's missing step and chunk 1's L-N). **① Calm is a `Buffs` payload, not a `Mob` field — §5.3 item 1 was wrong.** `AppliedEffects()` is *derived* from the buff store and `buffPayload` is a closed interface whose whole purpose is that a new kind **cannot compile without deciding its pip**; a bare `calmTicks` would have bypassed the one mechanism guaranteeing the client tell, and re-implemented aging + per-skill refresh the store already owns. The payload is **empty** (calm has no strength axis), and it cost exactly one new thing: **`DropCalm()`**, the store's only *targeted* removal — expiry and Cleanse-everything were the two shapes before it. **② ⚑ Skills loaded BEFORE factions at boot** (`aurad.go:62-63`), so D8's "resolved at content load" had nowhere to happen; factions now load first (they depend on nothing, so the swap is free) — 3 call sites plus a test-helper pass, since real-content tests need a **real** registry and `nil` is only safe for allowlist-free fixtures. **⭐ The D8 seam is where the PO's ruling met the code:** authored on the **skill** (PO's pick over the effect), resolved to a mask at load, then **stamped onto that skill's effects** — because the runtime gate belongs in `eligibleByTargetFlags`, the one predicate every targeted effect already passes through, whose own comment warns a per-site copy *is* how the gate gets forgotten. `TestCooldown_CalmScopeIsDataNotCode` is **L-L's acceptance test**: a second skill scoped to another faction needs no Go change. **⚑ L-O, the new landmine: skill-level JSON is parsed WITHOUT `DisallowUnknownFields`** — only *effects* get the strict key check, so a typo'd `targetFaction` would leave the mask 0, which reads as **unrestricted**. Fixed by making the allowlist **mandatory** for faction-scoped effect types (typo → boot error); **chunk 3 must add charm to `factionScopedEffects`**, the guard is per-type. **Also:** the break is checked **ahead of** the calm branch so the hit that breaks a calm retaliates on the **same tick** (its threat row is already written); calm gates `updateSupportTarget` too, because "out of combat" must mean it stops healing its pack; the aura gates off for free (no target → `modeIdle`); the pip is `AppliedEffectCalm` + one `PIP_STYLES` entry, **no schema change** (`applied_effects` is already a wire ubyte); a tooltip case, because the tooltip's `default:` warn makes a missing one a console warning and a literal `(calm)`, not a build error. **Content:** `api/skills/calm.json` id **62**, cooldown 600 ticks, radius 4.0, `calmTicks` 300 **+60/level**, scoped to `wildlife_prey` + `wildlife_predator` — all [PLACEHOLDER]. **Verified:** `go build`/`vet`/`test -count=1` green (**14 new Go tests**), frontend typecheck + **47 vitest** + prod build; **sim battery BYTE-IDENTICAL** across all four legs (**TTK 6.67 s / TTD 8.70 s stand**); boot `-content ../api` **0 errors 0 warnings 0 panics — 15 factions/84 skills (83 + Calm)/64 mobs/10 recipes/1 milestone/777 props/485 spawns/5 campfires**. **In-game harness 7/7** (new `chunk2-calm.mjs`, 0 console errors, 0 WebGL losses): an engaged wolf goes **0.59 → 2.88 and holds** while still inside its 5.4 aggro radius, then after expiry the player walks back in and it **closes again, 1.35 → 1.03**; the pip is confirmed by an **in-picture control** — present during the calm, absent on the same species after. ⚑ **Three harness iterations were needed and all three failures were the HARNESS, not the product**, each producing a plausible "calm is broken": after the 20 s camera settle the wolf has usually **already arrived** (so demanding a shrinking distance fails while calm works); the window must fit inside calm's **9.9 s** or it measures expiry; and a calmed mob **walks home past its own 5.4 sensor**, so "it never came back" was "it could not see me". There is also **no single `mob` scene layer — one per species** (`wildlife`, `bossMobs`, …), and the script tags its target by **object identity** so a second wolf cannot fake either result. **⚠ Two things to know:** **calm is unreachable in normal play** (no milestone/drop/teaching authors it — only the `SKILL` cheat), and **L-K did not bite only because a fresh player's aura slots are all Empty** — the first playtester with Damage equipped breaks their own calm instantly, and will read it as a bug. Full ledger `docs/plan-faction-flips.md` §9.
- **Prior: `plan-faction-flips.md` chunk 1 — the allegiance seam** ✅ `ec73634e` 2026-07-28 (backend-only) — **`SetFaction` is GONE**, replaced by three *defined* verbs: `Align()` / `RevertFaction()` / **`EnlistUnder(model.Allegiance)`**, plus a reflection tombstone guarding the general-setter SHAPE from returning under any name. ⭐ Its finding is **L-N**: the summon path has a **mob-caster leg** the design session missed because it is unreachable in content but **pinned by a shipped test** — a plain `Align()` would have raised an orc warlord's squad fighting for the player. The durable lesson: **a faction and its aggro mask are a pair, and every place that takes one without the other is a defect.** Sim battery byte-identical on every *reachable* path. Full ledger `docs/plan-faction-flips.md` §9.
...
- **⭐ NEXT — two independent threads, PO's pick.** ① **step 8 — accounts & persistence (a DESIGN session, not a schema session).** Pre-accounts hygiene is **COMPLETE** and `plan-faction-flips.md` **chunks 1 and 2 are now shipped**, so nothing is standing in front of it. The step-8 detail — ruling 4, backlog §32, the ops/security fold-in — is in the bullet below. ② **`plan-faction-flips.md` chunk 3 — charm. It is the LAST chunk in that plan**, and its own session (D5). ⭐ **It is the seam's real first consumer (D1): chunk 2 did NOT prove it** (§5.1 — calm never flips faction), so `RevertFaction()` still has no caller and `Align()`'s charm direction is still unexercised. **What it needs:** `charmer` + `CreditTo()`, `leader()` + the four `isFollower()` sites, `charmTicks`, `EffectTypeCharm`, the D13 pip, and **two** skills — charm wildlife + charm elementals (**L-L: the second is the acceptance test for D8, not a content detail**; if it needs a Go change the mechanism was hardcoded). **⚑ Four things to read first, all cheap and all load-bearing:** **L-B/L-M** — `Level()` reads the owner's level LIVE since chunk 1b, so reaching for `SetOwner` or letting `leader()` into the stat path shrinks a charmed elite to the charmer's level; **L-O (new, chunk 2)** — skill-level JSON has no `DisallowUnknownFields`, so **charm must be added to `factionScopedEffects`** or a typo'd `targetFactions` silently makes it universal; **chunk 2's D8 seam is already built and reusable as-is** — authored names on the skill, resolved at load, stamped onto the effects, gated once in `eligibleByTargetFlags` (charm authors `targetFactions` and gets the scope for free); and **`skills.Buffs` is the proven carrier for a timed state + its pip** — `calmPayload`/`ApplyCalm`/`DropCalm` is the shape `charmTicks` should copy rather than a `Mob` field. **Still open there:** the XP-credit trace against `PlayerTouches`, whether the pip shows duration, and — new from chunk 2 — **no skill in this plan is reachable in normal play** (calm is `SKILL`-cheat-only; charm will be too unless an unlock is authored).
