...
- **Last completed:** **Farm-band pre-chunk session (post-C8) — DONE 2026-07-21, PO-driven in-session (live playtest + 2 editor passes), committed `1b33be7a` + follow-up commit (turnip XP + wrap); full ledger: `plan-content-zones12.md` §13 farm-band pre-chunk banner.** Pulled the mob-authoring slice forward out of the planned Z2-hardening chunk: **three cL8-17 farm-band normals** — GiantSpider id 45 cL9 (VenomSpit lurker, guardrail-soft) / AlphaWolf id 46 cL10 (wolf-line apex, soft) / Marauder id 47 cL12 (**BanditBlades L3** + 85 baseline = the band's HARD normal; first non-encounter mob-skill level > 1, ProvingBoss precedent) — **XP kph-derived per the Session-⑥ rule: 205/275/175** (derivation re-run reproduces pinned Wolf 70 exactly; stance rule = facetank kph while viable, else kite ×0.5 — Marauder pays less than AlphaWolf because hard-but-kiteable, DireBear/Mammoth precedent). Dropless for now (drop table FINAL — additions are a PO call); spawns PO-placed same session. **Guardrail band-check extended (TDD):** cL8-17 "farm" band joins the Z1/Z2 soft+hard asserts (`cmd/simharness/guardrail_test.go`; farm: soft=AlphaWolf+GiantSpider, hard=Marauder). **Unique-art pass (PO ruling 2026-07-21: every mob its own sprite, higher tier looks meaner):** 5 new portrait SVGs; **DireWolf/DireBear lose their Wolf/Bear `entityType` reskins**; EntityType enum appended **66-70**, full 5-file wire path (proving-grounds reskins left alone on purpose — debug map). **PO editor passes ×2 folded in:** all three farm mobs placed (2/3/1), Troll 4→6, Bandit +3, Spider +4, wildlife thinned, campfires 3→**5**, props 798→**856**, spawns 347→**349**. Balance rulings: **KoboldVolley radius 4.5→2.2** ("halve ranged kobold range"), **turnip XP 0→1** (token pop reward; cheese stays dead, L1→2 costs 300). **Gray-band XP rule DECIDED: keep flat XP for now** (PO 2026-07-21 — live-mob camping explicitly accepted; the farm-band plan chunk no longer owns an open XP-rule decision). **Verified:** suite ×3 + `-race` green, `tsc` clean, prod+dev bundles serve the new classes, boot `78 skills/13 factions/47 mobs/10 recipes/856 props/349 spawns/5 campfires/14 npcs, 0 panics`, headless smoke (both dires render their new sprites in-world). **Watch (bit ×2 this session):** the zone editor drops NPC `entityType` on every moved/re-added NPC (Emberkeeper pass 1, LamplessTraveller pass 2 — hand-restored); the editor Download landed as plain `world.json` in the Windows Downloads folder this time (no space-name stray).
...
- **Next:** **PO priority queue (set 2026-07-21, after the Z2-hardening plan chunk was DISSOLVED** — difficulty/density covered by the farm-band mobs, farm-area/placements PO-manual in editor; dissolution note atop `plan-content-zones12.md` §13): **① Step-7 rebrand + Berryhunter cleanup — plan REFRESHED 2026-07-21, ready to execute** (`plan-rebrand-cleanup.md`; PO decisions locked: legacy separation = `"legacy": true` tag (A.5), skill naming = bare thematic names (A.6), Phase-B naming set decided at execution start; sequencing §6.5 = **A.4 scaffolding prune ✅ 2026-07-21 `93fba97e`** (drops[] chain + frontend equip/craft island + orphan assets; ledger in the plan §4 A.4 banner) → A.5 tag → A.6 renames → B+C atomic rename; **A.2 rating delete CANCELLED — PO 2026-07-21: rating + social links stay, Kringel Games project**; Phase C now explicitly covers the title-screen `<h1>BerryHunter.io</h1>`) → **② combat readability** (triage items 7 category ring colors + 15 tier frame ring; two append-only Mob-table wire bytes) → **③ reconnect-token persistence** (reload loses character; small plan-first chunk, session-scoped — NOT step-8 accounts) → **④ combat-feel SFX, optional** (the open step-6 remnant: hit/ability/mob-death/level-up sounds on the existing `frontend/src/features/audio/` scaffold — sound registry + assets + trigger hooks). PO continues manual placements/corrections in the zone editor in parallel (campfires, density, road corridor, post-intro polish — farm-mob coords are first-pass). Standing authoring rules for any future mob work: **new mobs inherit the Session-⑥ XP rule** (facetank kph while viable, else kite ×0.5), **every C1+ mob MUST be authored tier + baseline** (raw `maxHealth` hard-fails; manual §1), combat mobs need NO harvest entries but **gate obstacles must opt in**, and cL8-17 mobs fall under the "farm" guardrail band-check.
- **Deferred (per triage §Execution sequence):** **Post-C8 "combat readability"** — items **7** (category ring colors) + **15** (tier frame ring), two append-only Mob-table wire bytes. **Post-C8 tooling:** standalone browser map editor (**backlog §22**, seeded from the Session-④ density-pass renderer/generator in that session's scratchpad). **Step-7 rebrand** — items **22** (bare-name renames) + **12** (`legacy:true` tags). **Persistence step** — item **10** sacrifice loop (first consumer; GDD §11 amendment landed Session ①). **Anytime/annoying:** item **21** full `null.split` repro session. Placeholder values still open (none declared FINAL): Troll cL11 / Pyromancer cL6 / Dire cL6-7 tiers, **farm-band trio tiers/baselines** (GiantSpider cL9-90 / AlphaWolf cL10-80-spd0.74 / Marauder cL12-85-BladesL3) + XP 205/275/175, **KoboldVolley radius 2.2** (halved from 4.5, PO 2026-07-21), teacher gates Immolation 6 / Totem 5 / Revive 6, regen-taper floor 0.4, Vanguard shield 4+1/90t, WolfBite radius 1.0, **XP table v1** (rule `3600 × 1.15^(cL−1)` ÷ kph, Session ⑥; turnip → **1 XP** token, farm-band pre-chunk 2026-07-21, L1→2 on wolves; **gray-band: flat XP kept, decision closed**), **crit rework v2** (base `0.05` conf / default critFactor ×2 / KeenEye 2%/level ×5 / DireWolf drop 0.06 / dot +5% comp — backlog §23). Standing locks: growth 1.12 × maxLevel 30 (≈27×, band ≈ +5, lower-first); **regen 0.00033 ≈ 1%/s FINAL × level taper 1.0→0.4** (C8 settlements 2026-07-19 + 2026-07-20); **drop table FINAL + §11 no-pity FINAL + campfire `0.12` FINAL + heal cost `10 −2/level` FINAL** (C8 Session ⑤ 2026-07-20; KeenEye→DireWolf 0.06 added post-FINAL by PO choice 2026-07-20); **milestone table FINAL: Heal L3 / Haste L7** (C8 Session ④ 2026-07-20); **density standard: mob visible on every ⅔-screen (17×9.5 u) window** (C8 Session ④); tier frame: elite ≤ 25% facetank / boss kills the bot / normal = per-mob texture + Z1/Z2/farm(cL8-17) band-check (front exempt); **downtime 10 s** + chain 20; guardrail asserts LANDED (`cmd/simharness/guardrail_test.go`, deterministic). Dev cheats: GOD, WARP <x·120> <y·120>, SPEED [factor|off], XP, SKILL <name>, ANNOUNCE <text>, THREAT [id]. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; boot `-content ../api` for content iteration (schema/Go changes still need the rebuild).
