...
- **Two plans designed 2026-08-12, execution open** (`plan-code-health.md` completed whole + archived 2026-08-14): `plan-zone-editor-structure.md` (C1-C3: `kindOf` structure → controls + the talker round-trip that fixes the uneditable ascension stones → retire the legacy roster) · `plan-play-bot.md` (C0-C3: extract transport → the bot that fights → quests → the soak finding).
...
- **Fast-travel + map tuning, orphaned by the archive** (PO-seen): **marker sizing** — your own dot is invisible at your bound fire, *and* since flight D16 **every arrival ends with the flyer's dot occluded on every observer's map** (`r=3.5 px` under a `9.0 px` marker) - **ruled a non-issue for now (PO 2026-08-14), belongs to the UI-polish pass** · **flight speed 2.8× and viewport 1.2×** are PO-tuned but still [PLACEHOLDER].
