...
- **⭐ Last completed: conversation chunk Q1 — the dialogue system (`plan-conversation-journal.md` §6) ✅ DONE 2026-07-30 `d23670d7`.** All five parts of R1's reshape, test-first, **no wire change** (the `reply` field simply rides empty on locked rows). ① **`blockedLine` is GONE at every layer** — struct field deleted, the JSON key a tombstone naming its replacement (the `trigger` precedent; twelve authored lines across nine NPCs deleted), the *"required when gated"* loader rule retired, and a locked row is now **inert at both ends**: empty `Reply` from the presenter, silent refusal in `applyTeach`, no click handler + a `take()` guard in the client — the greying and the named wall are the whole answer, and the both-ends sweep pins that the two ends cannot disagree. ② ⭐ **A quest row is shown iff its ledger op would succeed** — `Ledger.CanApply(grant)`, built from `canAccept`/`canAdvanceDialogue` **extracted from** `Accept`/`AdvanceDialogue` so the mutating ops and the presenter run one judgement (L3; a `CanApply` written *beside* them would be N1 again). An Accept row vanishes the moment the quest runs while its sibling questions stay askable; a turn-in appears exactly when its edge is walkable. ⚑ The predicates deliberately avoid `progressOf()`, which **creates the map entry it looks up** — a predicate built on it would grow the ledger per tick just by being asked (pinned by `TestLedger_CanApplyMutatesNothing`). ③ **Combat no longer blocks talking** — all three gates deleted; range, death, disconnect and despawn are the only end conditions. The entity-model R2/D21 tests are **INVERTED, not deleted** (`TestSession_SurvivesCombat`, `TestInteractionSystem_CombatDoesNotWithdrawTheOffer`) — still the only eyes on the offer path, since no cheat can stamp player combat. ④ The synthetic **"Leave." row** at root, client-only (`view.showLeave` keeps the DOM layer logic-free; its handler is `leave()`, never `take()`). ⑤ Scrollbar padding on both panel bodies. **Verified:** full suite + vet · guardrails/alloc `-count=2` · 77/77 vitest + typecheck + prod build · boot both ways identical (86 skills/15 factions/64 mobs/10 recipes/1 milestone/**4 quests**/777 props/485 spawns/5 campfires, 0 errors 0 warnings) · harness each SOLO: `chunk3b-ii-conversation` **31/31** (rewritten *with* the chunk — inert-row leg, Leave legs, by-name root assertions, the permanent combat SKIP **deleted**), `chunk3b-interact` **14/14**, `chunkC4-quests` **28/28 + 1 deliberate SKIP**, `chunkC3-journal` **17/17** incl. probe-quest legs. ⚑ Two false alarms on record in the ledger (§29 ctx loss froze the Wanderer's drift; one missed abandon click) — both green on fresh-server re-runs. **Knock-on for Q4:** C4's `quest_at_stage` gates on offer nodes are now redundant-but-harmless; Q4 decides per conversant whether the greeting still changes or the row alone vanishes.
...
- **⭐ NEXT — `plan-conversation-journal.md` Q2, then Q3 → Q4.** The PO's call between the conversation pass and step 8 fell to the conversation pass, and **Q1 shipped 2026-07-30 `d23670d7`** (see Last completed). What remains: **Q2** — server-composed objective tracking (*"3/8 Wolves slain"* on `QuestProgress.objectives`, appended LAST — `server.fbs` is positional with no field ids — with the authored `tracker` override and the §4.4 no-per-tick-allocation cache; ⚑ the client reads this table by **hand-written unmarshal**, so `GameStateMessage.ts` and `JournalModel.ts` need the same edit) · **Q3** — the two-pane journal (selection is client state over a per-tick re-send; the sizing invariant must not mint a third hand-copy of the HUD strip's geometry) · **Q4** — the content restructure to R1's shape (nine flat NPCs get real trees, prose ≥50 % shorter, R3's lamp rewrite **plus deleting the Lantern drop from both kobolds** — making the quest the aura's only source, L5b — and Damage at level 1 via a creation-time milestone call). Sequencing per §3: Q2 before Q4 because a stage's `tracker` override is content Q4 writes; Q3 can move. **After the plan closes: step 8, accounts & persistence** (a DESIGN session, not a schema session) — detail in the bullet below. ⚑ The scoped-filler queue is EMPTY.
