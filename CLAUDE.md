...
- **Last completed:** **Step-7 A.5 legacy tag — DONE + verified 2026-07-21, committed `d1acf28d`; full ledger: `plan-rebrand-cleanup.md` §4 A.5 banner.** `"legacy": true` schema field on all three registries (skills/mobs tolerant decoders, factions strict — field declared) + `Zone.Legacy`. **Set correction — the item-12 audit list was stale:** Sessions ⑤–⑦ made all 5 audit-listed player skills world-reachable (SlowAura→BanditRanged / ToughPassive→Troll / WildAura→DireWolf / ReaperAura→EliteWolf drops; Revive→world-NPC teaching), and HealerAura is live via MedicCompanion (FieldMedics recipe). Actually tagged: **10 mobs / 0 player skills / 5 mob skills** (MammothAura, AngryMammothAura, AngryMammothStomp, SaberToothCatAura, DodoAura) **/ 3 factions** (predator/prey/tusker) **+ `proving-grounds.json` itself** tagged as the legacy zone (its expected-shape warnings suppressed) = 19 JSON files. **Leak warnings** (aggregated `slog.Warn` in `loaders.go`): live zone → legacy spawn/teaching refs (`Zone.LegacyRefs`); live mob → legacy skill/unlock/faction refs (`MobDefinition.LegacyRefs` — beyond the plan's letter, but the only enforcement for the faction/mob-skill tags). **Real-content pin** `TestDiskContent_LegacyTagging` (exact tagged sets, zero leaks on every live mob, world zone legacy-free — guards the zone editor's known field-dropping habit). Registry pins unchanged. **Verified:** suite `-count=1` green + `-race` on the 5 touched packages, `make -C backend build` (cp-defs), boot world zone **0 warnings** counts unchanged (`78 skills/13 factions/47 mobs/10 recipes/856 props/349 spawns/5 campfires/14 npcs, 0 panics`), boot proving-grounds 0 warnings, negative test (zone tag removed → WARN lists all 7 legacy spawn mobs) then restored; no client surface (no TS, no wire).
