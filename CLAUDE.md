...
- **Deferred tech debt / known bugs:**
  ...
  - **Dead character-variant code — remove.** `Graphics.ts` lists a single `player.svg`, so `Character.variants`/`pickVariant()`/the `hashCode(name)` selection are a no-op (`% 1`), and the ~13 `assets/characters/*.svg` are unreferenced. Old Berryhunter system, no longer used — all players render one knight portrait. Remove when building the new avatar selector (roadmap item 8).
...
- **Doc map (`docs/`, naming: core docs unprefixed; `plan-` = execution plan/record per work item; `research-` = point-in-time investigation/assessment; `archive-` = resolved, kept for rationale):**
  ...
  - docs/research-scripting-audit.md + docs/research-scripting-options.md — data-vs-Go audit and scripting/expression-layer options (decision open)
