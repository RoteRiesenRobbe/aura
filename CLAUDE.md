...
- **Last completed:** **Night-readability fix (ad-hoc bug session) — DONE + PO-verified in-game (full night played) 2026-07-21, committed `6afbee84`; full ledger: `plan-intermission-triage.md` §Night-readability fix.** PO report "damage aura active + night → character invisible" root-caused: **not an engine bug** — the DayCycle night tint (ColorMatrixFilter, multiplicative flood at 0.9) was applied via a stale hand-listed layer set predating the content pass, so the `characters` layer went near-black while every content-pass mob layer (wildlife/healer/companion/campfires/…) silently skipped the filter and stayed bright; the aura correlation was a day/night confound. **Fix (frontend-only, no wire):** night-filter list now **derived** (all layers minus exempt set: campfire/brazier light sources, darkness overlay §6.5, readability overlays) so new layers are night-correct by default; `FLOOD_OPACITY` 0.9→**0.6 [PLACEHOLDER]**; name/level/HP-shield bar moved to a world-space plate in the new unfiltered **`namePlates`** overlay (chat-bubble follow pattern, released in `hide()`); **mob layers moved under `characters`** (campfire art no longer covers a standing player). Side evidence: `null.split` (triage item 21) fired first-run in a fresh headless profile and froze the canvas — noted in §21. **Verified:** `tsc` + webpack prod clean, headless night battery (avatar readable on open ground + above campfire art, plate follows WARP, campfire glow intact, mobs/NPCs uniformly tinted), boot counts unchanged (`78 skills/13 factions/47 mobs/10 recipes/856 props/349 spawns/5 campfires/14 npcs, 0 panics`), PO played a full night in-game.
