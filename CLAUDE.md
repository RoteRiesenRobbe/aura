...
- **Last completed: pre-accounts hygiene, SESSION 1 — H2 → H1b/H1c → H3 → H4** (2026-07-28, backend + frontend + wire + config + docs, 25 files) ✅ `c183ce12` — the four parts that were supposed to change nothing, run together so that *"everything else is byte-identical"* stayed a **live check**. ⭐ **It held: the sim battery came out BYTE-IDENTICAL** across all four runs (default · `-chain` · `-levels` · `-content ../api` roster), diffed against a baseline taken before the first edit. **H1a is untouched — that is session 2 — so the standing baselines (TTK 6.67 s / TTD 8.70 s) still stand.** **H2 — the L12 collision guard:** a def carrying an `interaction` block must now author `body.collisionLayer`, because an omitted one is `0` and `mob.go:150` substitutes `Viewport|Action` — so the *default* for a conversant was aura-targetable and killable, under a content pin that read the same authored `0` and passed trivially. ⚑ **The guard constrains AUTHORING, not policy** — "a conversant must be unattackable" was rejected because it hard-blocks the teaching-guard-that-fights-bandits, the entity model's own proof that role and capabilities are orthogonal; the value stays entirely the author's, only *"unset"* stops being a way to say it. Content impact **zero** (all 14 conversants already author `97`). **H1b/H1c — one truth per config:** the embedded `cmd/aurad/conf.default.json` (the file a **fresh server actually runs on**) hand-synced — 7 dead keys out, `zone`/`baseHealth`/`skillPointsPerLevel`/`critChance`/day-cycle + the whole `mob` and `combat` blocks in — pinned by a new **map-comparison drift test** that excludes `server` on purpose (`frontendDir` vs `path` is the one real per-environment delta) and was red-first, enumerating the drift exactly; `heatFractionPerSecond` deleted everywhere. ⚑ **A fresh server was never actually broken by it**, checked not assumed: `gameconf.go:38` normalizes `BaseHealth` to 100 the same way `:48` normalizes the chase margin to 0.2, so the embedded file resolved to Go defaults rather than to wrong values — this is §35 tier 1's *tidiness* half, and H1a is the defect half. ⚑ **Deviation, widened:** the plan's "four tracked conf files" missed **`devops/conf.json`** — tracked, the LIVE server's, and carrying the dead key; cleaned, **and it is also drifted** (no `mob`/`combat` block ⇒ the live server runs on Go fallbacks for both), recorded rather than fixed. **H3:** both bare `33`s → `Constants.SERVER_TICKRATE` (the codebase had documented that rounded 33 as a past bug while still using it twice) + dead `damageAuraRadiusMeters`. **H4 — the wire prune:** `Vec3f` gone; `Resource.capacity`/`stock` gone and **`aabb` renumbered**, then every consumer in the plan's order — codec write, decode, `EntityManager`, `Resources.ts` (`baseScale`, the stock machinery, **both** empty `House`/`GateWall` overrides that existed only to dodge the rescale), `ResourceStockChangedEvent`, `ResourceJuice.ts` + its two mp3s. ⚑ **The renumber is pinned, not argued:** a new codec test reads **every** field a prop writes back off the wire including the moved `aabb`, because a mid-table renumber decodes as *garbage*, not as an error. **Verified:** `go build`/`vet`/`test ./...` clean, guardrails + alloc `-count=2`, frontend typecheck + **44 vitest** + prod build; boot `-content ../api` **0 errors 0 warnings 0 panics — 83 skills/15 factions/64 mobs/777 props/485 spawns/5 campfires**, and a **fresh-directory boot with no `conf.json`** writes one and reports the same `🎚️ tuning knobs` line. **Join smoke** (new `hygiene-wire-prune.mjs` — props are the only thing left on the Resource wire path, so "props render" *is* the renumber's assertion): 634 sprites, House props still at their authored non-square aspect, **0 console errors / 0 context losses**; **L-H6** `ctxloss-warning.mjs` PASS both ways with `clean` still **0** warnings at 5 contexts / 2 probe losses, so dropping two `registerPreload` calls did not move the boot GL profile. ⚑ **2 harness notes:** the first smoke run hit the **§29 lost-context flake** — an *invalid run*, not a failure, self-reported by name by the banner §29 shipped for exactly this; and the first `go generate` after a schema **deletion** prints `open …/Vec3f.go: no such file` — `go generate` walking a list the generator deletes mid-run, clean on a second run, nothing to fix. Full ledger `docs/plan-pre-accounts-hygiene.md` §11.
