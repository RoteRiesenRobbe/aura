...
- **No plan in flight** - `plan-effect-types.md` COMPLETE + archived 2026-08-17 (all four chunks, schema NONE throughout). Queued, designed and untouched: **`plan-portal-spells.md`** (designed 2026-08-17, C1 Open Portal + C2 Pull Through - was "queued after C4", so it is next in line) · `plan-npc-hails.md` (planned 2026-08-17, conditional name-personalized walk-by greetings, one chunk, schema NONE) · `plan-play-bot.md` (C0-C3) · `plan-prototype-projectile.md` (designed 2026-08-15). ⚑ C4 leftover, unowned: `TICKING_TYPES` in `SkillTooltip.ts` is a hand-maintained per-type set with no completeness pin and a SILENT failure mode (a new ticking type just loses its cadence line). ⚑ Zone-editor C3 leftovers, owned by nobody: the Go legacy plumbing is now dead code (Legacy/LegacyRefs fields + the unreachable loaders warning) · `wiki-generator/` is broken at HEAD independently of C3 (requires the §28-deleted `client-data/Items`) · ⭐ **C3 missed the `backend/pkg/api/mobs/` mirror** - all 10 deleted legacy mob JSONs still there naming `EntityType` values C3 removed, so an **embedded** boot may hard-fail (found 2026-08-17; `-content ../api` sidesteps it). ⚑ NOT reproducible at the 2026-08-17 C4 merge: git tracks no mob JSONs under that dir (gitignored) and the local mirror matches `api/mobs/` 1:1 after cp-defs - re-verify on the finder's checkout before acting.
