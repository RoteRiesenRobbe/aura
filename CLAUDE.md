...
- **⭐ Last completed: UI PASS C8 - TOOLTIP MAINTENANCE DEBT** ✅ 2026-09-02, `[uncommitted]` (ledger: `docs/plan-ui-pass.md` §6 C8; spec + rulings §5 C8, D1-D4): the three §2 debt shapes closed - ⭐ **the survey found NO live gap** (34 `case`s == 34 types, all 6 stats labelled, both gate keys, `TICKING_TYPES` == the 8 chargeable types), so shapes 2/3 got **PINS**: four vocabulary lists in `api/shared-constants.json`, Go twin in `pkg/aura/skills` (unexported maps, the accounts-codes precedent) + vitest twin incl. a per-type RENDER SWEEP and two PARTITION pins (`EFFECT_COLOR_KEYS` ∪ 13 named neutrals; `SELECTOR_LABELS` ∪ `all`); shape 1 = **D1** a served per-skill `description` (prose-only, `omitempty`, ONE field shared with C11) rendered **D2** italic under the subtitle before the numbers - the seven hardcoded ruling sentences left `effectBlock()` and the 10 affected skills author them. ⭐ **Durable: re-derive a debt note's counts from disk before ruling on it** (the §2 numbers had drifted; the drift was the evidence). ⚑ A render sweep over a server-fed switch needs payload-COMPLETE fixtures (bare `{type}` throws before `default:`). **Schema NONE** (HTTP catalog JSON only). Verified: vitest 580/580 (+9) · tsc · prod build · `go test -count=1` 34 pkgs ok (3 Martin-census reds = HEAD baseline) · `GET /skills` serves the 10 · `round4-tooltip` green incl. its NEW C8 leg (Frost Shield: the block is the third child, text read off the catalog) · `r3-lifesteal-burst` 7/7 · `c2-frost-shield` 7/7 · `c3-paralyze` 6/6 · probe: `.tooltipDescription` third child on Calm/Frost Shield, absent on Rejuvenation. Two Opus agents in parallel, reviewed line-by-line; ⭐ PO played 2026-09-02, ZERO change requests (two unrelated bug reports → `docs/feedback.md`).
...
- **Feedback flow + the UI pass:** new feedback lands ONLY in **`docs/feedback.md`** (four exit doors; plan docs never double as intake). **All UI work is consolidated in `docs/plan-ui-pass.md`**: direction C "Inked Panel" RATIFIED (⚑ the §4 CORRECTION block - the RENDERED mockup CSS - is the spec); §5 order C1-C11 RATIFIED; **C1-C8 SHIPPED** (2026-08-26 – 09-02), all PO-approved in-game; **next: C9 (mobile)**. ⚑ Two open intake rows in `docs/feedback.md` (2026-09-02: orphaned teaching-row tooltip · the shut book's unlock breadcrumb reads as no VFX) await PO rulings. ⏸ `plan-onboarding-cleanup.md` DEFERRED, trigger: a human coworker is actually about to join.
...
- **Known-inconclusive at HEAD**, all unowned: ⭐ **3 census tests in `pkg/aura/items/mobs` RED** - the collaborator's Martin NPC (`6c2e6d5c`) never bumped the hardcoded counts (hidden until C4's cp-defs synced the gitignored copy). Fix = bump 3 counts or hand it over · `chunk3-charm` 6–8/9 (D9 fragility) · `filler-batch` leg 1 (stale assert) · `chunk3b-ii-conversation` 28/34 (stale TownCrier assert + Leave-click race + Wanderer drift-pin) · `accounts.TestRepeatedFailuresAreThrottled` under `-race` ONLY · **`AuraTiledConvert.test.ts` byte-stability ×2** on a fresh checkout - they pin the PO's UNCOMMITTED world.json repair. ⚑ **Measure the rate before diagnosing a flake** (a misread cost `chunk3b-interact` two chunks); ⚑ **this host's wall clock is non-monotonic** - suspect it before any elapsed-time red.
