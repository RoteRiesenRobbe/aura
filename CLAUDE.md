...
- **⭐ Last completed: UI PASS C3 - THE SPELLBOOK STRUCTURAL REWORK** ✅ 2026-08-28, `9cc6cd74` (ledger: `docs/plan-ui-pass.md` §6 C3; spec + rulings §5 C3, PO choice prompts 2026-08-27): the book stops being always-on - ⭐ **D1 IN-PLACE TOGGLE** (B / open buttons / Escape; `'spellbook'` joined the C2 family + `PanelId` union, desktop equip flow untouched) · **D2 category tabs + pages within** (placeholder 8/page; page counts from DISCOVERED entries only, zero-hint kept) · **D3 utility category DEFERRED** with the tag axis · **D4 mobile full-screen book** - the ☰ sheet keeps only a Spellbook row; picking a PASSIVE closes the book and OPENS the sheet its slots live in (PO-confirmed) · **D5 hotkey B**. ⚑ **The harness-compat rule is LOAD-BEARING: the panel DOM stays RENDERED** - open/close + tab/page filtering toggle classes (`hidden`/`offPage`), nodes never removed; evaluate queries survive a shut book, boundingBox access opens it via new `lib/spellbook.mjs`, 27 verify scripts retrofitted through it. ⭐ Badge in BOTH places (PO look): open buttons AND panel title, one update path feeds every `.skillPointsBadge`. ⭐ The new legs caught 2 real defects: `#bottomCenter`'s dead strip ate pager clicks (new `@z-left-column`) · `.offPage` loses a same-specificity source-order tie unless it sits AFTER `.sectionHeader` - don't reorder. SimpleBar kept as overflow backstop only. **Schema NONE.** Verified: vitest 538/538 · tsc · prod build · new `c3-spellbook` 26/26 · **full 32-script sweep, every red settled** (charm 6/9 + conversation 28/34 baseline-identical; `r7-respec-cost`/`c0-honest-plate`/`chunkC4-quests` fail IDENTICALLY at HEAD via stash-and-rerun; `mobile-layout`'s red = the documented nag cover, screenshot-confirmed). Built by an Opus 5 agent, reviewed line-by-line; ⭐ PO clicked through 2026-08-28, the one wish (the badge) applied + re-verified same session.
...
- **⭐ REGION PRIMITIVE: C1-C5 ALL SHIPPED** (C5 `4937a977`, ledger §12; audio consumers unscheduled, `plan-region-audio.md`). ⏸ **`docs/plan-region-primitive.md` stays live for the OPEN look sitting**: texture picks, the 0.35 scale, the seam, blend width (placeholder 1.5) + mask density; `Forest`/`Ice` author no tile; D18's `tint` fork stays available. ⚑ The standing C4/C5 landmines (ONE `paintRegions` · ⛔ SVG tiles / `GroundTextureTypes` / `registerGameObjectSVG` · mask RTs per-pass and OWNED · `blend: 0` VALID) live in the plan doc + [[project-region-primitive]]. ⚑ At HEAD no interior seam exists - the band is near-invisible until a second region is drawn, and `c4-region-texture` leg 5 is INCONCLUSIVE by construction until then. Flatten-once follow-up: `c5-mask-cost.mjs`, worst case 1.07-1.09x. Schema NONE throughout.
