...
- **Last completed: `plan-faction-flips.md` chunk 1 — the allegiance seam** (2026-07-28, backend-only, 9 files) ✅ `ec73634e` — **`SetFaction` is GONE.** One general-looking setter with exactly one *defined* destination became **three named verbs**: `Align()` (the player side, mask `^Bit(Aligned)` — bit-for-bit today's behaviour), `RevertFaction()` (the species' authored faction + mask, off `m.definition`; no consumer until chunk 3) and **`EnlistUnder(model.Allegiance)`** (the summoner's side). ⭐ **The third verb is a CORRECTION to the plan, and it is the session's finding.** §4.1 claimed the summon path's caster is always a player — it is not: `processCooldowns` fires **mob** cooldowns, and `TestCooldown_MobCastSpawnHasNoOwner` is a **shipped test** asserting *"summon adopts the casting mob's faction"*, so a plain `Align()` turned it red and would have raised an orc warlord's squad **fighting for the player**. It is unreachable in content (no mob among the 64 defs equips any of the 6 spawn skills — checked, not assumed), which is exactly why the design session traced clean and missed it (**L-N**). ⚑ **`EnlistUnder` is therefore the one behaviour change in an otherwise behaviour-neutral chunk**: the old code adopted the caster's faction and *invented* `^casterBit`, i.e. **L2 in miniature** — an orc squad would have hunted every neutral it walked past. The new `model.Allegiance` capability (`Faction()` + `AggroMask()`) exists to make the pair **inseparable**; splitting it *is* the defect. **Also shipped:** `definitionAllegiance()` shared by `NewMob` and `RevertFaction` — ⚑ not cosmetic, because `NewMob` **rewrites** a zero-value (=aligned) definition faction to hostile, so reverting off the raw definition fields would land a reverted mob permanently player-aligned; `resetAggro()` on **every** edge (L-A — inert for both current callers, load-bearing for charm in both directions); `factions.go`'s built-in `aligned` entry made honest (`AggroMask: ^Bit(Aligned)` instead of an implicit `0` that read as *retaliation-only* — L-J re-checked, nothing walks `registry.All()` acting on masks); a **reflection tombstone** failing on `SetFaction` *or* `SetAggroMask` reappearing on `*Mob` (the trap is the SHAPE returning, under any name); 8 stale comments. **Considered and NOT taken:** a content pin forbidding mobs from equipping spawn skills — written, then deleted; once the behaviour is *defined*, constraining authoring is the wrong tool. **Verified:** `go build`/`vet`/`test -timeout 180s ./...` green, guardrails + alloc `-count=2` green, **sim battery BYTE-IDENTICAL** against a pre-change worktree across all four legs (default · `-chain` · `-levels` · `-content ../api`), boot `-content ../api` **0 errors 0 warnings 0 panics — 83 skills/15 factions/64 mobs/10 recipes/1 milestone/777 props/485 spawns/5 campfires**. No frontend surface, no wire change, no content change. ⚑ **Byte-identical on every *reachable* path, not literally every path** — the mob-caster summon leg changed and is invisible to every instrument the project has, because no content reaches it. Say so rather than claiming a clean sweep. Full ledger `docs/plan-faction-flips.md` §9.
...
- **⭐ NEXT — two independent threads, PO's pick.** ① **step 8 — accounts & persistence (a DESIGN session, not a schema session).** Pre-accounts hygiene is **COMPLETE** and `plan-faction-flips.md` **chunk 1 is now shipped**, so nothing is standing in front of it. The step-8 detail — ruling 4, backlog §32, the ops/security fold-in — is in the bullet below. ② **`plan-faction-flips.md` chunks 2 and 3, each its own session (D5).** **Chunk 2 = calm** — the cheapest real feature in the plan: it **never flips faction**, so it needs none of chunk 1's seam (`calmTicks` mirroring `ttlTicks`, a branch in `updateAggro` ahead of the pacifist case, break on any `tookDamage`, `EffectTypeCalm`) — **and it is where D8's per-skill faction-allowlist seam gets built**, authored names resolved to a mask at content load, then reused twice by chunk 3. ⚑ **Calm does NOT prove chunk 1's seam** (§5.1) and ⚑ **L-K: a calmed mob standing in the calmer's own damage aura breaks the calm next tick** — ruled as intended, so a harness run that casts calm and stands still will report it broken; write the test to move. **Chunk 3 = charm**, the seam's real first consumer (D1) and the only one with a verification tail: `charmer` + `CreditTo()`, `leader()` + the four `isFollower()` sites, `charmTicks`, `EffectTypeCharm`, the D13 pip, and **two** skills (charm wildlife + charm elementals — **L-L: the second one is the acceptance test for D8, not a content detail**; if it needs a Go change the mechanism was hardcoded). ⚑ **Read L-B/L-M before touching it** — `Level()` reads the owner's level LIVE since chunk 1b, so reaching for `SetOwner` or letting `leader()` into the stat path shrinks a charmed elite to the charmer's level. **Still open there:** calm's numbers, whether calm applies to a mob already attacking you, the XP-credit trace against `PlayerTouches`, and whether the pip shows duration (§8).
