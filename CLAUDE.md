...
- **Last completed:** **playtest round-4 chunk — ability tooltips now scale with character level DONE** (2026-07-26, full ledger in `docs/plan-playtest-feedback.md` §Round-4 chunk ledger) ✅ `[uncommitted]` — **⏳ PO TEST PENDING** (headless-verified only; 4-point acceptance checklist at the top of the ledger). 14 files. `SkillTooltip.ts` modelled the *skill*-level axis but never the character power curve, so Rejuvenation read `4 → 6` at character level 1 **and** at 30, where it actually ticks ≈107. **Backend:** `/skills` serves `{curve, skills}` (breaking; our client is the only consumer, both halves in one commit). The planned ⚑ DRY watch resolved by **extraction, not a third copy** — `curve.Curve{Growth: …, MaxLevel: …}` already existed twice, so `cfg.Config.LevelCurve()` is now THE construction point and both sites read it; the chunk *removed* a duplication. **Frontend:** `powerScaleAt(level)` mirrors `curve.F` including both degenerate cases (growth ≤ 0 neutral, level < 1 clamps); fetch failure → scale 1 = exactly the pre-fix rendering. `prog()` gained a trailing `scale` arg defaulting to neutral so "does this line ride f(L)?" stays visible **at each call site** — the client mirror of `casterPowerScale`'s HP-values-only boundary. Applied to seven lines, nothing else; the two `of max HP` branches carry inline comments saying why they stay curve-free. **⚑ Rounding:** new `hpFmt` mirrors `vitals.HP` (half up, min 1) — otherwise a 107 HP tick renders `106.99`. **⚠ 1 PO-visible behaviour change, not in the plan:** HP lines now read as **whole points even at character level 1** (an authored `6.3` shield reads `Shield: 6 HP`) — that is what the server grants and what the PO's "show what the player actually sees" ruling implies, but it is visible at low level. Checked: every authored base HP ≥ 4, every per-level step ≥ 1.2, so the `→ next` arrow still moves everywhere. **⭐ First JS test infra in the repo** (PO call 2026-07-26): `vitest` + `jsdom`, `vitest.config.ts`/`vitest.setup.ts`/`npm test`. Three landmines — **jsdom not node** (the module graph reaches `window` at import time: `Urls.ts`, PixiJS), **`vitest.setup.ts` stubs `fetch`** (`Skills.ts`/`Mobs.ts` fetch on *import* ⇒ real DNS otherwise), and **`skipLibCheck: true`** in `tsconfig.json` (vitest's own `.d.ts` uses private identifiers that tsc reports against the app's `es5` target; shipped bundle target unchanged). TDD: backend red→green first; 6 frontend tests written red, 3 genuinely red on behaviour — the load-bearing one is *"every non-HP line byte-identical across the scale"*, since over-applying the scale is the failure that looks like a fix. Drive-by per the plan: stale Rejuvenation drop `.1` → the authored `0.25` in `orc-warlord.json`'s `_comment` + `content-skill-inventory.md` (`content-auras.md` never carried the number). Verified: `go build`/`vet` clean, `go test ./...` **exit 0, 27 pkgs**, guardrails replay clean `-count=2`, `npm test` 6/6, `npm run typecheck` + `npm run build` clean, `make -C backend build` re-run **after** the JSON edit (§26 Chunk-2 lesson), boot `-content ../api` **0 errors 0 panics** (83 skills/14 factions/50 mobs/10 recipes/5 prop defs/1 milestone/777 props/471 spawns/5 campfires/14 npcs), headless in-game via the new **`.claude/skills/verify/round4-tooltip.mjs`** (kept as a repeatable check, PO call) — `4 → 6` at level 1, `107 → 160` at 30, radius byte-identical. **Harness notes:** `#gameUI`'s class is **not** `active` in this build (wait on `window.game?.character`), and `Character` has no `level` field — it lives only on `character.levelElement.text`.
...
- **Next: PO in-game verification of BOTH pending chunks in one session** — rounds 3 and 4 are both ⏳ PO-test-pending and independent of each other; each has a numbered acceptance checklist at the top of its ledger in `docs/plan-playtest-feedback.md`. **Round 3:** beat on a lone healer and confirm it now dies; disengage and confirm it still heals back to full; judge whether `RallyDrummer` no longer chasing players reads correctly; summon `FieldMedics`/`HoldTheLine` and confirm they support instead of chasing. **Round 4:** hover an ability at character level 1, `XP 99999999` to 30, hover again — Rejuvenation must go `4 → 6` ⇒ `107 → 160`; confirm the number matches the heal actually landing; confirm radius/crit/variance did NOT move; and sanity-check that the new whole-point rounding reads acceptably. **After that: step 8 (see below).** PO decisions on record: round-3 2026-07-25 — no universal auto-attack (parked in §31), support set = Heal+Shield, pacifist healers ignore their attacker, survivors-like fork **CLOSED — stays MMO-lite**; round-4 2026-07-25 — tooltips show **absolute numbers**, and 2026-07-26 — add a real JS test runner rather than skip the frontend guard.
