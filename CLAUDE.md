...
- **⭐ ASCENSION SITES is COMPLETE and archived, but the PO has not walked it yet** (`docs/archive/plan-ascension-sites.md` §9) - every chunk is harness-verified and nobody has stood at both stones by hand. What to look at: the village stone's rewards arrive in its **authored** order, each stone's price reads itself on the fallback row, and the front stone's own three-reward list is **unreachable in play** without five real orc kills (C1's recorded gap, PO-ruled to leave).
