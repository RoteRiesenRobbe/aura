...
- **⭐ NEXT: the pre-accounts hygiene chunk — `docs/plan-pre-accounts-hygiene.md`** (planned 2026-07-28, docs only ✅ `ec0732e9`; **4 PO decisions D1–D4 · 6 landmines L-H1–L-H6**). The last thing before step 8's design session. **Why these four and not the rest of the backlog: step 8 makes each of them more expensive, not less** — a fresh server writes `cmd/aurad/conf.default.json` as its `conf.json` and that file is the most stale of the five; the balancing harness is the instrument every content number comes out of; wire fields are positional, so deleting dead ones is free only while nothing durable is written in wire vocabulary; and the L12 pin is a tautology at exactly the point more content starts being authored. **⭐ RUN IT AS TWO SESSIONS, split at H1a** (§9.1): **session 1 = H2 → H1b/H1c → H3 → H4**, which share *one* acceptance criterion (byte-identical battery, boot counts unchanged) so a single battery run proves all four; **session 2 = H1a alone**, because it is the only part that moves a number, the only one needing a sign-off on a delta, and the only one that re-baselines the battery. The split keeps *"everything else is byte-identical"* a **live check** rather than an after-the-fact claim, and costs no extra baseline run (session 2 takes session 1's output as its baseline). **⚑ Ordering consequence: H1a is the final chunk before step 8 and it SUPERSEDES the standing baselines (TTK 6.67 s / TTD 8.70 s)** — the new numbers must land in the plan's §11 *and* here in the same session, or the next chunk's green byte-identity run is a false negative. **3 findings that shaped it:** ① **H1a is not a stale comment** — two Go defaults hold one value and disagree, `gameconf.go:48` normalizes to **0.2** (the live path: all five non-test `NewMob` callers pass the normalized config value) while `mob.go:305` falls back to **0.05**, so the fallback never fires in the running game and the harness copied a *test-only* number; since `stopDistance = auraR + targetR − margin`, the harness has been modelling mobs that stop **0.15 units short** of where they really stop. ⚑ That fallback is reached by **134 of 161 `NewMob` call sites**, all tests, so D1 runs as two separately-verified steps. ② **the obvious H2 guard would break the orthogonality case** — "a conversant must be unattackable" hard-blocks the teaching-guard-that-fights-bandits that `plan-entity-model.md` names as its proof that role and capabilities are orthogonal, so the guard constrains *authoring* instead (an `interaction` block requires an explicit `collisionLayer`, killing the "0 means unset" path that `mob.go:150` fills with `Viewport|Action`); all 14 conversants already author `97`, so it is content-change-free. ③ **`cfg.ReadConfig` has no `DisallowUnknownFields`** — that is why 7 dead keys survived in the embedded config; adding it would hard-fail every existing local *and deployed* `conf.json` on the next start, so it is recorded as needing a deploy plan rather than folded in, and the drift test pins the **`game` block only** (the two default configs legitimately differ in `server`: `frontendDir` vs `path`). **The 4 rulings:** ① harness follows live (0.2), accepting a one-time signed-off re-baseline · ② hand-sync the embedded config + a `game`-block drift test (map comparison, not a `cfg.Config` round-trip, which would drop exactly the dead keys the test exists to catch) · ③ delete `Vec3f` + `Resource.capacity`/`stock` and accept the `aabb` renumbering, leaving `Character.is_hit`/`active_skill_id` · ④ delete `ResourceJuice.ts` and both of its mp3s. Full ledger `docs/plan-pre-accounts-hygiene.md`.
