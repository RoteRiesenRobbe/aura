...
- **⭐ NEXT: AURA DRAWBACKS + the PLAYER SLOW DOOR, a PLANNING session** (PO 2026-09-25, `docs/feedback.md` row 2026-09-25, exit door → plan; the doc `plan-aura-drawbacks.md` does NOT exist yet): auras must be able to carry drawbacks that offset a higher power level (self slow, more damage taken, and the like) through a **while-active self modifier**, and mobs must be able to slow players through cooldowns or auras (the player slow door). ⚑ Measured at HEAD: the self-cost is the ONLY designed drawback; a self-vulnerability via `resist_aura` + `targetsSelf` + factor > 1 loads by accident; every slow on a player is inert by design. No production code in the planning session.
...
- **⭐ SKILL VFX owes the PO look at C3b + C3c, then the PHONE CHECK** (`docs/plan-skill-vfx.md` §13 C3b + C3c, both built 2026-09-25; ⚑ the gallery shows a 200 ms strike a fifth of its cycle, a look question): THREE decisions ride it: cap 96 vs 192, fill rate, the packer trigger (§12f.2). ⚑ The pyromancer's new `damage_aura` roughly doubled its output, unmeasured. ⚑ The harness census misses snapshots headless (C3a-ii ledger Findings).
