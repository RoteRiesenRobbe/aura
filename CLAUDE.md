...
- **Prior: SERVER PERFORMANCE CHUNK 3 — send-on-change owner-only state** ✅ 2026-09-06 `68946f78` (ledger: `docs/plan-server-performance.md` §Chunk 3): the spellbook block (slots, skill points, cost/damage factor, `quest_progress`) and the conversation TREE ride the wire **only when they change**, plus a ~5 s heartbeat. ⭐ Measured on the real encoder, 150-tick steady state: **384 → 233 B/tick/player (39.3 %)**; **672 → 275 B (59.1 %)** with a dialogue open. A mutation still lands the SAME tick (D1). ⭐ **No new dirty flags** — `SkillComponent.Revision()`/`Ledger` already counted this for `persist.go`'s forced saves, so the gate is that compare moved to the wire; **L2 fell out free** (reconnect mints a fresh client UUID → full send). ⚑ **`SkipOwnerState`/`SkipConversationTree` default FALSE = send everything.** ⭐ **A review then caught FOUR real defects, and the root of three is the lesson: a change-only field needs an EXPLICIT presence bit — every in-band inference collides with a legitimate value** (`spellbookLength() > 0` was unsound; a fresh spellbook is legitimately EMPTY, so genuine sends were discarded → 2nd appended field `owner_state:bool`). ⚑ **`Ledger.DisplayRevision()` is a SECOND counter on purpose**: `revision` drives forced SAVES, so bumping it per kill = a DB write per mob. Also fixed: a FROZEN cooldown sweep, and the journal not clearing on death. **Schema WIRE YES (2 appended); DB/content NONE.** Verified: build · vet · full `go test` · tsc · **vitest 586/586** · in-game `hygiene-wire-prune` 653/0 · **`c5-ability-bar` 30/30** (leg 5c is the sweep proof) · `chunkC3-journal` 14/14+2 SKIP · `r1-focus-cost` pass · **`chunk3b-ii-conversation` 28/34 = documented HEAD baseline**. ⚑ **OWED: the counter fix is proven by a mutation-verified Go test, not a browser.**
