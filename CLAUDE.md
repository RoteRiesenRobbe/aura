...
- **Last completed:** Aura collider re-sourced to skill definitions. **Decided: no per-skill colliders** (the never-wired `EquippedSkill.Collider` field is deleted) — each entity keeps its single aura sensor and `SkillSystem` resizes it per tick to `EquippedSkill.EffectiveRadius()` (level-scaled max of `radius + (level-1)*radiusPerLevel` over the skill's effects). `AuraCollider()` now returns the concrete `*phy.Circle`. A skill's radius parameters are effective for the first time; behavior-neutral today (skill JSON radii == legacy config radii == 1.0).
- **Legacy aura UI replacement:** 1a ✓, 1b ✓, collider re-sourcing ✓ — **next: Phase 5 cleanup.** Retires `#auras` buttons, the `AuraType`=slot hack, and the deprecated `aura`/`active_aura` fields; **decided: `aura_radius` stays** (sizes the ring for all clients, Phase-7-proof).
