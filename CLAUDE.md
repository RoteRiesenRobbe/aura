...
- **Backlog watch items** (`docs/backlog.md`): §25 D+E · §27.2.4–7 (⚑ §27.2.6 needs re-surveying) · §29 lost-WebGL-context trigger unknown · §37 skill-level/augment rework · §39 entity-presentation rework (no per-effect overlay art before it) · §34 hard collision · ~~§58 terrain select/move/rotate (PO-parked 2026-08-21)~~ **ANSWERED 2026-08-23 by a DIFFERENT editor** - Tiled does select/move/rotate/scale/flip/multi-select/undo over all 549 terrain blobs (`docs/archive/plan-tiled-authoring.md`); the in-game editor deliberately unchanged, §58's analysis still describes it; ⚑ not closed until the PO has moved a real texture in Tiled · round-6 item 4 target stickiness (re-opens on measured cost) · ⚑ **§57 attack-attribution lines: PROTOTYPE PO-played, deliberately NOT merged** (`prototype/attack-lines` `cf305284`; weakest exactly where it fires - a shipped version is a §39 consumer).
