...
- **⭐ ASCENSION follow-ups, both scoped and unstarted (`docs/plan-ascension.md` §13.7, PO 2026-08-11).** ① **The reward rows should carry the real ability tooltip** — hover a catalog row and see what the skill actually does, on **locked rows too** (knowing what you'd get is the point of a named gate). Design settled: an appended `skill_id` on `ConversationOption`, the `confirm_seconds` precedent, client feeds the existing by-id `SkillTooltip`. ⚑ It is a **WIRE change** — `.fbs` regen both sides, both builds, `hygiene-wire-prune`, and the next deploy becomes a both-sides one. ② **Leaving the world without a page reload** — see the backlog §52 bullet below; the PO has now asked for it.
