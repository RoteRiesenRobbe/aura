...
- **⏸ Parked 2026-08-20 (PO choice, resume later): `plan-prototype-projectile.md`** - P1 + five fixes shipped 2026-08-19; the SECOND in-game pass is OWED, not abandoned (§10 items 1, 2, 4, 5 + 13; setup `SKILL ThrowMine` / `SKILL ThrowBomb`, ⚑ **GOD makes a cooldown free**, so test the cost with god mode OFF). The verdict decides P2 (drift; now also owns the deferred travel-to-position ask) and P3 (mob thrower), or delete. **In flight instead: `plan-npc-hails.md`** (planned 2026-08-17, one chunk, schema NONE) · `plan-play-bot.md` (C0-C3). ⚑ Unowned leftovers: `TICKING_TYPES` in `SkillTooltip.ts` (hand-maintained per-type set, SILENT failure - a new ticking type loses its cadence line) · zone-editor C3: dead Go legacy plumbing (Legacy/LegacyRefs + the unreachable-loaders warning) · `wiki-generator/` broken at HEAD (needs the §28-deleted `client-data/Items`) · the reported stale `backend/pkg/api/mobs/` mirror is NOT reproducible here (two clean embedded boots since) - re-verify on the finder's checkout before acting.

- **⭐ DIRECTION SET 2026-08-22: the next map we build is the FIRST RELEASE MAP, and CAMPS ARE CONTENT** (`docs/plan-release-map.md` - the authoritative write-up; nothing built, the map itself not yet designed). Camp membership = a completed quest; exclusivity = `quest_at_stage` gates on the nodes that offer, teach and turn in; the whole Gothic arc (proving quests · a pledge closing the rivals · a camp-only finale with a locked rival door · camp-exclusive teaching · camp-gated ascension rewards · a free post-ascension reroll, because the quest ledger is keyed `character_id`) is authorable in shipped vocabulary - **no new vocabulary, no Go, schema NONE**. ⛑ Two engine rules constrain every camp file (`manual-content-authoring.md` §6): a terminal stage id never matches after completion, so *a durable choice must be its own quest id* · `not_started` re-matches after an abandon, so an exclusion seals at COMPLETION (author the pledge with no objective stage, accept + turn in in one conversation). ⛑ Permanence is a content pattern (every pledge offer carries every rival's `not_started`) guarded by a pin in `quests/content_test.go`. ⏸ `plan-camps.md` DEFERRED, not cancelled (still buys hostile standing + server-enforced permanence); its standing wipe defers with it - the guard flips: moving the quest ledger off `character_id` would silently kill the reroll. ⛔ `plan-test-world.md` DROPPED + archived, never executed (its §1 transfers: F1-F4 bounds + measured densities 46.8 spawns / 74.9 props / 51.8 textures per 1000 sq units; ⛑ F2 has moved, a spawn may author its own `level`). Execution-order slot + whether it replaces `world.json`: **open PO calls** (§7).
