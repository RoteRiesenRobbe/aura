...
- **Last completed:** **Playtest feedback pass: banner ×2, dot responsiveness, resist ring, Wildfire rework — DONE 2026-07-21, readability items PO-verified in-game ("all works"), committed `2c68be85`; full ledger: `plan-intermission-triage.md` §Playtest feedback pass.** PO-picked ad hoc from live play (ahead of queue item ②). **PO rulings (choice prompts):** dot speedup **player dots only** (Immolate + Wildfire `tickInterval` 40→20, ~0.67 s to first burn; backend refresh keeps the dot cadence, so responsiveness-only); spellbook section headers **1.1em + banner gold `#ffd75e`** (were 0.85em muted, smaller than the rows); projected `resist_aura` ring gets the **applied-pip teal `0x5fbfb0`** (new `AuraCategoryResist = 1<<6`, TDD; colour single-sourced in `AURA_CATEGORY_COLORS`, EffectPips references it — closes the item-7 deferral); alert banner **×2 for all kinds** (`0.55 → 1.1 × @uiElementHeight`); **one combined commit** incl. the parallel session's Immolation→Immolate rename. **Wildfire rework (PO: "the ultimate fire fantasy"):** answered the "why over Immolate" question (authored ~70% two-target side-grade) → PO retired the calibration: now **full Immolate damage on both targets, radius 1.4, dotTicks 4, caster-only fire resist** (`targetsSelf` without `targetsAllies` — no code change needed; 0.6 −0.05/lvl [PLACEHOLDER]); its ring shows purple+teal dual bands for free. **Watch item:** a **parallel session renamed `immolation.json` → `immolate.json` between my Read and Edit** — on unexpected Edit failures, `git status` before assuming a typo; the entanglement (3 shared files) was surfaced and the PO picked the combined commit. Stale sim pin updated (`serve_test.go` Immolate aura tick 40→20). **Verified:** `go build` clean, full suite green, `tsc` clean, prod build clean, boot `-content ../api` `81 skills/14 factions/50 mobs/10 recipes/847 props/383 spawns/5 campfires/14 npcs, 0 panics` ×2; PO "all works" pass also **closed the deferred mob-side pip glance**; Wildfire rework itself committed without an explicit post-rework in-game pass (feel check outstanding).
