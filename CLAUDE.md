...
- **Last completed:** Step-4 **chunk 5 (dash) DONE + VERIFIED IN-GAME 2026-07-14, committed** → `docs/plan-skill-vocab.md` banner. **Direction = movement vector, NOT facing (SUPERSEDES §3.8): Aura characters are non-turning icons with no facing/camera rotation, so dash aims along the last non-zero movement direction (current if pressed, else last recorded).** New `LastMoveDir()`/`SetLastMoveDir()` on PlayerEntity (concrete `player.lastMoveDir`, defaults to unit `{1,0}` so a never-moved player still aims); `core/input.go` records the normalized non-zero movement vector (same hook that cancels casts). `dash` effect type (21 total) + `DashParams{Distance,DistancePerLevel}`; `applyDash` = stepped `QueryCircleStatics` probe (mask PlayerStatic|Border, radius-sized steps, summonPosition precedent) marching the ray, clamp at last free point — cannot tunnel props/border, "dash up to the wall", zero-distance flush = still consumes cd (no whiff, player path consumes unconditionally). Player-only (mobs no-op, no last-move seam). Cast-cancel inherited FREE from chunk 4 (castTicks 0 → fires immediately via the different-slot hook). Frontend: **teleport snap lowered 600 → 180 px** so a 2.5-unit (300 px) dash snaps not smears; Skills.ts id 33. Content **Dash id 33** (cooldown 300, dashDistance 2.5 +0.5/lvl, cheat-granted via `SKILL Dash`); registry pin 33. **Prior:** chunk 3 (HoT + revive) VERIFIED IN-GAME + committed ce4033fa; chunk 4 (cast-time + Recall) committed 4287977e — key decisions (supersede §3.4/§3.5): same-slot re-request ignored, any other deliberate input cancels (different cooldown still fires; aura switch; movement ≠ 0); **damage-interrupt is OPT-IN per skill** (`castInterruptedByDamage`, only Recall sets it). 13 new tests (5 parse/validate + 8 behavior). Suite + build + tsc/webpack + boot smoke both content sources (33 skills) green.
- **Next:** Step-4 **chunk 6 = minimal tick-indicator wire + tick-rate seam** (§4.5/§5) — LAST chunk of Step 4. `aura_tick_interval`+`aura_tick_phase` ushort appends on Character+Mob (first-effect effective interval; phase = accumulator mod interval; zero when no active aura); `effectiveTickInterval` becomes caster-aware + composes a `tick_rate` buff payload (haste/tick-slow, clamped ≥ 1 tick) with one smoke consumer; frontend bare ring pulse/arc reading the two fields (own player + mobs + other players). Accumulator rescale-vs-restart + tick_rate combination rule = chunk-start decisions. NEW execution session. `docs/plan-skill-vocab.md` §4.5/§5 chunk 6
