...
- **Last completed:** Dead resource+placeable+decay prune **§26 FULLY DONE** (Chunks 1+2, plan-resource-decay-prune.md) ✅ `ee9d42e9`+`a2ab90b5` 2026-07-24, PO hand-tested. Chunk 1 removed the backend cluster (`model/resource`+`model/placeable`+`gen`+`sys/decay.go`+`model/decay.go`, 3 codec cases, `addPlaceableEntity`+`DecaySystem` → helpers 7→6/systems 16→15, 3 dead interfaces, 9 item JSONs); Chunk 2 swept the frontend `Placeable` decode path. Campfires (mobs) + props (`AnyEntityResource` wire path) untouched. **Two things to carry forward:** ① the deleted JSONs were 100% of nested item content, so `items.go`'s embed narrowed to `*.json` and the item registry is now the `None` sentinel alone — **backlog §28 (item-registry removal + Tier-3 `Placeable` schema prune) is now trivial**; ② Chunk 1 shipped a frontend-build regression (`client-data/Items.ts` `require`s the deleted JSONs) that the backend-only rebuild missed and Chunk 2 fixed — **rebuild frontend AND backend after content deletions.** Verified backend `go build`/`vet`/`test` (29 pkgs) + frontend `tsc`/webpack + 12/12 clean joins. Full ledger: `docs/plan-resource-decay-prune.md` §13.
- **Recent chunks (newest first; full ledgers in the plan docs):** §26 dead-layer prune Chunk 1 ✅ `ee9d42e9` (`plan-resource-decay-prune.md`); render-jitter buffered-interp fix ✅ `0e504c22`/`8a29a75c`/`c5064732` (`plan-render-jitter.md`); input-jitter held-state fix ✅ `cb7f011f` (`plan-input-jitter.md`); unlock source attribution ✅ `2bfee286` (`plan-unlock-attribution.md`); idle-loop alloc fix ✅ `fe0044d0` + day/night DEACTIVATED ✅ `e648ab88` (`plan-intermission-triage.md`); playtest-1 Passes A/B/C ✅ — **plan fully executed** (`plan-playtest1-feedback.md`); F&F deploy LIVE ✅ `a7a2267d` → `https://aura-game.duckdns.org/` (`plan-playtest-deploy.md`); content pass C1–C8 + rebrand step 7 complete. Earlier chunks: roadmap.md "Execution order" + the plan-*.md §13 banners.
