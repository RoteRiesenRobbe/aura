...
- **⭐ REGION ATMOSPHERE: the A-half is DONE, and what is next is B — the LINE OF SIGHT the PO originally asked for** (`docs/plan-region-atmosphere.md` §5): A0-A2 shipped `5b57d0c4`, **A4 shipped 2026-09-16** (ledger §10). ⭐ **B1 is next**: `Occluders.ts`, pure segment derivation from props (`occludesSight`, a DEFINITION flag per D8, not a placement one), blocking paths incl. P1’s `closed` wraparound, blocking polygons and the border — no rendering, 100 % vitest-reachable. Then B2 (the visibility polygon, also pure) and B3 (the stencil mask, D10 — FORCED by the engine: PixiJS 8.4.1’s `FillGradient` is LINEAR ONLY, so the "just fill the polygon with a radial gradient" shortcut does not exist). ⛔ **Today every light is an unconditional circle**, so a lantern erases darkness through a cave wall — that is what B fixes. ⚑ **The no-occluder case must return today’s circle** and that is a required test, not a nicety: it is what makes B safe to land. ⚑ **§8 Q3 is an OPEN PO CALL that B3 needs**: D9 ships LOS on the local player’s own light ONLY, because shadows do not compose by painting black (a point in wall A’s shadow but lit by torch B is lit, so each light needs its own clipped erase) — is an ally’s torch shining through a wall, beside your correctly shadowed one, worse than neither having shadows? ⛔ **B clips LIGHT, never AURAS** — aura LOS was cut 2026-07-10 and auras still pass through every wall. ⚑ **A3 stays a PO content call**: re-author `world.json`’s 35 `darkAreas` as 2-3 atmosphere shapes, which is also the trigger that retires the `darkAreas` primitive (D4). ⚑ **Owed from A4**: every number is [PLACEHOLDER], the look sitting has not happened, and `sight` is authored by NOTHING so D7’s `max()` has never been exercised by real content. ⚑ **And the camera cannot currently prove a clearing on screen** — the authored overlap with a dark bank is narrower than the player’s own light; a clearing whose dark overlap is comfortably wider would let `a4-clearing.mjs`’s pixel A/B answer instead of reporting inconclusive.
