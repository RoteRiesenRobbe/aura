...
- **Prior design session: runtime allegiance — ALL THREE CHUNKS NOW SHIPPED** ✅ `074d1a53` 2026-07-28 (docs only) — produced `docs/plan-faction-flips.md` (D1–D5, later D6–D14; landmines L-A–L-O). ⭐ **Its reframe is the durable part:** `SetFaction`'s `^f.Bit()` was not a wrong *value* but an **undefined destination** — correct for `aligned`, undefined for everything else — so the fix was named verbs, not a lookup. Three findings sized it: the restore direction needs no registry (`MobDefinition.AggroMask` **is** the authored mask), there is **no runtime faction registry** to look up anyway (`factions.Registry` is boot-only), and ⚑ the overwrite was **load-bearing** — a registry-derived "fix" would have silently stripped harm rights from every companion, totem and campfire. Full ledger `docs/plan-faction-flips.md`.
...
- **⭐ NEXT — step 8, accounts & persistence (a DESIGN session, not a schema session).** `plan-faction-flips.md` is **COMPLETE** (chunks 1 ✅ · 2 ✅ · 3 ✅) and pre-accounts hygiene is complete, so **nothing is standing in front of it**. The step-8 detail — ruling 4, backlog §32, the ops/security fold-in — is in the bullet below. ⚑ **Two things charm left on the table, both PO calls and neither blocking:** ① **nothing this plan shipped is reachable in normal play** — calm, charm-beast and charm-elemental are all `SKILL`-cheat-only, so no playtester can reach them until an unlock is authored (a milestone, drop or teaching; it is content, not code); ② **a charmed mob is focused by its former packmates and can die in ~8 s** against a 59.4 s duration — D9/L-E working as ruled, but the spell promises a minute and delivers eight seconds in a pack. Both are `docs/plan-faction-flips.md` §8 open questions 1 and 2. **Also still open there:** whether the D13 pip should show duration (a charmed wolf shows *a dot, and nothing else*).
