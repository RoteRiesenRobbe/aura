...
- **⭐ Last completed: ZONE EDITOR C2 - CONTROLS + THE TALKER ROUND-TRIP** ✅ 2026-08-16, `[uncommitted]` (`docs/plan-zone-editor-structure.md` §11): **the ascension stones are editable** - the spawn controls now follow what the picked species can DO, in both directions (picker change + marker select): respawn rows show only for defs without an `interaction`, movement rows only for `factors.speed > 0` (capability, never the bucket - Wanderer talks AND walks, Turnip is a fixture that dies, L4). Respawn went tri-state: a talker authors NO respawn keys (`world.json`'s 17 respawn-free spawns = exactly the 17 interaction carriers), and the serializer omits them. ⭐ Ruling: the omit predicate is **interaction-presence ONLY, never input emptiness** - an absent key parses to 0 server-side (respawn every tick), so a combat mob keeps the hard "Invalid respawn ticks" refusal; §4.6's "or empty" phrasing lost to its own bolded warning. The read/populate pair moved into the new pure `SpawnControls.ts` (the panel imports `require.context`, vitest can never reach it, L3); red-first via verbatim port + stubbed `capabilitiesOf` (10/22 new units red on assertions, the live stone refusal reproduced at the seam). The `world.json` round-trip test is now PERMANENT (L7 guard; passes at HEAD - the bug was panel-only). **Schema NONE.** Verified: vitest **350/350** (was 328) · typecheck · prod build · `go build` · `c3-zone-editor-level` 7/7 · throwaway stone harness 9/9 (Update on the stone accepted, **export byte-identical, P6**; a fresh stone with stale 900/0.2 inputs exports no keys; 0 console errors) · boot 0 WARN/0 ERROR · `harnessdb -cleanup`. ⚑ Post-WARP camera settle is ~40 s, not 20 (verify skill updated). **PO in-game checklist open** (stone → Update → move → re-export). **Next: C3** - retire the legacy roster, the one wire-touching chunk.
- **Prior: ZONE EDITOR C1 - THE STRUCTURE** ✅ 2026-08-15, `327f3828` (`docs/plan-zone-editor-structure.md` §11): the spawn editor's five unlike things now read as five. Derived **`kindOf`** (legacy → talker → fixture → companion → combat) lives in `ZoneModel.ts` (the only vitest-reachable home - `ZoneEditor.ts` opens with `require.context`, L3); the picker is five `<optgroup>`s with the `cL`/tier suffix on **Combat and Legacy only** (P2); markers colour per bucket - combat keeps the exact old green (435/488 placements), talkers pink, fixtures grey, companions brown, legacy faded green; selection stays full-strength yellow; every colour [PLACEHOLDER]. Census exact: 68 defs = 27/17/10/4/10, Brazier the sole legacy+structure overlap (P1). Export **byte-identical** before/after (P6, temp serialize-diff instrument, then deleted; ⚑ jsdom rewrites `import.meta.url`, so a node-fs vitest needs an absolute path). **Schema NONE.** Verified: red-first (`kindOf` stubbed to `'combat'` → 5/7 units red on assertions) · vitest **328/328** (was 321) · typecheck · prod build · `go build` · throwaway DOM harness all green (groups/census/suffix/D3/P1, 0 console errors) · **`c3-zone-editor-level` 7/7** (owns the touched panel + marker code) · boot 0 WARN/0 ERROR · residue cleaned · **PO in-game checklist 10/10 same day**. **Next: C2** - respawn tri-state + capability-gated controls, the chunk that fixes the uneditable ascension stones.
...
- **Two plans in flight** (`plan-code-health.md` completed whole + archived 2026-08-14): `plan-zone-editor-structure.md` **C1 ✅ 2026-08-15 · C2 ✅ 2026-08-16 - next is C3** (retire the legacy roster + `proving-grounds.json`, §4.7; the plan's ONE wire-touching chunk: `EntityType` values removed, regen both sides, both-parts deploy) · `plan-play-bot.md` untouched (C0-C3: extract transport → the bot that fights → quests → the soak finding). **Two more designed 2026-08-15, nothing built:** `plan-effect-types.md` (round 1: vulnerability, retaliate damage, invulnerability, ally speed - four chunks, schema NONE) · `plan-prototype-projectile.md`.
