...
- **Next:** **Step-6 chunk C3 (Kobold hideout + Dark Tunnel — the solo path) — execution session** per `plan-content-zones12.md` §13 C3: `kobold` + `spider` factions; Kobold melee (small radius, fast tick, flees low HP) + ranged (large radius, slow tick, uncapped `all` — the volley showcase); Light kill-drop (low %); hideout block-out SE-center + kobold clue signpost ("shiny stuff"); Tunnel N geometry + darkness; lit spider staging area at the mouth; Spider normal (lifesteal bite) + venom spider (`dot_aura`, first `poison` tag); Antivenom drops (first resist_passive drop pair); poison pools (brazier hazard pattern, XP 0); rockfall side passage (solid-mob pattern like the bramble — MUST opt in to a gate tag, `{"*":0,"harvest":1}` or its own, gate TBD §11); tunnel-warning signpost. Verify: solo tunnel run at level, light = advantage never requirement, poison DoT vs Antivenom, pools hurt but grant nothing. Then C4–C8 in order, one chunk per session. **Every C1+ mob MUST be authored tier + baseline** (raw `maxHealth` hard-fails; manual §1); combat mobs need NO harvest entries (gated damage tags — manual §1), but **gate obstacles must opt in**. Standing locks: growth 1.12 × maxLevel 30 (≈27×, band ≈ +5, lower-first); regen slow ~1%/s, tier placeholders normal ≤ ~50% / elite ≤ ~25% / boss dies; downtime 15 s + chain 20; guardrail asserts land in C8 against real mobs. Dev cheats: GOD, WARP <x·120> <y·120>, SPEED [factor|off], XP, SKILL <name>. `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; boot `-content ../api` for content iteration (schema/Go changes still need the rebuild).
