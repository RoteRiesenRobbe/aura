...
- **Last completed:** Step-4 **chunk 6 (tick-indicator wire + tick-rate seam) DONE + VERIFIED IN-GAME 2026-07-14, committed 3e9ab8e4 → STEP 4 COMPLETE** → `docs/plan-skill-vocab.md` banner. LAST chunk of Step 4. Post-verify fixes folded in: ring-glow indicator (over rejected dot + fill cuts), tick-cadence gate (no strobe on state auras), global cadence-doubling pass, and Skills.ts Haste registration (id 34 was defaulting to `Skill #34`/aura → listed under Auras + highlighted aura slots while the backend equipped it as the cooldown it is). **Chunk-start decisions locked: (1) accumulator = RAW MODULO (A) — the single monotonic `TickAccumulator` stays; fire test `acc % effectiveInterval` uses the current interval, wire phase = same formula so indicator and ticks stay in sync by construction; a large abrupt slow can cancel an almost-due tick (edge, no content yet), A→B rescale is a non-breaking future upgrade since the wire carries the EFFECTIVE value. (2) tick_rate combination = MULTIPLICATIVE (resist model) — strongest-per-skill (furthest from unity) × across skills, so haste and tick-slow cancel; floored ≥ 1 tick.** New `EffectiveTickInterval(e,level,factor)` in `skills/scaling.go` = THE cadence source of truth (round(scaled×factor), floor 1), called by both the firing loop (caster factor) and the model wire accessors (own factor, effect[0]). `tickRatePayload`+`ApplyTickRate`+`TickRateFactor()` in `skills.Buffs` (mirrors slow; 6 payloads). Firing loop reads the caster factor via a `tickRateBuffed` capability assert (default 1.0); `effectiveTickInterval(e,level)` stays the base (factor 1.0) wrapper for the VFX-style + buff-lifetime callers (haste must NOT flip hit style or instant-buff duration). `tick_rate` effect type (22 total) + `TickRateParams{Factor,DurationTicks}` — self-targeted haste/tick-slow cooldown, `applyTickRate` applies straight to caster (no query circle). Model: `AuraTickInterval()`/`AuraTickPhase()` on player+mob (mirror `AuraRadius`, 0 when none active) + `TickRateFactor()`/`ApplyTickRate` delegates. Wire: `aura_tick_interval`+`aura_tick_phase` ushort appended on Character+Mob; codec both branches; Go+TS regen. Frontend: bare `AuraTickIndicator` — a thin **ring-glow ramp**: a stroked ring at the aura edge whose alpha builds `phase/interval × 0.45` toward each tick and discharges at the beat (the full ring stays drawn, the hit keeps its slash/fire VFX). Iterated from two rejected cuts (orbiting dot = "didn't look good"; filled disc = "constant alarms all over the screen") — the glow only lights the ring edge, never floods the interior. On own player + other players + mobs; created LAZILY on `this.shape` (NOT in initShape — that runs during super() before the `= null` field initializer, which would clobber it → the "setRadius of null" crash the disc cut hit). NOT step-8 polish. **Two calming changes shipped with it: (a) tick-cadence GATE** — `skills.HasVisibleTickCadence(type)` gates `AuraTickInterval` to the four hit auras (damage/heal/dot/hot); slow/resist/light auras (often `tickInterval 1`) no longer strobe an indicator (wire 0). **(b) global cadence pass** — every hit-aura's `tickInterval` (+ dot/hotTickInterval) AND its per-tick output (`damageHP`/`healHP`/selfDamage, incl. perLevel) DOUBLED = DPS-neutral, half as many/twice as chunky ticks; 16 skill JSONs (player + mob), all [PLACEHOLDER]. Content **Haste id 34** (cooldown 300, tickRateFactor 0.5 for 90 ticks, cheat-granted `SKILL Haste`); registry pin 34. Buff-visibility (icons/timers) deferred to step 8; skill-metadata catalog stays the step-8 Skills.ts hand-sync direction. 14 new tests (7 tick_rate buff + 4 EffectiveTickInterval + haste-fires-faster + 5 parse/validate + codec round-trip). Suite + build + tsc/webpack + boot smoke both content sources (34 skills) green. **Prior:** chunk 5 (dash) VERIFIED IN-GAME + committed e8c4851b; chunk 3 (HoT+revive) committed ce4033fa; chunk 4 (cast-time+Recall) committed 4287977e.
- **Next:** Step 4 (skill-vocab fill) is **COMPLETE** (all 6 chunks verified in-game + committed). Per execution order (`docs/roadmap.md`) → **Step 5 = unlock-source systems** (clue anchors + NPC teaching) then the **content pass** (real auras/passives/cooldowns/combinations on the now-filled vocabulary) — confirm sequencing with the roadmap at step start. Plan-first, NEW session.
