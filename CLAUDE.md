...
- **⭐ Last completed: PLAYTEST INTAKE round 8 item 2 - A SPENT INFO ROW LEAVES WITH ITS QUEST** ✅ 2026-08-11, uncommitted: a third `quest_at_stage` sentinel, **`running`** (accepted, not yet finished, across every stage), plus one content gate, so the traveller's *"Where do they nest?"* goes when the lamp quest does. **Schema: DB NONE · FlatBuffers NONE · conf NONE** - Go + one content file, client untouched, so an ordinary deploy. Two PO calls (`running` alone, not also a `not_completed` twin whose one-arm edit stays available forever · the Town Crier's `news_who` stays readable). ⭐ **THE AUDIT WAS MOSTLY A FALSE ALARM AND FINDING THAT OUT WAS THE WORK**: a dump of all 16 authored trees showed six of seven candidates hang off `root` as siblings of *"Do you have a task for me?"*, i.e. standalone lore the design says must stay - `lamp_where` is the **only** node in the cast hanging off a QUEST node. One content line, not seven. ⛑ **A SENTINEL IS NAMED IN FOUR PLACES and the design found two**: the two it missed are error strings, and one is load-bearing - **`quests.CheckStageRef` refuses an unknown sentinel as a bad stage id, so it is a BOOT FAILURE**, which is exactly how the first run went red. ⛑ **L3 MADE THE FIX UNAUTHORABLE**: options carry no conditions, so a row is hidden by gating its destination node - but the loader refused any conditional node below the fallback, and hoisting it would have made it the *greeting*. A **navigation destination is now exempt**; the teeth stay on unreferenced nodes, with a negative-space test. Verified: TDD red-first at both surfaces, **2 mutations caught** · Go 0 FAIL (`-count=1`) bar the known `TestDwell` flake · `-race` clean · boot 0 WARN/0 ERROR · ⭐ **`chunkC4-quests` 42/42 (0 FAIL, 0 SKIP), leg D 9/9**, whose D2 asserted the OPPOSITE and was repaired with the change, and whose deliberate D5 skip became a real leg (six real kobolds, since `QUEST ADVANCE` cannot leave an objective stage). Ledger: `docs/plan-playtest-feedback.md` §Round-8 item 2. **Also recorded this session:** round-7 item 3 (totem tooltips) had already shipped 2026-08-09 `7e5233e2` and is now marked done in all four places.
- **Prior: ASCENSION follow-up ② - THE CEREMONY GETS THE STAGE; ⭐ THE PLAN IS COMPLETE AND ARCHIVED (`docs/archive/plan-ascension.md` §13.10)** ✅ 2026-08-11, `5b014c17`: starting the channel now **closes the panel that started it**, and the character is wrapped in a swarm of motes that collapses inward and flashes as the ceremony lands. Three PO calls (D29 own-player-only · D30 gathering motes · D31 drop the reply line, the cast bar says enough). **Schema: DB NONE · FlatBuffers NONE · conf NONE · content NONE** - it reads fields already on the wire. ⚑ **A CLIENT CANNOT CLOSE A CONVERSATION**: the tree leaving the next snapshot is what closes the panel, so the rule is a SERVER one, derived every tick in `refreshConversations` beside the death/range cases - which makes *E during the ceremony opens nothing* free. It **reads** the cast and never cancels it, so walking away is still the last escape (P7). ⛑ **A SURVIVING MUTATION MEANT DEAD CODE, not a missing test** - `castProgress`'s zero-guard was unreachable behind a total `clamp01`, and the line lost. ⛑ **The first FX pass was invisible** (a 2 px core on a ~17 px character) and only the harness screenshot said so; the screenshot's **timing** is load-bearing, since the eased collapse looks like nothing at 1 s. ⚑ Bystanders see nothing, by ruling. Verified: 4 session pins red-first, **1 mutation caught** · Go 0 FAIL (`-count=1`) bar the known `TestDwell` flake · `-race` clean · vitest **287/287** · ⭐ **`c2a` 29/29** (+2 legs) · `c3-memorial-catalog` 14/14 · `chunk3b-ii-conversation` 28/34, its exact baseline.
...
- **⭐ ASCENSION SITES, designed 2026-08-11, nothing built** (`docs/plan-ascension-sites.md`): many stones, each owning its price and its reward list. ⭐ **Half is already authorable** - node `conditions` are a list covering `minLevel`/`quest_at_stage`/`bloodline_ascensions`/`kills_this_life` - so the work is three blockers: `RequestAscension` overrides the content with a `levelCurve.maxLevel` check, one global catalog behind a `RowSource` that never learns which actor asks, and an unreadable gate. D1 **retires P1** (max level as the entry price). Schema NONE at every layer. C1 price → C2 readable gate → C3 rewards.
