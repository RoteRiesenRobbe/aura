...
- **Last completed:** **Friends & family playtest deployment — LIVE + PO-VERIFIED ("hat alles geklappt, wir können spielen") 2026-07-21, `[uncommitted]`; full ledger: `docs/plan-playtest-deploy.md`.** The game is publicly playable at **`https://aura-game.duckdns.org/`** (Hetzner CX23 `159.69.148.73`, Nürnberg; DuckDNS; single `aurad` process serves frontend + wss + `/skills` with built-in Let's Encrypt autocert; systemd auto-restart; runs `-content ./api` so content deploys skip the rebuild). **New:** `devops/deploy.sh` (full build+push / `--content-only` fast path for PO map edits), refreshed `devops/conf.json` prod template + `devops/README.md` runbook, port-80→https companion listener in `bootTlsServer` (ACME http-01 + redirect — 2-line change in `backend/cmd/aurad/aurad.go`). **Safety posture:** cheats token-gated (`/opt/aurad/tokens.list`, server-only, never in repo); accepted F&F risks recorded in the plan doc (open origin check, no rate limit, **no persistence — every restart wipes characters, `ANNOUNCE` before deploys**). **Verified:** `go build ./...` + cmd tests green, prod frontend build green, bundle self-boot smoke, live https/redirect/`/skills` probes, live Playwright join smoke (spawned at starting campfire, 0 page errors), live boot `82 skills/14 factions/50 mobs/10 recipes/815 props/399 spawns/5 campfires/14 npcs, 0 panics`, PO played live.
- **Prior:** **Triage pass ✅ `37878018`** 2026-07-21 PO-verified ("Perfect") — cast-bar stack in `#bottomCenter`, spider radii matched 1.0, **Strong passive id 136 on the new `damageDealt` stat** (pin **82**, CityGuard @L3), Wildfire light r4 +1/L; full ledger: `docs/plan-intermission-triage.md` §Triage pass.
- **Prior:** **UI-polish chunk 1 ✅ `ae51d8b5`** 2026-07-21 PO-verified ×2 — `GET /skills` serves the parsed registry (both muxes, CORS); `Skills.ts` hand-sync maps DELETED (catalog module + `SkillTooltip.ts` full-detail anchored hover tooltips, current→next, shared-line dedupe, colored labels via `AURA_CATEGORY_COLORS`); smoke 10/10; watch: `#gameUI` is a zero-size shell — Playwright waits on `classList`, not visibility; `AURA_CATEGORY_COLORS` (AuraRings.ts) is also a HUD dependency; commit incl. the PO's parallel zone-editor pass (`ac8eb9b5`); full ledger: `docs/plan-ui-polish.md` §Chunk 1 ledger.
