...
- **Last completed:** item-system removal **Chunk 3 (FlatBuffers dead-wire-enum prune) DONE** (2026-07-24, `docs/archive/plan-item-system-removal.md`) ✅ `8ed4ff4c`, PO-verified in-game — **§28 COMPLETE, plan archived**. 32 files, **131 insertions / 1033 deletions**. `server.fbs`: removed `table Placeable` (+ its `AnyEntity` member — last position, so `Character`/`Mob`/`Resource` keep 1/2/3), the 9 dead `EntityType` members and `StatusEffect.Yielded/Freezing/Starving`; **⭐ pinned EXPLICIT values on both enums at today's positions** — every survivor's wire value is unchanged and gaps (`EntityType` 1,3,6-8,12-14,16 · `StatusEffect` 1-3) mean **no future removal can ever renumber a survivor**; added `EntityType.NpcPlaceholder = 74`. **PO asked for a plan audit first — it caught 4 problems, one live:** the plan's `PlaceholderSprite` → `DebugCircle` repoint would have shipped a **self-deleting dev-only class** (60-tick TTL + `Develop.get()`) as the NPC fallback sprite. **PO rulings:** new authored art (**red `?` on a purple disc** — `npcPlaceholder.svg` + `NpcPlaceholder` class), fold in `Yielded` (dead, plan missed it), preserve ordinals, delete the orphaned classes/art. Frontend: 9 `gameObjectClasses` rows out + 1 in, 7 orphaned `Resources.*` classes + **11 SVGs + the `bush-hit` mp3**, the unreachable `forYielded` VFX, the `berryBush` render layer, 4 stranded imports; **deviation:** `internal-tools/art-references/` deleted (unbundled dev page whose entire subject was the removed `flower.svg`). Verified: `go build`/`vet`/`test` green, `tsc --noEmit` clean, backend + webpack prod builds green, boot embedded **and** `-content ../api` identical with 0 panics (83 skills/14 factions/50 mobs/10 recipes/5 prop defs/777 props/471 spawns/5 campfires/14 npcs — the decisive check, §27.2.1's loader hard-fails on a still-referenced removed EntityType), join smoke both muxes = character spawns / world renders / **0 console errors**. **⚠ Watch → backlog §29:** one cold develop-mux load threw `null.split` ×3 + a black world, **not reproduced in 5 further runs**, not attributable to the prune (a bad enum value fails *every* join, not 1 in 6; the failing run showed 40 s frame times). Spun off **backlog §30** (4 pre-existing render/asset vestiges left deliberately uncut). Full ledger: `plan-item-system-removal.md §13 Chunk 3`.
- **Prior:** §28 Chunk 2 (frontend scaffolding + delete `none.json`) ✅ `2f933634` 2026-07-24 — `features/items/` + 54 SVGs + `client-data/Items.ts` deleted, `BackendConstants` itemLookupTable unwired; 3 verify-surfaced deviations (credits.html graphics section, graphics.html gallery, dev-panel item-give block). §28 Chunk 1 (backend registry) ✅ `b9d01d33` 2026-07-24 — dead `items` Go package + embed + 8 importers gone (kept `items/mobs/`). EntityType name-fallback validation §27.2.1 ✅ `c3938be7` 2026-07-24 — a live-server crash-at-first-spawn turned into a boot error. Full ledgers: `docs/archive/plan-item-system-removal.md §13`, `docs/archive/plan-entitytype-validation.md`.
