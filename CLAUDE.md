...
- **`plan-flight-paths.md` continues at C3** (the client flight experience: camera zoom-out moving with the AOI, input lock, the indicator): C1 shipped inside world-map C2, and **C2 (the server state machine) shipped 2026-08-05** — headless-verified only, so C3 also owns the first in-game pass and the speed/zoom feel tuning. C4 shrank to the roster filter (snapshot invisibility came free with C2's structural design, D13).
