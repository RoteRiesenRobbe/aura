...
- **Last completed:** Step-5 (unlock-source systems, `docs/plan-npc-teaching.md`) **chunk 4 (speech origination, backend) DONE (backend-verified) 2026-07-14, committed.** The temp chunk-3 `slog "npc onApproach"` in `NpcSystem.Update` is replaced by a `speak(n, lines)` call. New `speak()` in `sys/npc.go` builds ONE `codec.EntityMessageFlatbufMarshal(builder, n.Basic().ID(), strings.Join(lines,"\n"))` and fans it out via `p.Client().SendMessage(bytes)` to every player in `n.Sensor().Collisions()` — reuses the existing chat wire (no `server.fbs` change). Latest-wins is automatic (one shared `entity_id`, newest `say`); the sensor ⊆ each player's viewport so the client already tracks the NPC. Imports swapped `log/slog`→`flatbuffers`+`codec`. **Sanity:** `go build ./...` clean; full `go test ./...` 0 fail; **new `sys/npc_test.go` test** `TestNpcSystem_SpeechReachesAllSensorPlayers` green (bystander already in range + newcomer crossing in on a later tick → newcomer's bubble ALSO reaches the non-moving bystander [fan-out to all, req 5]; decodes the wire back to NPC-anchored id + newline-joined text; added `addPlayerCollider`/`sentOf`/`decodeEntityMessage` helpers). Booted `-content ../api`: no panic, `20 *sys.NpcSystem` registered, `placed npcs count=1`. **NOT YET VISIBLE IN-GAME:** the NPC renders as a `Resource`, which has no `say()`, so `Chat.showMessage`'s `isFunction(gameObject.say)` guard silently drops the message (no crash). **The visible bubble requires chunk 5's frontend hoist** — this is the plan's deliberate backend/frontend boundary (backend-only: chunks 1–4). Backend origination is done, unit-verified, wire-confirmed by the decode test — but NOT PO-verified in-game (deferred to after chunk 5). **Prior:** chunk 3 (grant + edge-trigger) DONE+VERIFIED+committed (`35f3b151`); chunks 1 (schema+loader+TS mirror) + 2 (entity+placement) DONE+VERIFIED, committed (`f4ebe912`, `9d71f9ad`). Step 4 (skill-vocab fill) COMPLETE + committed.
- **Next:** Step-5 **chunk 5 (frontend bubble hoist + latest-wins + `Chat.showMessage` guard) in a NEW session** → `docs/plan-npc-teaching.md`. **Hoist `say`/`messages[]`/`messagesGroup`/expiry** from `Character` (`Character.ts:570`) to the shared base (`_GameObject`/`GameObject`) so the NPC's Resource-mapped render object can speak (this is what finally makes chunk 4's `EntityMessage` render a visible bubble over the NPC's head). Add a **single-slot latest-wins mode** (clear prior bubble children before pushing the new `Text` — a ~5-line branch in hoisted `say`) for NPC lines; player chat keeps today's stacking. **Harden `Chat.showMessage`** (`chat/logic/Chat.ts:58`) with an undefined-`gameObject` guard (latent throw-on-untracked crash — the `isFunction` check already covers the non-Character case, but `getObject` returning undefined still throws before it). Verify in-game (NOW achievable end-to-end): approach the Sage → bubble over its head; two nearby players both see the (latest) bubble; low-level player triggers the too-low line; lore guard/sign-post speaks. **Note:** `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; the Sage NPC (proving-grounds (4,3), teaches `HealAura`@L1 + `Dash`@L5) is committed content, boot `-content ../api` to exercise chunk 5. Chunk 6 (zone-editor `npc` mode) follows.
