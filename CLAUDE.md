...
- **Last completed:** **v1-roadmap item 11 (aura targeting) Steps 1–3, tested in-game** (see docs/v1-roadmap.md item 11 → Implementation status). (1) **Selector & cap machinery:** `EffectDef` gained `selector` (`nearest` default / `lowest_health` / `all`), `maxTargets` (0=uncapped), `maxTargetsPerLevel`, `tickIntervalPerLevel`; pipeline in `sys/targeting.go` (eligibility filter → selector sort → take N) applied uniformly to damage_aura/heal_aura/instant_damage so **every ability is cap-capable via config**; `lowest_health` ranks by current/max **%** (`HealthRatio()` on mob+player). (2) **Base auras flipped single-target:** DamageAura/HealAura/WildAura → `nearest`+`maxTargets:1`; **HealAura uses `lowest_health`** (heals most-wounded ally by %); NovaBurst/boss stomp stay uncapped AoE. (3) **Floating damage/heal/XP numbers:** per-tick accumulators on mob+player (`model.TickAccumulators.ResetTickNumbers()`, reset by StatusEffectsSystem prio 101), recorded at the accurate source (takeDamage/applyHealAura/AddExperience); wire `Mob.damage_taken`, `Character.damage_taken`/`heal_received`/`xp_gained` (VitalSign units for dmg/heal, raw XP); frontend `GameObject.showFloatingNumber` rises+fades on new topmost `floatingNumbers` world layer (red/green/gold), display scale placeholder full-HP≈1000 (`HEALTH_DISPLAY_SCALE`). **Prior:** Phase 8 complete (all three skill categories: auras, passives, cooldowns — 13 skills, 4 milestones; details in skill-system-design.md); Block 2 survival removal complete.
- **Next:** **Block 2 COMPLETE** (survival removal + resource unification, roadmap items 1+2 ✓ Done, verified end-to-end 2026-07-04) — see **docs/block2-resource-and-survival-removal.md**. `Health` is the single resource; DayCycle/heater/satiety/temperature, item actions/harvest/inventory/crafting/equipment, food+tool item defs, and the whole item **wire protocol** (`Action`/`OngoingAction`/`ItemStack`/`equipment`/`inventory`) are gone. Frontend item system removed (registry kept for placeable rendering; equipment rendering left inert). Resources decorative, campfires inert stubs. Dead Go types + dead survival `conf.json` config stripped. **Cosmetic leftovers only:** HUD item CSS in `HUD.less`; unused equipment-item icons kept alive by inert equipment rendering. **Next up:** **item 11 Steps 4–5** — Step 4 per-tick hit VFX (slash vs fire by tick cadence; design spec in docs/v1-roadmap.md item 11 → Remaining steps), Step 5 tick-interval verify-only; then Phase 9 (combinations impl; steps in skill-system-design.md Phase 9).
- **Current state:** new players start with DamageAura in slot 0 on spawn, server-authoritative from spawn. **Base auras are single-target (item 11):** DamageAura/WildAura hit the nearest one mob per tick; HealAura heals the lowest-%-HP ally; NovaBurst/boss stomp remain AoE-all. Floating damage/heal/XP numbers render over mobs + players. Unlock sources live: milestones (HealAura+Heal L2, SwiftPassive L3, NovaBurst L4), kill drops (WildAura: SaberToothCat 20%/boss 100%; SlowAura: Mammoth 20%; ToughPassive: Dodo 5%). Skill points: earned per player level, spent/refunded freely in the spellbook panel, equipped skills scale live. All combat participants (incl. healers, ~10s window) get full XP on mob death. `XP <amount>` cheat for manual leveling.
