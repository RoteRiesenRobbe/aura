...
- **Last completed: chunk B — mobs now push each other apart while they move** (2026-07-26, backend only, 5 files + 2 new) ✅ `8b045395` — **✅ PO-VERIFIED IN-GAME 2026-07-26** (*"feels much better"*; the stopped-mob limitation below was not raised as a blocker, so 0.45 stands and the tangential settle nudge stays offered, unscheduled). Round-6 item 3, PO decision *soft separation, no hard collision*; full ledger + acceptance checklist `docs/plan-playtest-feedback.md` §Round-6 chunk B ledger. All three planned pieces as specified: new **`phy.Space.AppendCircleDynamics`** (`QueryCircle` allocates a map *and* a slice per call; this path runs per mob per tick and would have re-opened `fe0044d0`), a query on **`LayerViewportCollision` filtered to `*Mob` via `UserData`** (there is no mob-body layer bit — and the type check is what makes *no player↔player / no player↔mob* structural), and the blend at **`mobSeparationWeight` 0.45 [PLACEHOLDER]**. **3 decisions taken inside the chunk:** ① separation is kept **out of the head-on latch entirely** — ⚑ the half that would have bitten is not *setting* `steerSide` but **holding** it (it clears on `rep == 0`, so a trailing mob would have kept it alive forever and walked its neighbour sideways); ② the sum is **clamped to unit length** with weight < 1, so no crowd can reverse a mob's pull; ③ **⚑ the welding landmine was not where the plan expected.** Co-located mobs barely exist (sequential per-tick updates break the tie in one step); the case that matters is **a mob directly BEHIND another** — the push is straight backwards, and steering sets direction, not speed, so normalizing hands back the same direction. Single file is what a chase converges every pack into. Fix: `blendSeparation` fades in a **perpendicular** component as the push lines up with the path. **⚠ Known limitation, by design: a STOPPED mob does not separate** (`shouldApproach` halts it, `steer` only runs from the move calls) — packs *arrive* spread but a settled ring keeps its arrival spacing; a tangential settle nudge would fix it and is **offered, not taken**. Measured weight sweep (4 mobs, real space): today = 0.01 mean gap (total weld), 0.45 = 0.68 at rest / 0.75 chasing, 1.20 = 1.09 but > 1 lets packmates hold a mob out of its own attack range. Verified: `go build`/`vet` clean, `go test ./...` **exit 0, 27 pkgs**, guardrails `-count=2`, **0 allocations** on the whole `steer` path, **sim battery + level curve + pack matrix byte-identical** to the stashed baseline, TDD 6 pins red-first on 3 behavioural assertions, boot `-content ../api` **0 errors 0 panics** (83 skills/14 factions/50 mobs/10 recipes/5 prop defs/1 milestone/777 props/471 spawns/5 campfires/14 npcs), headless via new `.claude/skills/verify/mob-separation.mjs` (warp into the densest wolf cluster, gather, then walk so the pack chases — 0 console errors, 0 WebGL losses; it deliberately measures **no** spacing, `window.game` has no entity manager).
