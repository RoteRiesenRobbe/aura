...
- **Last completed:** Phase 6.1 — mob migration to the SkillSystem (strict 1:1), plus the zombie-mob bug fix (death check before out-of-combat regen). Each mob has its own aura skill JSON in `api/skills/mobs/` (per-mob damage values made a shared skill impossible); `damage_aura` gained `structureDamageFraction`/`targetsStructures`; mob damage goes through the preserved `MobTouches(Factors)` double dispatch (SkillSystem fills the payload from the active skill). Target flags are now enforced (player casters too, behavior-neutral); the aura sensor's mask is derived per tick via `model.AuraMaskFor`, radius likewise — **mob aura switching is ready** for future boss scripts. `skillEntity` interface split (heal capabilities via type assertion; mobs lack player vitals). Removed from mob JSONs: `damageRadius`, `damages`, `damageFraction`, `structureDamageFraction`; `aggroRadius` is required now. Load order: items → skills → mobs (skill names resolve at startup, unknown = fail).
- **Remaining in Block 3:** Item 10 participation XP (all combat participants incl. recent healers get full XP on mob death; drops stay with last toucher) → 6.2 monster-kill unlocks (`unlocks` in mob JSON, per-participant rolls, glow via existing spellbook diff) → 6.3 AngryMammoth boss designation (JSON-only: biggest size+aura, guaranteed unlock source).
