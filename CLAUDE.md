...
- **Last completed:** **Triage pass — cast-bar stack + spider range match + Strong passive + Wildfire light — DONE + PO-VERIFIED ("Perfect") 2026-07-21, `37878018`; full ledger: `docs/plan-intermission-triage.md` §Triage pass.** Cast bar now stacks directly above the action bars in a shared `#bottomCenter` flex column (idle bar keeps its slot via `visibility` — no jump on cast start); SpiderBite 1.2→**1.0** / VenomSpit 2.0→**1.0** matched [PLACEHOLDER]; **Strong** (id 136, passive, +4% all damage +2%/pt, maxLevel 5) rides the new **`damageDealt` stat** (fifth validStat: `DerivedStats.DamageDealtBonus` + `sys.casterDamageFactor` — the `casterCritChance` twin — multiplied at all three outgoing-damage base sites incl. dot application; never heals/CC; TDD +3 tests; registry pin **82**; tooltip label "All damage"); **PO ruling:** Strong taught by the existing **CityGuard @L3** — the "inform the city" quest reward (TownCrier opens the thread; teachings + tooLowLine added in world.json). Wildfire gained `light_aura` r4 +1/L (exactly Light's values per PO spec — zero code, `LightRadius()` is already max-over-effects; note its L5 light tops out r8 vs Light's r6). **Verified:** full suite green, `tsc` + prod build clean, boot `-content ../api` `82 skills/14 factions/50 mobs/10 recipes/828 props/373 spawns/5 campfires/14 npcs, 0 panics`, `GET /skills` serves all three changes, Playwright smoke **8/8 PASS** (stack geometry, catalog name, "All damage: +4% → 6%" tooltip, Wildfire light lines), PO in-game. Docs: skill inventory regenerated (47 player/82 pin, 14 teachings across 10 NPCs) + passives catalog row.
