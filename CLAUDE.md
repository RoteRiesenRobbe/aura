...
- **⭐ NEXT — quest chunks C0–C4 (`docs/plan-quests.md`), THEN step 8, accounts & persistence (a DESIGN session, not a schema session).** Quest ruling **D12** put the quest first pass in front of step 8 (see the ⭐ quest banner at the top; roadmap "Execution order" carries the pre-step-8 insert) — **C0 (interaction hardening: the §8b N1 both-ends fix + 255-index guard) is the first execution chunk and is also standalone filler**; C1→C4 follow in order. `plan-faction-flips.md` is **COMPLETE and ARCHIVED** (chunks 1 ✅ · 2 ✅ · 3 ✅ · in-world teachers ✅ `3b1b3ef6` · all §8 questions closed), pre-accounts hygiene is complete, and the open-questions sweep above cleared the standing PO queue. The step-8 detail — ruling 4, backlog §32, the ops/security fold-in, **now also the live quest ledger from D12** (`quests`/`killCounts`/`talkedTo`, plan §10) — is in the bullet below. ⚑ **Other scoped filler:** §35 C3–C4 (`docs/plan-conf-duplication.md`; C1 + C2 shipped 2026-07-29, see Last completed) — and both small chunks previously queued here have shipped: **Swift → cooldown** `a29fe986` and **R4** `a4225e8b`, the latter closing and archiving `plan-entity-model.md`.
