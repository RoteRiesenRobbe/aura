...
- **One plan in flight: `plan-effect-types.md`** (round 1, schema NONE all four chunks) - C1 vulnerability ✅ 2026-08-16; open: **C2 `retaliate_damage`** (next) → C3 invulnerability → C4 ally speed. Untouched: `plan-play-bot.md` (C0-C3: extract transport → the bot that fights → quests → the soak finding) · `plan-prototype-projectile.md` (designed 2026-08-15). ⚑ Zone-editor C3 leftovers, owned by nobody: the Go legacy plumbing is now dead code (Legacy/LegacyRefs fields + the unreachable loaders warning) · `wiki-generator/` is broken at HEAD independently of C3 (requires the §28-deleted `client-data/Items`) · ⭐ **C3 missed the `backend/pkg/api/mobs/` mirror** — all 10 deleted legacy mob JSONs are still committed there (found 2026-08-17), and they name `EntityType` values C3 removed from the enum, so an **embedded** boot may hard-fail; `cp-defs` fixes it locally but stages 10 deletions, and `-content ../api` sidesteps it entirely.
