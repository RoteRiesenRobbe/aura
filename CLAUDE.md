...
- **Last completed:** **Sim-harness Chunk 2 DONE — PO-verified ("the tool works") + committed `eed434f5` (2026-07-15)** per `docs/plan-sim-harness.md` §8: the decided §5 `f(character level)` curve implemented + validated in the tool. `sim/curve.go` = `Curve` (`f(L)=growth^(L-1)`) + `XPModel` (level-up rule mirrors `player.experienceForNextLevel` exactly; modeled kill XP → kills-per-level) + `Fixture` generator (`PlayerAt`/`MobAt` scale **HP values only** — max health + aura damage, never radius/tick/variance/crit — pinned by tests); `sim/sweep.go` = same-tier level sweep, cross-tier gap sweep (TTK/TTD/win-rate vs Δlevel), linked-triple table (measured wall Δ = first Δ≥0 with TTK win-rate <50% [PLACEHOLDER def] × analytic inflation per max-level candidate), sweep points parallel across NumCPU (race-clean, fixed-seed reproducibility kept); CLI `-levels` mode + web-explorer "level curve" panel (`POST /curve`, SVG charts, win-rate bars, triple table). 12+2 new tests; suite + build green; browser-verified via the new committed **`.claude/skills/run-simharness/` run skill** (`920c8bf2` — no-root playwright harness + driver.mjs). **Findings: same-tier perfectly flat across 30 levels (Philosophy A holds in the real systems); with current content baselines (TTD/TTK≈1.3) the wall is Δ≈+2 regardless of growth — the band is set by growth AND the TTK:TTD ratio; at a target-shaped ratio (~1:2.5, mob dmg 8→4) the wall spreads 1.08→>6 / 1.10→+6 / 1.12→+5 / 1.15→+4, so the ~4-level band points at growth ≈1.15.** Chunk-1 recap: committed `5faf7aa6` — `sim/` drives the real ECS headlessly, distributions over N seeded runs, `-serve` explorer + mob presets, `SkillSystem.SeedRNG` seam; TTD ≈ 8.7 s vs ~20–25 s target.
...
- **Next:** **Execute Chunk 3 of `docs/plan-sim-harness.md`** (1-vs-N matrix: multi-mob worlds, sweep player-build × pack-size, survival / clear-time / overwhelm point as distributions). Then chunk 4, pausing for PO review between. **⚑ OPEN (PO): lock exact `growth` + max level from the chunk-2 tool output** (working read: ~4-level band at a ~1:2.5 TTK:TTD ratio → growth ≈1.15; the current-content ratio ≈1.3 compresses the band to +2 regardless of growth). After the harness: **Step 6 = the initial content pass (item 12)** — two large zones with intra-zone difficulty areas, full mob roster (hand-authored per-tier against the §5 curve), boss scripts, first real balance pass; live-game `f` multiplier lands at the top of step 6 (§5 Decision 5). `make build` runs `cp-defs` which reverts embedded `backend/pkg/api/` from source `api/`; the committed Sage (proving-grounds (4,3), teaches `HealAura`@L1 + `Dash`@L5) is content — boot `-content ../api` for content iteration.
