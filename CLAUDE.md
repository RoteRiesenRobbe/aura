...
- **Prior: MOB ATTACKS HIT ONE TARGET, and the companion defends itself** ✅ 2026-09-19 `de57fd4f` (ledger: `docs/archive/status-history.md` holds the full entry + `manual-content-authoring.md` §2 "Mob attacks hit one target"; no plan doc): ⭐ **a mob fights one versus one, WoW style** (PO ruling): every attack authors `selector` + `maxTargets`, cap **1** unless the world explains more (2 = two weapons, 3 = a cleave, `all` = a PLACE or a telegraphed AoE). ⭐ The target is the **NEAREST, not the aggro pick**; authored per file, **no loader default and no validator** (rule over machine). ⭐ A mob then spent its one hit on a companion that never answered, so the companion's own `highestThreatTarget()` is its third and LAST candidate. ⚑ An absent `maxTargets` loads clean as "hits everyone", silent BY CHOICE · elite/boss re-pricing owed (watch item). **Schema DB/wire/conf NONE; content: 21 mob skill files.** Verified: 35 pkgs 0 failures · `-validate` 0 both ways · smoke 0/114 · `chunk2-follower.mjs` 6/6. ⭐ **PO walk 2026-09-20: "walked it, works."**

- **Prior: SKILL VFX C2a, the engine + four kinds** ✅ 2026-09-19 `512d4afd` (ledger: `docs/plan-skill-vfx.md` §13 C2a, spec §12b + amendment §12c): ⭐ **a `SkillFx` manager on its own layer above `flyers`, below `darkness`**, fed once from `Backend.receiveSnapshot` off C1's `SkillEvent`s, budget 96 + pools, a deterministic math module, placeholder bodies. Kinds `impact` · `projectile` · `beam` (+ a VISUAL-ONLY `chain`) and ⭐ **the caster-anchored `strike`, which the first PO look forced** (§12c). ⭐ **The `hitStyle` lever is DELETED end to end** and `AuraTickIndicator.ts` is absorbed (the wind-up glow draws ABOVE entities). PO: author every damaging skill, no engine fallback. ⚑ `width` is PIXELS. **Schema DB/conf NONE; WIRE `aura_hit_style` deprecated; content: `visual` on 59 skills.** Verified: 35 pkgs 0 failures · smoke 0/114 · frontend 742/42 · `skill-fx.mjs` 7/7. ⭐ **PO: "works for now as placeholders, ingame look passes."**
