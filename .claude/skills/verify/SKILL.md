...
| `content-editor-skills-tab.mjs` | the **content editor's Skills tab** (spell builder C1+C3+C4): every player skill renders from the served vocabulary with no console error, Save present and `id` locked on every editable skill, the parked pair (`ThrowBomb`/`ThrowMine`) fully read-only, a sources-panel jump switching the sidebar tab; then the EDIT legs: a rename refused as a guard, an illegal cost refused by `aurad -validate`, a real save round-tripped through `api/skills/damage.json` and restored byte for byte, the type-change / Delete / `maxLevel` confirms; then the C4 legs: the test-rig badge on exactly the three Omni rows and headers, the icon picker offering the whole vendored set with the file's own glyph checked, the role-grouped `spawnMob` picker and its jump into the Mobs tab, the "After saving" test link on every skill, and the **new-skill flow** end to end (the prompt, a draft with no category and `id` = max over both skill folders + 1, a duplicate "+ New" refused client-side, a real save whose file lands on disk with exactly the authored keys and whose rendered checklist carries the recounted registry pin, and a new file reusing id 1 refused by the seam); then the **C3b Visuals legs** (part 2d, `plan-skill-vfx.md` §12f.5): Damage renders its `strike`/`hit`/`sword` layer with a loaded body thumbnail, the moment picker offers `applied` on Blight and not on Damage, a new layer on OmniPassive offers no `wave`/`orbit` and `hit` alone, `applied` on Damage is refused by the seam, a `wave` layer is SAVED onto `damage.json` and restored, and in the Mobs tab the WolfBite look block saves `ms` onto `api/skills/mobs/wolf-bite.json` through `/api/save/skill-visual` with `wolf.json` untouched and is restored; screenshots Damage / OmniStrike / ThrowBomb / NovaBurst / Blight / Wolf-mobs-tab; then the **C3c preview legs** (part 2e): the row Preview toggle and "Show all kinds" both answer the ready handshake from `fx-preview.html` on 2001 (a NOTE, never a problem, when that dev server is down), the debounced re-post on an edit, Reset with a preview open, `Damage-preview.png`. ⚑ It WRITES `api/skills/harness-test-skill.json` and DELETES it in a `finally` (a leftover file reddens the Go registry count pin for everyone; if a run dies mid-leg, `rm -f api/skills/harness-test-skill.json` and `git checkout api/skills/damage.json api/skills/mobs/wolf-bite.json`). ⚑ Needs `tools/content-editor/server.mjs` AND a built, mtime-fresh `backend/aurad` (no DB); runnable from any cwd (the script resolves `playwright` from `~/.cache/aurahunter-run` itself). Pair it with `npm run smoke` in `tools/content-editor/`, which owns the vocabulary-vs-presentation completeness and the save path's unit legs offline | `tools/content-editor/public/app.js`'s skill editor + the Mobs tab's look block, `save-skill.mjs` (both writers), `skill-references.mjs`, `skill-presentation.mjs`, `skill-visual-hints.mjs`, `skill-icons.mjs`, `skill-fx.mjs`, `/api/save/skill`, `/api/save/skill-visual`, `/api/skill-fx/body/*.png`, `api/skill-vocabulary.json` |
...
| `round4-tooltip.mjs` | skill tooltips scaled to character level; the `/skills` payload shape; **the served `description` block** (UI pass C8: Frost Shield renders it as the THIRD tooltip child under the subtitle, text read off the catalog so a re-authored sentence stays green; Rejuvenation renders none) | `SkillTooltip.ts`, the skills catalog endpoint, a skill JSON `description`, `#skillTooltip`'s child order |
...
| `skill-fx.mjs` | **the `SkillFx` engine drawing authored `visual` layers off the per-hit `SkillEvent` wire** (plan-skill-vfx.md C2a + C2b + C3a + §12g + §12h, §12b + the §12c `strike` amendment + §12d + §12f): 20 legs, asserted through the manager's own counters (`window.game.skillFx()` -> `{live, spawnedByKind, evicted}`), which count at SPAWN time, so a throttled scene poll cannot miss a 220 ms mark. ⭐ **Since §12g the round mark on the victim is the ENGINE'S** (every landed Damage/Crit hit, no file authors it, still counted as `impact`), so every damage leg asserts it (`wantImpact`), leg 10 proves `off` hides it and leg 15 proves a HEAL landing draws none. Nothing spawns without a skill event · Damage draws a caster-anchored `strike` plus the mark (and the wolves' own bite STRIKES land here, which is the mob half) · LongRangeStrike draws `projectile` + the mark · LightningStrike draws a chained `beam` · `skillFx` sits below `darkness` · a bandit's swing and a troll's overhead land on the OWN player · **C3a's legs: 13 the three pilot BODIES (`sword` on Damage, `arrow` on Long-Range Strike, `wolf-jaw` on the wolves' bite) draw as SPRITES rather than placeholders, read off the `sprites` counter in legs 1-3's OWN windows (a pure re-read: it never raises the global inconclusive flag, because that flag gates the eight legs after it) with Lightning Strike - bodiless on every layer - as the control, plus the page's `[skill-fx] body ...` WARNINGS (spider-fang included since C3a-ii), which the error collector cannot see and which are the only signal that a body silently fell back to its placeholder · 14 the wolves' `bite` strike draws its jaw PNG FROM THE WOLF (own aura off, so every strike in the window is a wolf's and sprites == strikes), photographed with the armed 8x-slowed clock (`leg14-wolf-bite.png`), which needs GOD off like legs 5+6 and therefore sits AFTER the XP cheat and BEFORE them (at the bandit camp the player can die, and a dead client never re-points `window.game.character`, so every later warp check reads a corpse's position) · 15 sits INSIDE leg 11 with Heal still on at the quiet campfire: `DAMAGE 90` (writes the pool directly, no hit event, works under GOD) and the heal landings that follow draw 0 marks; a full player is NOT a heal landing (`player.go`), which is why a first draft riding leg 14's wolf bites judged nothing (a level-30 pool regenerates before Heal comes on).** ⚑ Leg 14 is a BOUND, not an equality: the wolf camp's boars gore too (id 112, bodiless), measured 91 sprites for 102 strikes. **C2b's legs: a campfire's AMBIENT mist with no combat at all · Frostbite's swirl on the own player, disposed when the aura is switched off · density `low` (another actor's ambient emitter absent, the own one present) · density `off` (a real fight spawns 0 Fx while the glows live) · Heal's two emitters · Whirling Axes' orbit**, and leg 2 also asserts the `cast-pose`. ⚑ `counters()` also reports `ambient`, `glows` and `density`, and `window.game.settings()` drives the slider. ⚑ **No XP cheat before the aura legs** - a levelled player one-shots the pack and the chain has nothing to jump to. ⚑ **Every equip happens BEFORE the first fight** (equips are combat-locked client-side). ⚑ Screenshots arm on the spawn COUNTER and slow the page clock 8× - a fixed-time shot misses a 260 ms beam. ⚑ **GOD short-circuits the player's own `takeDamage`**, so the two mob-strike legs drop GOD for their armed window only, or nothing ever draws on the player. Tri-state: a warp off-venue, an aura that never activates, an equip that never lands or a leg whose skill never landed is INCONCLUSIVE, not red. **C3a-ii's legs (§12h, 2026-09-23): the wire's `phase` (Direct 0 / Applied 1 / Tick 2) is counted by an in-page CENSUS that reads `skillEvents().last` only when `total` moved and reports the snapshots it missed as `gaps` (the headless page stalls past several snapshots, measured 4-20 gaps a window, so an exact bound is scored only on a gap-free census and a CADENCE bound, distinct casters x window / aura beat, stands in otherwise); `plannedBound` reads the authored layers off `api/skills` to bound each per-kind counter by the skills that actually landed · 14 also measures the RIM BITE off the live jaw sprites (the only sprites anchored (0, 1)): length = max(40, player radius x 1.4) = 42 px against the wolf's 120 px reach, hinge on the player's rim, pair opening toward the player's centre, plus a close-up shot (`leg14-wolf-bite-closeup.png`) · 19 Shockwave: one `wave` Fx per cast, an equality against the census's FIRED events, cast at open ground with every aura off (`leg19-shockwave.png`) · 16 the venom spiders: Applied on the wire, no Direct from a DoT-only skill, projectiles within the application bound, marks on the ticks · 17 the giant spiders at the camp's LIT western edge (the middle is dark, and five giants at once killed a level-30 player in 14 s): the white `spider-fang` sprites on `hit`, and the gap-proof discriminator, spits tracking fang pairs beat for beat (one shared aura beat) instead of doubling per tick, photographed with a close-up · 18 the pyromancer: Direct hits (beam) beside Applied (emitter burst). ⚑ 16-18 sit AFTER legs 6 and 5 and each rests 8 s at the quiet campfire first: there is no heal cheat, and a drained player died to the trolls. ⚑ A leg in which the player DIES marks the run inconclusive (a dead client never re-points `window.game.character`)** | `frontend/src/features/skill-fx/**`, the `SkillEvent` decode, the `visual` vocabulary (`skills/visual.go`, the fixture) or any authored `visual` on a skill file |
| `skill-fx-scale.mjs` | **what the `SkillFx` layer COSTS at world scale** (plan-skill-vfx.md C4, §12e): a MEASUREMENT script, its deliverable is the tables + JSON it prints, its only hard guard is 10× `full` `update()` p95 under a 16.7 ms frame. Seven legs: real 1× at the western bandit camp (LEVELLED, GOD OFF, or the mob→player stream never records) · the synthetic driver's honesty gate against it · 10× at `off` / `low` / `full` · ambient alone (50 owners) · the cap at 48 / 96 / 192 · a phone-shaped page (390×844, DPR 3) · the CEILING ramp (×2 per step until 96 evicts, then the three caps where they differ). The load is the dev-only `window.game.skillFxStress()` fed through the real `onSnapshot` + `setAmbient`; `skillFxMeasure` / `skillFxStats` / `skillFxBudget` are the instrument. `AURA_FXSCALE_RAMP_ONLY=1` runs legs 1, 2 and 7. ⭐ `AURA_FXSCALE_SKILL_IDS=1,45,110` (C3a) points the driver at the body-carrying skills only, for pricing the SPRITE path against C4's Graphics numbers - ⛑ none of those three authors an ambient layer, so such a run carries NO ambient (≈ 93 % of C4's display objects) and is comparable only to another run of the same script, not to the C4 ledger row. ⚑ **Headless absolutes do not transfer**: the page renders at ≈ 3 fps, a p95 over ~16 frames is the second-highest sample (read p50 beside it), rAF ratios are noise, fill rate cannot be judged. ⚑ No 8× clock trick: every clock here reads `performance.now`. ⚑ `reset()` (a death) restores the budget override. Inconclusive when the venue is empty | the `SkillFx` budget, pools, ambient reconciler or per-frame update, `SkillFxStress.ts`, particle counts or the density rule |
| `skill-fx-wave-probe.mjs` | **the `wave` kind executes** (plan-skill-vfx.md §12g.2): the dev-only stress driver pointed at the mammoth stomp alone (`skillIds: [105]`) feeds `WaveFx` through the real `onSnapshot`; asserts `spawnedByKind.wave` rose, no page error, and photographs a ring with the 8x-slowed clock (`wave-stomp.png`). ⚑ It exists because no PLACED mob authors `wave` and the scale JSON reduces the counters to a row, so nothing else ever runs the class. ⚑ Its negative claim (a fired stomp spawns nothing else) scores only when a 12 s control window at open ground is quiet, and it usually is not (mob-vs-mob fights at the viewport's edge); NOTE otherwise. Warps to (-23, 14) with a position-checked retry | `WaveFx`, `waveRing`/`waveCountOf`, `drawWaveRing`, `angry-mammoth-stomp.json`'s layer, the `fired` planning path |
| `skill-fx-preview.mjs` | **the dev-only live VFX preview page** (plan-skill-vfx.md C3c, §12f.7): `fx-preview.html`, the second webpack entry the content editor iframes, drawing with the REAL `SkillFx` renderer from two stub actors and no server, read through `window.__fxPreview.counters()`. `?gallery` spawns every one of the seven kinds (plus the engine hit mark) and keeps looping, seven labelled slots, then one shot PER SLOT armed on that kind's counter with the page clock slowed 8x (`gallery-slot-N-<kind>.png`, look at them) · message mode spawns NOTHING before a message, a posted `wave` layer draws waves only (`single-wave.png`), a malformed message is ignored · a host page on `http://127.0.0.2:<port>` (untrusted by the origin guard) iframes both modes: the `aura-fx-preview-ready` handshake reaches the PARENT from both (matched by `event.source`, as the editor does), its posted wave is ignored, and the same frame answers a trusted post (the control). ⚑ Not a `data:` host, which the spec named: Chrome refuses to load the localhost frame under a data: or public-address parent at all (Private Network Access, the frame lands on `chrome-error://`). ⚑ Needs the webpack DEV server (`./scripts/dev-restart.sh frontend`); no aurad, no DB. Args: dev-server URL (default `http://localhost:2001`), shots dir. The one console error ignored is the 404 on `/skills` (Skills.ts fetches the catalog at import; the page registers its own definitions) | `frontend/src/fx-preview.ts` / `.html`, `SkillFxPreview.ts`, the `registerSkillDefinition` / `forgetVisual` / `loadSkillFxBodies` seams, the dev entry in `webpack.dev.js`, any `SkillFx` kind |
| `immune-feedback.mjs` | the floating grey **"Immune"** label over a fully mitigated hit (plan-immune-feedback.md): a normal damage aura ticking on a `{"*": 0}` structure pops the label at the target, in the D3 grey, with **no damage number** there (since skill VFX C1 both come from `skill_events`: an `Immune` HIT draws the label, and D6 draws only own-caused numbers). ⚑ **Venue is the Bramble wall (-68.7..-65.1, -4.8), deliberately NOT the Rockfall the plan names** - the Rockfall sits in the Dark Tunnel, where `showFloatingText`'s darkness suppression swallows the label and the run proves nothing. ⚑ The "no damage number" half is scoped BY POSITION (within ~2.5 u of a bramble) - a global "no red text" assert would flake on any unrelated wolf fight in the viewport. ⚑ GOD is survival-only here: it never touches the MOB's takeDamage, but the player-side label never appears under GOD (D6), so don't extend this script to the own-head case without dropping GOD. Tri-state: a warp landing off-target or an aura that never activates is INCONCLUSIVE, not red | `takeDamage`'s mitigation branches, `SkillEvent` / `HitKind.Immune` on the wire, `SkillEventNumbers.ts`, the consumer in `Backend.ts`, `IMMUNE_COLOR`, `showFloatingText` |
...
| `start-screen-tour.mjs` | the **start screen's live backdrop** (2026-09-21): the PRE-JOIN spectator tours the map. It samples the camera and `#spectateFade` for ~50 s and asserts the view is zoomed OUT (scale ~0.44 against the in-game ~0.88), that it SWEEPS (~1 u/s), that at least one CUT happens, that no sample shows the NEW place with the fade open, that the black lifts again, and that the world is populated wherever it looks. ⚑ **The only script that never joins**, so it leaves no `hrnss_` residue. ⚑ **`window.game` does not exist before a join** (it, and `Develop.isActive()`, switch on with the first Pong, which a client on the start screen never gets), so the script reads a `?develop`-keyed hook instead, `window.spectatorTour` (`Spectator.ts`). ⚑ Headless renders at a few fps, so the cover usually LOSES its race with the cut here (opacity ~0.5 then 1.0): that is the hard-cut-to-black fallback working, not a defect, and the leg is phrased as *never lit across a cut* for that reason. ⚑ It found one real defect on its first run: stop detection by wall-clock read a main-thread stall at load as "the view stopped" and faded for nothing; `SpectateFade` counts SNAPSHOTS now. ⚑ Takes `[label] [url]`; screenshots `start-tour-*.png` are for the eye | `model/spectator/tour.go`, `NewTouringSpectator`, `AppendWakePositions`' quadrant tiling, `Zoom`'s spectate override, `SpectateFade(.Overlay).ts`, `Spectator.ts`, the spectator branch of `Backend.ts`'s GameState handler |
...
`[label] [url]`, but **eleven scripts take the URL as the first argument** and die
...
`r3-lifesteal-burst`, `r7-respec-cost`, `r7-strong`, `round4-tooltip`,
`skill-fx` (plus `filler-batch`). Don't trust this list against a growing suite — a sweep runner
should DERIVE it:
