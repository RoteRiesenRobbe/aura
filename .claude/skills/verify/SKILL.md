...
| `content-editor-skills-tab.mjs` | the **content editor's Skills tab** (spell builder C1): every player skill renders from the served vocabulary with no console error, every control disabled, no Save button, no dirty dot, the parked-type banner, a sources-panel jump switching the sidebar tab. ⚑ Needs only `tools/content-editor/server.mjs` (no aurad/DB); run from `~/.cache/aurahunter-run`. Pair it with `npm run smoke` in `tools/content-editor/`, which owns the vocabulary-vs-presentation completeness offline and, since spell builder C2, also needs a built `backend/aurad` for its save-seam legs (f)+(g) | `tools/content-editor/public/app.js`'s skill editor, `skill-presentation.mjs`, `/api/data`'s `skills`/`skillVocabulary`/`ticksPerSecond`, `api/skill-vocabulary.json` |
