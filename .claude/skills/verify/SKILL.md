...
| `skill-fx.mjs` | **the `SkillFx` engine drawing authored `visual` layers off the per-hit `SkillEvent` wire** (plan-skill-vfx.md C2a, §12b + the §12c `strike` amendment): 7 legs, asserted through the manager's own counters (`window.game.skillFx()` -> `{live, spawnedByKind, evicted}`), which count at SPAWN time, so a throttled scene poll cannot miss a 180 ms impact. Nothing spawns without a skill event · Damage draws a caster-anchored `strike` (and the wolves' own `wolf-bite` impacts land here, which is the mob half) · LongRangeStrike draws `projectile` + `impact` · LightningStrike draws a chained `beam` · `skillFx` sits below `darkness` · a bandit's swing and a troll's overhead land on the OWN player. ⚑ **No XP cheat before the aura legs** - a levelled player one-shots the pack and the chain has nothing to jump to. ⚑ **Every equip happens BEFORE the first fight** (equips are combat-locked client-side). ⚑ Screenshots arm on the spawn COUNTER and slow the page clock 8× - a fixed-time shot misses a 260 ms beam. ⚑ **GOD short-circuits the player's own `takeDamage`**, so the two mob-strike legs drop GOD for their armed window only, or nothing ever draws on the player. Tri-state: a warp off-venue, an aura that never activates, an equip that never lands or a leg whose skill never landed is INCONCLUSIVE, not red | `frontend/src/features/skill-fx/**`, the `SkillEvent` decode, the `visual` vocabulary (`skills/visual.go`, the fixture) or any authored `visual` on a skill file |
